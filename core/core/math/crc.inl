namespace V {
     ...
     */
    namespace {
        inline constexpr unsigned int crc_table[256] = {
    ...
    //
    //=========================================================================
    constexpr Crc32::Crc32(VStd::string_view view)
        : m_value{ 0 }
    {
        if (!view.empty())
        {
    ...
    namespace Internal
    {
        ...
        inline static constexpr V::Crc32 CompileTimeCrc32 = V::Crc32(CrcValue);

        constexpr unsigned int ComputeCrc32Octet(unsigned int currentCrc, uint8_t dataOctet)
        {
            return crc_table[(static_cast<int>(currentCrc) ^ dataOctet) & 0xff] ^ (currentCrc >> 8);
        }

        template<typename CharType>
        constexpr void Crc32Set(const CharType* data, size_t size, bool forceLowerCase, V::u32& value)
        {
            ...
            else
            {
                ...
                value = crc ^ 0xffffffffL;     /* (instead of ~c for 64-bit machines) */
            }
        }
    }
    ...
    // Crc32 - Add
    //=========================================================================
    constexpr void Crc32::Add(VStd::string_view view)
    {
        if (!view.empty())
        {
