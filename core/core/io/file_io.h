#ifndef V_FRAMEWORK_CORE_io_FILE_io_H
#define V_FRAMEWORK_CORE_io_FILE_io_H

#include <core/base.h>
#include <core/std/function/function_fwd.h>
...
namespace V {
    namespace IO {
        ...
        V_DEFINE_ENUM_BITWISE_OPERATORS(OpenMode)
        ...
        public:
            ...
            virtual Result Tell(HandleType fileHandle, V::u64& offset) = 0;
            virtual Result Seek(HandleType fileHandle, V::s64 offset, SeekType type) = 0;
            virtual Result Read(HandleType fileHandle, void* buffer, V::u64 size, bool failOnFewerThanSizeBytesRead = false, V::u64* bytesRead = nullptr) = 0;
            virtual Result Write(HandleType fileHandle, const void* buffer, V::u64 size, V::u64* bytesWritten = nullptr) = 0;
            ...
            /// across sessions.
            virtual V::u64 ModificationTime(HandleType fileHandle) = 0;
            virtual V::u64 ModificationTime(const char* filePath) = 0;

            /// Get the size of the file.  Returns Success if we report size.
            virtual Result Size(const char* filePath, V::u64& size) = 0;
            virtual Result Size(HandleType fileHandle, V::u64& size) = 0;
            ...
            typedef VStd::function<bool(const char*)> FindFilesCallbackType;
            ...
            virtual void SetDeprecatedAlias(VStd::string_view oldAlias, VStd::string_view newAlias) = 0;
            ...
            virtual VStd::optional<V::u64> ConvertToAlias(char* inOutBuffer, V::u64 bufferLength) const = 0;
            ...
            //! if no alias matches the start of the path, the path is returned as a V::IO::FixedPath
            //! If the path cannot fit in V::IO::FixedMaxPath false is returned
            virtual bool ConvertToAlias(V::IO::FixedMaxPath& convertedPath, const V::IO::PathView& path) const = 0;
            VStd::optional<V::IO::FixedMaxPath> ConvertToAlias(const V::IO::PathView& path) const;
            ...
            virtual bool ResolvePath(const char* path, char* resolvedPath, V::u64 resolvedPathSize) const = 0;
            ...
            virtual bool ResolvePath(V::IO::FixedMaxPath& resolvedPath, const V::IO::PathView& path) const = 0;
            VStd::optional<V::IO::FixedMaxPath> ResolvePath(const V::IO::PathView& path) const;
            ...
            //! returns true if the resulting path can fit within V::IO::FixedMaxPath buffer
            virtual bool ReplaceAlias(V::IO::FixedMaxPath& replacedAliasPath, const V::IO::PathView& path) const = 0;

            /// Divulge the filename used to originally open that handle.
            virtual bool GetFilename(HandleType fileHandle, char* filename, V::u64 filenameSize) const = 0;
        ...
        {
        public:
            V_CLASS_ALLOCATOR(FileIOStream, SystemAllocator, 0);
            FileIOStream();
            FileIOStream(HandleType fileHandle, V::IO::OpenMode mode, bool ownsHandle);
            FileIOStream(const char* path, V::IO::OpenMode mode, bool errorOnFailure = false);
            ~FileIOStream() override;

            bool Open(const char* path, V::IO::OpenMode mode);
        ...
        private:
            ...
            VStd::string m_filename; ///< Stores filename for reopen support
