 ...
 *
 */
#ifndef V_FRAMEWORK_CORE_STD_FUNCTIONAL_BASIC_H
#define V_FRAMEWORK_CORE_STD_FUNCTIONAL_BASIC_H
...
namespace VStd
{
    ...
    struct plus<void> {
        ...
        constexpr auto operator()(T&& left, U&& right) const -> decltype(VStd::forward<T>(left) + VStd::forward<U>(right)) {
            return VStd::forward<T>(left) + VStd::forward<U>(right);
        }
    ...
    struct negate<void> {
        template <class T>
        constexpr auto operator()(T&& left) const -> decltype(-VStd::forward<T>(left)) {
            return -VStd::forward<T>(left);
        }
        using is_transparent = void;
    };
...
#endif // V_FRAMEWORK_CORE_STD_FUNCTIONAL_BASIC_H