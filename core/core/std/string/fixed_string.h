 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_STRING_FIXED_STRING_H
#define V_FRAMEWORK_CORE_STD_STRING_FIXED_STRING_H
...
namespace VStd
{
    ...
    //! Due to being non-allocating this class is also safe to use in static memory unlike VStd::string
    ...
    private:
        ...
        using internal_size_type =
            VStd::conditional_t<(MaxElementCount < std::numeric_limits<uint8_t>::max()), uint8_t,
            VStd::conditional_t<(MaxElementCount < std::numeric_limits<uint16_t>::max()), uint16_t,
            VStd::conditional_t<(MaxElementCount < std::numeric_limits<uint32_t>::max()), uint32_t, size_type
            >>>;
    ...
    public:
        ...
        // #9
        constexpr basic_fixed_string(VStd::initializer_list<Element> ilist);

        // #10
        template<typename T, typename = VStd::enable_if_t<is_convertible_v<const T&, basic_string_view<Element, Traits>>
        ...
        // #12
        constexpr basic_fixed_string(VStd::nullptr_t) = delete;

        constexpr operator VStd::basic_string_view<Element, Traits>() const;
        ...
        constexpr auto operator=(VStd::initializer_list<Element> ilist) -> basic_fixed_string&;
        template<typename T>
        constexpr auto operator=(const T& convertible_to_view)
            -> VStd::enable_if_t<is_convertible_v<const T&, basic_string_view<Element, Traits>>
            && !is_convertible_v<const T&, const Element*>, basic_fixed_string&>;
        constexpr auto operator=(VStd::nullptr_t) -> basic_fixed_string& = delete;
        ...
        constexpr auto append(VStd::initializer_list<Element> ilist) -> basic_fixed_string&;
        ...
        constexpr auto assign(VStd::initializer_list<Element> ilist) -> basic_fixed_string&;
        ...
        constexpr auto insert(const_iterator insertPos, VStd::initializer_list<Element> ilist) -> iterator;
        ...
        constexpr auto replace(const_iterator first, const_iterator last, InputIt first2, InputIt last2)
            -> enable_if_t<Internal::is_input_iterator_v<InputIt> && !is_convertible_v<InputIt, size_type>, basic_fixed_string&>;
        constexpr auto replace(const_iterator first, const_iterator last, VStd::initializer_list<Element> ilist) -> basic_fixed_string&;
    ...
    basic_fixed_string(T, U...)-> basic_fixed_string<T, 1 + sizeof...(U), VStd::char_traits<T>>;

    // Guide for making a basic_fixed_string out of a string literal
    // i.e basic_fixed_string elem = "Test"; // basic_fixed_string<char, 4, VStd::char_traits<char>>
    template<class T, auto N>
    basic_fixed_string(const T (&)[N])-> basic_fixed_string<T, N - 1, VStd::char_traits<T>>; // subtract 1 to account for null-terminator
