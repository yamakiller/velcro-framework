 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_STRING_WILDCARD_H
#define V_FRAMEWORK_CORE_STD_STRING_WILDCARD_H
...
namespace VStd {
    ...
    constexpr bool wildcard_match(VStd::string_view filter, VStd::string_view name, BinaryOp binaryOp) {
        VStd::string_view findCharPos = name;
        VStd::string_view findFilterPos = filter;
        ...
        if (findFilterPos.front() == '*') {
            ...
            findFilterPos.remove_prefix(nonMatchZeroOrMoreOffset != VStd::string_view::npos ? nonMatchZeroOrMoreOffset : findFilterPos.size());
            ...
            for (auto findEndCharPos = findCharPos.rbegin(); findEndCharPos != findCharPos.rend(); ++findEndCharPos) {
                ...
                if (findFilterPos.front() == '?' || binaryOp(findFilterPos.front(), *findEndCharPos)) {
                    ...
                    VStd::string_view startFindSubView{ findCharPos.data() + VStd::distance(findCharPos.begin(), findEndCharPos.base()) - 1, findCharPos.data() + findCharPos.size() };
    ...
    inline constexpr bool wildcard_match(VStd::string_view filter, VStd::string_view name) {
    ...
    inline constexpr bool wildcard_match_case(VStd::string_view filter, VStd::string_view name) {
        return wildcard_match(filter, name, [](char filterChar, char findChar) {
...
#endif // V_FRAMEWORK_CORE_STD_STRING_WILDCARD_H