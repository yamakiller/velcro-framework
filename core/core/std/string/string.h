 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_STRING_STRING_H
#define V_FRAMEWORK_CORE_STD_STRING_STRING_H
...
namespace VStd
{
     ...
     */
    template<class Element, class Traits = char_traits<Element>, class Allocator = VStd::allocator >
    class basic_string
    ...
    public:
        ...
        */
        ...
        {
            assign(VStd::forward<this_type>(rhs));
        }
        ...
        basic_string(VStd::basic_string_view<Element, Traits> view, const Allocator& alloc = Allocator())
            : basic_string(view.begin(), view.end(), alloc)
        {
        }

        // C++23 overload to prevent initializing a string_view via a nullptr or integer type
        constexpr basic_string(VStd::nullptr_t) = delete;
        ...
        operator VStd::basic_string_view<Element, Traits>() const
        {
            return VStd::basic_string_view<Element, Traits>(data(), size());
        }

        inline iterator begin()             { return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer)); }
        inline const_iterator begin() const { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer)); }
        inline const_iterator cbegin() const    { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer)); }
        inline iterator end()                   { return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, (SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer) + m_size)); }
        inline const_iterator end() const       { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, (SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer) + m_size)); }
        inline const_iterator cend() const      { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, (SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer) + m_size)); }
        ...
        inline this_type& operator=(this_type&& rhs) { return assign(VStd::forward<this_type>(rhs)); }
        inline this_type& operator=(VStd::basic_string_view<Element, Traits> view) { return assign(view); }
        inline this_type& operator=(const_pointer ptr)          { return assign(ptr); }
        inline this_type& operator=(Element ch)             { return assign(1, ch); }
        inline this_type& operator=(VStd::nullptr_t) = delete;
        ...
        {   // append rhs [rhsOffset, rhsOffset + count)
            VSTD_CONTAINER_ASSERT(rhs.size() >= rhsOffset, "Invalid offset!");
            size_type num = rhs.m_size - rhsOffset;
            if (num < count)
            {
                count = num;    // trim count to size
            }
            VSTD_CONTAINER_ASSERT(npos - m_size > count && m_size + count >= m_size, "result is too long!");
            num = m_size + count;
            if (count > 0  && grow(num))
            {
        ...
        {
            // append count * ch
            VSTD_CONTAINER_ASSERT(npos - m_size > count, "result is too long");
            size_type num  = m_size + count;
            if (count > 0 && grow(num))
            {
                pointer data = SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer;
        ...
        {   // append [first, last)
            return append_iter(first, last, VStd::is_integral<InputIterator>());
        }
        ...
        {   // assign rhs [rhsOffset, rhsOffset + count)
            VSTD_CONTAINER_ASSERT(rhs.m_size >= rhsOffset, "Invalid offset");
            size_type num = rhs.m_size - rhsOffset;
            if (count < num)
            {
        ...
        {
            // assign count * ch
            VSTD_CONTAINER_ASSERT(count != npos, "result is too long!");
            if (grow(count))
            {   // make room and assign new stuff
                pointer data = SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer;
                if (count == 1)
        ...
        template<class InputIterator>
        inline this_type& assign(InputIterator first, InputIterator last)   { return assign_iter(first, last, VStd::is_integral<InputIterator>()); }
        ...
        this_type& insert(size_type offset, const this_type& rhs, size_type rhsOffset, size_type count)
        {
            ...
            VSTD_CONTAINER_ASSERT(m_size >= offset && rhs.m_size >= rhsOffset, "Invalid offset(s)");
            size_type num = rhs.m_size - rhsOffset;
            if (num < count)
            {
        ...
        this_type&  insert(size_type offset, const_pointer ptr, size_type count)
        {
            ...
            VSTD_CONTAINER_ASSERT(m_size >= offset, "Invalid offset");
            VSTD_CONTAINER_ASSERT(npos - m_size > count, "Result is too long");
            size_type num = m_size + count;
            if (count > 0 && grow(num))
            {   // make room and insert new stuff
        ...
        iterator insert(const_iterator insertPos, Element ch)
        {
            ...
            insert(offset, 1, ch);
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, data + offset));
        }
        ...
        iterator erase(const_iterator erasePos)
        {
            ...
            erase(count, 1);
            data = SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer;
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, data + count));
        }
        ...
        this_type& replace(size_type offset, size_type count, const this_type& rhs, size_type rhsOffset, size_type rhsCount)
        {
            ...
            VSTD_CONTAINER_ASSERT(m_size >= offset && rhs.m_size >= rhsOffset, "Invalid offsets");
            if (m_size - offset < count)
            {
            ...
            VSTD_CONTAINER_ASSERT(npos - rhsCount > m_size - count, "Result is too long");
        ...
        this_type& replace(size_type offset, size_type count, const_pointer ptr, size_type ptrCount)
        {
            ...
            VSTD_CONTAINER_ASSERT(npos - ptrCount > m_size - count, "Result too long");
        ...
        {   // replace [offset, offset + count) with num * ch
            ...
            VSTD_CONTAINER_ASSERT(npos - num > m_size - count, "Result is too long");
            size_type nm = m_size - count - offset;
        ...
        {
            VSTD_CONTAINER_ASSERT(m_size != 0, "VStd::string::front - string is empty!");
            pointer data = SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer;
            return data[0];
        }
        ...
        {
            VSTD_CONTAINER_ASSERT(m_size != 0, "VStd::string::back - string is empty!");
            pointer data = SSO_BUF_SIZE <= m_capacity ? m_data : m_buffer;
        ...
        inline size_type        max_size() const
        {
            ...
            return VStd::allocator_traits<allocator_type>::max_size(m_allocator) / sizeof(value_type);
        }
        ...
        {   // exchange contents with _Right
            ...
            if (m_allocator == rhs.m_allocator)
            {
                ...
                VStd::swap(m_capacity, rhs.m_capacity);
            }
            else
            {
        ...
        {
            V_Assert(ptr != NULL, "Invalid input!");
        ...
        // starts_with
        bool starts_with(VStd::basic_string_view<Element, Traits> prefix) const
        {
            return VStd::basic_string_view<Element, Traits>(data(), size()).starts_with(prefix);
        }
        ...
        // ends_with
        bool ends_with(VStd::basic_string_view<Element, Traits> suffix) const
        {
            return VStd::basic_string_view<Element, Traits>(data(), size()).ends_with(suffix);
        }
            ...
            if (m_capacity != numElements)
            {
                ...
                else
                {
                    ...
                    if (m_capacity >= SSO_BUF_SIZE)
                    {
                        ...
                        V_Assert(expandedSize % sizeof(node_type) == 0, "Expanded size not a multiply of node type. This should not happen");
                    ...
                    VSTD_CONTAINER_ASSERT(newData != 0, "VStd::string allocation failed!");
...
#if V_COMPILER_CLANG || defined(_PREFAST_) || defined(_RELEASE)
#    if V_COMPILER_CLANG
#        define FORMAT_FUNC      __attribute__((format(printf, 1, 2)))
#        define FORMAT_FUNC_ARG
#    elif V_COMPILER_MSVC
#        define FORMAT_FUNC
...
#    undef FORMAT_FUNC_ARG
...
#else // !V_COMPILER_CLANG && !defined(_PREFAST_) && !defined(_RELEASE)
        ...
        static basic_string<char, char_traits<char>, Allocator> format(const char* formatStr, Args... args)
        {
            ...
            {
                constexpr bool isValid = VSTD_IS_CONVERTIBLE(decltype(value), ValidFormatArg);
...
#endif // V_COMPILER_CLANG || defined(_PREFAST_) || defined(_RELEASE)
    ...
    protected:
        ...
        void orphan_range(pointer first, pointer last) const
        {
            ...
            while (iter != 0)
            {
                V_Assert(iter->m_container == static_cast<const checked_container_base*>(this), "basic_string::orphan_range - iterator was corrupted!");
...
#if !defined(V_STRING_EXPLICIT_SPECIALIZATION) && defined(V_COMPILER_MSVC)

    // extern explicit specialization - to speed up build time.
    extern template class basic_string<char>;
    //extern template class basic_string<wchar_t>;

#endif // defined(V_STRING_EXPLICIT_SPECIALIZATION) && defined(V_COMPILER_MSVC)
...
#endif // V_FRAMEWORK_CORE_STD_STRING_STRING_H