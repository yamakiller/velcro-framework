...
#include <core/std/typetraits/type_id.h>
...
#define VSTD_FUNCTION_TARGET_FIX(x)
#define VSTD_FUNCTION_ENABLE_IF_NOT_INTEGRAL(Functor, Type)  VStd::enable_if_t<!std::is_integral_v<Functor> && !std::is_null_pointer_v<Functor>, Type>
...
namespace VStd {
    namespace Internal {
        namespace function_util {
             ...
             * VStd::function. It is a union containing function pointers,
            ...
            union function_buffer {
                ...
                // For pointers to VStd::type_id objects
                struct type_t {
            ...
            /**
             * Determine if VStd::function can use the small-object
            ...
            struct functor_manager_common {
                typedef VStd::decay_t<Functor> functor_type;
            ...
            private:
                ...
                // Function objects that fit in the small-object buffer.
                static inline void
                manager(const function_buffer& in_buffer, function_buffer& out_buffer, functor_manager_operation_type op, VStd::true_type)
                {
                ...
                static inline void
                manager(const function_buffer& in_buffer, function_buffer& out_buffer, functor_manager_operation_type op, VStd::false_type)
                {
                    if (op == clone_functor_tag) {
                        // Clone the functor
                        ...
                        VStd::allocator a;
                        functor_type* new_f = new (a.allocate(sizeof(functor_type), VStd::alignment_of<functor_type>::value))functor_type(*f);
                ...
                {
                    manager(in_buffer, out_buffer, op, VStd::integral_constant<bool, function_allows_small_object_optimization<functor_type>::value >());
                }
                ...
                {
                    manager(in_buffer, out_buffer, op, VStd::true_type());
                }
            ...
            private:
                ...
                manager(const function_buffer& in_buffer, function_buffer& out_buffer, functor_manager_operation_type op, VStd::false_type)
                {
                    ...
                    if (op == clone_functor_tag)
                    {
                        ...
                        functor_wrapper_type* new_f = new (f->allocate(sizeof(functor_wrapper_type), VStd::alignment_of<functor_wrapper_type>::value))functor_wrapper_type(*f);
                        out_buffer.obj_ptr = new_f;
                    } else if (op == move_functor_tag) {
                        out_buffer.obj_ptr = in_buffer.obj_ptr;
                        in_buffer.obj_ptr = 0;
                        ...
                        wrapper_allocator.deallocate(victim, sizeof(functor_wrapper_type), VStd::alignment_of<functor_wrapper_type>::value);
                        out_buffer.obj_ptr = 0;
                    } else if (op == check_functor_type_tag) {
                        const type_id& check_type = out_buffer.type.type;
                        if (vtypeid_cmp(check_type, vtypeid(functor_type)))
    ...
    /**
     * The bad_function_call exception class is thrown when a VStd::function
     * object is invoked
     */
    //class bad_function_call : public std::runtime_error
    //{
    //public:
    //  bad_function_call() : std::runtime_error("call to empty VStd::function") {}
    //};
    ...
    template<typename Functor>
    VSTD_FUNCTION_ENABLE_IF_NOT_INTEGRAL(Functor, bool)
