 ...
 *
 */
#ifndef V_FRAMEWORK_CORE_STD_HASH_TABLE_H
#define V_FRAMEWORK_CORE_STD_HASH_TABLE_H

#include <core/std/containers/node_handle.h>
#include <core/std/containers/vector.h>
#include <core/std/containers/fixed_vector.h>
#include <core/std/containers/list.h>
#include <core/std/containers/fixed_list.h>

#include <core/std/hash.h>
#include <core/std/tuple.h>
#include <core/std/utils.h>
#include <core/std/functional_basic.h>
#include <core/std/allocator_ref.h>
...
namespace VStd
{
    namespace Internal
    {
        ...
        public:
            ...
            typedef VStd::list<typename Traits::value_type, VStd::allocator_ref<allocator_type> >  list_type;
            typedef typename list_type::size_type                                                   size_type;

            typedef VStd::pair<size_type, typename list_type::iterator>                             vector_value_type;
            typedef VStd::vector<vector_value_type, VStd::allocator_ref<allocator_type> >          vector_type;

            V_FORCE_INLINE hash_table_storage(const allocator_type& alloc)
                : m_allocator(alloc)
            ...
            V_FORCE_INLINE hash_table_storage(const this_type& rhs)
            ...
            V_FORCE_INLINE vector_value_type* buckets()            { return m_buckets; }
            V_FORCE_INLINE vector_value_type* buckets()    const   { return m_buckets; }

            V_FORCE_INLINE size_type         get_num_buckets()             { return m_numBuckets; }
            V_FORCE_INLINE size_type         get_num_buckets() const       { return m_numBuckets; }
            ...
            void swap(this_type& rhs)
            {
                ...
                VStd::swap(m_startBucket.first, rhs.m_startBucket.first);
                m_startBucket.second = m_list.begin();
                rhs.m_startBucket.second = rhs.m_list.begin();
                VStd::swap(m_numBuckets, rhs.m_numBuckets);
                VStd::swap(m_max_load_factor, rhs.m_max_load_factor);
            ...
            V_FORCE_INLINE void init_buckets()
            {
            ...
            void        rehash(HashTable* table, size_type numBucketsMin)
            {
                ...
                numBucketsMin = (VStd::max)(numBucketsMin, (size_type)ceilf((float)m_list.size() / m_max_load_factor));
                ...
                Debug::checked_iterator_base* debugIterList;
                {
                    ...
                    V_GLOBAL_SCOPED_LOCK(m_list.get_global_section());
#endif
                ...
                while (!m_list.empty())
                {
                    ...
                    else
                    {
                        ...
                        if (!table->find_insert_position(valueKey, table->m_keyEqual, insertIter, numElements, integral_constant<bool, Traits::has_multi_elements>()))
                        {
                            ...
                            V_Assert(false, "Found a duplicate element when rehashing. "
            ...
            template<class HashTable>
            V_FORCE_INLINE void         rehash_if_needed(HashTable* table)
            {
        ...
        */
        ...
        public:
            typedef typename Traits::allocator_type                                     allocator_type;
            typedef VStd::fixed_list<typename Traits::value_type, Traits::fixed_num_elements>  list_type;
            typedef typename list_type::size_type   size_type;

            typedef VStd::pair<size_type, typename list_type::iterator>                 vector_value_type;
            typedef VStd::fixed_vector<vector_value_type, Traits::fixed_num_buckets>    vector_type;

            V_FORCE_INLINE hash_table_storage(const allocator_type&)
            {
                init_buckets();
            }

            V_FORCE_INLINE hash_table_storage(const this_type&)
            {
                init_buckets();
            }

            V_FORCE_INLINE float           max_load_factor() const                 { return (float)m_vector.size() / (float)m_vector.capacity(); }
            V_FORCE_INLINE void            max_load_factor(float newMaxLoadFactor) { (void)newMaxLoadFactor; }

            V_FORCE_INLINE vector_value_type*          buckets()           { return &m_vector[0]; }
            V_FORCE_INLINE const vector_value_type*    buckets()   const   { return &m_vector[0]; }

            V_FORCE_INLINE size_type         get_num_buckets()             { return m_vector.size(); }
            V_FORCE_INLINE size_type         get_num_buckets() const       { return m_vector.size(); }
            ...
            V_FORCE_INLINE void pre_copy(const this_type&)
            {
                m_vector.assign(m_vector.capacity(), vector_value_type(0));
                m_list.clear();
            }
            ...
            template<class HashTable>
            V_FORCE_INLINE void    rehash_if_needed(HashTable*)    {}

            template<class HashTable>
            V_FORCE_INLINE void    rehash(HashTable*, size_type) {}
    ...
    {
        using allocator_type = typename VStd::allocator_traits<Allocator>::template rebind_alloc<NodeType>;
        using allocator_traits = VStd::allocator_traits<allocator_type>;
    ...
    class hash_table
    {
        ...
        typedef VStd::integral_constant<bool, Traits::is_dynamic>   is_dynamic;
    ...
    public:
        ...
        typedef VStd::pair<iterator, bool>                     pair_iter_bool;
        typedef VStd::pair<iterator, iterator>                 pair_iter_iter;
        typedef VStd::pair<const_iterator, const_iterator>     pair_citer_citer;
        ...
        V_FORCE_INLINE explicit hash_table(const hasher& hash, const key_eq& keyEqual, const allocator_type& alloc = allocator_type())
            : m_data(alloc)
            , m_keyEqual(keyEqual)
            , m_hasher(hash)
        {}
        ...
        V_FORCE_INLINE hash_table(const value_type* first, const value_type* last, const hasher& hash, const key_eq& keyEqual, const allocator_type& alloc)
            : m_data(alloc)
            , m_keyEqual(keyEqual)
            , m_hasher(hash)
        {
        ...
        V_FORCE_INLINE hash_table(const this_type& rhs)
            : m_data(rhs.m_data)
        ...
        V_FORCE_INLINE hash_table(this_type&& rhs)
        ...
        V_FORCE_INLINE iterator            begin()         { return m_data.m_list.begin(); }
        V_FORCE_INLINE const_iterator      begin() const   { return m_data.m_list.begin(); }
        V_FORCE_INLINE iterator            end()           { return m_data.m_list.end(); }
        V_FORCE_INLINE const_iterator      end() const     { return m_data.m_list.end(); }
        ...
        // START UNIQUE
        V_FORCE_INLINE reverse_iterator        rbegin()        { return m_data.m_list.rbegin(); }
        V_FORCE_INLINE const_reverse_iterator  rbegin() const  { return m_data.m_list.rbegin(); }
        V_FORCE_INLINE reverse_iterator        rend()          { return m_data.m_list.rend(); }
        V_FORCE_INLINE const_reverse_iterator  rend() const    { return m_data.m_list.rend(); }
        // END UNIQUE

        V_FORCE_INLINE size_type       size() const                { return m_data.m_list.size(); }
        V_FORCE_INLINE size_type       max_size() const            { return m_data.m_list.max_size(); }
        V_FORCE_INLINE bool            empty() const               { return m_data.m_list.empty(); }
        V_FORCE_INLINE key_eq          key_equal() const           { return m_keyEqual; }
        V_FORCE_INLINE hasher          get_hasher() const          { return m_hasher; }
        V_FORCE_INLINE size_type       bucket_count() const        { return m_data.get_num_buckets(); }
        V_FORCE_INLINE size_type       max_bucket_count() const    { return m_data.m_vector.max_size(); }
        V_FORCE_INLINE size_type       bucket(const key_type& keyValue) const { return bucket_from_hash(m_hasher(keyValue)); }
        V_INLINE size_type             bucket_size(size_type bucket) const { return m_data.buckets()[bucket].first; }

        V_FORCE_INLINE float           load_factor() const         {   return ((float)m_data.m_list.size() / (float)m_data.get_num_buckets()); }
        V_FORCE_INLINE float           max_load_factor() const     {   return m_data.max_load_factor();    }
        V_FORCE_INLINE void            max_load_factor(float newMaxLoadFactor) { m_data.max_load_factor(newMaxLoadFactor); m_data.rehash_if_needed(this); }
        V_FORCE_INLINE pair_iter_bool  insert(const value_type& value) { return insert_impl(value); } // try to insert node with value value
        V_FORCE_INLINE iterator        insert(const_iterator, const value_type& value)
        {
        ...
        V_FORCE_INLINE pair_iter_bool  insert(value_type&& value)
        {
            return insert_impl(VStd::forward<value_type>(value));
        }
        V_FORCE_INLINE iterator  insert(const_iterator, value_type&& value)
        {
            return insert_impl(VStd::forward<value_type>(value)).first;
        }

        template <typename... Args>
        V_FORCE_INLINE pair_iter_bool emplace(Args&&... arguments)
        {
            return insert_impl_emplace(VStd::forward<Args>(arguments)...);
        }

        V_FORCE_INLINE void insert(std::initializer_list<value_type> list)
        {
        ...
        void            insert(Iterator first, Iterator last)
        {
            ...
            for (; first != last; ++first)
            {
                ...
                {
                    if (!find_insert_position(valueKey, m_keyEqual, iter, numElements, VStd::integral_constant<bool, Traits::has_multi_elements>()))
                    {
                        continue;
                    }
    ...
    public:
        ...
        V_FORCE_INLINE void            reserve(size_type numBucketsMin) { rehash((size_type)::ceilf((float)numBucketsMin / max_load_factor())); }
        V_FORCE_INLINE void            rehash(size_type numBucketsMin) { m_data.rehash(this, numBucketsMin); }
        ...
        iterator        erase(const_iterator erasePos)
        {
            ...
            V_Assert(bucket.first >= 1, "VStd::hash_table::erase - This bucket has not elements!");
            --bucket.first;
        ...
        size_type       erase(const key_type& keyValue)
        {
            ...
            for (size_type i = 0; i < numElements; ++i, ++eraseFirst)
            {
                ...
                {
                    size_type iNext = i + 1;
                    for (eraseLast = VStd::next(eraseFirst); iNext < numElements && m_keyEqual(Traits::key_from_value(*eraseLast), keyValue); ++eraseLast, ++iNext)
                    {
                    }
                    numErased = iNext - i;
                    break;
            ...
            if (numErased > 0)
            {
                ...
                {
                    VStd::advance(bucket.second, numErased);
                }
        ...
        iterator        erase(const_iterator first, const_iterator last)
        {
            ...
            else
            {
                ...
                return VStd::Internal::ConstIteratorCast<iterator>(first);
            }
        }
        ...
        auto find(const ComparableToKey& key)
            -> enable_if_t<(Internal::is_transparent<key_eq, ComparableToKey>::value && Internal::is_transparent<hasher, ComparableToKey>::value) || VStd::is_convertible_v<ComparableToKey, key_type>, iterator>
        {
        ...
        auto find(const ComparableToKey& key) const
            -> enable_if_t<(Internal::is_transparent<key_eq, ComparableToKey>::value && Internal::is_transparent<hasher, ComparableToKey>::value) || VStd::is_convertible_v<ComparableToKey, key_type>, const_iterator>
        {
            return lower_bound(key);
        }
        ...
        auto contains(const ComparableToKey& key) const
            -> enable_if_t<(Internal::is_transparent<key_eq, ComparableToKey>::value && Internal::is_transparent<hasher, ComparableToKey>::value) || VStd::is_convertible_v<ComparableToKey, key_type>, bool>
        {
            return find(key) != end();
        }
        ...
        auto count(const ComparableToKey& key) const
            -> enable_if_t<(Internal::is_transparent<key_eq, ComparableToKey>::value && Internal::is_transparent<hasher, ComparableToKey>::value) || VStd::is_convertible_v<ComparableToKey, key_type>, size_type>
        {   // count all elements that match keyValue
            pair_citer_citer pair = equal_range(key);
            size_type num = VStd::distance(pair.first, pair.second);
            return num;
        }
        ...
        auto equal_range(const ComparableToKey& key)
            -> enable_if_t<(Internal::is_transparent<key_eq, ComparableToKey>::value && Internal::is_transparent<hasher, ComparableToKey>::value) || VStd::is_convertible_v<ComparableToKey, key_type>, VStd::pair<iterator, iterator>>
        {
            vector_value_type& bucket = m_data.buckets()[bucket_from_hash(m_hasher(key))];
            iterator iter = bucket.second;
            size_type numElements = bucket.first;
        ...
        auto equal_range(const ComparableToKey& key) const
            -> enable_if_t<(Internal::is_transparent<key_eq, ComparableToKey>::value && Internal::is_transparent<hasher, ComparableToKey>::value) || VStd::is_convertible_v<ComparableToKey, key_type>, VStd::pair<const_iterator, const_iterator>>
        {
            VStd::pair<iterator, iterator> non_const_range = const_cast<hash_table*>(this)->equal_range(key);
        ...
        */
        ...
        V_FORCE_INLINE allocator_type&         get_allocator()         { return m_data.m_allocator; }
        V_FORCE_INLINE const allocator_type&   get_allocator() const   { return m_data.m_allocator; }
        ...
        * \ref VStdExamples. The main idea is that you don't to create unnecessary expensive temporaries.
        */
        ...
        pair_iter_bool insert_from(const U& userValue, const Converter& convert, const Hasher& hash, const KeyEqual& keyEq)
        {
            ...
            {
                if (!find_insert_position(valueKey, keyEq, iter, numElements, VStd::integral_constant<bool, Traits::has_multi_elements>()))
                {
    ...
    protected:
        V_FORCE_INLINE size_type bucket_from_hash(const size_type key) const   { return key % m_data.get_num_buckets(); }
        ...
        void copy(const this_type& rhs)
        {
            ...
            for (size_type iBucket = 0; iBucket < rhs.m_data.get_num_buckets(); ++iBucket)
            {
                ...
                if (srcBucket.first > 0)
                {
                    ...
                    // copy the elements
                    m_data.m_list.insert(m_data.m_list.begin(), srcBucket.second, VStd::next(srcBucket.second, srcBucket.first));
                    bucket.second = m_data.m_list.begin();
        ...
        pair_iter_bool insert_impl(T&& value)
        {
            ...
            if (numElements == 0)
            {
                m_data.m_list.push_front(VStd::forward<T>(value));
                insertPos = m_data.m_list.begin();
                bucket.second = insertPos;
            }
            else
            {
                ...
                insertPos = m_data.m_list.insert(iter, VStd::forward<T>(value));
            }
            ++numElements;
        ...
        pair_iter_bool insert_impl_emplace(Args&&... arguments)
        {
            ...
            m_data.m_list.emplace_front(VStd::forward<Args>(arguments)...);
        ...
        */
        struct ConvertFromValue
        {
            ...
            static V_FORCE_INLINE const key_type&      to_key(const value_type& value)     { return Traits::key_from_value(value); }
            static V_FORCE_INLINE const value_type&    to_value(const value_type& value)   { return value; }
        };
    };
    ...
    InsertReturnType hash_table<Traits>::node_handle_insert(NodeHandle&& nodeHandle)
    {
        ...
        {
            if (!find_insert_position(valueKey, m_keyEqual, iter, numElements, VStd::bool_constant<Traits::has_multi_elements>()))
            {
                ...
                return { iter, false, VStd::move(nodeHandle) };
            }
    ...
    {
        return node_handle_insert<insert_return_type<iterator, NodeHandle>>(VStd::move(nodeHandle)).position;
    }
    ...
    inline auto hash_table<Traits>::try_emplace_transparent(ComparableToKey&& key, Args&&... arguments) -> pair_iter_bool
    {
        ...
        iterator insertIter = VStd::next(bucketFrontIter, numElements);
        ...
        if (numElements == 0)
        {
            ...
            insertIter = m_data.m_list.emplace(m_data.m_list.begin(), VStd::piecewise_construct, VStd::forward_as_tuple(VStd::forward<ComparableToKey>(key)),
                VStd::forward_as_tuple(VStd::forward<Args>(arguments)...));
            bucketFrontIter = insertIter;
        }
        else
        {
            // The returned iterator from list::emplace is pointing at the newly inserted element
            insertIter = m_data.m_list.emplace(insertIter, VStd::piecewise_construct, VStd::forward_as_tuple(VStd::forward<ComparableToKey>(key)),
                VStd::forward_as_tuple(VStd::forward<Args>(arguments)...));
        }
    ...
    {
        return try_emplace_transparent(VStd::forward<ComparableToKey>(key), VStd::forward<Args>(arguments)...).first;
    }
    ...
    inline auto hash_table<Traits>::insert_or_assign_transparent(ComparableToKey&& key, MappedType&& value) -> pair_iter_bool
    {
        ...
        if (iterator findIter = find(key); findIter != m_data.m_list.end())
        {
            ...
            findIter->second = VStd::forward<MappedType>(value);
            return { findIter, false };
        }
        ...
        if (numElements == 0)
        {
            ...
            // emplace the element at the beginning of the list and update the bucket front iterator to point to it
            insertIter = m_data.m_list.emplace(m_data.m_list.begin(), VStd::piecewise_construct, VStd::forward_as_tuple(VStd::forward<ComparableToKey>(key)),
                VStd::forward_as_tuple(VStd::forward<MappedType>(value)));
            bucketFrontIter = insertIter;
        }
        else
        {
    ...
    {
        return insert_or_assign_transparent(VStd::forward<ComparableToKey>(key), VStd::forward<MappedType>(value)).first;
    }
}

#endif // V_FRAMEWORK_CORE_STD_HASH_TABLE_H