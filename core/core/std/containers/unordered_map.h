 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_UNORDERED_MAP_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_UNORDERED_MAP_H

#include <core/std/containers/node_handle.h>
#include <core/std/hash_table.h>
...
namespace VStd {
    namespace Internal {
         ...
         * value. This rely on that VStd::pair (map value type) can be constructed with a key only (first element).
         */
        ...
        struct ConvertKeyType
        {
            ...
            V_FORCE_INLINE const KeyType&      to_key(const KeyType& key) const { return key; }
            // We return key as the value so the pair is constructed using Pair(first) ctor.
            V_FORCE_INLINE const KeyType&      to_value(const KeyType& key) const  { return key; }
        };
    }
     ...
     * Check the unordered_map \ref VStdExamples.
     */
    ...
    public:
        ...
        V_FORCE_INLINE unordered_map()
            : base_type(hasher(), key_eq(), allocator_type()) {}
        explicit unordered_map(const allocator_type& alloc)
            : base_type(hasher(), key_eq(), alloc) {}
        V_FORCE_INLINE unordered_map(const unordered_map& rhs)
            : base_type(rhs) {}
        /// This constructor is VStd extension (so we don't rehash/allocate memory)
        V_FORCE_INLINE unordered_map(const hasher& hash, const key_eq& keyEqual, const allocator_type& allocator)
            : base_type(hash, keyEqual, allocator) {}
        V_FORCE_INLINE unordered_map(size_type numBucketsHint)
            : base_type(hasher(), key_eq(), allocator_type())
        {
            base_type::rehash(numBucketsHint);
        }
        V_FORCE_INLINE unordered_map(size_type numBucketsHint, const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual, allocator_type())
        {
            base_type::rehash(numBucketsHint);
        }
        V_FORCE_INLINE unordered_map(size_type numBucketsHint, const hasher& hash, const key_eq& keyEqual, const allocator_type& allocator)
            : base_type(hash, keyEqual, allocator)
        {
            base_type::rehash(numBucketsHint);
        }
        template<class Iterator>
        V_FORCE_INLINE unordered_map(Iterator first, Iterator last)
            : base_type(hasher(), key_eq(), allocator_type())
        {
        ...
        template<class Iterator>
        V_FORCE_INLINE unordered_map(Iterator first, Iterator last, size_type numBucketsHint)
            : base_type(hasher(), key_eq(), allocator_type())
        {
            base_type::rehash(numBucketsHint);
            for (; first != last; ++first)
        ...
        template<class Iterator>
        V_FORCE_INLINE unordered_map(Iterator first, Iterator last, size_type numBucketsHint, const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual, allocator_type())
        {
            base_type::rehash(numBucketsHint);
            for (; first != last; ++first)
        ...
        template<class Iterator>
        V_FORCE_INLINE unordered_map(Iterator first, Iterator last, size_type numBucketsHint, const hasher& hash, const key_eq& keyEqual, const allocator_type& allocator)
            : base_type(hash, keyEqual, allocator)
        {
            base_type::rehash(numBucketsHint);
            for (; first != last; ++first)
        ...
        V_FORCE_INLINE unordered_map(const std::initializer_list<value_type>& list, const hasher& hash = hasher(), const key_eq& keyEqual = key_eq(), const allocator_type& allocator = allocator_type())
            : base_type(hash, keyEqual, allocator)
        {
        ...
        V_FORCE_INLINE unordered_map(this_type&& rhs)
            : base_type(VStd::move(rhs))
        {
        }

        this_type& operator=(this_type&& rhs)
        {
            base_type::operator=(VStd::move(rhs));
            return *this;
        }
         ...
         */
        V_FORCE_INLINE mapped_type& operator[](const key_type& key)
        {
         ...
         */
        V_FORCE_INLINE mapped_type& at(const key_type& key)
        {
            iterator iter = base_type::find(key);
            VSTD_CONTAINER_ASSERT(iter != base_type::end(), "Element with key is not present");
            return iter->second;
        }
        V_FORCE_INLINE const mapped_type& at(const key_type& key) const
        {
        ...
        {
            VSTD_CONTAINER_ASSERT(nodeHandle.empty() || nodeHandle.get_allocator() == base_type::get_allocator(),
                "node_type with incompatible allocator passed to unordered_map::insert(node_type&& nodeHandle)");
            return base_type::template node_handle_insert<insert_return_type>(VStd::move(nodeHandle));
        }
        iterator insert(const_iterator hint, node_type&& nodeHandle)
        {
            ...
            return base_type::node_handle_insert(hint, VStd::move(nodeHandle));
        }
        ...
        {
            return base_type::insert_or_assign_transparent(key, VStd::forward<M>(value));
        }
        template <typename M>
        pair_iter_bool insert_or_assign(key_type&& key, M&& value)
        {
            return base_type::insert_or_assign_transparent(VStd::move(key), VStd::forward<M>(value));
        }
        template <typename M>
        iterator insert_or_assign(const_iterator hint, const key_type& key, M&& value)
        {
            return base_type::insert_or_assign_transparent(hint, key, VStd::forward<M>(value));
        }
        template <typename M>
        iterator insert_or_assign(const_iterator hint, key_type&& key, M&& value)
        {
            return base_type::insert_or_assign_transparent(hint, VStd::move(key), VStd::forward<M>(value));
        }
        ...
        {
            return base_type::try_emplace_transparent(key, VStd::forward<Args>(arguments)...);
        }
        template <typename... Args>
        pair_iter_bool try_emplace(key_type&& key, Args&&... arguments)
        {
            return base_type::try_emplace_transparent(VStd::move(key), VStd::forward<Args>(arguments)...);
        }
        template <typename... Args>
        iterator try_emplace(const_iterator hint, const key_type& key, Args&&... arguments)
        {
            return base_type::try_emplace_transparent(hint, key, VStd::forward<Args>(arguments)...);
        }
        template <typename... Args>
        iterator try_emplace(const_iterator hint, key_type&& key, Args&&... arguments)
        {
            return base_type::try_emplace_transparent(hint, VStd::move(key), VStd::forward<Args>(arguments)...);
        }
         ...
         */
        V_FORCE_INLINE pair_iter_bool insert_key(const key_type& key)
        {
    ...
    V_FORCE_INLINE void swap(unordered_map<Key, MappedType, Hasher, EqualKey, Allocator>& left, unordered_map<Key, MappedType, Hasher, EqualKey, Allocator>& right)
    {
        left.swap(right);
    }

    template <class Key, class MappedType, class Hasher, class EqualKey, class Allocator>
    V_FORCE_INLINE bool operator==(const unordered_map<Key, MappedType, Hasher, EqualKey, Allocator>& a, const unordered_map<Key, MappedType, Hasher, EqualKey, Allocator>& b)
    {
     ...
     *
     * Check the unordered_multimap \ref VStdExamples.
     */
    template<class Key, class MappedType, class Hasher = VStd::hash<Key>, class EqualKey = VStd::equal_to<Key>, class Allocator = VStd::allocator >
    ...
    public:
        ...
        V_FORCE_INLINE unordered_multimap()
            : base_type(hasher(), key_eq(), allocator_type()) {}
        explicit unordered_multimap(const allocator_type& alloc)
            : base_type(hasher(), key_eq(), alloc){}
        V_FORCE_INLINE unordered_multimap(const unordered_multimap& rhs)
            : base_type(rhs) {}
        /// This constructor is VStd extension (so we don't rehash/allocate memory)
        V_FORCE_INLINE unordered_multimap(const hasher& hash, const key_eq& keyEqual, const allocator_type& allocator)
            : base_type(hash, keyEqual, allocator) {}
        V_FORCE_INLINE unordered_multimap(size_type numBuckets)
            : base_type(hasher(), key_eq(), allocator_type())
        {
            base_type::rehash(numBuckets);
        }
        V_FORCE_INLINE unordered_multimap(size_type numBuckets, const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual, allocator_type())
        {
            base_type::rehash(numBuckets);
        }
        V_FORCE_INLINE unordered_multimap(size_type numBuckets, const hasher& hash, const key_eq& keyEqual, const allocator_type& allocator)
            : base_type(hash, keyEqual, allocator)
        {
            base_type::rehash(numBuckets);
        }
        template<class Iterator>
        V_FORCE_INLINE unordered_multimap(Iterator first, Iterator last)
            : base_type(hasher(), key_eq(), allocator_type())
        {
            for (; first != last; ++first)
            {
        ...
        template<class Iterator>
        V_FORCE_INLINE unordered_multimap(Iterator first, Iterator last, size_type numBuckets)
            : base_type(hasher(), key_eq(), allocator_type())
        {
            base_type::rehash(numBuckets);
            for (; first != last; ++first)
        ...
        template<class Iterator>
        V_FORCE_INLINE unordered_multimap(Iterator first, Iterator last, size_type numBuckets, const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual, allocator_type())
        {
            base_type::rehash(numBuckets);
            for (; first != last; ++first)
        ...
        template<class Iterator>
        V_FORCE_INLINE unordered_multimap(Iterator first, Iterator last, size_type numBuckets, const hasher& hash, const key_eq& keyEqual, const allocator_type& allocator)
            : base_type(hash, keyEqual, allocator)
        {
            base_type::rehash(numBuckets);
            for (; first != last; ++first)
        ...
        V_FORCE_INLINE unordered_multimap(const std::initializer_list<value_type>& list, const hasher& hash = hasher(), const key_eq& keyEqual = key_eq(), const allocator_type& allocator = allocator_type())
            : base_type(hash, keyEqual, allocator)
        {
            base_type::rehash(list.size());
            for (const value_type& i : list)
        ...
        V_FORCE_INLINE unordered_multimap(this_type&& rhs)
            : base_type(VStd::move(rhs))
        {
        }
        ...
        V_FORCE_INLINE iterator insert(const value_type& value)
        {
        ...
        iterator insert(node_type&& nodeHandle)
        {
            ...
            using insert_return_type = insert_return_type<iterator, node_type>;
            return base_type::template node_handle_insert<insert_return_type>(VStd::move(nodeHandle)).position;
        }
        iterator insert(const_iterator hint, node_type&& nodeHandle)
        {
            VSTD_CONTAINER_ASSERT(nodeHandle.empty() || nodeHandle.get_allocator() == base_type::get_allocator(),
    ...
    template<class Key, class MappedType, class Hasher, class EqualKey, class Allocator >
    V_FORCE_INLINE void swap(unordered_multimap<Key, MappedType, Hasher, EqualKey, Allocator>& left, unordered_multimap<Key, MappedType, Hasher, EqualKey, Allocator>& right)
    {
        left.swap(right);
    }

    template <class Key, class MappedType, class Hasher, class EqualKey, class Allocator>
    V_FORCE_INLINE bool operator==(const unordered_multimap<Key, MappedType, Hasher, EqualKey, Allocator>& a, const unordered_multimap<Key, MappedType, Hasher, EqualKey, Allocator>& b)
    {
        return (a.size() == b.size() && equal(a.begin(), a.end(), b.begin()));
    }
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_UNORDERED_MAP_H