#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_ARRAY_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_ARRAY_H
...
// Because the V_Assert macro invokes the "V::Debug::Trace::Instance().GetAssertNoOp()" function
...
#define VSTD_CONTAINER_COMPILETIME_ASSERT(expression, ...) \
    if (!(expression)) \
    { \
        V_Assert(expression, __VA_ARGS__); \
    }
...
namespace VStd {
     ...
     * Check the array \ref VStdExamples.
     */
    ...
    public:
        ...
        constexpr reference operator[](size_type i) {
            VSTD_CONTAINER_COMPILETIME_ASSERT(i < N, "out of range");
            return m_elements[i];
        }
        ...
        constexpr reference at(size_type i) { VSTD_CONTAINER_COMPILETIME_ASSERT(i < N, "out of range");  return m_elements[i]; }
        constexpr const_reference at(size_type i) const { VSTD_CONTAINER_COMPILETIME_ASSERT(i < N, "out of range");  return m_elements[i]; }
        ...
        // swap
        void swap(this_type& other) { VStd::swap_ranges(m_elements, pointer(m_elements + N), other.m_elements); }
        ...
        {
            VStd::Internal::copy(rhs.m_elements, rhs.m_elements + N, m_elements);
            return *this;
        }

        constexpr void fill(const T& value)
        {
            VStd::fill_n(m_elements, N, value);
        }
    ...
    public:
        ...
        constexpr reference front() { VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0");  return m_elements[0]; }
        constexpr const_reference front() const { VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0"); return m_elements[0]; }
        constexpr reference back() { VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0"); return m_elements[0]; }
        constexpr const_reference back() const { VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0"); return m_elements[0]; }

        constexpr reference operator[](size_type)
        {
            VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0");
            return m_elements[0];
        }
        ...
        constexpr reference at(size_type) { VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0");  return m_elements[0]; }
        constexpr const_reference at(size_type) const { VSTD_CONTAINER_COMPILETIME_ASSERT(false, "out of range. Cannot access elements in an array of size 0");  return m_elements[0]; }
    ...
    // Deduction Guide for VStd::array
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_ARRAY_H