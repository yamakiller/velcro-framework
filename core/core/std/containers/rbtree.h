 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_RBTREE_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_RBTREE_H
...
namespace VStd
{
    #define VSTD_RBTREE_RED    0
    #define VSTD_RBTREE_BLACK  1
    ...
    namespace Internal
    {
        struct rbtree_node_base
        {
            ...
            V_FORCE_INLINE color_type get_color() const
            {
                return static_cast<color_type>(reinterpret_cast<VStd::size_t>(m_parentColor) & 1);
            }
            V_FORCE_INLINE void set_color(color_type color)
            {
                VStd::size_t parentColor = reinterpret_cast<VStd::size_t>(m_parentColor);
                parentColor &= ~(VStd::size_t)1;
                parentColor |= (VStd::size_t)color;
                m_parentColor = reinterpret_cast<BaseNodePtr>(parentColor);
            }
            V_FORCE_INLINE BaseNodePtr get_parent() const
            {
                VStd::size_t parentColor = reinterpret_cast<VStd::size_t>(m_parentColor);
                parentColor &= ~(VStd::size_t)1;
                return reinterpret_cast<BaseNodePtr>(parentColor);
            }
            V_FORCE_INLINE void set_parent(BaseNodePtr parent)
            {
                VSTD_CONTAINER_ASSERT((reinterpret_cast<VStd::size_t>(parent) & 1) == 0, "We require nodes to be at least 2 bytes aligned!");
                VStd::size_t newParentColor = reinterpret_cast<VStd::size_t>(parent);
                VStd::size_t parentColor = reinterpret_cast<VStd::size_t>(m_parentColor);
                parentColor &= 1;
            ...
            {
                color_type col = static_cast<color_type>(reinterpret_cast<VStd::size_t>(m_parentColor) & 1);
                color_type rhsCol = static_cast<color_type>(reinterpret_cast<VStd::size_t>(rhs.m_parentColor) & 1);
                col ^= rhsCol;
                rhsCol ^= col;
                col ^= rhsCol;
                VStd::size_t parentColor = reinterpret_cast<VStd::size_t>(m_parentColor);
                VStd::size_t rhsParentColor = reinterpret_cast<VStd::size_t>(rhs.m_parentColor);
                parentColor &= ~(VStd::size_t)1;
                rhsParentColor &= ~(VStd::size_t)1;
                parentColor |= (VStd::size_t)col;
                rhsParentColor |= (VStd::size_t)rhsCol;
                m_parentColor = reinterpret_cast<BaseNodePtr>(parentColor);
                rhs.m_parentColor = reinterpret_cast<BaseNodePtr>(rhsParentColor);
            }


            V_FORCE_INLINE BaseNodePtr minimum()
            {
            ...
            V_FORCE_INLINE BaseNodePtr maximum()
            {
                BaseNodePtr node = this;
    ...
    public:
        typedef VStd::bidirectional_iterator_tag   iterator_category;
        ...
        V_FORCE_INLINE rbtree_const_iterator()
            : m_node(0) {}
        V_FORCE_INLINE rbtree_const_iterator(base_node_ptr_type baseNode)
            : m_node(static_cast<node_ptr_type>(baseNode)) {}
        V_FORCE_INLINE rbtree_const_iterator(node_ptr_type node)
            : m_node(node) {}
        //copy constructor for iterator and constructor from iterator for const_iterator
        V_FORCE_INLINE rbtree_const_iterator(const rbtree_const_iterator& rhs)
            : m_node(rhs.m_node) {}

        V_FORCE_INLINE reference operator*() const { return m_node->m_value;  }
        V_FORCE_INLINE pointer operator->() const  { return &(m_node->m_value); }
        ...
        inline this_type& operator--()
        {
            if (m_node->get_color() == VSTD_RBTREE_RED && m_node->get_parent()->get_parent() == m_node)
            {
        ...
        V_FORCE_INLINE bool operator == (this_type rhs) const { return m_node == rhs.m_node; }
        V_FORCE_INLINE bool operator != (this_type rhs) const { return m_node != rhs.m_node;  }
    protected:
    ...
    public:
        ...
        V_FORCE_INLINE rbtree_iterator() {}
        V_FORCE_INLINE rbtree_iterator(base_node_ptr_type baseNode)
            : rbtree_const_iterator<T>(baseNode) {}
        V_FORCE_INLINE rbtree_iterator(node_ptr_type node)
            : rbtree_const_iterator<T>(node) {}
        V_FORCE_INLINE reference operator*() const { return base_type::m_node->m_value; }
        V_FORCE_INLINE pointer operator->() const { return &(base_type::m_node->m_value); }
        V_FORCE_INLINE this_type& operator++() { base_type::operator++();  return *this;   }
        V_FORCE_INLINE this_type operator++(int)
        {
            this_type temp = *this;
            ++*this;
     ...
     * VStd extensions and requirements, we use compressed node which saves about ~25%
     * of the tree overhead. This is the base container used for VStd::set,VStd::multiset,VStd::map and VStd::multimap.
     *
    ...
    public:
        V_FORCE_INLINE rbtree()
            : m_numElements(0)
            , m_keyEq(key_eq())
            , m_allocator(allocator_type())
        {
            m_head.set_color(VSTD_RBTREE_RED); // used to distinguish header from root, in iterator.operator++
            m_head.set_parent(0);
        ...
        V_FORCE_INLINE rbtree(const key_eq& keyEq, const allocator_type& allocator)
            : m_numElements(0)
            , m_keyEq(keyEq)
            , m_allocator(allocator)
        {
        ...
        {
            m_head.set_color(VSTD_RBTREE_RED);
        ...
        // accessors:
        V_FORCE_INLINE key_eq  key_comp() const                { return m_keyEq; }

        V_FORCE_INLINE iterator begin()                        { return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, m_head.m_left)); }
        V_FORCE_INLINE const_iterator begin() const            { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, m_head.m_left)); }
        ...
        rbtree(this_type&& rhs)
            ...
            , m_keyEq(VStd::move(rhs.m_keyEq))
            , m_allocator(VStd::move(rhs.m_allocator))
        {
            m_head.set_color(VSTD_RBTREE_RED); // used to distinguish header from root, in iterator.operator++
            m_head.set_parent(0);
            m_head.m_left = &m_head;
        ...
        VStd::pair<iterator, bool> insert_unique(value_type&& value)
        {
            ...
            if (insertNode != &m_head) // !end()
            {
                if (m_keyEq(Traits::key_from_value(value), Traits::key_from_value(static_cast<node_ptr_type>(insertNode)->m_value)))
                {
                    ...
                    insertPos = insert_unique_node(insertPos, create_node(VStd::forward<value_type>(value)));
                    isInserted = true;
                }
            }
            else
            ...
            return VStd::pair<iterator, bool>(insertPos, isInserted);
        }
        ...
        iterator insert_unique(const_iterator insertPos, value_type&& value)
        {
            ...
            node_ptr_type newNode = static_cast<node_ptr_type>(create_node(VStd::move(value)));
        ...
        {
            return insert_equal_node(create_node(VStd::forward<value_type>(value)));
        }

        template<class ... InputArguments>
        VStd::pair<iterator, bool> emplace_unique(InputArguments&& ... arguments)
        {
            // we need to construct the node just to get the key/value
            node_ptr_type newNode = static_cast<node_ptr_type>(create_node(VStd::forward<InputArguments>(arguments) ...));
            VStd::pair<iterator, bool> result = insert_unique_node(newNode);
            if (!result.second)
            {
                pointer toDestroy = &newNode->m_value;
                Internal::destroy<pointer>::single(toDestroy);
        ...
        {
            return insert_equal_node(create_node(VStd::forward<InputArguments>(arguments) ...));
        }

        template<class ... InputArguments>
        iterator emplace_equal(const_iterator insertPos, InputArguments&& ... arguments)
        {
            return insert_equal_node(insertPos, create_node(VStd::forward<InputArguments>(arguments) ...));
        }
        ...
        // insert/erase
        VStd::pair<iterator, bool> insert_unique(const value_type& value)
        {
            bool isInserted = false;
            iterator insertPos = lower_bound(Traits::key_from_value(value));
            base_node_ptr_type insertNode = insertPos.m_node;
        ...
        V_FORCE_INLINE void insert_equal(Iterator first, Iterator last)
        {
        ...
        template<class Iterator>
        V_FORCE_INLINE void insert_unique(Iterator first, Iterator last)
        {
            for (; first != last; ++first)
            {
                insert_unique(*first);
        ...
        VStd::pair<iterator, bool> try_emplace_unique(ComparableToKey&& key, Args&&... arguments);
        ...
        VStd::pair<iterator, bool> insert_or_assign_unique(ComparableToKey&& key, MappedType&& value);
...
#endif
            iterator next(VSTD_CHECKED_ITERATOR(iterator_impl, eraseNode));
            ++next;
        ...
        {
            VStd::pair<iterator, iterator> p = equal_range(key);
            size_type n = VStd::distance(p.first, p.second);
        ...
        inline iterator erase(const_iterator first, const_iterator last)
        {
            ...
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, lastNode));
        }

        V_FORCE_INLINE void erase(const key_type* first, const key_type* last)
        {
            while (first != last)
            {
                erase(*first++);
        ...
        auto find(const ComparableToKey& key)
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, iterator>
        {
        ...
        auto find(const ComparableToKey& key) const
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, const_iterator>
        {
        ...
        auto contains(const ComparableToKey& key) const
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, bool>
        {
            return find(key) != end();
        }
        ...
        auto count(const ComparableToKey& key) const
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, size_type>
        {
            VStd::pair<const_iterator, const_iterator> p = equal_range(key);
            return VStd::distance(p.first, p.second);
        }

        template<class ComparableToKey>
        auto equal_range(const ComparableToKey& key)
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, VStd::pair<iterator, iterator>>
        {
            return { lower_bound(key), upper_bound(key) };
        }
        template<class ComparableToKey>
        auto equal_range(const ComparableToKey& key) const
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, VStd::pair<const_iterator, const_iterator>>
        {
            return { lower_bound(key), upper_bound(key) };
        }
        ...
        auto equal_range_unique(const ComparableToKey& key) const
            -> enable_if_t<Internal::is_transparent<key_eq, ComparableToKey>::value || VStd::is_convertible_v<ComparableToKey, key_type>, VStd::pair<const_iterator, const_iterator>>
        {
            VStd::pair<iterator, iterator> non_const_range(const_cast<rbtree*>(this)->equal_range_unique(key));
    ...
    private:
        inline base_node_ptr_type create_node(const value_type& value)
        {
            ...
            VSTD_CONTAINER_ASSERT(newNode != NULL, "VStd::rb_tree::create_node - failed to allocate node!");
        ...
        {
            node_ptr_type newNode = reinterpret_cast<node_ptr_type>(m_allocator.allocate(sizeof(node_type), alignment_of<node_type>::value));
            VSTD_CONTAINER_ASSERT(newNode, "VStd::rb_tree::create_node - failed to allocate node!");

            pointer ptr = &newNode->m_value;
            Internal::construct<pointer>::single(ptr, VStd::forward<InputArguments>(arguments) ...);
        ...
        V_FORCE_INLINE base_node_ptr_type clone_node(base_node_ptr_type node)
        {
        ...
        {
            base_node_ptr_type tmp = create_node(VStd::move(static_cast<node_ptr_type>(node)->m_value));
            tmp->set_color(node->get_color());
            return tmp;
        }
        ...
        VStd::pair<iterator, bool> insert_unique_node(base_node_ptr_type newNode)
        {
            base_node_ptr_type y = &m_head;
            base_node_ptr_type x = m_head.get_parent();
            bool comp = true;
            ...
            iterator j(VSTD_CHECKED_ITERATOR(iterator_impl, y));

            if (comp)
            {
                if (j == begin())
                {
                    return VStd::pair<iterator, bool>(insert_node(y, newNode, y), true);
                }
                else
                {
                    --j;
            ...
            {
                return VStd::pair<iterator, bool>(insert_node(y, newNode, x), true);
            }
            return VStd::pair<iterator, bool>(j, false);
        }
        ...
        iterator insert_unique_node(const_iterator insertPos, base_node_ptr_type newNode)
        {
            ...
            { // begin()
              // if the container is empty, fall back on insert_unique.
                ...
                else
                {
                    ...
                    {
                        return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, insertPos.m_node));
                    }
                    //Below __comp_pos_v == true

                    // Standard-conformance - does the insertion point fall immediately AFTER
                    // the hint?
                    iterator after(VSTD_CHECKED_ITERATOR(iterator_impl, insertPos.m_node));
                    ++after;
#ifdef VSTD_HAS_CHECKED_ITERATORS
            ...
            {
                iterator before(VSTD_CHECKED_ITERATOR(iterator_impl, insertPos.m_node));
                --before;
#ifdef VSTD_HAS_CHECKED_ITERATORS
        ...
        {
            x->set_color(VSTD_RBTREE_RED);
            base_node_ptr_type xParent = x->get_parent();
            while (x != m_head.get_parent() && xParent->get_color() == VSTD_RBTREE_RED)
            {
                ...
                if (xParent == xParentParent->m_left)
                {
                    ...
                    if (y && y->get_color() == VSTD_RBTREE_RED)
                    {
                        xParent->set_color(VSTD_RBTREE_BLACK);
                        y->set_color(VSTD_RBTREE_BLACK);
                        xParentParent->set_color(VSTD_RBTREE_RED);
                        x = xParentParent;
            ...
            m_head.get_parent()->set_color(VSTD_RBTREE_BLACK);
        }
        ...
        base_node_ptr_type rebalance_for_erase(base_node_ptr_type z, base_node_ptr_type& leftmost, base_node_ptr_type& rightmost)
        {
            ...
            if (y->get_color() != VSTD_RBTREE_RED)
            {
                while (x != m_head.get_parent() && (x == 0 || x->get_color() == VSTD_RBTREE_BLACK))
                {
                    if (x == xParent->m_left)
                    {
                        base_node_ptr_type w = xParent->m_right;
                        if (w->get_color() == VSTD_RBTREE_RED)
                        {
                            w->set_color(VSTD_RBTREE_BLACK);
                            xParent->set_color(VSTD_RBTREE_RED);
                            rotate_left(xParent);
                            w = xParent->m_right;
                        }
                        if ((w->m_left == 0 || w->m_left->get_color() == VSTD_RBTREE_BLACK) &&
                            (w->m_right == 0 || w->m_right->get_color() == VSTD_RBTREE_BLACK))
                        {
                            w->set_color(VSTD_RBTREE_RED);
                            x = xParent;
                            xParent = xParent->get_parent();
                        }
                        else
                        {
                            if (w->m_right == 0 || w->m_right->get_color() == VSTD_RBTREE_BLACK)
                            {
                                if (w->m_left)
                                {
                                    w->m_left->set_color(VSTD_RBTREE_BLACK);
                                }
                                w->set_color(VSTD_RBTREE_RED);
                                rotate_right(w);
                                w = xParent->m_right;
                            ...
                            if (w->m_right)
                            {
                                w->m_right->set_color(VSTD_RBTREE_BLACK);
                            }
                            rotate_left(xParent);
                            break;
                        }
                    ...
                    else
                    {
                        ...
                        if ((w->m_right == 0 || w->m_right->get_color() == VSTD_RBTREE_BLACK) &&
                            (w->m_left == 0 ||  w->m_left->get_color() == VSTD_RBTREE_BLACK))
                        {
                            w->set_color(VSTD_RBTREE_RED);
                            x = xParent;
                            xParent = xParent->get_parent();
        ...
        inline void orphan_node(node_ptr_type node) const
        {
            ...
            while (iter != 0)
            {
                V_Assert(iter->m_container == static_cast<const checked_container_base*>(this), "rbtree::orphan_node - iterator was corrupted!");
    ...
    inline InsertReturnType rbtree<Traits>::node_handle_insert_unique(NodeHandle&& nodeHandle)
    {
        ...
        if (!result.inserted)
        {
            result.node = VStd::move(nodeHandle);
        }
    ...
    inline NodeHandle rbtree<Traits>::node_handle_extract(const_iterator extractPos)
    {
        ...
        iterator next(VSTD_CHECKED_ITERATOR(iterator_impl, extractNode));
        ++next;
    ...
    template <class Traits>
    template <typename ComparableToKey, typename... Args>
    auto rbtree<Traits>::try_emplace_unique(ComparableToKey&& key, Args&&... arguments) -> VStd::pair<iterator, bool>
    {
        ...
        base_node_ptr_type newNode = create_node(VStd::piecewise_construct, VStd::forward_as_tuple(VStd::forward<ComparableToKey>(key)),
    ...
    template <class Traits>
    template <typename ComparableToKey, typename MappedType>
    auto rbtree<Traits>::insert_or_assign_unique(ComparableToKey&& key, MappedType&& value) -> VStd::pair<iterator, bool>
    {
        // Check if the key has a corresponding node in the container
        iterator insertIter = lower_bound(key);
        if (insertIter.m_node != &m_head && !m_keyEq(key, insertIter->first))
        {
            ...
            insertIter->second = VStd::forward<MappedType>(value);
            return { insertIter, false };
        }
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_RBTREE_H