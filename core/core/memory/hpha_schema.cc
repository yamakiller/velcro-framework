...
#include <core/memory/hpha_schema.h>

#include <core/math/random.h>
#include <core/memory/osallocator.h> // required by certain platforms
...
#include <core/std/containers/set.h>
#endif

// Enable if V_Assert is making things worse (since V_Assert may end up doing allocations)
//#include <assert.h>
//#define _HPHA_ASSERT1(_exp)         assert(_exp)
//#define _HPHA_ASSERT2(_exp, reason) assert(_exp)
#define _HPHA_ASSERT1(_exp)          V_Assert(_exp, "HPHA Assert, expression: \"" #_exp "\"")
#define _HPHA_ASSERT2(_exp, _reason) V_Assert(_exp, "HPHA Assert, expression: \"" #_exp "\", possible reason: " #_reason)
...
#define OS_VIRTUAL_PAGE_SIZE V_PAGE_SIZE
    //////////////////////////////////////////////////////////////////////////
...
#define USE_MUTEX_PER_BUCKET
    ...
    //////////////////////////////////////////////////////////////////////////
    // TODO: Replace with VStd::intrusive_list
    ...
    public:
        ...
        class bucket
        {
            ...
            mutable VStd::mutex mLock;
    #endif
#endif
            size_t          mMarker;
#ifdef MULTITHREADED
    #if defined (USE_MUTEX_PER_BUCKET)
            unsigned char _padding[sizeof(void*) * 16 - sizeof(page_list) - sizeof(VStd::mutex) - sizeof(size_t)];
    #else
...
#ifdef MULTITHREADED
    #if defined (USE_MUTEX_PER_BUCKET)
            VStd::mutex& get_lock() const {return mLock; }
    #endif
#endif
        ...
        {
            return (page*)V::PointerAlignDown((char*)ptr, m_poolPageSize);
        }
...
#ifdef MULTITHREADED
                    VStd::lock_guard<VStd::mutex> lock(mBuckets[bi].get_lock());
#endif
        ...
        struct free_node
            : public VStd::intrusive_multiset_node<free_node>
        {
        ...
        using free_node_tree = VStd::intrusive_multiset<free_node, VStd::intrusive_multiset_base_hook<free_node>, VStd::less<> >;
        ...
        mutable VStd::recursive_mutex mTreeMutex;
#endif
        ...
        private:
            ...
            V::Debug::StackFrame mCallStack[MAX_CALLSTACK_DEPTH];
        };
        ...
        public:
            ...
            V_FORCE_INLINE pointer_type allocate(size_t byteSize, size_t alignment, int = 0)
            {
                return V_OS_MALLOC(byteSize, alignment);
            }
            V_FORCE_INLINE size_type resize(pointer_type, size_type)
            {
                return 0;
            }
            V_FORCE_INLINE void deallocate(pointer_type ptr, size_type, size_type)
            {
                V_OS_FREE(ptr);
            }
        };
        ...
        class debug_record_map
            : public VStd::set<debug_record, VStd::less<debug_record>, DebugAllocator >
        {
            typedef VStd::set<debug_record, VStd::less<debug_record>, DebugAllocator > base;
...
#ifdef MULTITHREADED
        mutable VStd::mutex mDebugMutex;
#endif
    ...
    public:
        HpAllocator(V::HphaSchema::Descriptor desc);
        ~HpAllocator();
        ...
        inline void* alloc(size_t size, size_t alignment)
        {
            ...
            {
                size = clamp_small_allocation(size);
                void* ptr = bucket_alloc_direct(bucket_spacing_function(V::SizeAlignUp(size + MEMORY_GUARD_SIZE, alignment)));
                debug_add(ptr, size, DEBUG_SOURCE_BUCKETS);
                return ptr;
            }
            else
        ...
        void* realloc(void* ptr, size_t size, size_t alignment)
        {
            ...
            else
            {
                ...
                {
                    size = clamp_small_allocation(size);
                    newPtr = bucket_alloc_direct(bucket_spacing_function(V::SizeAlignUp(size + MEMORY_GUARD_SIZE, alignment)));
                    if (newPtr)
                    {
                        debug_add(newPtr, size, DEBUG_SOURCE_BUCKETS);
                        HPPA_ASSERT(size <= ptr_get_block_header(ptr)->size() - MEMORY_GUARD_SIZE);
        ...
        inline size_t resize(void* ptr, size_t size)
        {
            ...
            size = VStd::GetMax(size, MAX_SMALL_ALLOCATION + MIN_ALLOCATION);
        ...
        inline void free(void* ptr, size_t origSize, size_t oldAlignment)
        {
            ...
            if (m_isPoolAllocations && is_small_allocation(origSize) && oldAlignment <= MAX_SMALL_ALLOCATION)
            {
                ...
                debug_remove(ptr, origSize, DEBUG_SOURCE_BUCKETS);
                return bucket_free_direct(ptr, bucket_spacing_function(V::SizeAlignUp(origSize + MEMORY_GUARD_SIZE, oldAlignment)));
            }
            debug_remove(ptr, origSize, DEBUG_SOURCE_TREE);
            tree_free(ptr);
        }
    ...
    //////////////////////////////////////////////////////////////////////////
    HpAllocator::HpAllocator(V::HphaSchema::Descriptor desc)
...
#   endif // MULTITHREADED
#endif // V_TRAIT_OS_HAS_CRITICAL_SECTION_SPIN_COUNT
        ...
        if (!m_fixedBlock)
        {
            ...
            if (!mFreeTree.empty())
            {
                ...
                while (node != end)
                {
                    ...
                    V_TracePrintf("HPHA", "Block in free tree: %p, size=%zi bytes", cur, cur->size());
                    ++node;
                }
            }
#endif
...
#   endif // MULTITHREADED
        ...
        V::SimpleLcgRandom randGenerator = V::SimpleLcgRandom(reinterpret_cast<u64>(static_cast<void*>(this)));
    ...
    void* HpAllocator::bucket_alloc(size_t size)
    {
        ...
        VStd::lock_guard<VStd::mutex> lock(mBuckets[bi].get_lock());
    #else
        VStd::lock_guard<VStd::mutex> lock(m_mutex);
    #endif
#endif
    ...
    void* HpAllocator::bucket_realloc(void* ptr, size_t size)
    {
        ...
        memcpy(newPtr, ptr, VStd::GetMin(elemSize - MEMORY_GUARD_SIZE, size - MEMORY_GUARD_SIZE));
        bucket_free(ptr);
        return newPtr;
    }
    ...
    void* HpAllocator::bucket_realloc_aligned(void* ptr, size_t size, size_t alignment)
    {
        ...
        void* newPtr = bucket_alloc_direct(bucket_spacing_function(V::SizeAlignUp(size, alignment)));
        if (!newPtr)
        {
            return nullptr;
        }
...
#ifdef MULTITHREADED
    #if defined (USE_MUTEX_PER_BUCKET)
            VStd::lock_guard<VStd::mutex> lock(mBuckets[i].get_lock());
    #else
            VStd::lock_guard<VStd::mutex> lock(m_mutex);
    #endif
#endif
    ...
    size_t HpAllocator::bucket_get_unused_memory(bool isPrint) const
    {
        ...
        for (int i = (int)NUM_BUCKETS - 1; i > 0; i--)
        {
            ...
            for (const page* p = mBuckets[i].page_list_begin(); p != pageEnd; )
            {
                ...
                if (isPrint)
                {
                    V_TracePrintf("System", "Unused Bucket %d page %p elementSize: %d available: %d elements\n", i, p, elementSize, availableMemory / elementSize);
                }
                p = p->next();
            }
        }
    ...
    void* HpAllocator::tree_system_alloc(size_t size)
    {
        ...
        size_t allocSize = V::SizeAlignUp(size, OS_VIRTUAL_PAGE_SIZE);
    ...
    HpAllocator::block_header* HpAllocator::tree_grow(size_t size)
    {
        ...
        const size_t newSize = (sizeWithBlockHeaders < m_treePageSize) ? V::SizeAlignUp(sizeWithBlockHeaders, m_treePageSize) : sizeWithBlockHeaders;
    ...
    HpAllocator::block_header* HpAllocator::tree_extract_aligned(size_t size, size_t alignment)
    {
        ...
        while (bestNode != lastNode)
        {
            ...
            size_t alignmentOffs = V::PointerAlignUp((char*)node, alignment) - (char*)node;
    ...
    HpAllocator::block_header* HpAllocator::tree_extract_bucket_page()
    {
        ...
        while (bestNode != lastNode)
        {
            ...
            size_t alignmentOffs = V::PointerAlignUp((char*)bestBlock, alignment) - (char*)bestBlock;
    ...
    {
#ifdef MULTITHREADED
        VStd::lock_guard<VStd::recursive_mutex> lock(mTreeMutex);
#endif
        ...
        size = V::SizeAlignUp(size, sizeof(block_header));
    ...
    void* HpAllocator::tree_alloc_aligned(size_t size, size_t alignment)
    {
        ...
        HPPA_ASSERT(newBl && newBl->size() >= size);
        size_t alignmentOffs = V::PointerAlignUp((char*)newBl->mem(), alignment) - (char*)newBl->mem();
    ...
    void* HpAllocator::tree_alloc_bucket_page()
    {
        ...
        size_t alignmentOffs = V::PointerAlignUp((char*)newBl, alignment) - (char*)newBl;
    ...
    void* HpAllocator::tree_realloc_aligned(void* ptr, size_t size, size_t alignment)
    {
        ...
        block_header* prev = bl->prev();
        size_t prevSize = prev->used() ? 0 : prev->size() + sizeof(block_header);
        size_t alignmentOffs = prev->used() ? 0 : V::PointerAlignUp((char*)prev->mem(), alignment) - (char*)prev->mem();
    ...
    void HpAllocator::tree_free_bucket_page(void* ptr)
    {
        ...
        VStd::lock_guard<VStd::recursive_mutex> lock(mTreeMutex);
#endif
        HPPA_ASSERT(V::PointerAlignDown(ptr, m_poolPageSize) == ptr);
    ...
    void HpAllocator::tree_purge_block(block_header* bl)
    {
        ...
        //  {
        //      char* alignedEnd = V::PointerAlignDown((char*)bl->next()-requiredFrontSize,m_treePageSize);
        //      char* memStart = (char*)bl->prev();
        ...
        //  {
        //      char* alignedStart = V::PointerAlignUp((char*)bl+requiredEndSize,m_treePageSize);
    ...
    size_t HpAllocator::tree_get_unused_memory(bool isPrint) const
    {
        ...
        for (free_node_tree::const_iterator it = mFreeTree.begin(); it != mFreeTree.end(); ++it)
        {
            ...
            if (isPrint)
            {
                V_TracePrintf("System", "Unused Treenode %p size: %zu\n", it->get_block()->mem(), it->get_block()->size());
            }
        }
        return unusedMemory;
    }
    ...
    HpAllocator::GetMaxAllocationSize() const
    {
        ...
        size_t maxSize = 0;
        maxSize = VStd::GetMax(bucket_get_max_allocation(), maxSize);
        maxSize = VStd::GetMax(tree_get_max_allocation(), maxSize);
        return maxSize;
    }
    ...
    HpAllocator::SystemAlloc(size_t size, size_t align)
    {
        ...
        V_Assert(align % OS_VIRTUAL_PAGE_SIZE == 0, "Invalid allocation/page alignment %d should be a multiple of %d!", size, OS_VIRTUAL_PAGE_SIZE);
        return V_OS_MALLOC(size, align);
    }
    ...
    {
        V::Debug::SymbolStorage::StackLine stackLines[MAX_CALLSTACK_DEPTH] = { { 0 } };
        V::Debug::SymbolStorage::DecodeFrames(mCallStack, MAX_CALLSTACK_DEPTH, stackLines);
        ...
        for (int i = 0; i < MAX_CALLSTACK_DEPTH; ++i)
        {
            if (stackLines[i][0] != 0)
            {
                ...
                if ((lineLength + 1) < V_ARRAY_SIZE(stackLines[i]))
                {
                    stackLines[i][lineLength] = '\n';
                    stackLines[i][lineLength + 1] = 0;
                }
                V::Debug::Trace::Output("HpAllocator", stackLines[i]);
            }
        }
    }
    ...
    {
        V::Debug::StackRecorder::Record(mCallStack, MAX_CALLSTACK_DEPTH, 6);
    }
    ...
    void HpAllocator::debug_add(void* ptr, size_t size, debug_source source, memory_debugging_flags flags)
    {
        if (ptr)
        {
            {
#ifdef MULTITHREADED
                VStd::lock_guard<VStd::mutex> lock(mDebugMutex);
#endif
    ...
    void HpAllocator::report()
    {
        ...
        VStd::lock_guard<VStd::mutex> lock(mDebugMutex);
    #endif
        V_TracePrintf("HPHA", "REPORT =================================================\n");
        V_TracePrintf("HPHA", "Total requested size=%zi bytes\n", mTotalDebugRequestedSize[DEBUG_SOURCE_BUCKETS] + mTotalDebugRequestedSize[DEBUG_SOURCE_TREE]);
        V_TracePrintf("HPHA", "Total allocated size=%zi bytes\n", mTotalDebugRequestedSize[DEBUG_SOURCE_BUCKETS] + mTotalDebugRequestedSize[DEBUG_SOURCE_TREE]);
        V_TracePrintf("HPHA", "Currently allocated blocks:\n");
        for (debug_record_map::const_iterator it = mDebugMap.begin(); it != mDebugMap.end(); ++it)
        {
            V_TracePrintf("HPHA", "ptr=%zX, size=%zi\n", (size_t)it->ptr(), it->size());
            it->print_stack();
        }
        V_TracePrintf("HPHA", "===========================================================\n");
    }
    ...
    { 
        ...
        if (m_capacity == V_CORE_MAX_ALLOCATOR_SIZE)
        {
