#ifndef V_FRAMEWORK_CORE_EVENT_BUS_INTERNAL_BUS_CONTAINER_H
#define V_FRAMEWORK_CORE_EVENT_BUS_INTERNAL_BUS_CONTAINER_H

#include <core/std/functional.h>
#include <core/std/parallel/thread.h>
#include <core/std/smart_ptr/intrusive_ptr.h>

#include <core/event_bus/internal/call_stack_entry.h>
#include <core/event_bus/internal/handlers.h>
#include <core/event_bus/internal/storage_policies.h>
    ...
    namespace Internal
    {
        ...
        namespace
        {
            ...
            class MidDispatchDisconnectFixer
                : public V::Internal::CallstackEntry<typename Bus::InterfaceType, typename Bus::Traits>
            {
                using Base = V::Internal::CallstackEntry<typename Bus::InterfaceType, typename Bus::Traits>;
            ...
            public:
                ...
                MidDispatchDisconnectFixer(typename Bus::Context* context, const typename Bus::BusIdType* busId, PreRemoveHandler&& pre, PostRemoveHandler&& post)
                    : Base(context, busId)
                    , m_onPreDisconnect(VStd::forward<PreRemoveHandler>(pre))
                    , m_onPostDisconnect(VStd::forward<PostRemoveHandler>(post))
                {
                    static_assert(VStd::is_invocable<PreHandler, typename Bus::InterfaceType*>::value, "Pre handler requires accepting a parameter of Bus::InterfaceType pointer");
                    static_assert(VStd::is_invocable<PostHandler>::value, "Post handler does not accept any parameters");
                }
            ...
            {
                return MidDispatchDisconnectFixer<Bus, PreHandler, PostHandler>(context, busId, VStd::forward<PreHandler>(remove), VStd::forward<PostHandler>(post));
            }
        }

// Executes router handling in a generic way
#define EVENTBUS_DO_ROUTING(contextParam, id, isQueued, isReverse) \
    do {                                                                                        \
        ...
        template <typename Interface, typename Traits, EventBusAddressPolicy addressPolicy = Traits::AddressPolicy, EventBusHandlerPolicy handlerPolicy = Traits::HandlerPolicy>
        struct EventBusContainer
        {
        public:
            using ContainerType = EventBusContainer;
            ...
            using HandlerNode = V::Internal::HandlerNode<Interface, Traits, HandlerHolder>;
            ...
            struct Dispatcher
            {
                ...
                static void Event(const IdType& id, Function&& func, ArgsT&&... args)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        EVENTBUS_DO_ROUTING(*context, &id, false, false);
                ...
                static void EventReverse(const IdType& id, Function&& func, ArgsT&&... args)
                {
                    ...
                    {
                        typename Bus::Context::DispatchLockGuard lock(context->m_contextMutex);
                        EVENTBUS_DO_ROUTING(*context, &id, false, true);
                ...
                static void Event(const BusPtr& busPtr, Function&& func, ArgsT&&... args)
                {
                    if (busPtr)
                    {
                        ...
                        EVENTBUS_ASSERT(context, "Internal error: context deleted with bind ptr outstanding.");
                        typename Bus::Context::DispatchLockGuard lock(context->m_contextMutex);

                        EVENTBUS_DO_ROUTING(*context, &busPtr->m_busId, false, false);
                ...
                static void EventReverse(const BusPtr& busPtr, Function&& func, ArgsT&&... args)
                {
                    if (busPtr)
                    {
                        ...
                        EVENTBUS_DO_ROUTING(*context, &busPtr->m_busId, false, true);
                ...
                static void Broadcast(Function&& func, ArgsT&&... args)
                {
                    ...
                    {
                        typename Bus::Context::DispatchLockGuard lock(context->m_contextMutex);
                        EVENTBUS_DO_ROUTING(*context, nullptr, false, false);
                ...
                static void BroadcastReverse(Function&& func, ArgsT&&... args)
                {
                    ...
                    {
                        typename Bus::Context::DispatchLockGuard lock(context->m_contextMutex);
                        EVENTBUS_DO_ROUTING(*context, nullptr, false, true);
                        ...
                        while (addressIt != addresses.rend())
                        {
                            ...
                            if (addressIt != addresses.rbegin())
                            {
                                ...
                                nextHolder = &*VStd::prev(addressIt);
                                nextHolder->add_ref();
                            }
                ...
                static void EnumerateHandlers(Callback&& callback)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        {
                            if (!context->m_buses.EnumerateHandlersImpl(context, *addressIt++, VStd::forward<Callback>(callback)))
                            {
                                break;
                            }
                        }
                ...
                static void EnumerateHandlersId(const IdType& id, Callback&& callback)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        if (addressIt != addresses.end())
                        {
                            context->m_buses.EnumerateHandlersImpl(context, *addressIt, VStd::forward<Callback>(callback));
                        }
                    }
                }
                template <class Callback>
                static void EnumerateHandlersPtr(const BusPtr& ptr, Callback&& callback)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        if (ptr)
                        {
                            context->m_buses.EnumerateHandlersImpl(context, *ptr, VStd::forward<Callback>(callback));
                        }
                    }
                }
            };
            ...
            {
                EVENTBUS_ASSERT(!handler.m_holder,
            ...
            {
                EVENTBUS_ASSERT(handler.m_holder, "Internal error: disconnecting handler that is incompletely connected");
        ...
        template <typename Interface, typename Traits, EventBusAddressPolicy addressPolicy>
        struct EventBusContainer<Interface, Traits, addressPolicy, EventBusHandlerPolicy::Single>
        {
        public:
            using ContainerType = EventBusContainer;
            using IdType = typename Traits::BusIdType;
            ...
            struct Dispatcher
            {
                ...
                static void Event(const IdType& id, Function&& func, ArgsT&&... args)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        if (addressIt != addresses.end() && addressIt->m_interface)
                        {
                            ...
                            Traits::EventProcessingPolicy::Call(VStd::forward<Function>(func), addressIt->m_interface, VStd::forward<ArgsT>(args)...);
                        }
                    }
                }
                template <typename Results, typename Function, typename... ArgsT>
                static void EventResult(Results& results, const IdType& id, Function&& func, ArgsT&&... args)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        {
                            CallstackEntry entry(context, &addressIt->m_busId);
                            Traits::EventProcessingPolicy::CallResult(results, VStd::forward<Function>(func), addressIt->m_interface, VStd::forward<ArgsT>(args)...);
                        }
                    }
                }
                template <typename Function, typename... ArgsT>
                ...
                static void Event(const BusPtr& busPtr, Function&& func, ArgsT&&... args)
                {
                    if (busPtr)
                    {
                        ...
                        {
                            CallstackEntry entry(context, &busPtr->m_busId);
                            Traits::EventProcessingPolicy::Call(VStd::forward<Function>(func), busPtr->m_interface, VStd::forward<ArgsT>(args)...);
                        }
                    }
                }
                template <typename Results, typename Function, typename... ArgsT>
                static void EventResult(Results& results, const BusPtr& busPtr, Function&& func, ArgsT&&... args)
                {
                    if (busPtr)
                    {
                        ...
                        {
                            CallstackEntry entry(context, &busPtr->m_busId);
                            Traits::EventProcessingPolicy::CallResult(results, VStd::forward<Function>(func), busPtr->m_interface, VStd::forward<ArgsT>(args)...);
                        }
                    }
                }
                template <typename Function, typename... ArgsT>
            ...
            struct HandlerHolder
            {
                ...
                HandlerHolder(HandlerHolder&& rhs)
                    : m_busContainer(rhs.m_busContainer)
                    , m_busId(rhs.m_busId)
                    , m_handler(VStd::move(rhs.m_handler))
        ...
        template <typename Interface, typename Traits, EventBusHandlerPolicy handlerPolicy>
        struct EventBusContainer<Interface, Traits, EventBusAddressPolicy::Single, handlerPolicy>
        {
        public:
            using ContainerType = EventBusContainer;
            using IdType = typename Traits::BusIdType;
        ...
        struct EventBusContainer<Interface, Traits, EventBusAddressPolicy::Single, EventBusHandlerPolicy::Single>
        {
        public:
            using ContainerType = EventBusContainer;
            using IdType = typename Traits::BusIdType;
            ...
            struct Dispatcher
            {
                ...
                static void Broadcast(Function&& func, ArgsT&&... args)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        {
                            CallstackEntry entry(context, nullptr);
                            Traits::EventProcessingPolicy::Call(VStd::forward<Function>(func), handler, VStd::forward<ArgsT>(args)...);
                        }
                    }
                }
                template <typename Results, typename Function, typename... ArgsT>
                static void BroadcastResult(Results& results, Function&& func, ArgsT&&... args)
                {
                    if (auto* context = Bus::GetContext())
                    {
                        ...
                        {
                            CallstackEntry entry(context, nullptr);
                            Traits::EventProcessingPolicy::CallResult(results, VStd::forward<Function>(func), handler, VStd::forward<ArgsT>(args)...);
                        }
                    }
                }
                template <typename Function, typename... ArgsT>
            ...
            void Connect(HandlerNode& handler, const IdType&)
            {
                V_Assert(!m_handler, "Bus already connected to!");
                m_handler = handler;
            }
...
#endif // V_FRAMEWORK_CORE_EVENT_BUS_INTERNAL_BUS_CONTAINER_H
