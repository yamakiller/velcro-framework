...
namespace V {
    ...
    template <typename... Params>
    class Event final {
        friend class EventHandler<Params...>;
        public:
            using Callback = VStd::function<void(Params...)>;
            using Handler  = EventHandler<Params...>;

            V_CLASS_ALLOCATOR(Event<Params...>, SystemAllocator, 0);
        ...
        private:
            ...
            mutable bool m_updating = false; //< Raised during a Signal, false otherwise, used to guard m_handlers during handler iterati
    };

    /// A handler class that can connect to an Event 
    template <typename... Params>
    class EventHandler final {
        friend class Event<Parmas...>;
    public:
        using Callback = VStd::function<void(Params...)>;

        V_CLASS_ALLOCATOR(EventHandler<Params...>, SystemAllocator, 0);
...
namespace V {
    ...
    EventHandler<Params...>& EventHandler<Params...>::operator=(EventHandler&& rhs) {
        if (this != &rhs) {
            Disconnect();
            // Moves all of the data of the r-value handle, fixup the event to point to them, and 
            // revert the r-value handle to it's original construction state
            m_event = rhs.m_event;
            m_index = rhs.m_index;
            m_callback = VStd::move(rhs.m_callback);
    ...
    void Event<Params...>::Signal(const Params&... params) const {
        ...
        if (!m_addList.empty()) {
            for (Handler* handler : m_addList) {
                V_Assert(handler, "NULL handler encountered in Event addList");
                if (m_freeList.empty()) {
                    handler->m_index = vnumeric_cast<int32_t>(m_handlers.size());
                    m_handlers.push_back(handler);
                } else {
                    handler->m_index = vnumeric_cast<int32_t>(m_freeList.top());
                    m_freeList.pop();
    ...
    inline void Event<Params...>::BindHandlerEventPointers() {
        for (Handler* handler : m_handlers) {
            if (handler) {
                // This should have happened as part of a move so none of the pointers should refer to this event
                // (they should also all refer to the same event)
    ...
    inline void Event<Params...>::Connect(Handler& handler) const {
        if (m_updating) {
            handler.m_index = -vnumeric_cast<int32_t>(m_addList.size() + 1);
