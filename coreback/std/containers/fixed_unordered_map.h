#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_FIXED_UNORDERED_MAP_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_FIXED_UNORDERED_MAP_H
...
namespace VStd
{
    namespace Internal
    {
        template<class Key, class MappedType, class Hasher, class EqualKey, bool isMultiMap, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements>
     ...
     * Check the fixed_unordered_map \ref VStdExamples.
     */
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, class Hasher = VStd::hash<Key>, class EqualKey = VStd::equal_to<Key> >
    ...
    public:
        ...
        V_FORCE_INLINE fixed_unordered_map()
            : base_type(hasher(), key_eq()) {}
        V_FORCE_INLINE fixed_unordered_map(const hasher& hash, const key_eq& keyEqual)
        ...
        fixed_unordered_map(const VStd::initializer_list<value_type>& list, const hasher& hash = hasher(),
            const key_eq& keyEqual = key_eq())
            : fixed_unordered_map(list.begin(), list.end(), hash, keyEqual)
        {
        }

        V_FORCE_INLINE pair_iter_bool insert(const value_type& value)
        {
        ...
        */
        V_FORCE_INLINE mapped_type& at(const key_type& key)
        {
            ...
            VSTD_CONTAINER_ASSERT(iter != base_type::end(), "Element with key %d is not present", key);
            return iter->second;
        }
    ...
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, class Hasher, class EqualKey >
    V_FORCE_INLINE void swap(fixed_unordered_map<Key, MappedType, FixedNumBuckets, FixedNumElements, Hasher, EqualKey>& left, fixed_unordered_map<Key, MappedType, FixedNumBuckets, FixedNumElements, Hasher, EqualKey>& right)
    {
        left.swap(right);
    }
     ...
     * a prime number and about 1/3-1/4 of the FixedNumElements.
     *
     * Check the fixed_unordered_multimap \ref VStdExamples.
     */
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, class Hasher = VStd::hash<Key>, class EqualKey = VStd::equal_to<Key> >
    ...
    public:
        ...
        V_FORCE_INLINE fixed_unordered_multimap()
            : base_type(hasher(), key_eq()) {}
        V_FORCE_INLINE fixed_unordered_multimap(const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual) {}
        template<class Iterator>
        ...
        fixed_unordered_multimap(const VStd::initializer_list<value_type>& list, const hasher& hash = hasher(),
            const key_eq& keyEqual = key_eq())
    ...
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, class Hasher, class EqualKey >
    V_FORCE_INLINE void swap(fixed_unordered_multimap<Key, MappedType, FixedNumBuckets, FixedNumElements, Hasher, EqualKey>& left, fixed_unordered_multimap<Key, MappedType, FixedNumBuckets, FixedNumElements, Hasher, EqualKey>& right)
    {
        left.swap(right);
    }
}

#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_FIXED_UNORDERED_MAP_H