...
namespace V::IO
{
    ...
    extern template void swap<VStd::string>(Path& lhs, Path& rhs) noexcept;
    ...
    extern template size_t hash_value<VStd::string>(const Path& pathToHash);
    extern template size_t hash_value<FixedMaxPathString>(const FixedMaxPath& pathToHash);

    // Append operator explicit declarations
    extern template BasicPath<VStd::string> operator/<VStd::string>(const BasicPath<VStd::string>& lhs, const PathView& rhs);
    ...
    extern template BasicPath<VStd::string> operator/<VStd::string>(const BasicPath<VStd::string>& lhs, VStd::string_view rhs);
    extern template BasicPath<FixedMaxPathString> operator/<FixedMaxPathString>(const BasicPath<FixedMaxPathString>& lhs, VStd::string_view rhs);
    extern template BasicPath<VStd::string> operator/<VStd::string>(const BasicPath<VStd::string>& lhs,
...
namespace V::IO
{
    ...
    constexpr PathView::PathView(VStd::string_view pathView) noexcept
        : m_path(pathView)
    {}

    constexpr PathView::PathView(VStd::string_view pathView, const char preferredSeparator) noexcept
        : m_path(pathView)
    ...
    constexpr auto PathView::operator=(VStd::string_view path) noexcept -> PathView&
    {
        m_path = path;
        return *this;
    }
    ...
    constexpr auto PathView::Native() const noexcept -> VStd::string_view
    {
        return m_path;
    }

    constexpr PathView::operator VStd::string_view() const noexcept
    {
        return m_path;
    }

    // path.decompose
    constexpr auto PathView::root_name_view() const -> VStd::string_view
    {
    ...
    constexpr auto PathView::root_directory_view() const -> VStd::string_view
    {
        auto pathParser = parser::PathParser::CreateBegin(m_path, m_preferred_separator);
        if (pathParser.m_parser_state == parser::PS_InRootName)
        {
    ...
    constexpr auto PathView::root_path_raw_view() const -> VStd::string_view
    {
        auto pathParser = parser::PathParser::CreateBegin(m_path, m_preferred_separator);
        if (pathParser.m_parser_state == parser::PS_InRootName)
        {
            ...
            {
                ++pathParser;
                return VStd::string_view{ m_path.begin(), pathParser.m_path_raw_entry.end() };
            }
            return pathParser.m_path_raw_entry;
        }
        return (pathParser.m_parser_state == parser::PS_InRootDir) ? *pathParser : VStd::string_view{};
    }

    constexpr auto PathView::relative_path_view() const -> VStd::string_view
    {
        auto pathParser = parser::PathParser::CreateBegin(m_path, m_preferred_separator);
        ...
        VStd::string_view relativePathStart = pathParser.m_path_raw_entry;
        // Retrieve the last filename entry and use it's end as the end of the relative path
        VStd::string_view lastFilenameEntry = *(--parser::PathParser::CreateEnd(m_path, m_preferred_separator));
        return VStd::string_view(relativePathStart.begin(), lastFilenameEntry.end());
    }

    constexpr auto PathView::parent_path_view() const -> VStd::string_view
    {
        if (empty())
        {
            return {};
        ...
        // a string representing that path
        {
            ...
            --pathParser;
            return VStd::string_view(m_path.data(), pathParser.m_path_raw_entry.data() + pathParser.m_path_raw_entry.size());
        }
    }

    constexpr auto PathView::filename_view() const -> VStd::string_view
    {
        if (empty())
        {
            return {};
    ...
    constexpr auto PathView::stem_view() const -> VStd::string_view
    {
        return parser::SeparateFilename(filename_view()).first;
    }

    constexpr auto PathView::extension_view() const -> VStd::string_view
    {
    ...
    constexpr int PathView::Compare(VStd::string_view pathView) const noexcept
    {
    ...
    constexpr VStd::fixed_string<MaxPathLength> PathView::FixedMaxPathString() const noexcept
    {
        return VStd::fixed_string<MaxPathLength>(m_path.begin(), m_path.end());
    }

    // as_posix
    constexpr VStd::fixed_string<MaxPathLength> PathView::FixedMaxPathStringAsPosix() const noexcept
    {
        VStd::fixed_string<MaxPathLength> resultPath(m_path.begin(), m_path.end());
        VStd::replace(resultPath.begin(), resultPath.end(), V::IO::WindowsPathSeparator, V::IO::PosixPathSeparator);
        return resultPath;
    }
    ...
    [[nodiscard]] constexpr bool PathView::Match(VStd::string_view pathPattern) const
    {
        // Initialize a path view for the pattern using preferred separator for the current path
        V::IO::PathView pathPatternView{ pathPattern, m_preferred_separator };
        ...
        if (m_preferred_separator == PosixPathSeparator)
        {
            ...
            {
                if (!VStd::wildcard_match_case(*patternParserEnd, *pathParserEnd))
                {
                    return false;
                }
            }
        ...
        else
        {
            for (--pathParserEnd, --patternParserEnd; pathParserEnd && patternParserEnd; --pathParserEnd, --patternParserEnd)
            {
                if (!VStd::wildcard_match(*patternParserEnd, *pathParserEnd))
                {
                    return false;
                }
            }
    ...
    constexpr void PathView::MakeRelativeTo(PathIterable& pathIterable, const V::IO::PathView& path, const V::IO::PathView& base) noexcept
    {
    ...
    constexpr auto PathView::AppendNormalPathParts(PathIterable& pathIterable, const V::IO::PathView& path) noexcept -> void
    {
        ...
        for (auto pathParser = parser::PathParser::CreateBegin(path.m_path, path.m_preferred_separator); pathParser; ++pathParser)
        {
            ...
            case parser::PathPartKind::PK_Filename:
            {
                ...
                VStd::string_view filenameView{ *pathParser };
                ...
                {
                    V::IO::PathView fileNamePath{ VStd::string_view{ filenameView.begin(), path.m_path.end() },
            ...
            case parser::PathPartKind::PK_None:
                V_Assert(false, "Path Parser is in an invalid state");
            }
        }
    }

    constexpr auto PathView::GetNormalPathParts(const V::IO::PathView& path) noexcept -> PathIterable
    {
...
namespace V::IO
{
    ...
    template <typename StringType>
    constexpr BasicPath<StringType>::BasicPath(VStd::string_view src, const char preferredSeparator) noexcept
        : m_path(src)
        , m_preferred_separator(preferredSeparator) {}
    ...
    constexpr BasicPath<StringType>::BasicPath(const value_type* src, const char preferredSeparator) noexcept
        ...
        , m_preferred_separator(preferredSeparator) {}
    template <typename StringType>
    constexpr BasicPath<StringType>::BasicPath(VStd::string_view src) noexcept
        : m_path(src) {}
    ...
    template <typename StringType>
    constexpr auto BasicPath<StringType>::operator=(VStd::string_view src) noexcept -> BasicPath&
    {
        return Assign(src);
    }
    ...
    template <typename StringType>
    constexpr auto BasicPath<StringType>::Assign(VStd::string_view src) noexcept -> BasicPath&
    {
    ...
    template <typename StringType>
    constexpr auto BasicPath<StringType>::operator/=(VStd::string_view src) -> BasicPath&
    {
        return Append(src.begin(), src.end());
    }
    template <typename StringType>
    ...
    template <typename StringType>
    constexpr auto BasicPath<StringType>::Append(VStd::string_view src) -> BasicPath&
    {
        return Append(src.begin(), src.end());
    }
    template <typename StringType>
    ...
    constexpr auto BasicPath<StringType>::Append(InputIt first, InputIt last) -> BasicPath&
    {
        ...
        VStd::string_view rootNameView{ m_path.begin(), postRootNameIter };
        ...
        auto compareRootName = [ToLower = VStd::move(ToLower), path_separator = m_preferred_separator](const char lhs, const char rhs) constexpr
        {
            return path_separator == PosixPathSeparator ? lhs == rhs : ToLower(lhs) == ToLower(rhs);
        };
        if (first != otherPostRootNameIter && !VStd::equal(rootNameView.begin(), rootNameView.end(), first, otherPostRootNameIter, compareRootName))
        {
            m_path.assign(first, last);
            return *this;
        }
    ...
    constexpr auto BasicPath<StringType>::MakePreferred() -> BasicPath&
    {
        if (m_preferred_separator != PosixPathSeparator)
        {
            VStd::replace(m_path.begin(), m_path.end(), PosixPathSeparator, m_preferred_separator);
        }
        else
        {
            VStd::replace(m_path.begin(), m_path.end(), WindowsPathSeparator, m_preferred_separator);
        }
        return *this;
    }
    ...
    template <typename StringType>
    VStd::string BasicPath<StringType>::String() const
    {
        return VStd::string(m_path.begin(), m_path.end());
    }

    // extension: fixed string types with MaxPathLength capacity
    template <typename StringType>
    constexpr VStd::fixed_string<MaxPathLength> BasicPath<StringType>::FixedMaxPathString() const
    {
        return VStd::fixed_string<MaxPathLength>(m_path.begin(), m_path.end());
    }
    ...
    template <typename StringType>
    VStd::string BasicPath<StringType>::StringAsPosix() const
    {
        VStd::string resultPath(m_path.begin(), m_path.end());
        VStd::replace(resultPath.begin(), resultPath.end(), WindowsPathSeparator, PosixPathSeparator);
        return resultPath;
    }

    template <typename StringType>
    constexpr VStd::fixed_string<MaxPathLength> BasicPath<StringType>::FixedMaxPathStringAsPosix() const noexcept
    {
        VStd::fixed_string<MaxPathLength> resultPath(m_path.begin(), m_path.end());
        VStd::replace(resultPath.begin(), resultPath.end(), WindowsPathSeparator, PosixPathSeparator);
        return resultPath;
    ...
    template <typename StringType>
    constexpr int BasicPath<StringType>::Compare(VStd::string_view pathView) const noexcept
    {
    ...
    template <typename StringType>
    [[nodiscard]] constexpr bool BasicPath<StringType>::Match(VStd::string_view pathPattern) const
    {
    ...
    template <typename StringType>
    constexpr BasicPath<StringType> operator/(const BasicPath<StringType>& lhs, VStd::string_view rhs)
    {
        BasicPath<StringType> result(lhs);
        result /= rhs;
        return result;
    ...
    {
        return VStd::hash<BasicPath<StringType>>{}(pathToHash);
    }
}


//! V::PathView functions which returns FixedMaxPath
namespace V::IO
{
    [[nodiscard]] constexpr bool PathView::IsRelativeTo(const PathView& base) const
    {
        ...
        [[maybe_unused]] auto [thisPathIter, basePathIter] = VStd::mismatch(thisPathParts.begin(), thisPathParts.end(),
...
//! V::Path iterator implementation
namespace V::IO
{
    // PathIterator
    ...
    {
        V_Assert(m_state != Singular, "attempting to increment a singular iterator (i.e a default constructed iterator)");
        V_Assert(m_state != AtEnd, "attempting to increment the path end iterator is not allowed");
        return increment();
    }
    ...
    constexpr auto PathIterator<PathType>::operator--() -> PathIterator&
    {
        V_Assert(m_state != Singular, "attempting to decrement a singular iterator (i.e a default constructed iterator");
        V_Assert(m_path_entry_view.data() != m_path_ref->Native().data(), "attempting to decrement the path begin iterator is not allowed");
        return decrement();
    }
...
namespace VStd
{
    ...
    {
        size_t operator()(const V::IO::PathView& pathToHash) noexcept
        {
            auto pathParser = V::IO::parser::PathParser::CreateBegin(pathToHash.Native(), pathToHash.m_preferred_separator);
            return V::IO::parser::HashPath(pathParser);
        }
    };
    template <typename StringType>
    struct hash<V::IO::BasicPath<StringType>>
    {
        size_t operator()(const V::IO::BasicPath<StringType>& pathToHash) noexcept
        {
            return VStd::hash<V::IO::PathView>{}(pathToHash);
        }
    };

    // Instantiate the hash function for Path and FixedMaxPath
    template struct hash<V::IO::Path>;
    template struct hash<V::IO::FixedMaxPath>;
}
...
namespace V::IO
{
    ...
    {
        return VStd::hash<PathView>{}(pathToHash);
    }
}