    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                ~rw_mutex()
                {
                    __TBB_ASSERT(!m_state.load(std::memory_order_relaxed),
                ...
                void lock()
                {
                    ...
                    while (!try_lock()) {
                        if (!(m_state.load(std::memory_order_relaxed) &
                              WRITER_PENDING)) {  // no pending writers
                            m_state |= WRITER_PENDING;
                        }

                        auto wakeup_condition = [&] {
                            return !(m_state.load(std::memory_order_relaxed) & BUSY);
                        };
                ...
                void unlock()
                {
                    ...
                    state_type curr_state =
                        (m_state &= READERS | WRITER_PENDING);  // Returns current state
                ...
                void lock_shared()
                {
                    ...
                    while (!try_lock_shared()) {
                        ...
                        auto       wakeup_condition = [&] {
                            return !(m_state.load(std::memory_order_relaxed) & has_writer);
                        };
                        adaptive_wait_on_address(this, wakeup_condition, READER_CONTEXT);
                    }
                    __TBB_ASSERT(m_state.load(std::memory_order_relaxed) & READERS,
                                 "invalid state of a read lock: no readers");
                }
            ...
            private:
                /** Detail non ISO C++ standard API **/
                //! This API is used through the scoped_lock class

                //! Upgrade reader to become a writer.
                /** Returns whether the upgrade happened without releasing and re-acquiring the
                 * lock */
                bool upgrade()
                {
                    state_type s = m_state.load(std::memory_order_relaxed);
                    ...
                    // (with multiple readers and pending writer, another upgrade could have
                    // been requested)
                    while ((s & READERS) == ONE_READER || !(s & WRITER_PENDING)) {
                        if (m_state.compare_exchange_strong(s, s | WRITER | WRITER_PENDING)) {
                            auto wakeup_condition = [&] {
                                return (m_state.load(std::memory_order_relaxed) & READERS) ==
                                       ONE_READER;
                            };
                            while ((m_state.load(std::memory_order_relaxed) & READERS) !=
                                   ONE_READER) {
                ...
                void downgrade()
                {
                    ...
                    __TBB_ASSERT(m_state.load(std::memory_order_relaxed) & READERS,
                                 "invalid state after downgrade: no readers");
                }
