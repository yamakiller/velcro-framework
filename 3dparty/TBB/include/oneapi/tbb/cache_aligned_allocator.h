...
namespace tbb {
    namespace detail {
        ...
        namespace r1 {
            TBB_EXPORT void* __TBB_EXPORTED_FUNC  cache_aligned_allocate(size_t size);
            TBB_EXPORT void __TBB_EXPORTED_FUNC   cache_aligned_deallocate(void* p);
            TBB_EXPORT size_t __TBB_EXPORTED_FUNC cache_line_size();
        }  // namespace r1
        ...
        namespace d1 {
            ...
            public:
                ...
                {
                    return (~size_t(0) - r1::cache_line_size()) / sizeof(value_type);
                }
            ...
            private:
                ...
                void* do_allocate(size_t bytes, size_t alignment) override
                {
                    ...
                    size_t         space = correct_size(bytes) + cache_line_alignment;
                    std::uintptr_t base =
                        reinterpret_cast<std::uintptr_t>(m_upstream->allocate(space));
                    __TBB_ASSERT(base != 0, "Upstream resource returned NULL.");

                    // Round up to the next cache line (align the base address)
                    std::uintptr_t result =
                        (base + cache_line_alignment) & ~(cache_line_alignment - 1);
                    __TBB_ASSERT((result - base) >= sizeof(std::uintptr_t),
                                 "Can`t store a base pointer to the header");
                    __TBB_ASSERT(space - (result - base) >= bytes,
                                 "Not enough space for the storage");
                ...
                void do_deallocate(void* ptr, size_t bytes, size_t alignment) override
                {
                    if (ptr) {
                        ...
                        m_upstream->deallocate(reinterpret_cast<void*>(base),
                                               correct_size(bytes) +
                ...
                bool do_is_equal(const std::pmr::memory_resource& other) const noexcept override
                {
                    ...
                    const cache_aligned_resource* other_res =
                        dynamic_cast<const cache_aligned_resource*>(&other);
