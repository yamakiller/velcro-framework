...
#include "TBB/include/oneapi/tbb/detail/_config.h"
#include "TBB/include/oneapi/tbb/detail/_namespace_injection.h"
#include "spin_mutex.h"
#include "null_mutex.h"
#include "spin_rw_mutex.h"
#include "null_rw_mutex.h"
#include "TBB/include/oneapi/tbb/detail/_pipeline_filters.h"
#include "TBB/include/oneapi/tbb/detail/_task.h"
#include "TBB/include/oneapi/tbb/detail/_small_object_pool.h"
#include "cache_aligned_allocator.h"
#include "TBB/include/oneapi/tbb/detail/_exception.h"
#include "TBB/include/oneapi/tbb/detail/_template_helpers.h"
#include "TBB/include/oneapi/tbb/detail/_aggregator.h"
#include "TBB/include/oneapi/tbb/detail/_allocator_traits.h"
#include "TBB/include/oneapi/tbb/detail/_utils.h"
#include "profiling.h"
...
namespace tbb {
    namespace detail {
        ...
        namespace d1 {
            ...
            //! An enumeration the provides the two most common concurrency levels: unlimited
            //! and serial
        ...
        namespace d0 {
            ...
            concept node_body_return_type =
                std::same_as<OutputType, tbb::detail::d1::continue_msg> ||
                std::same_as<OutputType, ReturnType>;

            template <typename Body, typename Output>
            concept continue_node_body =
                std::copy_constructible<Body> &&
                requires(Body& body, const tbb::detail::d1::continue_msg& v) {
                    {
                        body(v)
                        } -> node_body_return_type<Output>;
                };

            template <typename Body, typename Input, typename Output>
            concept function_node_body =
                std::copy_constructible<Body> && requires(Body& body, const Input& v) {
                                                     {
                                                         body(v)
                                                         } -> node_body_return_type<Output>;
                                                 };
            ...
            concept async_node_body = std::copy_constructible<Body> &&
                                      requires(Body& body, const Input& v,
                                               GatewayType& gateway) { body(v, gateway); };
        ...
        namespace d1 {
            ...
            class limiter_node;  // needed for resetting decrementer
...
//! The graph class
#include "TBB/include/oneapi/tbb/detail/_flow_graph_impl.h"

namespace tbb {
    namespace detail {
        namespace d1 {

            static inline std::pair<graph_task*, graph_task*> order_tasks(graph_task* first,
                                                                          graph_task* second)
            {
            ...
            static inline graph_task* combine_tasks(graph& g, graph_task* left,
                                                    graph_task* right)
            {
            ...
            protected:
                ...
                friend class broadcast_cache;
                template <typename X, typename Y>
                friend class round_robin_cache;
                virtual graph_task* try_put_task(const T& t) = 0;
                virtual graph&      graph_reference() const = 0;

                template <typename TT, typename M>
                friend class successor_cache;
                virtual bool is_continue_receiver()
                {
                    return false;
                }

                // TODO revamp: reconsider the inheritance and move node priority out of
                // receiver
                virtual node_priority_t priority() const
                {
                    return no_priority;
                }
            ...
            //! m_base class for receivers of completion messages
            /** These receivers automatically reset, but cannot be explicitly waited on */
            class continue_receiver : public receiver<continue_msg> {
            protected:
                //! Constructor
                explicit continue_receiver(int             number_of_predecessors,
                                           node_priority_t a_priority)
                {
                ...
                {
                    my_predecessor_count = my_initial_predecessor_count =
                        src.my_initial_predecessor_count;
                    my_current_count = 0;
                ...
                /** Does not Assert to see if the removal of the predecessor now makes the
                   current count exceed the new threshold.  So removing a predecessor while the
                   graph is active can cause unexpected results. */
                ...
                // the friend declaration in the base class did not eliminate the "protected
                // class" error in gcc 4.1.2
                ...
                template <typename TT, typename M>
                friend class successor_cache;
                bool is_continue_receiver() override
                {
                    return true;
                }

                node_priority_t priority() const override
                {
                    return my_priority;
                }
            };  // class continue_receiver
...
#include "TBB/include/oneapi/tbb/detail/_flow_graph_trace_impl.h"
#include "TBB/include/oneapi/tbb/detail/_hash_compare.h"

namespace tbb {
    namespace detail {
        namespace d1 {

#include "TBB/include/oneapi/tbb/detail/_flow_graph_body_impl.h"
#include "TBB/include/oneapi/tbb/detail/_flow_graph_cache_impl.h"
#include "TBB/include/oneapi/tbb/detail/_flow_graph_types_impl.h"

            using namespace graph_policy_namespace;

            template <typename C, typename N>
            graph_iterator<C, N>::graph_iterator(C* g, bool begin)
                : my_graph(g), current_node(NULL)
            {
                if (begin)
                    current_node = my_graph->my_nodes;
            ...
            inline graph::graph()
                : my_wait_context(0), my_nodes(NULL), my_nodes_last(NULL), my_task_arena(NULL)
            {
                prepare_task_arena();
                own_context = true;
                cancelled = false;
                caught_exception = false;
                my_context = new (r1::cache_aligned_allocate(sizeof(task_group_context)))
                    task_group_context(FLOW_TASKS);
                fgt_graph(this);
                my_is_active = true;
            }
            ...
            inline void graph::remove_node(graph_node* n)
            {
                {
                    spin_mutex::scoped_lock lock(nodelist_mutex);
                    __TBB_ASSERT(my_nodes && my_nodes_last,
            ...
            inline void graph::reset(reset_flags f)
            {
                ...
                // Reattach the arena. Might be useful to run the graph in a particular
                // task_arena while not limiting graph lifetime to a single
                // task_arena::execute() call.
            ...
            inline graph::const_iterator graph::begin() const
            {
                return const_iterator(this, true);
            }

            inline graph::const_iterator graph::end() const
            {
                return const_iterator(this, false);
            }

            inline graph::const_iterator graph::cbegin() const
            {
                return const_iterator(this, true);
            }

            inline graph::const_iterator graph::cend() const
            {
                return const_iterator(this, false);
            }
...
#include "TBB/include/oneapi/tbb/detail/_flow_graph_node_impl.h"

            //! An executable node that acts as a source, i.e. it has no predecessors

            template <typename Output>
            __TBB_requires(std::copyable<Output>) class input_node : public graph_node,
            ...
            public:
                ...
                __TBB_requires(input_node_body<Body, Output>) __TBB_NOINLINE_SYM
                ...
                __TBB_requires(input_node_body<Body, Output>)
                    input_node(const node_set<order::preceding, Successors...>& successors,
                               Body                                             body)
                ...
                /** true = item has been released and so remains in sender, dest must request or
                 * reserve future items */
                bool try_release() override
                {
                    spin_mutex::scoped_lock lock(my_mutex);
                    __TBB_ASSERT(my_reserved && my_has_cached_item,
                ...
                Body copy_function_object()
                {
                    ...
                    return dynamic_cast<input_body_leaf<output_type, Body>&>(body_ref)
                        .get_body();
                }
            ...
            private:
                ...
                //! Applies the body.  Returning SUCCESSFULLY_ENQUEUED okay; forward_task_bypass
                //! will handle it.
            ...
            __TBB_requires(std::default_initializable<Input>&& std::copy_constructible<Input>&&
                               std::copy_constructible<Output>) class function_node
                : public graph_node,
            ...
            public:
                ...
                typedef Output output_type;
                typedef function_input<input_type, output_type, Policy, internals_allocator>
                                                                              input_impl_type;
                ...
                template <typename Body>
                __TBB_requires(function_node_body<Body, Input, Output>) __TBB_NOINLINE_SYM
                    function_node(graph& g, size_t concurrency, Body body, Policy = Policy(),
                                  node_priority_t a_priority = no_priority)
                    : graph_node(g),
                      input_impl_type(g, concurrency, body, a_priority),
                      fOutput_type(g)
                {
                ...
                __TBB_NOINLINE_SYM function_node(const function_node& src)
                    : graph_node(src.my_graph), input_impl_type(src), fOutput_type(src.my_graph)
                {
                    fgt_node_with_body(CODEPTR(), FLOW_FUNCTION_NODE, &this->my_graph,
                                       static_cast<receiver<input_type>*>(this),
                                       static_cast<sender<output_type>*>(this), this->my_body);
            ...
            protected:
                ...
                broadcast_cache<output_type>& successors() override
                {
                ...
                void reset_node(reset_flags f) override
                {
                    ...
                    __TBB_ASSERT(!(f & rf_clear_edges) || successors().empty(),
                                 "function_node successors not empty");
                    __TBB_ASSERT(this->my_predecessors.empty(),
            ...
            __TBB_requires(std::default_initializable<Input>&&
                               std::copy_constructible<Input>) class multifunction_node
                : public graph_node,
                  public multifunction_input<
                      Input,
                      typename wrap_tuple_elements<std::tuple_size<Output>::value,  // #elements
                                                                                    // in tuple
                                                   multifunction_output,  // wrap this around
                                                                          // each element
                                                   Output  // the tuple providing the types
                                                   >::type,
                      Policy, cache_aligned_allocator<Input>> {
                typedef cache_aligned_allocator<Input> internals_allocator;
            ...
            public:
                ...
                typedef typename wrap_tuple_elements<N, multifunction_output, Output>::type
                    output_ports_type;
                typedef multifunction_input<input_type, output_ports_type, Policy,
                                            internals_allocator>
                                                                              input_impl_type;
                typedef function_input_queue<input_type, internals_allocator> input_queue_type;
            ...
            public:
                ...
                __TBB_requires(multifunction_node_body<Body, Input, output_ports_type>)
                    __TBB_NOINLINE_SYM
                    multifunction_node(const node_set<Args...>& nodes, size_t concurrency,
                                       Body body, Policy p = Policy(),
                                       node_priority_t a_priority = no_priority)
                    : multifunction_node(nodes.graph_reference(), concurrency, body, p,
                                         a_priority)
                {
                    make_edges_in_order(nodes, *this);
                }
            ...
            public:
                ...
                typedef typename wrap_tuple_elements<N,  // #elements in tuple
                                                     multifunction_output,  // wrap this around
                                                                            // each element
                ...
                __TBB_NOINLINE_SYM explicit split_node(graph& g)
                    : graph_node(g),
                      my_output_ports(
                          init_output_ports<output_ports_type>::call(g, my_output_ports))
                {
                ...
                __TBB_NOINLINE_SYM split_node(const node_set<Args...>& nodes)
                    : split_node(nodes.graph_reference())
                {
                    make_edges_in_order(nodes, *this);
                }
#endif
                ...
                __TBB_NOINLINE_SYM split_node(const split_node& other)
                    : graph_node(other.my_graph),
                      base_type(other),
                      my_output_ports(init_output_ports<output_ports_type>::call(
                          other.my_graph, my_output_ports))
                {
                    fgt_multioutput_node<N>(CODEPTR(), FLOW_SPLIT_NODE, &this->my_graph,
                                            static_cast<receiver<input_type>*>(this),
                                            this->output_ports());
            ...
            protected:
                ...
                {
                    // Sending split messages in parallel is not justified, as overheads would
                    // prevail. Also, we do not have successors here. So we just tell the task
                    // returned here is successful.
                ...
                void reset_node(reset_flags f) override
                {
                    ...
                    __TBB_ASSERT(!(f & rf_clear_edges) ||
                                     clear_element<N>::this_empty(my_output_ports),
            ...
            __TBB_requires(std::copy_constructible<Output>) class continue_node
                : public graph_node,
                  public continue_input<Output, Policy>,
                  public function_output<Output> {
            public:
                typedef continue_msg                               input_type;
                typedef Output                                     output_type;
                ...
                template <typename Body>
                __TBB_requires(continue_node_body<Body, Output>) __TBB_NOINLINE_SYM
                    continue_node(graph& g, Body body, Policy = Policy(),
                                  node_priority_t a_priority = no_priority)
                    : graph_node(g), input_impl_type(g, body, a_priority), fOutput_type(g)
                {
                ...
                __TBB_requires(continue_node_body<Body, Output>)
                    continue_node(const node_set<Args...>& nodes, int number_of_predecessors,
                                  Body body, Policy p = Policy(),
                                  node_priority_t a_priority = no_priority)
                    : continue_node(nodes.graph_reference(), number_of_predecessors, body, p,
                                    a_priority)
                {
                    make_edges_in_order(nodes, *this);
                }
            ...
            public:
                __TBB_NOINLINE_SYM explicit broadcast_node(graph& g)
                    : graph_node(g), my_successors(this)
                {
                ...
                broadcast_node(const node_set<Args...>& nodes)
                    : broadcast_node(nodes.graph_reference())
                {
                    make_edges_in_order(nodes, *this);
                }
#endif
            ...
            protected:
                ...
                void reset_node(reset_flags f) override
                {
                    ...
                    __TBB_ASSERT(!(f & rf_clear_edges) || my_successors.empty(),
            ...
            template <typename T>
            class buffer_node : public graph_node,
            ...
            protected:
                ...
                public:
                    ...
                    buffer_operation(const T& e, op_type t)
                        : type(char(t)), elem(const_cast<T*>(&e)), ltask(NULL)
                    {
                    }
                ...
                {
                    __TBB_ASSERT(static_cast<class_type*>(derived) == this,
                    ...
                    while (op_list) {
                        ...
                        switch (tmp->type) {
                        case reg_succ:
                            internal_reg_succ(tmp);
                            try_forwarding = true;
                            break;
                        case rem_succ:
                            internal_rem_succ(tmp);
                            break;
                        case req_item:
                            internal_pop(tmp);
                            break;
                        case res_item:
                            internal_reserve(tmp);
                            break;
                        case rel_res:
                            internal_release(tmp);
                            try_forwarding = true;
                            break;
                        case con_res:
                            internal_consume(tmp);
                            try_forwarding = true;
                            break;
                        case put_item:
                            try_forwarding = internal_push(tmp);
                            break;
                        case try_fwd_task:
                            internal_forward_task(tmp);
                            break;
                        }
                    }
                    ...
                    if (try_forwarding && !forwarder_busy) {
                        if (is_graph_active(this->my_graph)) {
                            ...
                            small_object_allocator                  allocator{};
                            graph_task* new_task = allocator.new_object<task_type>(
                                graph_reference(), allocator, *this);
                            my_graph.reserve_wait();
                            // tmp should point to the last item handled by the aggregator. This
                            // is the operation the handling thread enqueued.  So modifying that
                            // record will be okay.
                            // TODO revamp: Assert that the issue is still present
                            // workaround for icc bug  (at least 12.0 and 13.0)
                            // error: function "tbb::flow::interfaceX::combine_tasks" cannot be
                            // called with the given argument list
                            //        argument types are: (graph, graph_task *, graph_task *)
                            graph_task* z = tmp->ltask;
                            graph&      g = this->my_graph;
                            tmp->ltask = combine_tasks(
                                g, z, new_task);  // in case the op generated a task
                        }
                    }
                }  // handle_operations
            ...
            public:
                ...
                __TBB_NOINLINE_SYM explicit buffer_node(graph& g)
                    : graph_node(g),
                      reservable_item_buffer<T, internals_allocator>(),
                      receiver<T>(),
                      sender<T>(),
                      my_successors(this),
                      forwarder_busy(false)
                ...
                buffer_node(const node_set<Args...>& nodes)
                    : buffer_node(nodes.graph_reference())
                {
                    make_edges_in_order(nodes, *this);
                }
#endif
                ...
                bool remove_successor(successor_type& r) override
                {
                    ...
                    op_data.r = &r;
                    my_aggregator.execute(&op_data);
                    // even though this operation does not cause a forward, if we are the
                    // handler, and a forward is scheduled, we may be the first to reach this
                    // point after the aggregator, and so should Assert for the task.
                    (void)enqueue_forwarding_task(op_data);
                    return true;
                }
            ...
            protected:
                ...
                graph_task* try_put_task(const T& t) override
                {
                    ...
                    my_aggregator.execute(&op_data);
                    graph_task* ft = grab_forwarding_task(op_data);
                    // sequencer_nodes can return failure (if an item has been previously
                    // inserted) We have to spawn the returned task if our own operation fails.
            ...
            template <typename T>
            __TBB_requires(std::copyable<T>) class sequencer_node : public queue_node<T> {
            ...
            public:
                ...
                __TBB_requires(sequencer<Sequencer, T>) __TBB_NOINLINE_SYM
                ...
                __TBB_NOINLINE_SYM sequencer_node(const sequencer_node& src)
                    : queue_node<T>(src), my_sequencer(src.my_sequencer->clone())
                {
                    fgt_node(CODEPTR(), FLOW_SEQUENCER_NODE, &(this->my_graph),
                             static_cast<receiver<input_type>*>(this),
                             static_cast<sender<output_type>*>(this));
            ...
            public:
                ...
                //! Constructor
                __TBB_NOINLINE_SYM explicit priority_queue_node(graph&         g,
...
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
                template <typename... Args>
                priority_queue_node(const node_set<Args...>& nodes,
                                    const Compare&           comp = Compare())
            ...
            private:
                ...
                // in case a reheap has not been done after a push, Assert if the mark item is
                // higher than the 0'th item
                bool prio_use_tail()
                {
                    __TBB_ASSERT(mark <= this->my_tail, "mark outside bounds before test");
                    return mark < this->my_tail &&
                           compare(this->get_my_item(0), this->get_my_item(this->my_tail - 1));
                }

                // prio_push: checks that the item will fit, expand array if necessary, put at
                // end
                ...
                // 0, move last item to 0 and reheap.  If end of array, just destroy and
                // decrement tail and mark.  Assumes the array has already been tested for
                // emptiness; no failure.
                void prio_pop()
                {
                    if (prio_use_tail()) {
                ...
                void reheap()
                {
                    ...
                    while (child < mark) {
                        ...
                        if (child + 1 < mark &&
                            compare(this->get_my_item(child), this->get_my_item(child + 1)))
                            ++target;
                        // target now has the higher priority child
                        if (compare(this->get_my_item(target), this->get_my_item(cur_pos)))
                            break;
                        // swap
            ...
            public:
                ...
                typedef typename receiver<input_type>::predecessor_type predecessor_type;
                typedef typename sender<output_type>::successor_type    successor_type;
                // TODO: There is a lack of predefined types for its controlling "decrementer"
                // port. It should be fixed later.
            ...
            private:
                ...
                {
                    {
                        ...
                        if (delta > 0 && size_t(delta) > my_count) {
                            ...
                            my_count -=
                                size_t(delta);  // absolute value of delta is sufficiently small
                        }
                        __TBB_ASSERT(my_count <= my_threshold,
                                     "counter values are truncated to be inside the [0, "
                                     "threshold] interval");
                    }
                    return forward_task();
                }
                ...
                {  // always called under lock
                    return (my_count + my_tries < my_threshold && !my_predecessors.empty() &&
                            !my_successors.empty());
                }
                ...
                graph_task* forward_task()
                {
                        ...
                        if ((rval = my_successors.try_put_task(v)) != NULL) {
                            {
                                ...
                                if (check_conditions()) {
                                    if (is_graph_active(this->my_graph)) {
                                        typedef forward_task_bypass<
                                            limiter_node<T, DecrementType>>
                                                               task_type;
                                        small_object_allocator allocator{};
                                        graph_task* rtask = allocator.new_object<task_type>(
                                            my_graph, allocator, *this);
                                        my_graph.reserve_wait();
                    ...
                    // if we can reserve but can't put, we decrement the tries and release the
                    // reservation
                    {
                        spin_mutex::scoped_lock lock(my_mutex);
                        --my_tries;
                        ...
                        if (check_conditions()) {
                            if (is_graph_active(this->my_graph)) {
                                small_object_allocator allocator{};
                                typedef forward_task_bypass<limiter_node<T, DecrementType>>
                                            task_type;
                                graph_task* t =
                                    allocator.new_object<task_type>(my_graph, allocator, *this);
                                my_graph.reserve_wait();
                ...
                void initialize()
                {
                    fgt_node(CODEPTR(), FLOW_LIMITER_NODE, &this->my_graph,
                             static_cast<receiver<input_type>*>(this),
                             static_cast<receiver<DecrementType>*>(&decrement),
                             static_cast<sender<output_type>*>(this));
                }
            ...
            public:
                ...
                limiter_node(const limiter_node& src)
                    : limiter_node(src.my_graph, src.my_threshold)
                {
                }

                //! The interface for accessing internal receiver< DecrementType > that adjusts
                //! the count
                ...
                bool register_successor(successor_type& r) override
                {
                    ...
                    if (was_empty && !my_predecessors.empty() &&
                ...
                bool remove_successor(successor_type& r) override
                {
                    // TODO revamp: investigate why qualification is needed for
                    // remove_predecessor() call
                    tbb::detail::d1::remove_predecessor(r, *this);
                    my_successors.remove_successor(r);
                    return true;
                }
...
#include "TBB/include/oneapi/tbb/detail/_flow_graph_join_impl.h"

            template <typename OutputTuple, typename JP = queueing>
            class join_node;

            template <typename OutputTuple>
            class join_node<OutputTuple, reserving>
                : public unfolded_join_node<std::tuple_size<OutputTuple>::value, reserving_port,
                                            OutputTuple, reserving> {
            private:
                static const int N = std::tuple_size<OutputTuple>::value;
                typedef unfolded_join_node<N, reserving_port, OutputTuple, reserving>
                    unfolded_type;
...
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
                template <typename... Args>
                __TBB_NOINLINE_SYM join_node(const node_set<Args...>& nodes,
                                             reserving = reserving())
            ...
            class join_node<OutputTuple, queueing>
                : public unfolded_join_node<std::tuple_size<OutputTuple>::value, queueing_port,
                                            OutputTuple, queueing> {
            private:
                static const int N = std::tuple_size<OutputTuple>::value;
                typedef unfolded_join_node<N, queueing_port, OutputTuple, queueing>
                    unfolded_type;
            ...
            template <typename OutputTuple, typename K, typename... Functions, size_t... Idx>
            void join_node_function_objects_helper(std::index_sequence<Idx...>)
                requires(std::tuple_size_v<OutputTuple> == sizeof...(Functions)) &&
                        (... &&
                         join_node_function_object<Functions,
                                                   std::tuple_element_t<Idx, OutputTuple>, K>);

            template <typename OutputTuple, typename K, typename... Functions>
            concept join_node_functions =
                requires {
                    join_node_function_objects_helper<OutputTuple, K, Functions...>(
            ...
            // tag_matching join_node is a specialization of key_matching, and is
            // source-compatible.
            template <typename OutputTuple, typename K, typename KHash>
            class join_node<OutputTuple, key_matching<K, KHash>>
                : public unfolded_join_node<std::tuple_size<OutputTuple>::value,
                                            key_matching_port, OutputTuple,
                                            key_matching<K, KHash>> {
            private:
                static const int N = std::tuple_size<OutputTuple>::value;
                typedef unfolded_join_node<N, key_matching_port, OutputTuple,
                                           key_matching<K, KHash>>
                    unfolded_type;
            ...
            public:
                ...
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1>)
                    ...
                    : unfolded_type(g, b0, b1)
                {
                    fgt_multiinput_node<N>(CODEPTR(), FLOW_JOIN_NODE_TAG_MATCHING,
                                           &this->my_graph, this->input_ports(),
                                           static_cast<sender<output_type>*>(this));
                }
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2>
                __TBB_requires(
                    join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2>)
                    __TBB_NOINLINE_SYM
                    join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2)
                    : unfolded_type(g, b0, b1, b2)
                {
                    fgt_multiinput_node<N>(CODEPTR(), FLOW_JOIN_NODE_TAG_MATCHING,
                                           &this->my_graph, this->input_ports(),
                                           static_cast<sender<output_type>*>(this));
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2,
                          typename __TBB_B3>
                __TBB_requires(
                    join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3>)
                    __TBB_NOINLINE_SYM
                    join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3)
                    : unfolded_type(g, b0, b1, b2, b3)
                {
                    fgt_multiinput_node<N>(CODEPTR(), FLOW_JOIN_NODE_TAG_MATCHING,
                                           &this->my_graph, this->input_ports(),
                                           static_cast<sender<output_type>*>(this));
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2,
                          typename __TBB_B3, typename __TBB_B4, typename __TBB_B5>
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2,
                                                   __TBB_B3, __TBB_B4, __TBB_B5>)
                    __TBB_NOINLINE_SYM
                    join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3,
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2,
                          ...
                          typename __TBB_B6>
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2,
                                                   __TBB_B3, __TBB_B4, __TBB_B5, __TBB_B6>)
                    __TBB_NOINLINE_SYM
                    join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3,
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2,
                          typename __TBB_B3, typename __TBB_B4, typename __TBB_B5,
                          typename __TBB_B6, typename __TBB_B7>
                __TBB_requires(
                    join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3,
                                        __TBB_B4, __TBB_B5, __TBB_B6, __TBB_B7>)
                    __TBB_NOINLINE_SYM
                    join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3,
                              __TBB_B4 b4, __TBB_B5 b5, __TBB_B6 b6, __TBB_B7 b7)
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2,
                          typename __TBB_B3, typename __TBB_B4, typename __TBB_B5,
                          typename __TBB_B6, typename __TBB_B7, typename __TBB_B8>
                __TBB_requires(
                    join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3,
                                        __TBB_B4, __TBB_B5, __TBB_B6, __TBB_B7, __TBB_B8>)
                    __TBB_NOINLINE_SYM
                    join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3,
                              __TBB_B4 b4, __TBB_B5 b5, __TBB_B6 b6, __TBB_B7 b7, __TBB_B8 b8)
                ...
                template <
                    ...
                    // clang 3.4 misdeduces 'Args...' for 'node_set' while it can cope with
                    // template template parameter.
                    template <typename...> class node_set,
    #endif
                    typename... Args, typename... Bodies>
                __TBB_requires((sizeof...(Bodies) == 0) ||
                               join_node_functions<OutputTuple, K, Bodies...>)
                    __TBB_NOINLINE_SYM
                    join_node(const node_set<Args...>& nodes, Bodies... bodies)
...
// indexer node
#include "TBB/include/oneapi/tbb/detail/_flow_graph_indexer_impl.h"
            ...
            public:
                ...
                indexer_node(const node_set<Args...>& nodes)
                    : indexer_node(nodes.graph_reference())
                {
                    make_edges_in_order(nodes, *this);
                }
#endif
            ...
            class indexer_node<T0, T1, T2>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2>> {
            private:
            ...
            class indexer_node<T0, T1, T2, T3>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2, T3>> {
            private:
            ...
            class indexer_node<T0, T1, T2, T3, T4>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2, T3, T4>> {
            private:
            ...
            class indexer_node<T0, T1, T2, T3, T4, T5>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2, T3, T4, T5>> {
            private:
            ...
            class indexer_node<T0, T1, T2, T3, T4, T5, T6>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2, T3, T4, T5, T6>> {
            private:
            ...
            class indexer_node<T0, T1, T2, T3, T4, T5, T6, T7>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2, T3, T4, T5, T6, T7>> {
            private:
            ...
            class indexer_node<T0, T1, T2, T3, T4, T5, T6, T7, T8>
                : public unfolded_indexer_node<std::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8>> {
            private:
            ...
            class indexer_node /*default*/ : public unfolded_indexer_node<std::tuple<
            ...
            // Makes an edge from port 0 of a multi-output predecessor to port 0 of a
            // multi-input successor.
            template <typename T, typename V, typename = typename T::output_ports_type,
            ...
            // Removes an edge between port 0 of a multi-output predecessor and port 0 of a
            // multi-input successor.
            template <typename T, typename V, typename = typename T::output_ports_type,
                      typename = typename V::input_ports_type>
            inline void remove_edge(T& output, V& input)
            {
                remove_edge(std::get<0>(output.output_ports()),
                            std::get<0>(input.input_ports()));
            }
            ...
            public:
                ...
                {
                    fgt_multiinput_multioutput_node(CODEPTR(), FLOW_COMPOSITE_NODE, this,
                                                    &this->my_graph);
                }
                ...
                {
                    static_assert(NUM_INPUTS == std::tuple_size<input_ports_type>::value,
                                  "number of arguments does not match number of input ports");
                    static_assert(NUM_OUTPUTS == std::tuple_size<output_ports_type>::value,
                                  "number of arguments does not match number of output ports");

                    fgt_internal_input_alias_helper<T1, NUM_INPUTS>::alias_port(
                        this, input_ports_tuple);
                    fgt_internal_output_alias_helper<T2, NUM_OUTPUTS>::alias_port(
                        this, output_ports_tuple);

                    my_input_ports.reset(
                        new input_ports_type(std::forward<T1>(input_ports_tuple)));
                    my_output_ports.reset(
                ...
                {
                    add_nodes_impl(this, true, n...);
                }

                template <typename... NodeTypes>
                void add_nodes(const NodeTypes&... n)
                {
                    add_nodes_impl(this, false, n...);
                }
            ...
            public:
                ...
                void set_external_ports(T&& input_ports_tuple)
                {
                    ...
                    fgt_internal_input_alias_helper<T, NUM_INPUTS>::alias_port(
                        this, input_ports_tuple);
            ...
            public:
                ...
                void set_external_ports(T&& output_ports_tuple)
                {
                    ...
                    fgt_internal_output_alias_helper<T, NUM_OUTPUTS>::alias_port(
                        this, output_ports_tuple);
            ...
            template <typename Input, typename Output, typename Policy = queueing_lightweight>
            __TBB_requires(std::default_initializable<Input>&&
                               std::copy_constructible<Input>) class async_node
                : public multifunction_node<Input, std::tuple<Output>, Policy>,
                  public sender<Output> {
                typedef multifunction_node<Input, std::tuple<Output>, Policy> base_type;
                typedef multifunction_input<Input, typename base_type::output_ports_type,
                                            Policy, cache_aligned_allocator<Input>>
                    mfn_input_type;
            ...
            private:
                ...
                public:
                    ...
                    void release_wait() override
                    {
                        ...
                        graph*      g = &n->my_graph;
                        g->release_wait();
                        fgt_async_commit(static_cast<typename async_node::receiver_type*>(n),
                                         g);
                    }

                    //! Implements gateway_type::try_put for an external activity to submit a
                    //! message to FG
                ...
                // The substitute of 'this' for member construction, to prevent compiler
                // warnings
                async_node* self()
                {
                    return this;
            ...
            public:
                ...
                __TBB_requires(async_node_body<Body, input_type, gateway_type>)
                    __TBB_NOINLINE_SYM
                    async_node(graph& g, size_t concurrency, Body body, Policy = Policy(),
                               node_priority_t a_priority = no_priority)
                    : base_type(g, concurrency,
                                async_body<Input, typename base_type::output_ports_type,
                                           gateway_type, Body>(body, &my_gateway),
                                a_priority),
                      my_gateway(self())
                {
                ...
                template <typename Body>
                __TBB_requires(async_node_body<Body, input_type, gateway_type>)
                    __TBB_NOINLINE_SYM async_node(graph& g, size_t concurrency, Body body,
                                                  node_priority_t a_priority)
                ...
                __TBB_requires(async_node_body<Body, input_type, gateway_type>)
                    __TBB_NOINLINE_SYM
                    async_node(const node_set<Args...>& nodes, size_t concurrency, Body body,
                               Policy = Policy(), node_priority_t a_priority = no_priority)
                ...
                __TBB_NOINLINE_SYM async_node(const async_node& other)
                    : base_type(other), sender<Output>(), my_gateway(self())
                {
                    static_cast<async_body_base_type*>(this->my_body->get_body_ptr())
                        ->set_gateway(&my_gateway);
                    static_cast<async_body_base_type*>(this->my_init_body->get_body_ptr())
                        ->set_gateway(&my_gateway);
                ...
                Body copy_function_object()
                {
                    typedef multifunction_body<input_type,
                                               typename base_type::output_ports_type>
                        mfn_body_type;
                    typedef async_body<Input, typename base_type::output_ports_type,
                                       gateway_type, Body>
                                    async_body_type;
                    mfn_body_type&  body_ref = *this->my_body;
                    async_body_type ab = *static_cast<async_body_type*>(
                        dynamic_cast<multifunction_body_leaf<
                            input_type, typename base_type::output_ports_type,
                            async_body_type>&>(body_ref)
                            .get_body_ptr());
                    return ab.get_body();
                }
...
#include "TBB/include/oneapi/tbb/detail/_flow_graph_node_set_impl.h"
            ...
            public:
                ...
                overwrite_node(const node_set<Args...>& nodes)
                    : overwrite_node(nodes.graph_reference())
                {
                    make_edges_in_order(nodes, *this);
                }
#endif
                ...
                bool register_successor(successor_type& s) override
                {
                    ...
                    if (my_buffer_is_valid && is_graph_active(my_graph)) {
                        ...
                        if (ret) {
                            ...
                            // In case of reservation a race between the moment of reservation
                            // and register_successor can appear, because failed reserve does
                            // not mean that register_successor is not ready to put a message
                            // immediately. We have some sort of infinite loop: reserving node
                            // tries to set pull state for the edge, but overwrite_node tries to
                            // return push state back. That is why we have to break this loop
                            // with task creation.
                            small_object_allocator            allocator{};
                            typedef register_predecessor_task task_type;
                            graph_task* t = allocator.new_object<task_type>(
                                graph_reference(), allocator, *this, s);
            ...
            protected:
                ...
                struct register_predecessor_task : public graph_task {
                    register_predecessor_task(graph& g, small_object_allocator& allocator,
            ...
            public:
                ...
                {
                    fgt_node(CODEPTR(), FLOWWRITE_ONCE_NODE, &(this->my_graph),
                             static_cast<receiver<input_type>*>(this),
                             static_cast<sender<output_type>*>(this));
                }
                ...
                write_once_node(const node_set<Args...>& nodes)
                    : write_once_node(nodes.graph_reference())
                {
                    make_edges_in_order(nodes, *this);
                }
#endif
            ...
            inline void set_name(const function_node<Input, Output, Policy>& node,
                                 const char*                                 name)
            {
                fgt_node_desc(&node, name);
            }
            ...
            template <typename Input, typename Output, typename Policy>
            inline void set_name(const multifunction_node<Input, Output, Policy>& node,
                                 const char*                                      name)
            {
            ...
            template <typename InputTuple, typename OutputTuple>
            inline void set_name(const composite_node<InputTuple, OutputTuple>& node,
                                 const char*                                    name)
            {
            ...
            template <typename Input, typename Output, typename Policy>
            inline void set_name(const async_node<Input, Output, Policy>& node,
                                 const char*                              name)
            {
                fgt_multioutput_node_desc(&node, name);
            }
...
#include "TBB/include/oneapi/tbb/detail/_flow_graph_nodes_deduction.h"
