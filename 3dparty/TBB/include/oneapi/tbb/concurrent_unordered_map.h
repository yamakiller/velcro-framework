...
#include "TBB/include/oneapi/tbb/detail/_concurrent_unordered_base.h"
        ...
        namespace d1 {
            ...
            class concurrent_unordered_map
                : public concurrent_unordered_base<concurrent_unordered_map_traits<
                      Key, T, Hash, KeyEqual, Allocator, false>> {
                using traits_type =
                    concurrent_unordered_map_traits<Key, T, Hash, KeyEqual, Allocator, false>;
            ...
            public:
                ...
                void merge(concurrent_unordered_map<key_type, mapped_type, OtherHash,
                                                    OtherKeyEqual, allocator_type>& source)
                {
                ...
                template <typename OtherHash, typename OtherKeyEqual>
                void merge(concurrent_unordered_multimap<key_type, mapped_type, OtherHash,
                                                         OtherKeyEqual, allocator_type>& source)
                {
                    this->internal_merge(source);
                }
                ...
                template <typename OtherHash, typename OtherKeyEqual>
                void
                merge(concurrent_unordered_multimap<key_type, mapped_type, OtherHash,
                                                    OtherKeyEqual, allocator_type>&& source)
                {
                    this->internal_merge(std::move(source));
                }
            ...
            template <typename It, typename Hash = std::hash<iterator_key_t<It>>,
            ...
            concurrent_unordered_map(It, It, size_t = {}, Hash = Hash(), KeyEq = KeyEq(),
                                     Alloc = Alloc())
                -> concurrent_unordered_map<iterator_key_t<It>, iterator_mapped_t<It>, Hash,
                                            KeyEq, Alloc>;
            ...
            template <typename Key, typename T,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<KeyEq>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_map(std::initializer_list<std::pair<Key, T>>, size_t = {},
            ...
            template <typename It, typename Alloc,
                      typename = std::enable_if_t<is_input_iterator_v<It>>,
                      typename = std::enable_if_t<is_allocator_v<Alloc>>>
            concurrent_unordered_map(It, It, size_t, Alloc)
            ...
            // TODO: investigate if a deduction guide for concurrent_unordered_map(It, It,
            // Alloc) is needed
            ...
            template <typename It, typename Hash, typename Alloc,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<Hash>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_map(It, It, size_t, Hash, Alloc)
                -> concurrent_unordered_map<iterator_key_t<It>, iterator_mapped_t<It>, Hash,
                                            std::equal_to<iterator_key_t<It>>, Alloc>;

            template <typename Key, typename T, typename Alloc,
                      typename = std::enable_if_t<is_allocator_v<Alloc>>>
            concurrent_unordered_map(std::initializer_list<std::pair<Key, T>>, size_t, Alloc)
            ...
            template <typename Key, typename T, typename Hash, typename Alloc,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<Hash>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_map(std::initializer_list<std::pair<Key, T>>, size_t, Hash,
                                     Alloc)
            ...
            class concurrent_unordered_multimap
                : public concurrent_unordered_base<concurrent_unordered_map_traits<
                      Key, T, Hash, KeyEqual, Allocator, true>> {
                using traits_type =
                    concurrent_unordered_map_traits<Key, T, Hash, KeyEqual, Allocator, true>;
                using base_type = concurrent_unordered_base<traits_type>;
            ...
            public:
                ...
                operator=(const concurrent_unordered_multimap&) = default;
                concurrent_unordered_multimap&
                operator=(concurrent_unordered_multimap&&) = default;
            ...
            template <typename It, typename Hash = std::hash<iterator_key_t<It>>,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<KeyEq>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_multimap(It, It, size_t = {}, Hash = Hash(), KeyEq = KeyEq(),
                                          Alloc = Alloc())
            ...
            template <typename Key, typename T,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<KeyEq>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_multimap(std::initializer_list<std::pair<Key, T>>, size_t = {},
                                          Hash = Hash(), KeyEq = KeyEq(), Alloc = Alloc())
                -> concurrent_unordered_multimap<std::remove_const_t<Key>, T, Hash, KeyEq,
                                                 Alloc>;

            template <typename It, typename Alloc,
                      typename = std::enable_if_t<is_input_iterator_v<It>>,
                      typename = std::enable_if_t<is_allocator_v<Alloc>>>
            concurrent_unordered_multimap(It, It, size_t, Alloc)
                -> concurrent_unordered_multimap<iterator_key_t<It>, iterator_mapped_t<It>,
                                                 std::hash<iterator_key_t<It>>,
                                                 std::equal_to<iterator_key_t<It>>, Alloc>;
            ...
            template <typename It, typename Hash, typename Alloc,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<Hash>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_multimap(It, It, size_t, Hash, Alloc)
                -> concurrent_unordered_multimap<iterator_key_t<It>, iterator_mapped_t<It>,
                                                 Hash, std::equal_to<iterator_key_t<It>>,
                                                 Alloc>;
            ...
            concurrent_unordered_multimap(std::initializer_list<std::pair<Key, T>>, size_t,
                                          Alloc)
                -> concurrent_unordered_multimap<
                    std::remove_const_t<Key>, T, std::hash<std::remove_const_t<Key>>,
                    std::equal_to<std::remove_const_t<Key>>, Alloc>;
