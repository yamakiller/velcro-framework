...
#include "TBB/include/oneapi/tbb/detail/_mutex_common.h"

#include "profiling.h"

#include "TBB/include/oneapi/tbb/detail/_assert.h"
#include "TBB/include/oneapi/tbb/detail/_utils.h"
#include "TBB/include/oneapi/tbb/detail/_scoped_lock.h"
                    ...
                    while ((s & READERS) == ONE_READER || !(s & WRITER_PENDING)) {
                        if (m_state.compare_exchange_strong(s, s | WRITER | WRITER_PENDING)) {
                            ...
                            while ((m_state.load(std::memory_order_relaxed) & READERS) !=
                                   ONE_READER)
                                backoff.pause();
                            __TBB_ASSERT((m_state & (WRITER_PENDING | WRITER)) ==
...
#include "TBB/include/oneapi/tbb/detail/_rtm_rw_mutex.h"
