    ...
    namespace detail {
        namespace d1 {
            ...
            private:
                ...
                using aggregator_function_type =
                    aggregating_functor<lru_cache_type, aggregator_operation_type>;
                using aggregator_type =
                    aggregator<aggregator_function_type, aggregator_operation_type>;
            ...
            private:
                ...
                const size_t      my_history_list_capacity;  // history list's allowed capacity

                // interface
            public:
                concurrent_lru_cache(value_function_type value_function, size_t cache_capacity)
                    : my_value_function(value_function),
                      my_history_list_capacity(cache_capacity)
                {
            ...
            private:
                ...
                {
                    storage_map_iterator_type map_it =
                        my_storage_map.find(map_record_ref.first);

                    __TBB_ASSERT(map_it != my_storage_map.end(),
                                 "cache should not return past-end iterators to outer world");
                    __TBB_ASSERT(
                        &(*map_it) == &map_record_ref,
                        "dangling reference has been returned to outside world: data race?");
                    __TBB_ASSERT(std::find(my_history_list.begin(), my_history_list.end(),
                                           map_it) == my_history_list.end(),
                    ...
                    if (!--(map_it->second.my_ref_counter)) {
                        ...
                        if (my_history_list.size() >= my_history_list_capacity) {
                            size_t number_of_elements_to_evict =
                                1 + my_history_list.size() - my_history_list_capacity;

                            for (size_t i = 0; i < number_of_elements_to_evict; ++i) {
                                storage_map_iterator_type map_it_to_evict =
                                    my_history_list.back();

                                __TBB_ASSERT(
                                    map_it_to_evict->second.my_ref_counter == 0,
                                    "item to be evicted should not have a live references");

                                // TODO: can we use forward_list instead of list? pop_front /
                                // insert_after last
                ...
                storage_map_reference_type retrieve_serial(key_type key,
                                                           bool&    is_new_value_needed)
                {
                    ...
                    if (map_it == my_storage_map.end()) {
                        map_it = my_storage_map.emplace_hint(
                            map_it, std::piecewise_construct, std::make_tuple(key),
                            std::make_tuple(value_type(), 0, my_history_list.end(), false));
                        is_new_value_needed = true;
                    } else {
                        history_list_iterator_type list_it =
                            map_it->second.my_history_list_iterator;
                        if (list_it != my_history_list.end()) {
                            __TBB_ASSERT(
                                map_it->second.my_ref_counter == 0,
                                "item to be evicted should not have a live references");

                            // Item is going to be used. Therefore it is not a subject for
                            // eviction, so we remove it from LRU history.
            ...
            public:
                ...
                // interface
            public:
                storage_map_value_type(value_type const& value, ref_counter_type ref_counter,
                                       history_list_iterator_type history_list_iterator,
                                       bool                       is_ready)
                    : my_value(value),
                      my_ref_counter(ref_counter),
                      my_history_list_iterator(history_list_iterator),
                      my_is_ready(is_ready)
                {
                }
            };
            ...
            private:
                ...
                // interface
            public:
                handle_object() : my_lru_cache_ptr(nullptr), my_map_record_ptr(nullptr) {}
                handle_object(lru_cache_type&            lru_cache_ref,
                              storage_map_reference_type map_record_ref)
                ...
                {
                    __TBB_ASSERT((other.my_lru_cache_ptr != nullptr &&
                ...
                value_type& value()
                {
                    ...
                    __TBB_ASSERT(my_map_record_ptr,
            ...
            public:
                ...
                //   - as a statically typed variant type or CRTP? (static, dependent on the use
                //   case)
                ...
                void cast_and_handle(lru_cache_type& lru_cache_ref)
                {
                    if (my_op == op_type::retrieve)
                        ...
                        static_cast<signal_end_of_usage_aggregator_operation*>(this)->handle(
                            lru_cache_ref);
                }
            };

            template <typename KeyT, typename ValT, typename KeyToValFunctorT>
            struct concurrent_lru_cache<KeyT, ValT,
                                        KeyToValFunctorT>::retrieve_aggregator_operation
            ...
            public:
                ...
                {
                    my_map_record_ptr =
                        &lru_cache_ref.retrieve_serial(my_key, my_is_new_value_needed);
                }
            ...
            template <typename KeyT, typename ValT, typename KeyToValFunctorT>
            struct concurrent_lru_cache<
                KeyT, ValT, KeyToValFunctorT>::signal_end_of_usage_aggregator_operation
                : aggregator_operation,
                  private no_assign {
            private:
