...
#include "TBB/include/oneapi/tbb/detail/_concurrent_skip_list.h"
    ...
    namespace detail {
        namespace d2 {
            ...
            template <typename Key, typename Value, typename KeyCompare,
                      typename RandomGenerator, typename Allocator, bool AllowMultimapping>
            struct map_traits {
                static constexpr size_t max_level = RandomGenerator::max_level;
                ...
                static value_compare value_comp(compare_type comp)
                {
            ...
            template <typename Key, typename Value, typename Compare = std::less<Key>,
                      typename Allocator = tbb::tbb_allocator<std::pair<const Key, Value>>>
            class concurrent_map
                : public concurrent_skip_list<
                      map_traits<Key, Value, Compare, concurrent_geometric_level_generator<32>,
                                 Allocator, false>> {
                using base_type = concurrent_skip_list<
                    map_traits<Key, Value, Compare, concurrent_geometric_level_generator<32>,
                               Allocator, false>>;
            ...
            public:
                ...
                void
                merge(concurrent_map<key_type, mapped_type, OtherCompare, Allocator>& source)
                {
                ...
                template <typename OtherCompare>
                void merge(
                    concurrent_multimap<key_type, mapped_type, OtherCompare, Allocator>& source)
                {
                    this->internal_merge(source);
                }

                template <typename OtherCompare>
                void merge(concurrent_multimap<key_type, mapped_type, OtherCompare, Allocator>&&
                               source)
                {
                    this->internal_merge(std::move(source));
                }
            ...
            template <typename It, typename Comp = std::less<iterator_key_t<It>>,
            ...
            template <typename Key, typename T,
                      ...
                      typename = std::enable_if_t<is_allocator_v<Alloc>>,
                      typename = std::enable_if_t<!is_allocator_v<Comp>>>
            concurrent_map(std::initializer_list<std::pair<Key, T>>, Comp = Comp(),
                           Alloc = Alloc())
            ...
            concurrent_map(std::initializer_list<std::pair<Key, T>>, Alloc)
                -> concurrent_map<std::remove_const_t<Key>, T,
                                  std::less<std::remove_const_t<Key>>, Alloc>;
            ...
            template <typename Key, typename T,
                      ...
                      typename = std::enable_if_t<is_allocator_v<Alloc>>,
                      typename = std::enable_if_t<!is_allocator_v<Comp>>>
            concurrent_multimap(std::initializer_list<std::pair<Key, T>>, Comp = Comp(),
                                Alloc = Alloc())
            ...
            concurrent_multimap(std::initializer_list<std::pair<Key, T>>, Alloc)
                -> concurrent_multimap<std::remove_const_t<Key>, T,
                                       std::less<std::remove_const_t<Key>>, Alloc>;
