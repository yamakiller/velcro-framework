...
#include "TBB/include/oneapi/tbb/detail/_aligned_space.h"
...
namespace tbb {
    namespace detail {
        ...
        inline namespace d0 {
            ...
            concept parallel_for_each_body =
        ...
        namespace d1 {
            ...
            class feeder {
                ...
                friend class d2::feeder_impl;

            public:
                //! Add a work item to a running parallel_for_each.
                void add(const Item& item)
                {
                    internal_add_copy(item);
                }
                void add(Item&& item)
                {
                    internal_add_move(std::move(item));
                }
            };
    ...
    #pragma warning(push)
            ...
            struct feeder_item_task : public task {
                ...
                feeder_item_task(ItemType&& input_item, feeder_type& feeder,
                ...
                //! Hack for resolve ambiguity between calls to the body with and without moving
                //! the stored copy Executing body with moving the copy should have higher
                //! priority
                using first_priority = int;
                using second_priority = double;

                template <typename BodyType, typename ItemType, typename FeederType>
                static auto call(const BodyType& call_body, ItemType& call_item,
                                 FeederType& call_feeder, first_priority)
                    -> decltype(parallel_for_each_operator_selector<Body>::call(
                                    call_body, std::move(call_item), &call_feeder),
                                void())
                {
                    parallel_for_each_operator_selector<Body>::call(
                        call_body, std::move(call_item), &call_feeder);
                }

                template <typename BodyType, typename ItemType, typename FeederType>
                static void call(const BodyType& call_body, ItemType& call_item,
                                 FeederType& call_feeder, second_priority)
                {
                    parallel_for_each_operator_selector<Body>::call(call_body, call_item,
                                                                    &call_feeder);
                }
            ...
            class feeder_impl : public feeder<Item> {
                // Avoiding use of copy constructor in a virtual method if the type does not
                // support it
                ...
                {
                    __TBB_ASSERT(false, "Overloading for r-value reference doesn't work or "
                                        "it's not movable and not copyable object");
                }

                void internal_add_copy(const Item& item) override
                {
                    internal_add_copy_impl(typename std::is_copy_constructible<Item>::type(),
                                           item);
                }
            ...
            public:
                feeder_impl(const Body& body, wait_context& w_context,
                            task_group_context& context)
                    : my_body(body), my_wait_context(w_context), my_execution_context(context)
                {
                }
            ...
            struct for_each_iteration_task : public task {
                ...
                for_each_iteration_task(Iterator input_item_ptr, const Body& body,
                                        feeder_impl<Body, Item>* feeder_ptr,
                                        wait_context&            wait_context)
                    : item_ptr(input_item_ptr),
                      my_body(body),
                      my_feeder_ptr(feeder_ptr),
                      parent_wait_context(wait_context)
                {
                }
                ...
                {
                    parallel_for_each_operator_selector<Body>::call(my_body, *item_ptr,
                                                                    my_feeder_ptr);
                    finalize();
                    return nullptr;
                }
            ...
            // If the element can be passed to the body as an rvalue - this iterator should be
            // move_iterator
            ...
            struct input_iteration_task_iterator_helper<
                Body, Item,
                tbb::detail::void_t<decltype(parallel_for_each_operator_selector<Body>::call(
                    std::declval<const Body&>(), std::declval<Item&&>(),
                    std::declval<feeder_impl<Body, Item>*>()))>> {
                using type = std::move_iterator<Item*>;
            };

            /** Split one block task to several(max_block_size) iteration tasks for input
               iterators
                @ingroup algorithms **/
            template <typename Body, typename Item>
            struct input_block_handling_task : public task {
                ...
                using feeder_type = feeder_impl<Body, Item>;
                using iteration_task_iterator_type =
                    typename input_iteration_task_iterator_helper<Body, Item>::type;
                using iteration_task =
                    for_each_iteration_task<iteration_task_iterator_type, Body, Item>;

                input_block_handling_task(wait_context&       root_wait_context,
                                          task_group_context& e_context, const Body& body,
                                          feeder_impl<Body, Item>* feeder_ptr,
                                          small_object_allocator&  alloc)
                    : my_size(0),
                      my_wait_context(0),
                      my_root_wait_context(root_wait_context),
                      my_execution_context(e_context),
                      my_allocator(alloc)
                {
                    auto item_it = block_iteration_space.begin();
                    for (auto* it = task_pool.begin(); it != task_pool.end(); ++it) {
                        new (it) iteration_task(iteration_task_iterator_type(item_it++), body,
                                                feeder_ptr, my_wait_context);
                    }
                }
                ...
                task* execute(execution_data& ed) override
                {
                    ...
                    for (size_t counter = 1; counter < my_size; ++counter) {
                        my_wait_context.reserve();
                        spawn(*(task_pool.begin() + counter), my_execution_context);
                    }
                    my_wait_context.reserve();
                    execute_and_wait(*task_pool.begin(), my_execution_context, my_wait_context,
                ...
                {
                    for (size_t counter = 0; counter < max_block_size; ++counter) {
            ...
            /** Split one block task to several(max_block_size) iteration tasks for forward
               iterators
                @ingroup algorithms **/
            template <typename Iterator, typename Body, typename Item>
            struct forward_block_handling_task : public task {
                ...
                forward_block_handling_task(Iterator first, size_t size,
                                            wait_context&       w_context,
                                            task_group_context& e_context, const Body& body,
                                            feeder_impl<Body, Item>* feeder_ptr,
                                            small_object_allocator&  alloc)
                    : my_size(size),
                      my_wait_context(0),
                      my_root_wait_context(w_context),
                      my_execution_context(e_context),
                      my_allocator(alloc)
                {
                    ...
                    for (size_t i = 0; i < size; i++) {
                        new (task_it++)
                            iteration_task(first, body, feeder_ptr, my_wait_context);
                        ++first;
                    }
                }
                ...
                {
                    for (size_t counter = 0; counter < my_size; ++counter) {
                        (task_pool.begin() + counter)->~iteration_task();
                    }
                }
            ...
            /** Body for parallel_for algorithm.
              * Allows to redirect operations under random access iterators range to the
            ...
            public:
                parallel_for_body_wrapper(Iterator first, const Body& body,
                                          feeder_impl<Body, Item>* feeder_ptr)
    ...
    #pragma ivdep
#endif
                    for (size_t count = range.begin(); count != range.end(); count++) {
                        parallel_for_each_operator_selector<Body>::call(
                            my_body, *(my_first + count), my_feeder_ptr);
                    }
                }
            };  // class parallel_for_body_wrapper
            ...
            template <typename It>
            using iterator_tag_dispatch = typename std::conditional<
                std::is_base_of<std::random_access_iterator_tag, tag<It>>::value,
                std::random_access_iterator_tag,
                typename std::conditional<
                    std::is_base_of<std::forward_iterator_tag, tag<It>>::value,
                    std::forward_iterator_tag, std::input_iterator_tag>::type>::type;

            template <typename Body, typename Iterator, typename Item>
            using feeder_is_required = tbb::detail::void_t<decltype(std::declval<const Body>()(
            ...
            template <typename Iterator, typename Body, typename Item>
            class feeder_holder<Iterator, Body, Item,
                                feeder_is_required<Body, Iterator, Item>> {
            public:
                feeder_holder(wait_context& w_context, task_group_context& context,
                              const Body& body)
            ...
            public:
                for_each_root_task_base(Iterator first, Iterator last, const Body& body,
                    ...
                    : my_first(first),
                      ...
                      my_execution_context(e_context),
                      my_body(body),
                      my_feeder_holder(my_wait_context, my_execution_context, my_body)
                {
                    my_wait_context.reserve();
                }
            ...
            /** parallel_for_each algorithm root task - most generic version
              * Splits input range to blocks
                @ingroup algorithms **/
            template <typename Iterator, typename Body, typename Item,
                      typename IteratorTag = iterator_tag_dispatch<Iterator>>
            ...
            private:
                task* execute(execution_data& ed) override
                {
                    ...
                    small_object_allocator alloc{};
                    auto block_handling_task = alloc.new_object<block_handling_type>(
                        ed, this->my_wait_context, this->my_execution_context, this->my_body,
                        this->my_feeder_holder.feeder_ptr(), alloc);

                    auto* block_iterator = block_handling_task->block_iteration_space.begin();
                    for (; !(this->my_first == this->my_last) &&
                           block_handling_task->my_size < block_handling_type::max_block_size;
                         ++this->my_first) {
            ...
            private:
                task* execute(execution_data& ed) override
                {
                    using block_handling_type =
                        forward_block_handling_task<Iterator, Body, Item>;
                    if (this->my_first == this->my_last) {
                        this->my_wait_context.release();
                        return nullptr;
                    }
                    ...
                    for (; !(this->my_first == this->my_last) &&
                           block_size < block_handling_type::max_block_size;
                         ++this->my_first) {
                        ++block_size;
                    }
                    ...
                    small_object_allocator alloc{};
                    auto block_handling_task = alloc.new_object<block_handling_type>(
                        ed, first_block_element, block_size, this->my_wait_context,
                        this->my_execution_context, this->my_body,
                        this->my_feeder_holder.feeder_ptr(), alloc);
            ...
            /** parallel_for_each algorithm root task - random access iterator based
              specialization
              * Splits input range to blocks
                @ingroup algorithms **/
            template <typename Iterator, typename Body, typename Item>
                ...
                task* execute(execution_data&) override
                {
                    tbb::parallel_for(
                        tbb::blocked_range<size_t>(
                            0, std::distance(this->my_first, this->my_last)),
                        parallel_for_body_wrapper<Iterator, Body, Item>(
                            this->my_first, this->my_body, this->my_feeder_holder.feeder_ptr()),
            ...
            /** Helper for getting item type. If item type can be deduced from feeder - got it
               from feeder, if feeder is generic - got item type from range.
                @ingroup algorithms */
            template <typename Body, typename Item, typename FeederArg>
            auto feeder_argument_parser(void (Body::*)(Item, feeder<FeederArg>&) const)
                -> FeederArg;

            template <typename Body, typename>
            decltype(feeder_argument_parser<Body>(&Body::operator()))
            get_item_type_impl(int);  // for (T, feeder<T>)
            template <typename Body, typename Item>
            Item get_item_type_impl(...);  // stub
            ...
            concept parallel_for_each_iterator_body = parallel_for_each_body<
                Body, iterator_reference_type<Iterator>,
                feeder_item_type<Body, iterator_reference_type<Iterator>>>;

            template <typename Body, typename Range>
            concept parallel_for_each_range_body =
                parallel_for_each_body<Body, range_reference_type<Range>,
                                       feeder_item_type<Body, range_reference_type<Range>>>;
#endif

            /** Implements parallel iteration over a range.
                @ingroup algorithms */
            template <typename Iterator, typename Body>
            void run_parallel_for_each(Iterator first, Iterator last, const Body& body,
                                       task_group_context& context)
            {
                if (!(first == last)) {
                    using ItemType =
                    ...
                    for_each_root_task<Iterator, Body, ItemType> root_task(first, last, body,
                                                                           w_context, context);
            ...
            /** \page parallel_for_each_body_req Requirements on parallel_for_each body
                ...
                - \code
                  ...
                  \endcode                                               Process item.
                                                                         May be invoked
            concurrently  for the same \c this but different \c item.
            ...
            **/
            ...
            /** \name parallel_for_each
                See also requirements on \ref parallel_for_each_body_req "parallel_for_each
               Body". **/
            //@{
            //! Parallel iteration over a range, with optional addition of more work.
            /** @ingroup algorithms */
            template <typename Iterator, typename Body>
            __TBB_requires(std::input_iterator<Iterator>&& parallel_for_each_iterator_body<
                           Body, Iterator>) void parallel_for_each(Iterator    first,
                                                                   Iterator    last,
                                                                   const Body& body)
            {
            ...
            __TBB_requires(container_based_sequence<Range, std::input_iterator_tag>&&
                               parallel_for_each_range_body<
                                   Body, Range>) void parallel_for_each(Range&      rng,
                                                                        const Body& body)
            {
            ...
            __TBB_requires(container_based_sequence<Range, std::input_iterator_tag>&&
                               parallel_for_each_range_body<
                                   Body, Range>) void parallel_for_each(const Range& rng,
                                                                        const Body&  body)
            {
                parallel_for_each(std::begin(rng), std::end(rng), body);
            }

            //! Parallel iteration over a range, with optional addition of more work and
            ...
            template <typename Range, typename Body>
            __TBB_requires(
                container_based_sequence<Range, std::input_iterator_tag>&&
                    parallel_for_each_range_body<
                        Body, Range>) void parallel_for_each(Range& rng, const Body& body,
                                                             task_group_context& context)
            {
            ...
            __TBB_requires(
                container_based_sequence<Range, std::input_iterator_tag>&&
                    parallel_for_each_range_body<
                        Body, Range>) void parallel_for_each(const Range& rng, const Body& body,
                                                             task_group_context& context)
            {
                parallel_for_each(std::begin(rng), std::end(rng), body, context);
            }
