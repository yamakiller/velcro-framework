...
namespace tbb {
    namespace detail {
        namespace d1 {
            ...
            //! m_base of thread-safe pool allocator for variable-size requests
            class pool_base : no_copy {
                // Pool interface is separate from standard allocator classes because it has
                // to maintain internal state, no copy or assignment. Move and swap are
                // possible.
            public:
                //! Reset pool to reuse its memory (free all objects at once)
                void recycle()
                {
                    rml::pool_reset(my_pool);
                }
    ...
    #pragma warning(disable : 4100)
            ...
            protected:
                ...
                friend bool operator==(const memory_pool_allocator<V, R>& a,
    ...
    #pragma warning(pop)
            ...
            //! Analogous to std::allocator<void>, as defined in ISO C++ Standard,
            //! Section 20.4.1
            /** @ingroup memory_allocation */
            template <typename P>
            ...
            inline bool operator==(const memory_pool_allocator<T, P>& a,
                                   const memory_pool_allocator<U, P>& b)
            {
                return a.my_pool == b.my_pool;
            }
            ...
            public:
                ...
                {
                    destroy();
                }  // call the callbacks first and destroy my_alloc latter
            };
    ...
    #pragma warning(disable : 4702)
                                                       ...
                                                       size_t raw_bytes)
            {
                ...
                self.my_alloc.deallocate(static_cast<typename Alloc::value_type*>(raw_ptr),
                                         raw_bytes / unit_size);
                return 0;
            }
#if __TBB_MSVC_UNREACHABLE_CODE_IGNORED
