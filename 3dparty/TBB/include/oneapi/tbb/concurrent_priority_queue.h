    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                explicit concurrent_priority_queue(
                    size_type init_capacity, const allocator_type& alloc = allocator_type())
                    : mark(0), my_size(0), my_compare(), data(alloc)
                {
                ...
                explicit concurrent_priority_queue(
                    size_type init_capacity, const Compare& compare,
                ...
                {
                    if (this != &other) {
                        // TODO: Assert if exceptions from std::vector::operator=(vector&&)
                ...
                /* This operation can be safely used concurrently with other push, try_pop or
                 * emplace operations. */
            ...
            private:
                ...
                void handle_operations(cpq_operation* op_list)
                {
                    ...
                    // First pass processes all constant (amortized; reallocation may happen)
                    // time pushes and pops.
                    while (op_list) {
                        // ITT note: &(op_list->status) tag is used to cover accesses to op_list
                        // node. This thread is going to handle the operation, and so will
                        // acquire it and perform the associated operation w/o triggering a race
                        // condition; the thread that created the operation is waiting on the
                        // status field, so when this thread is done with the operation, it will
                        // perform a store_with_release to give control back to the waiting
                        // thread in aggregator::insert_operation.
                        // TODO: enable
                        ...
                        if (tmp->type == POP_OP) {
                            if (mark < data.size() && my_compare(data[0], data.back())) {
                                // there are newly pushed elems and the last one is higher than
                                // top
                                *(tmp->elem) = std::move(data.back());
                                my_size.store(my_size.load(std::memory_order_relaxed) - 1,
                                              std::memory_order_relaxed);
                                tmp->status.store(uintptr_t(SUCCEEDED),
                            ...
                            __TBB_ASSERT(tmp->type == PUSH_OP || tmp->type == PUSH_RVALUE_OP,
                                         "Unknown operation");
                ...
                void push_back_helper_impl(const T& value,
                                           /*is_copy_constructible = */ std::true_type)
                {
                    data.push_back(value);
                }

                void push_back_helper_impl(const T&,
                                           /*is_copy_constructible = */ std::false_type)
                {
                    __TBB_ASSERT(false,
                                 "error: calling tbb::concurrent_priority_queue.push(const "
                                 "value_type&) for move-only type");
                }
            ...
            template <typename It, typename Comp = std::less<iterator_value_t<It>>,
            ...
            concurrent_priority_queue(It, It, Alloc)
                -> concurrent_priority_queue<iterator_value_t<It>,
