...
// Intel Compiler on OSX cannot create atomics objects that instantiated from non-fundamental
// types
#if __INTEL_COMPILER && __APPLE__
            template <>
            struct internal_ets_key_selector<sizeof(size_t)> {
            ...
            public:
                ...
                struct slot {
                    ...
                    {
                        // TODO: maybe claim ptr, because key_type is not guaranteed to fit into
                        // word size
            ...
            protected:
                ...
                void free(array* a)
                {
                    ...
                    free_array(static_cast<void*>(a), size_t(sizeof(array) + n * sizeof(slot)));
                }
                                            ...
                                            void* (*add_element)(ets_base<E2>&, void*))
                {
                    ...
                    if (!other.my_root.load(std::memory_order_relaxed))
                        return;
                    array* root =
                        allocate(other.my_root.load(std::memory_order_relaxed)->lg_size);
                    ...
                    for (array* r = other.my_root.load(std::memory_order_relaxed); r;
                         r = r->next) {
                        for (size_t i = 0; i < r->size(); ++i) {
                            slot& s1 = r->at(i);
                            if (!s1.empty()) {
                                for (size_t j = root->start(std::hash<key_type>{}(
                                         s1.key.load(std::memory_order_relaxed)));
                                     ; j = (j + 1) & mask) {
                                    slot& s2 = root->at(j);
                                    if (s2.empty()) {
                                        s2.ptr = add_element(static_cast<ets_base<E2>&>(*this),
                                                             s1.ptr);
            ...
            void* ets_base<ETS_key_type>::table_lookup(bool& exists)
            {
                ...
                // Key does not yet exist.  The density of slots in the table does not exceed
                // 0.5, for if this will occur a new table is allocated with double the current
                // table size, which is swapped in as the new root table.  So an empty slot is
                // guaranteed.
                exists = false;
                found = create_local();
                {
                    ...
                    if (!r || c > r->size() / 2) {
                        ...
                        for (;;) {
                            ...
                            if (new_r->lg_size >= s) {
                                // Another thread inserted an equal or  bigger array, so our
            ...
            insert:
                // Whether a slot has been found in an older table, or if it has been inserted
                // at this level, it has already been accounted for in the total.  Guaranteed to
                // be room for it, and it is not present, so search for empty slot and use it.
            ...
            class ets_base<ets_key_per_instance> : public ets_base<ets_no_key> {
...
#if WIN32 || WIN64
    #if __TBBWIN8UI_SUPPORT
                using tls_key_t = DWORD;
                void create_key()
                {
                    my_key = FlsAlloc(NULL);
                }
                void destroy_key()
                {
                    FlsFree(my_key);
                ...
                virtual void* create_array(size_t _size) override = 0;  // _size in bytes
                virtual void  free_array(void*  ptr,
                                         size_t _size) override = 0;  // size in bytes
            protected:
                ets_base()
                {
                    create_key();
            ...
            class enumerable_thread_specific_iterator {
                ...
                operator-(const enumerable_thread_specific_iterator<C, T>& i,
                          const enumerable_thread_specific_iterator<C, U>& j);
            ...
            public:
                ...
                enumerable_thread_specific_iterator(const Container&              container,
                    ...
                    : my_container(&const_cast<Container&>(container)),
                      my_index(index),
                      my_value(nullptr)
                {
                }
                ...
                //! Default constructor
                enumerable_thread_specific_iterator()
                    : my_container(nullptr), my_index(0), my_value(nullptr)
                {
                }
                ...
                enumerable_thread_specific_iterator(
                    ...
                    : my_container(other.my_container),
                      ...
                      my_value(const_cast<Value*>(other.my_value))
                {
                }

                enumerable_thread_specific_iterator operator+(std::ptrdiff_t offset) const
                {
                    return enumerable_thread_specific_iterator(*my_container,
                                                               my_index + offset);
                }

                friend enumerable_thread_specific_iterator
                operator+(std::ptrdiff_t offset, enumerable_thread_specific_iterator v)
                {
                    return enumerable_thread_specific_iterator(*v.my_container,
                                                               v.my_index + offset);
                }
            ...
            class segmented_iterator {
                template <typename C, typename T, typename U>
                friend bool operator==(const segmented_iterator<C, T>& i,
            ...
            public:
                ...
                segmented_iterator(const segmented_iterator<SegmentedContainer, U>& other)
                    : my_segcont(other.my_segcont),
                      ...
                      // can we assign a default-constructed iterator to inner if we're at the
                      // end?
                ...
                template <typename U>
                segmented_iterator&
                operator=(const segmented_iterator<SegmentedContainer, U>& other)
                {
                    my_segcont = other.my_segcont;
                    outer_iter = other.outer_iter;
                ...
                segmented_iterator& operator=(const outer_iterator& new_outer_iter)
                {
                    ...
                    // Assert that this iterator points to something inside the segmented
                    // container
                    for (outer_iter = new_outer_iter; outer_iter != my_segcont->end();
                         ++outer_iter) {
                        if (!outer_iter->empty()) {
                ...
                {
                    __TBB_ASSERT(my_segcont != nullptr, NULL);
                    return (outer_iter == other_outer && (outer_iter == my_segcont->end() ||
                                                          inner_iter == outer_iter->begin()));
                }
                ...
                {
                    __TBB_ASSERT(my_segcont != nullptr, NULL);
                    __TBB_ASSERT(outer_iter != my_segcont->end(),
            ...
            private:
                ...
                void advance_me()
                {
                    __TBB_ASSERT(my_segcont != nullptr, NULL);
                    __TBB_ASSERT(outer_iter != my_segcont->end(),
                                 NULL);  // not true if there are no inner containers
                    __TBB_ASSERT(inner_iter != outer_iter->end(),
                                 NULL);  // not true if the inner containers are all empty.
                    ++inner_iter;
                    while (inner_iter == outer_iter->end() &&
                           ++outer_iter != my_segcont->end()) {
                        inner_iter = outer_iter->begin();
                    }
                }
            ...
            {
                if (i.my_segcont != j.my_segcont)
                    return false;
                if (i.my_segcont == nullptr)
                    return true;
                if (i.outer_iter != j.outer_iter)
                    return false;
                if (i.outer_iter == i.my_segcont->end())
                    return true;
            ...
            struct construct_by_args : no_assign {
                ...
                {
                    call(
                        [where](const typename std::decay<P>::type&... args) {
                            new (where) T(args...);
                        },
                        pack);
                }
            ...
            class callback_leaf : public callback_base, Constructor {
                template <typename... P>
                callback_leaf(P&&... params) : Constructor(std::forward<P>(params)...)
                {
                }
                // TODO: make the construction/destruction consistent (use
                ...
                void destroy() override
                {
                    ...
                    tbb::detail::allocator_traits_base<my_allocator_type>::destroy(alloc, this);
                    tbb::detail::allocator_traits_base<my_allocator_type>::deallocate(alloc,
                                                                                      this, 1);
                }
            ...
            */
            ...
            // A predicate that can be used for a compile-time compatibility Assert of ETS
            // instances Ideally, it should have been declared inside the ETS class, but
            // unfortunately in that case VS2013 does not enable the variadic constructor.
            template <typename T, typename ETS>
            struct is_compatible_ets : std::false_type {};
            ...
            // expression
            template <typename T>
            using has_empty_braces_operator = decltype(std::declval<T>()());
            template <typename T>
            using is_callable_no_args = supports<T, has_empty_braces_operator>;

            //! The enumerable_thread_specific container
            /** enumerable_thread_specific has the following properties:
                - thread-local copies are lazily created, with default, exemplar or function
            initialization.
                - thread-local copies do not move (during lifetime, and excepting clear()) so
            the address of a copy is invariant.
                - the contained objects need not have operator=() defined if combine is not
            used.
                - enumerable_thread_specific containers may be copy-constructed or assigned.
                - thread-local copies can be managed by hash-table, or can be accessed via TLS
            storage for speed.
                - outside of parallel contexts, the contents of all thread-local copies are
            accessible by iterator or using combine or combine_each methods

            @par Segmented iterator
                When the thread-local objects are containers with input_iterators defined, a
            segmented iterator may be used to iterate over all the elements of all thread-local
            copies.
            ...
            @par combine and combine_each
                ...
                - neither method modifies the contents of the object (though there is no
            guarantee that the applied methods do not modify the object.)
                - Both are evaluated in serial context (the methods are assumed to be
            non-benign.)
            ...
            class enumerable_thread_specific : ets_base<ETS_key_type> {
                ...
                // TODO: consider unifying the callback mechanism for all create_local* methods
                // below
                ...
                static void* create_local_by_copy(ets_base<ETS_key_type>& base, void* p)
                {
                    enumerable_thread_specific& ets =
                        static_cast<enumerable_thread_specific&>(base);
            ...
            public:
                ...
                // Iterator types
                using iterator =
                    enumerable_thread_specific_iterator<internal_collection_type, value_type>;
                using const_iterator =
                    enumerable_thread_specific_iterator<internal_collection_type,
                                                        const value_type>;
                ...
                //! Constructor with initializer functor. Each local instance of T is
                //! constructed by T(finit()).
                template <typename Finit,
                          typename = typename std::enable_if<is_callable_no_args<
                              typename std::decay<Finit>::type>::value>::type>
                ...
                //! Constructor with exemplar. Each local instance of T is copy-constructed from
                //! the exemplar.
                ...
                //! Variadic constructor with initializer arguments.  Each local instance of T
                //! is constructed by T(args...)
                template <typename P1, typename... P,
                          typename = typename std::enable_if<
                              !is_callable_no_args<typename std::decay<P1>::type>::value &&
                              !is_compatible_ets<T, typename std::decay<P1>::type>::value &&
                              !std::is_same<T, typename std::decay<P1>::type>::value>::type>
                enumerable_thread_specific(P1&& arg1, P&&... args)
                    : my_construct_callback(callback_leaf<construct_by_args<T, P1, P...>>::make(
                          std::forward<P1>(arg1), std::forward<P>(args)...))
                {
                }
                ...
                ~enumerable_thread_specific()
                {
                    ...
                    // Deallocate the hash table before overridden free_array() becomes
                    // inaccessible
            ...
            private:
                ...
                void internal_copy(const enumerable_thread_specific<T, A2, C2>& other)
                {
                    ...
                    static_assert(
                        (is_compatible_ets<T,
                                           typename std::decay<decltype(other)>::type>::value),
                        "is_compatible_ets fails");
                    // Initialize my_construct_callback first, so that it is valid even if rest
                    // of this routine throws an exception.
            ...
            public:
                ...
                {
                    __TBB_ASSERT(static_cast<void*>(this) != static_cast<const void*>(&other),
                                 NULL);  // Objects of different types
                    this->clear();
                    my_construct_callback->destroy();
                    internal_copy(other);
                    return *this;
            ...
            public:
                ...
                flattened2d(const Container& c, typename Container::const_iterator b,
