...
#include "TBB/include/oneapi/tbb/detail/_segment_table.h"
...
namespace tbb {
    namespace detail {
        namespace d1 {
            ...
            template <typename FVector, typename Value>
            class vector_iterator {
                using vector_type = FVector;
            ...
            public:
                ...
                friend vector_iterator<Vec, Val>
                operator+(typename vector_iterator<Vec, Val>::difference_type,
                          const vector_iterator<Vec, Val>&);

                template <typename Vec, typename Val1, typename Val2>
                friend typename vector_iterator<Vec, Val1>::difference_type
                operator-(const vector_iterator<Vec, Val1>&, const vector_iterator<Vec, Val2>&);

                template <typename Vec, typename Val1, typename Val2>
                friend bool operator==(const vector_iterator<Vec, Val1>&,
                                       const vector_iterator<Vec, Val2>&);
            ...
            private:
                vector_iterator(const vector_type& vector, size_type index,
                                value_type* item = nullptr)
                    : my_vector(const_cast<vector_type*>(&vector)),
                      my_index(index),
                      my_item(item)
                {
                }
            ...
            public:
                vector_iterator()
                    : my_vector(nullptr), my_index(~size_type(0)), my_item(nullptr)
                {
                }

                vector_iterator(
                    const vector_iterator<vector_type, typename vector_type::value_type>& other)
                ...
                vector_iterator& operator++()
                {
                    ...
                    if (my_item != nullptr) {
                        if (vector_type::is_first_element_in_segment(my_index)) {
                            // If the iterator crosses a segment boundary, the pointer become
                            // invalid as possibly next segment is in another memory location
                            my_item = nullptr;
                        } else {
                            ++my_item;
                        }
                ...
                vector_iterator& operator--()
                {
                    __TBB_ASSERT(my_index > 0, "operator--() applied to iterator already at "
                                               "beginning of concurrent_vector");
                    --my_index;
                    if (my_item != nullptr) {
                        if (vector_type::is_first_element_in_segment(my_index)) {
                            // If the iterator crosses a segment boundary, the pointer become
            ...
            private:
                ...
                // Caches my_vector *it;
                // If my_item == nullptr cached value is not available use
                // internal_subscript(my_index)
                mutable value_type* my_item;
            };  // class vector_iterator

            template <typename FVector, typename T>
            vector_iterator<FVector, T>
            operator+(typename vector_iterator<FVector, T>::difference_type offset,
                      const vector_iterator<FVector, T>&                    v)
            {
                return vector_iterator<FVector, T>(*v.my_vector, v.my_index + offset);
            }

            template <typename FVector, typename T, typename U>
            typename vector_iterator<FVector, T>::difference_type
            operator-(const vector_iterator<FVector, T>& i,
                      const vector_iterator<FVector, U>& j)
            {
                using difference_type = typename vector_iterator<FVector, T>::difference_type;
                return static_cast<difference_type>(i.my_index) -
                       static_cast<difference_type>(j.my_index);
            }

            template <typename FVector, typename T, typename U>
            bool operator==(const vector_iterator<FVector, T>& i,
                            const vector_iterator<FVector, U>& j)
            {
            ...
            static constexpr size_t embedded_table_num_segments = 3;

            template <typename T, typename Allocator = tbb::cache_aligned_allocator<T>>
            class concurrent_vector
                : private segment_table<T, Allocator, concurrent_vector<T, Allocator>,
                                        embedded_table_num_segments> {
                using self_type = concurrent_vector<T, Allocator>;
                using base_type =
                    segment_table<T, Allocator, self_type, embedded_table_num_segments>;

                friend class segment_table<T, Allocator, self_type,
                                           embedded_table_num_segments>;
                ...
                public:
                    ...
                    generic_range_type(const generic_range_type<U>& r)
                        : blocked_range<Iterator>(r.begin(), r.end(), r.grainsize())
                    {
                    }
                    generic_range_type(generic_range_type& r, split)
                        : blocked_range<Iterator>(r, split())
                    {
                    }
            ...
            public:
                ...
                {
                    try_call([&] { grow_by(count, value); }).on_exception([&] {
                        base_type::clear();
                    });
                }
                ...
                explicit concurrent_vector(size_type             count,
                    ...
                    : concurrent_vector(alloc)
                {
                    try_call([&] { grow_by(count); }).on_exception([&] { base_type::clear(); });
                }
                ...
                concurrent_vector(InputIterator first, InputIterator last,
                    ...
                    : concurrent_vector(alloc)
                {
                    try_call([&] { grow_by(first, last); }).on_exception([&] {
                        base_type::clear();
                    });
                }
                ...
                concurrent_vector(const concurrent_vector& other)
                    : base_type(
                          segment_table_allocator_traits::select_on_container_copy_construction(
                              other.get_allocator()))
                {
                    try_call([&] { grow_by(other.begin(), other.end()); }).on_exception([&] {
                        base_type::clear();
                    });
                }
                ...
                concurrent_vector&
                operator=(concurrent_vector&& other) noexcept(is_noexcept_assignment)
                {
                ...
                typename std::enable_if<is_input_iterator<ForwardIterator>::value,
                                        iterator>::type
            ...
            private:
                ...
                using segment_table_allocator_traits =
                    typename base_type::segment_table_allocator_traits;
                using segment_index_type = typename base_type::segment_index_type;

                using segment_element_type = typename base_type::value_type;
                using segment_element_allocator_type =
                    typename allocator_traits_type::template rebind_alloc<segment_element_type>;
                using segment_element_allocator_traits =
                    tbb::detail::allocator_traits_base<segment_element_allocator_type>;

                segment_table_type
                allocate_long_table(const typename base_type::atomic_segment* embedded_table,
                                    size_type                                 start_index)
                {
                    __TBB_ASSERT(start_index <= this->embedded_table_size,
                                 "Start index out of embedded table");

                    // If other threads are trying to set pointers in the short segment, wait
                    // for them to finish their assignments before we copy the short segment to
                    // the long segment. Note: grow_to_at_least depends on it
                    for (segment_index_type i = 0; this->segment_base(i) < start_index; ++i) {
                        spin_wait_while_eq(embedded_table[i], segment_type(nullptr));
                    }

                    // It is possible that the table was extend by a thread allocating
                    // first_block, need to Assert this.
                    if (this->get_table() != embedded_table) {
                        return nullptr;
                    }

                    // Allocate long segment table and fill with null pointers
                    segment_table_type new_segment_table =
                        segment_table_allocator_traits::allocate(base_type::get_allocator(),
                                                                 this->pointers_per_long_table);
                    // Copy segment pointers from the embedded table
                    for (size_type segment_index = 0;
                         segment_index < this->pointers_per_embedded_table; ++segment_index) {
                        segment_table_allocator_traits::construct(
                            base_type::get_allocator(), &new_segment_table[segment_index],
                            embedded_table[segment_index].load(std::memory_order_relaxed));
                    }
                    for (size_type segment_index = this->pointers_per_embedded_table;
                         segment_index < this->pointers_per_long_table; ++segment_index) {
                        segment_table_allocator_traits::construct(
                            base_type::get_allocator(), &new_segment_table[segment_index],
                            nullptr);
                    }
                ...
                segment_type create_segment(segment_table_type table,
                                            segment_index_type seg_index, size_type index)
                {
                    size_type first_block =
                        this->my_first_block.load(std::memory_order_relaxed);
                    // First block allocation
                    if (seg_index < first_block) {
                        // If 0 segment is already allocated, then it remains to wait until the
                        // segments are filled to requested
                        if (table[0].load(std::memory_order_acquire) != nullptr) {
                            spin_wait_while_eq(table[seg_index], segment_type(nullptr));
                            return nullptr;
                        }

                        segment_element_allocator_type segment_allocator(
                            base_type::get_allocator());
                        segment_type new_segment = nullptr;
                        size_type    first_block_size = this->segment_size(first_block);
                        try_call([&] {
                            new_segment = segment_element_allocator_traits::allocate(
                                segment_allocator, first_block_size);
                        }).on_exception([&] {
                            segment_type disabled_segment = nullptr;
                            if (table[0].compare_exchange_strong(
                                    disabled_segment, this->segment_allocation_failure_tag)) {
                                size_type end_segment = table == this->my_embedded_table
                                                            ? this->pointers_per_embedded_table
                                                            : first_block;
                                for (size_type i = 1; i < end_segment; ++i) {
                                    table[i].store(this->segment_allocation_failure_tag,
                        ...
                        if (table[0].compare_exchange_strong(disabled_segment, new_segment)) {
                            ...
                            // Other threads can wait on a snapshot of an embedded table, need
                            // to fill it.
                            for (size_type i = 1;
                                 i < first_block && i < this->pointers_per_embedded_table;
                                 ++i) {
                                this->my_embedded_table[i].store(new_segment,
                                                                 std::memory_order_release);
                            }
                        } else if (new_segment != this->segment_allocation_failure_tag) {
                            // Deallocate the memory
                            segment_element_allocator_traits::deallocate(
                                segment_allocator, new_segment, first_block_size);
                            // 0 segment is already allocated, then it remains to wait until the
                            // segments are filled to requested
                            spin_wait_while_eq(table[seg_index], segment_type(nullptr));
                        }
                    } else {
                        ...
                        if (index == offset) {
                            __TBB_ASSERT(table[seg_index].load(std::memory_order_relaxed) ==
                                             nullptr,
                                         "Only this thread can enable this segment");
                            segment_element_allocator_type segment_allocator(
                                base_type::get_allocator());
                            segment_type new_segment = this->segment_allocation_failure_tag;
                            try_call([&] {
                                new_segment = segment_element_allocator_traits::allocate(
                                    segment_allocator, this->segment_size(seg_index));
                ...
                size_type number_of_elements_in_segment(segment_index_type seg_index)
                {
                    ...
                    if (seg_index == 0) {
                        ...
                        return curr_segment_base * 2 > curr_vector_size
                                   ? curr_vector_size - curr_segment_base
                                   : curr_segment_base;
                    }
                }
                ...
                void deallocate_segment(segment_type address, segment_index_type seg_index)
                {
                    ...
                    if (seg_index >= first_block) {
                        segment_element_allocator_traits::deallocate(
                            segment_allocator, address, this->segment_size(seg_index));
                    } else if (seg_index == 0) {
                        size_type elements_to_deallocate = first_block > 0
                                                               ? this->segment_size(first_block)
                                                               : this->segment_size(0);
                        segment_element_allocator_traits::deallocate(segment_allocator, address,
                                                                     elements_to_deallocate);
                    }
                }
                ...
                void destroy_segment(segment_type address, segment_index_type seg_index)
                {
                    ...
                    for (size_type i = 0; i < elements_to_destroy; ++i) {
                        segment_element_allocator_traits::destroy(segment_allocator,
                                                                  address + i);
                    }
                ...
                void copy_segment(segment_index_type seg_index, segment_type from,
                                  segment_type to)
                {
                    size_type i = 0;
                    try_call([&] {
                        for (; i != number_of_elements_in_segment(seg_index); ++i) {
                            segment_table_allocator_traits::construct(
                                base_type::get_allocator(), to + i, from[i]);
                        }
                    }).on_exception([&] {
                        // Zero-initialize items left not constructed after the exception
                        zero_unconstructed_elements(this->get_segment(seg_index) + i,
                                                    this->segment_size(seg_index) - i);

                        segment_index_type last_segment = this->segment_index_of(
                            this->my_size.load(std::memory_order_relaxed));
                        ...
                        for (segment_index_type j = seg_index + 1; j != last_segment; ++j) {
                            ...
                            if (curr_segment) {
                                zero_unconstructed_elements(curr_segment +
                                                                this->segment_base(j),
                                                            this->segment_size(j));
                            }
                        }
                        this->my_size.store(this->segment_size(seg_index) + i,
                                            std::memory_order_relaxed);
                    });
                }

                // move_segment function is required by the segment_table base class
                void move_segment(segment_index_type seg_index, segment_type from,
                                  segment_type to)
                {
                    size_type i = 0;
                    try_call([&] {
                        for (; i != number_of_elements_in_segment(seg_index); ++i) {
                            segment_table_allocator_traits::construct(
                                base_type::get_allocator(), to + i, std::move(from[i]));
                        }
                    }).on_exception([&] {
                        // Zero-initialize items left not constructed after the exception
                        zero_unconstructed_elements(this->get_segment(seg_index) + i,
                ...
                {
                    // An element is the first in a segment if its index is equal to a power of
                    // two
                ...
                {
                    __TBB_ASSERT(index < this->my_size.load(std::memory_order_relaxed),
                                 "Invalid subscript index");
                    return base_type::template internal_subscript<
                        /*allow_out_of_range_access=*/false>(index);
                }

                const_reference internal_subscript_with_exceptions(size_type index) const
                {
                    return const_cast<self_type*>(this)->internal_subscript_with_exceptions(
                        index);
                }
                ...
                reference internal_subscript_with_exceptions(size_type index)
                {
                    ...
                    segment_table_type table =
                        this->my_segment_table.load(std::memory_order_acquire);
                ...
                iterator internal_emplace_back(Args&&... args)
                {
                    ...
                    auto element_address = &base_type::template internal_subscript<
                        /*allow_out_of_range_access=*/true>(old_size);
                    ...
                    segment_table_allocator_traits::construct(base_type::get_allocator(),
                                                              element_address,
                ...
                template <typename... Args>
                void internal_loop_construct(segment_table_type table, size_type start_idx,
                                             size_type end_idx, const Args&... args)
                {
                    static_assert(sizeof...(Args) < 2, "Too many parameters");
                    for (size_type idx = start_idx; idx < end_idx; ++idx) {
                        auto element_address = &base_type::template internal_subscript<
                            /*allow_out_of_range_access=*/true>(idx);
                        // try_call API is not convenient here due to broken
                        // variadic capture on GCC 4.8.5
                        auto value_guard = make_raii_guard([&] {
                            segment_index_type last_allocated_segment =
                                this->find_last_allocated_segment(table);
                            ...
                            for (size_type i = idx; i < end_idx; ++i) {
                                zero_unconstructed_elements(&this->internal_subscript(i),
                                                            /*count =*/1);
                            }
                        });
                        segment_table_allocator_traits::construct(base_type::get_allocator(),
                                                                  element_address, args...);
                        value_guard.dismiss();
                    }
                }
                ...
                template <typename ForwardIterator>
                void internal_loop_construct(segment_table_type table, size_type start_idx,
                                             size_type end_idx, ForwardIterator first,
                                             ForwardIterator)
                {
                    for (size_type idx = start_idx; idx < end_idx; ++idx) {
                        auto element_address = &base_type::template internal_subscript<
                        ...
                        try_call([&] {
                            segment_table_allocator_traits::construct(
                                base_type::get_allocator(), element_address, *first++);
                        }).on_exception([&] {
                            segment_index_type last_allocated_segment =
                                this->find_last_allocated_segment(table);
                            size_type segment_size = this->segment_size(last_allocated_segment);
                ...
                template <typename... Args>
                iterator internal_grow(size_type start_idx, size_type end_idx,
                                       const Args&... args)
                {
                    this->assign_first_block_if_necessary(this->segment_index_of(end_idx - 1) +
                                                          1);
                    ...
                    if (seg_index > this->my_first_block.load(std::memory_order_relaxed)) {
                        // So that other threads be able to work with the last segment of
                        // grow_by, allocate it immediately. If the last segment is not less
                        ...
                        if (table[seg_index].load(std::memory_order_relaxed) == nullptr) {
                            ...
                            if (first_element >= start_idx && first_element < end_idx) {
                                segment_type segment =
                                    table[seg_index].load(std::memory_order_relaxed);
                    ...
                    return iterator(*this, start_idx,
                                    &base_type::template internal_subscript<
                                        /*allow_out_of_range_access=*/false>(start_idx));
                }
                ...
                iterator internal_grow_to_at_least(size_type new_size, const Args&... args)
                {
                    ...
                    while (old_size < new_size &&
                           !this->my_size.compare_exchange_weak(old_size, new_size)) {
                    }
                    ...
                    for (segment_index_type seg_idx = 0; seg_idx <= end_segment; ++seg_idx) {
                        if (this->get_table()[seg_idx].load(std::memory_order_relaxed) ==
                            nullptr) {
                            atomic_backoff backoff(true);
                            while (this->get_table()[seg_idx].load(std::memory_order_relaxed) ==
                                   nullptr) {
                                backoff.pause();
                            }
                        }
                ...
                void internal_resize(size_type n, const Args&... args)
                {
                    ...
                    if (n > old_size) {
                        ...
                        // Delete segments
                        for (size_type seg_idx = this->segment_index_of(n - 1) + 1;
                             seg_idx <= last_segment; ++seg_idx) {
                            this->delete_segment(seg_idx);
                        }

                        // If n > segment_size(n) => we need to destroy all of the items in the
                        // first segment Otherwise, we need to destroy only items with the index
                        // < n
                        size_type n_segment = this->segment_index_of(n - 1);
                        size_type last_index_to_destroy = std::min(
                            this->segment_base(n_segment) + this->segment_size(n_segment),
                            old_size);
                        // Destroy elements in curr segment
                        for (size_type idx = n; idx < last_index_to_destroy; ++idx) {
                            segment_table_allocator_traits::destroy(
                                base_type::get_allocator(),
                                &base_type::template internal_subscript<
                                    /*allow_out_of_range_access=*/false>(idx));
                        }
                ...
                void destroy_elements()
                {
                    ...
                    for (size_type i = 0; i < this->my_size.load(std::memory_order_relaxed);
                         ++i) {
                        allocator_traits_type::destroy(
                            alloc, &base_type::template internal_subscript<
                                       /*allow_out_of_range_access=*/false>(i));
                    }
                ...
                static bool incompact_predicate(size_type size)
                {
                    ...
                    return size < page_size ||
                           ((size - 1) % page_size < page_size / 2 && size < page_size * 128);
                }
                ...
                void internal_compact()
                {
                    ...
                    segment_table_type       table = this->get_table();
                    const segment_index_type k_end =
                        this->find_last_allocated_segment(table);  // allocated segments
                    const segment_index_type k_stop =
                        curr_size ? this->segment_index_of(curr_size - 1) + 1
                                  : 0;  // number of segments to store existing items: 0=>0;
                                        // 1,2=>1; 3,4=>2; [5-8]=>3;..
                    const segment_index_type first_block =
                        this->my_first_block;  // number of merged segments, getting values from
                                               // atomics
                    ...
                    if (k_stop < first_block) {
                        ...
                        while (k < k_stop &&
                               incompact_predicate(this->segment_size(k) * sizeof(value_type)))
                            k++;
                    }
                    ...
                    if (k != first_block && k) {
                        ...
                        auto buffer_table = segment_table_allocator_traits::allocate(
                            base_type::get_allocator(), max_block);

                        for (size_type seg_idx = 0; seg_idx < max_block; ++seg_idx) {
                            segment_table_allocator_traits::construct(
                                base_type::get_allocator(), &buffer_table[seg_idx],
                        ...
                        size_type index = 0;
                        try_call([&] {
                            for (; index < std::min(this->segment_size(max_block), curr_size);
                                 ++index) {
                                auto                                element_address =
                                    &static_cast<base_type*>(this)->operator[](index);
                                segment_index_type seg_idx = this->segment_index_of(index);
                                segment_table_allocator_traits::construct(
                                    base_type::get_allocator(), element_address,
                                    std::move_if_noexcept(buffer_table[seg_idx].load(
                                        std::memory_order_relaxed)[index]));
                            }
                        }).on_exception([&] {
                            segment_element_allocator_type allocator(
                                base_type::get_allocator());
                            for (size_type i = 0; i < index; ++i) {
                                auto element_adress = &this->operator[](i);
                                segment_element_allocator_traits::destroy(allocator,
                                                                          element_adress);
                            }
                            segment_element_allocator_traits::deallocate(
                                allocator, table[0].load(std::memory_order_relaxed),
                                this->segment_size(max_block));

                            for (size_type seg_idx = 0; seg_idx < max_block; ++seg_idx) {
                                table[seg_idx].store(
                                    buffer_table[seg_idx].load(std::memory_order_relaxed),
                                    std::memory_order_relaxed);
                                buffer_table[seg_idx].store(nullptr, std::memory_order_relaxed);
                            }
                            segment_table_allocator_traits::deallocate(
                                base_type::get_allocator(), buffer_table, max_block);
                        ...
                        // in order for the segment deletion to work correctly, set the
                        // first_block size that was earlier, destroy the unnecessary segments.
                        this->my_first_block.store(first_block, std::memory_order_relaxed);
                        for (size_type seg_idx = max_block; seg_idx > 0; --seg_idx) {
                            auto curr_segment =
                                buffer_table[seg_idx - 1].load(std::memory_order_relaxed);
                            if (curr_segment != nullptr) {
                                destroy_segment(
                                    buffer_table[seg_idx - 1].load(std::memory_order_relaxed) +
                                        this->segment_base(seg_idx - 1),
                                    seg_idx - 1);
                            }
                        }
                        ...
                        for (size_type seg_idx = 0; seg_idx < max_block; ++seg_idx) {
                            segment_table_allocator_traits::destroy(base_type::get_allocator(),
                                                                    &buffer_table[seg_idx]);
                        }
                    ...
                    if (k_stop < k_end) {
                        ...
                        if (!k)
                            this->my_first_block.store(0, std::memory_order_relaxed);
                        ;
                    }
                }

                // Lever for adjusting the size of first_block at the very first insertion.
                // TODO: consider >1 value, Assert performance
            ...
            template <typename T, typename Allocator>
            tbb::detail::synthesized_three_way_result<
                typename concurrent_vector<T, Allocator>::value_type>
