...
namespace tbb {
    namespace detail {
        namespace d2 {
            ...
            concept ch_map_rw_scoped_lockable =
                rw_scoped_lockable<Mutex> && requires(const typename Mutex::scoped_lock& sl) {
                                                 {
                                                     sl.is_writer()
            ...
            static void* const rehash_req_flag = reinterpret_cast<void*>(size_t(3));
            // Rehashed empty bucket flag
            static void* const empty_rehashed_flag = reinterpret_cast<void*>(size_t(0));
            ...
            public:
                ...
                using allocator_traits_type =
                    tbb::detail::allocator_traits_base<allocator_type>;
                using bucket_allocator_type =
                    typename allocator_traits_type::template rebind_alloc<bucket>;
                using bucket_allocator_traits =
                    tbb::detail::allocator_traits_base<bucket_allocator_type>;
                ...
                // Count of segments in the first block
                static constexpr size_type first_block =
                    8;  // including embedded_block. perfect with bucket size 16, so the
                        // allocations are power of 4096
                // Size of a pointer / table size
                static constexpr size_type pointers_per_table =
                    sizeof(segment_index_type) * 8;  // one segment per bit
                ...
                hash_map_base(const allocator_type& alloc)
                    : my_allocator(alloc), my_mask(embedded_buckets - 1), my_size(0)
                {
                    for (size_type i = 0; i != embedded_buckets; ++i) {
                        my_embedded_segment[i].node_list.store(nullptr,
                                                               std::memory_order_relaxed);
                    }

                    for (size_type segment_index = 0; segment_index < pointers_per_table;
                         ++segment_index) {
                        auto argument = segment_index < embedded_block
                                            ? my_embedded_segment + segment_base(segment_index)
                                            : nullptr;
                        my_table[segment_index].store(argument, std::memory_order_relaxed);
                    }

                    __TBB_ASSERT(embedded_block <= first_block,
                ...
                void init_buckets_impl(segment_ptr_type ptr, size_type sz, Args&&... args)
                {
                    for (size_type i = 0; i < sz; ++i) {
                        bucket_allocator_traits::construct(my_allocator, ptr + i,
                ...
                void init_buckets(segment_ptr_type ptr, size_type sz, bool is_initial)
                {
                    if (is_initial) {
                        ...
                        init_buckets_impl(ptr, sz,
                                          reinterpret_cast<node_base*>(rehash_req_flag));
                    }
                }
                ...
                {
                    __TBB_ASSERT(!rehash_required(b->node_list.load(std::memory_order_relaxed)),
                                 nullptr);
                    n->next = b->node_list.load(std::memory_order_relaxed);
                    b->node_list.store(
                        n, std::memory_order_relaxed);  // its under lock and flag is set
                }
                ...
                void enable_segment(segment_index_type k, bool is_initial = false)
                {
                    ...
                    size_type sz;
                    __TBB_ASSERT(!is_valid(my_table[k].load(std::memory_order_relaxed)),
                    ...
                    if (k >= first_block) {
                        ...
                        segment_ptr_type ptr = nullptr;
                        try_call([&] {
                            ptr = bucket_allocator_traits::allocate(my_allocator,
                                                                    sz - embedded_buckets);
                        }).on_exception([&] {
                            my_table[k].store(nullptr, std::memory_order_relaxed);
                        });
                        ...
                        for (segment_index_type i = embedded_block; i < first_block;
                             i++)  // calc the offsets
                ...
                void mark_rehashed_levels(hashcode_type h) noexcept
                {
                    ...
                    while (segment_ptr_type seg = my_table[++s].load(std::memory_order_relaxed))
                        if (rehash_required(seg[h].node_list.load(std::memory_order_relaxed))) {
                            seg[h].node_list.store(
                                reinterpret_cast<node_base*>(empty_rehashed_flag),
                                std::memory_order_relaxed);
                            mark_rehashed_levels(
                                h + ((hashcode_type)1 << s));  // optimized segment_base(s)
                        }
                }
                ...
                // Process mask race, Assert for rehashing collision
                bool check_rehashing_collision(const hashcode_type h, hashcode_type m_old,
                                               hashcode_type m) const
                {
                    __TBB_ASSERT(
                        m_old != m,
                        nullptr);  // TODO?: m arg could be optimized out by passing h = h&m
                    if ((h & m_old) != (h & m)) {  // mask changed for this hashcode, rare event
                        // condition above proves that 'h' has some other bits set beside
                        // 'm_old' find next applicable mask after m_old    //TODO: look at bsl
                        // instruction
                        for (++m_old; !(h & m_old);
                             m_old <<=
                             1)  // at maximum few rounds depending on the first block size
                            ;
                        m_old = (m_old << 1) - 1;  // get full mask from a bit
                        __TBB_ASSERT((m_old & (m_old + 1)) == 0 && m_old <= m, nullptr);
                        // Assert whether it is rehashing/ed
                        if (!rehash_required(get_bucket(h & m_old)->node_list.load(
                ...
                // Insert a node and Assert for load factor. @return segment index to enable.
                segment_index_type insert_new_node(bucket* b, node_base* n, hashcode_type mask)
                {
                    size_type sz = ++my_size;  // prefix form is to enforce allocation after the
                                               // first item inserted
                    add_to_bucket(b, n);
                    // Assert load factor
                    if (sz >= mask) {  // TODO: add custom load_factor
                        segment_index_type new_seg =
                            tbb::detail::log2(mask + 1);  // optimized segment_index_of
                        __TBB_ASSERT(
                            is_valid(my_table[new_seg - 1].load(std::memory_order_relaxed)),
                            "new allocations must not publish new mask until segment has "
                            "allocated");
                ...
                void internal_swap_content(hash_map_base& table)
                {
                    ...
                    for (size_type i = 0; i < embedded_buckets; i++) {
                        auto temp =
                            my_embedded_segment[i].node_list.load(std::memory_order_relaxed);
                        my_embedded_segment[i].node_list.store(
                            table.my_embedded_segment[i].node_list.load(
                                std::memory_order_relaxed),
                            std::memory_order_relaxed);
                        table.my_embedded_segment[i].node_list.store(temp,
                                                                     std::memory_order_relaxed);
                    }
                ...
                {
                    my_mask.store(other.my_mask.load(std::memory_order_relaxed),
                                  std::memory_order_relaxed);
                    ...
                    for (size_type i = 0; i < embedded_buckets; ++i) {
                        my_embedded_segment[i].node_list.store(
                            other.my_embedded_segment[i].node_list, std::memory_order_relaxed);
                        other.my_embedded_segment[i].node_list.store(nullptr,
                                                                     std::memory_order_relaxed);
                    }
            ...
            protected:
                ...
                std::atomic<size_type> my_size;  // It must be in separate cache line from
                                                 // my_mask due to performance effects
                // Zero segment
                bucket my_embedded_segment[embedded_buckets];
                // Segment pointers table. Also prevents false sharing between my_mask and
                // my_size
            ...
            public:
                ...
                {
                    __TBB_ASSERT(map_base::is_valid(my_node),
            ...
            private:
                ...
                friend bool operator==(const hash_map_iterator<C, T>& i,
                ...
                template <typename C, typename T, typename U>
                friend ptrdiff_t operator-(const hash_map_iterator<C, T>& i,
                                           const hash_map_iterator<C, U>& j);
                ...
                {  // TODO?: refactor to iterator_base class
                    ...
                    while (k <= my_map->my_mask.load(std::memory_order_relaxed)) {
                        ...
                        my_node = static_cast<node*>(
                            my_bucket->node_list.load(std::memory_order_relaxed));
                ...
                template <typename Key, typename T, typename HashCompare, typename A
                          ...
                          typename M>
                __TBB_requires(
                    tbb::detail::hash_compare<HashCompare, Key>&& ch_map_rw_scoped_lockable<M>)
#else
                          >
                ...
                hash_map_iterator(const Container& map, size_t index, const bucket* b,
                                  node_base* n)
                    : my_map(&map),
                      my_index(index),
                      my_bucket(b),
            ...
            bool operator==(const hash_map_iterator<Container, T>& i,
            ...
            public:
                ...
                hash_map_range(const map_type& map, size_type grainsize_ = 1)
                    : my_begin(Iterator(
                          map, 0, map.my_embedded_segment,
                          map.my_embedded_segment->node_list.load(std::memory_order_relaxed))),
                      my_end(
                          Iterator(map, map.my_mask.load(std::memory_order_relaxed) + 1, 0, 0)),
            ...
            void hash_map_range<Iterator>::set_midpoint() const
            {
                ...
                if (m > my_grainsize) {
                    ...
                    my_midpoint = Iterator(*my_begin.my_map, m, b,
...
#endif
            {
                ...
                template <typename I>
                friend class hash_map_range;
                using allocator_traits_type = tbb::detail::allocator_traits_base<Allocator>;
            ...
            public:
                using key_type = Key;
                using mapped_type = T;
                // type_identity is needed to disable implicit deduction guides for
                // std::initializer_list constructors and copy/move constructor with explicit
                // allocator argument
            ...
            protected:
                ...
                using node_allocator_type =
                    typename base_type::allocator_traits_type::template rebind_alloc<node>;
                using node_allocator_traits =
                    tbb::detail::allocator_traits_base<node_allocator_type>;
                ...
                void delete_node(node_base* n)
                {
                    ...
                    node_allocator_traits::destroy(node_allocator,
                                                   static_cast<node*>(n)->storage());
                ...
                static node* create_node(bucket_allocator_type& allocator, Args&&... args)
                {
                    ...
                    node_allocator_traits::construct(node_allocator, node_ptr->storage(),
                                                     std::forward<Args>(args)...);
                    guard.dismiss();
                    return node_ptr;
                }

                static node* allocate_node_copy_construct(bucket_allocator_type& allocator,
                                                          const Key& key, const T* t)
                {
                    return create_node(allocator, key, *t);
                }

                static node* allocate_node_move_construct(bucket_allocator_type& allocator,
                                                          const Key& key, const T* t)
                {
                ...
                static node* allocate_node_default_construct(bucket_allocator_type& allocator,
                                                             const K& key, const T*)
                {
                ...
                node* search_bucket(const K& key, bucket* b) const
                {
                    ...
                    __TBB_ASSERT(!rehash_required(n),
                ...
                public:
                    bucket_accessor(concurrent_hash_map* base, const hashcode_type h,
                                    bool writer = false)
                    {
                        acquire(base, h, writer);
                    }
                    // find a bucket by masked hashcode, optionally rehash, and acquire the lock
                    inline void acquire(concurrent_hash_map* base, const hashcode_type h,
                                        bool writer = false)
                    {
                        my_b = base->get_bucket(h);
                        // TODO: actually, notification is unnecessary here, just hiding
                        // double-Assert
                        if (rehash_required(my_b->node_list.load(std::memory_order_acquire)) &&
                            bucket::scoped_type::try_acquire(my_b->mutex, /*write=*/true)) {
                            if (rehash_required(
                                    my_b->node_list.load(std::memory_order_relaxed)))
                                base->rehash_bucket(my_b, h);  // recursive rehashing
                        } else
                            bucket::scoped_type::acquire(my_b->mutex, writer);
                        __TBB_ASSERT(
                            !rehash_required(my_b->node_list.load(std::memory_order_relaxed)),
                            nullptr);
                    }
                ...
                void rehash_bucket(bucket* b_new, const hashcode_type hash)
                {
                    ...
                    b_new->node_list.store(reinterpret_cast<node_base*>(empty_rehashed_flag),
                                           std::memory_order_release);  // mark rehashed
                    hashcode_type mask = (1u << tbb::detail::log2(hash)) -
                                         1;  // get parent mask from the topmost bit
                ...
                restart:
                    ...
                    while (this->is_valid(curr)) {
                        hashcode_type curr_node_hash =
                            my_hash_compare.hash(static_cast<node*>(curr)->value().first);
                ...
                template <typename U>
                using hash_compare_is_transparent =
                    dependent_bool<comp_is_transparent<hash_compare_type>, U>;

            public:
                class accessor;
                // Combines data access, locking, and garbage collection.
                class const_accessor
                    : private node::scoped_type /*which derived from no_copy*/ {
#if __TBB_PREVIEW_CONCURRENT_HASH_MAP_EXTENSIONS
                ...
                public:
                    ...
                    {
                        my_node =
                            nullptr;  // scoped lock's release() is called in its destructor
                    }

                protected:
                    bool is_writer()
                    {
                        return node::scoped_type::is_writer();
                    }
                    node*         my_node;
                    hashcode_type my_hash;
                };

                // Allows write access to elements and combines data access, locking, and
                // garbage collection.
                ...
                explicit concurrent_hash_map(const hash_compare_type& compare,
                    ...
                    : base_type(a), my_hash_compare(compare)
                {
                }
                ...
                // Construct empty table with n preallocated buckets. This number serves also as
                ...
                concurrent_hash_map(const concurrent_hash_map& table)
                    : concurrent_hash_map(
                          node_allocator_traits::select_on_container_copy_construction(
                              table.get_allocator()))
                {
                    try_call([&] { internal_copy(table); }).on_exception([&] {
                        this->clear();
                    });
                }
                    ...
                    : concurrent_hash_map(a)
                {
                    ...
                    internal_move_construct_with_allocator(std::move(table), a,
                                                           is_equal_type());
                }
                    ...
                    : concurrent_hash_map(a)
                {
                    ...
                    }).on_exception([&] { this->clear(); });
                }

                template <typename I>
                concurrent_hash_map(I first, I last, const hash_compare_type& compare,
                                    const allocator_type& a = allocator_type())
                    : concurrent_hash_map(compare, a)
                {
                    try_call([&] {
                ...
                {
                    if (this != &table) {
                        using pocma_type = typename node_allocator_traits::
                            propagate_on_container_move_assignment;
                        using is_equal_type = typename node_allocator_traits::is_always_equal;
                        move_assign_allocators(this->my_allocator, table.my_allocator);
                        internal_move_assign(
                            std::move(table),
                            tbb::detail::disjunction<is_equal_type, pocma_type>());
                    }
                    return *this;
                }
                ...
                /** Useful to optimize performance before or after concurrent operations.
                    Also enables using of find() and count() concurrent methods in serial
                   context. */
                void rehash(size_type sz = 0)
                {
                    ...
                    hashcode_type b =
                        (mask + 1) >> 1;  // size or first index of the last segment
                    __TBB_ASSERT((b & (b - 1)) == 0, nullptr);  // zero or power of 2
                    bucket* bp = this->get_bucket(
                        b);  // only the last segment should be scanned for rehashing
                    for (; b <= mask; b++, bp++) {
                        node_base* n = bp->node_list.load(std::memory_order_relaxed);
                        __TBB_ASSERT(this->is_valid(n) || empty_rehashed(n) ||
                                         rehash_required(n),
                                     "Broken internal structure");
                        __TBB_ASSERT(*reinterpret_cast<intptr_t*>(&bp->mutex) == 0,
                                     "concurrent or unexpectedly terminated operation during "
                                     "rehash() execution");
                        if (rehash_required(
                                n)) {  // rehash bucket, conditional because rehashing of a
                                       // previous bucket may affect this one
                            hashcode_type h = b;
                            bucket*       b_old = bp;
                            do {
                                __TBB_ASSERT(h > 1, "The lowermost buckets can't be rehashed");
                                hashcode_type m = (1u << tbb::detail::log2(h)) -
                                                  1;  // get parent mask from the topmost bit
                                b_old = this->get_bucket(h &= m);
                            } while (rehash_required(
                                b_old->node_list.load(std::memory_order_relaxed)));
                            // now h - is index of the root rehashed bucket b_old
                            this->mark_rehashed_levels(h);  // mark all non-rehashed children
                                                            // recursively across all segments
                            node_base* prev = nullptr;
                            node_base* curr = b_old->node_list.load(std::memory_order_relaxed);
                            while (this->is_valid(curr)) {
                                hashcode_type curr_node_hash = my_hash_compare.hash(
                                    static_cast<node*>(curr)->value().first);

                                if ((curr_node_hash & mask) != h) {  // should be rehashed
                                    node_base* next = curr->next;
                                    // exclude from b_old
                                    if (prev == nullptr) {
                                        b_old->node_list.store(curr->next,
                                                               std::memory_order_relaxed);
                                    } else {
                                        prev->next = curr->next;
                                    }
                                    bucket* b_new = this->get_bucket(curr_node_hash & mask);
                                    __TBB_ASSERT(!rehash_required(b_new->node_list.load(
                                                     std::memory_order_relaxed)),
                                                 "hash() function changed for key in table or "
                                                 "internal error");
                                    this->add_to_bucket(b_new, curr);
                                    curr = next;
                                } else {
                ...
                void clear()
                {
                    ...
                    __TBB_ASSERT(s + 1 == this->pointers_per_table ||
                                     !this->my_table[s + 1].load(std::memory_order_relaxed),
                                 "wrong mask or concurrent grow");
                    do {
                        __TBB_ASSERT(
                            this->is_valid(this->my_table[s].load(std::memory_order_relaxed)),
                            "wrong mask or concurrent grow");
                        segment_ptr_type buckets_ptr =
                            this->my_table[s].load(std::memory_order_relaxed);
                        size_type sz = this->segment_size(s ? s : 1);
                        for (segment_index_type i = 0; i < sz; i++)
                            for (node_base* n =
                                     buckets_ptr[i].node_list.load(std::memory_order_relaxed);
                                 this->is_valid(n);
                                 n = buckets_ptr[i].node_list.load(std::memory_order_relaxed)) {
                                buckets_ptr[i].node_list.store(n->next,
                                                               std::memory_order_relaxed);
                                delete_node(n);
                            }
                ...
                {
                    return iterator(
                        *this, 0, this->my_embedded_segment,
                        this->my_embedded_segment->node_list.load(std::memory_order_relaxed));
                }
                ...
                {
                    return this->my_size.load(std::memory_order_acquire);
                }
                ...
                {
                    return this->my_mask.load(std::memory_order_relaxed) + 1;
                }
                ...
                {
                    return const_cast<concurrent_hash_map*>(this)->lookup</*insert*/ false>(
                        key, nullptr, nullptr, /*write=*/false, &do_not_allocate_node);
                }

#if __TBB_PREVIEW_CONCURRENT_HASH_MAP_EXTENSIONS
                template <typename K>
                typename std::enable_if<hash_compare_is_transparent<K>::value, size_type>::type
                ...
                bool find(const_accessor& result, const Key& key) const
                {
                    result.release();
                    return const_cast<concurrent_hash_map*>(this)->lookup</*insert*/ false>(
                        key, nullptr, &result, /*write=*/false, &do_not_allocate_node);
                }
                ...
                {
                    result.release();
                    return lookup</*insert*/ false>(key, nullptr, &result, /*write=*/true,
                                                    &do_not_allocate_node);
                }

#if __TBB_PREVIEW_CONCURRENT_HASH_MAP_EXTENSIONS
                template <typename K>
                typename std::enable_if<hash_compare_is_transparent<K>::value, bool>::type
                find(const_accessor& result, const K& key)
                {
                    result.release();
                    return lookup</*insert*/ false>(key, nullptr, &result, /*write=*/false,
                                                    &do_not_allocate_node);
                }
                ...
                {
                    result.release();
                    return lookup</*insert*/ true>(key, nullptr, &result, /*write=*/false,
                ...
                {
                    result.release();
                    return lookup</*insert*/ true>(key, nullptr, &result, /*write=*/true,
                                                   &allocate_node_default_construct<>);
                }
                ...
                typename std::enable_if<hash_compare_is_transparent<K>::value &&
                                        ...
                                        bool>::type
                insert(const_accessor& result, const K& key)
                {
                    result.release();
                    return lookup</*insert*/ true>(key, nullptr, &result, /*write=*/true,
                                                   &allocate_node_default_construct<K>);
                }
                ...
                typename std::enable_if<hash_compare_is_transparent<K>::value &&
                                            std::is_constructible<key_type, const K&>::value,
                                        bool>::type
                insert(accessor& result, const K& key)
                {
                    result.release();
                    return lookup</*insert*/ true>(key, nullptr, &result, /*write=*/true,
                                                   &allocate_node_default_construct<K>);
                ...
                // Insert item by copying if there is no such key present already and acquire a
                // read lock on the item.
                /** Returns true if item is new. */
                bool insert(const_accessor& result, const value_type& value)
                {
                    result.release();
                    return lookup</*insert*/ true>(value.first, &value.second, &result,
                                                   /*write=*/false,
                ...
                {
                    return lookup</*insert*/ true>(value.first, &value.second, nullptr,
                                                   /*write=*/false,
                                                   &allocate_node_copy_construct);
                }
            ...
            protected:
                ...
                node* allocate_node_helper(const K& key, const T* t,
                                           AllocateNodeType allocate_node, std::true_type)
                {
                    return allocate_node(base_type::get_allocator(), key, t);
                }

                template <typename K, typename AllocateNodeType>
                node* allocate_node_helper(const K&, const T*, AllocateNodeType,
                                           std::false_type)
                {
                    __TBB_ASSERT(
                        false,
                        "allocate_node_helper with std::false_type should never been called");
                    return nullptr;
                }

                // Insert or find item and optionally acquire a lock on the item.
                template <bool OpInsert, typename K, typename AllocateNodeType>
                bool lookup(const K& key, const T* t, const_accessor* result, bool write,
                            AllocateNodeType allocate_node, node* tmp_n = 0)
                {
                ...
                restart : {  // lock scope
                    ...
                    if (OpInsert) {
                        ...
                        if (!n) {
                            if (!tmp_n) {
                                tmp_n = allocate_node_helper(
                                    key, t, allocate_node,
                                    std::integral_constant<bool, OpInsert>{});
                            }
                            while (!b.is_writer() &&
                                   !b.upgrade_to_writer()) {  // TODO: improved insertion
                                // Rerun search list, in case another thread inserted the intem
                                // during the upgrade
                                n = search_bucket(key, b());
                                if (this->is_valid(n)) {  // unfortunately, it did
                                    if (!b.downgrade_to_reader()) {
                                        // If the lock was downgraded with reacquiring the mutex
                                        // Rerun search list in case another thread removed the
                                        // item during the downgrade
                                        n = search_bucket(key, b());
                                        if (!this->is_valid(n)) {
                        ...
                        if (!n) {
                            if (this->check_mask_race(h, m))
                                goto restart;  // b.release() is done in ~b(). TODO: replace by
                                               // continue
                            return false;
                        }
                        return_value = true;
                    ...
                    if (!result->try_acquire(n->mutex, write)) {
                        for (tbb::detail::atomic_backoff backoff(true);;) {
                            ...
                            if (!backoff.bounded_pause()) {
                                ...
                                b.release();
                                __TBB_ASSERT(!OpInsert || !return_value,
                ...
                bool generic_move_insert(Accessor&& result, value_type&& value)
                {
                    ...
                    return lookup</*insert*/ true>(
                        value.first, &value.second, accessor_location(result),
                ...
                bool generic_emplace(Accessor&& result, Args&&... args)
                {
                    result.release();
                    node* node_ptr =
                        create_node(base_type::get_allocator(), std::forward<Args>(args)...);
                    return lookup</*insert*/ true>(
                        node_ptr->value().first, nullptr, accessor_location(result),
                ...
                search:
                    ...
                    while (this->is_valid(erase_node) &&
                           !my_hash_compare.equal(
                               key, static_cast<node*>(erase_node)->value().first)) {
                        prev = erase_node;
                    ...
                    if (erase_node == nullptr) {  // not found, but mask could be changed
                        ...
                        if (this->check_mask_race(hash,
                                                  mask))  // contended upgrade, Assert mask
                            goto restart;
                        goto search;
                    }
                    ...
                    {
                        typename node::scoped_type item_locker(erase_node->mutex,
                                                               /*write=*/true);
                    }
                    // note: there should be no threads pretending to acquire this mutex again,
                    // do not try to upgrade const_accessor!
                    delete_node(erase_node);  // Only one thread can delete it due to write lock
                                              // on the bucket
                    return true;
                }

                // Returns an iterator for an item defined by the key, or for the next item
                // after it (if upper==true)
                ...
                void internal_copy(const concurrent_hash_map& source)
                {
                    ...
                    if (this->my_mask.load(std::memory_order_relaxed) ==
                        mask) {  // optimized version
                        this->reserve(source.my_size.load(
                            std::memory_order_relaxed));  // TODO: load_factor?
                        bucket *dst = 0, *src = 0;
                        bool    rehashing_required = false;
                        for (hashcode_type k = 0; k <= mask; k++) {
                            if (k & (k - 2))
                                ++dst, src++;  // not the beginning of a segment
                            else {
                                dst = this->get_bucket(k);
                                src = source.get_bucket(k);
                            }
                            __TBB_ASSERT(!rehash_required(
                                             dst->node_list.load(std::memory_order_relaxed)),
                                         "Invalid bucket in destination table");
                            node* n = static_cast<node*>(
                                src->node_list.load(std::memory_order_relaxed));
                            if (rehash_required(n)) {  // source is not rehashed, items are in
                                                       // previous buckets
                                ...
                                for (; n; n = static_cast<node*>(n->next)) {
                                    node* node_ptr =
                                        create_node(base_type::get_allocator(),
                        ...
                        internal_copy(source.begin(), source.end(),
                                      source.my_size.load(std::memory_order_relaxed));
                }
                ...
                void internal_copy(I first, I last, size_type reserve_size)
                {
                    ...
                    for (; first != last; ++first) {
                        ...
                        __TBB_ASSERT(
                            !rehash_required(b->node_list.load(std::memory_order_relaxed)),
                            "Invalid bucket in destination table");
                        node* node_ptr = create_node(base_type::get_allocator(), (*first).first,
                                                     (*first).second);
                ...
                void internal_move_construct_with_allocator(concurrent_hash_map&& other,
                    ...
                    if (a == other.get_allocator()) {
                        ...
                        try_call([&] {
                            internal_copy(std::make_move_iterator(other.begin()),
                                          std::make_move_iterator(other.end()), other.size());
                        }).on_exception([&] { this->clear(); });
                    }
                }
                ...
                void internal_swap(concurrent_hash_map& other,
                ...
                // Fast find when no concurrent erasure is used. For internal use inside TBB
                // only!
            ...
            concurrent_hash_map(It, It, HashCompare = HashCompare(), Alloc = Alloc())
                -> concurrent_hash_map<iterator_key_t<It>, iterator_mapped_t<It>, HashCompare,
                                       Alloc>;
            ...
            concurrent_hash_map(It, It, Alloc)
                -> concurrent_hash_map<iterator_key_t<It>, iterator_mapped_t<It>,
                                       d1::tbb_hash_compare<iterator_key_t<It>>, Alloc>;
            ...
            concurrent_hash_map(std::initializer_list<std::pair<Key, T>>,
                                HashCompare = HashCompare(), Alloc = Alloc())
            ...
            concurrent_hash_map(std::initializer_list<std::pair<Key, T>>, Alloc)
                -> concurrent_hash_map<std::remove_const_t<Key>, T,
                                       d1::tbb_hash_compare<std::remove_const_t<Key>>, Alloc>;

#endif /* __TBB_CPP17_DEDUCTION_GUIDES_PRESENT */

            template <typename Key, typename T, typename HashCompare, typename A1, typename A2>
            inline bool operator==(const concurrent_hash_map<Key, T, HashCompare, A1>& a,
            ...
            template <typename Key, typename T, typename HashCompare, typename A>
            inline void swap(concurrent_hash_map<Key, T, HashCompare, A>& a,
