...
namespace tbb {
    namespace detail {
        ...
        namespace d1 {
            ...
            inline size_t get_initial_auto_partitioner_divisor()
            {
            ...
            class affinity_partitioner_base : no_copy {
                ...
                {
                    // Check factor to avoid asking for number of workers while there might be
                    // no arena.
                    unsigned max_threads_in_arena = max_concurrency();
                    size_t   new_size = factor ? factor * max_threads_in_arena : 0;
                    if (new_size != my_size) {
                        if (my_array) {
                            r1::cache_aligned_deallocate(my_array);
                            // Following two assignments must be done here for sake of exception
                            // safety.
                            my_array = nullptr;
                            my_size = 0;
                        }
                        if (new_size) {
                            my_array = static_cast<slot_id*>(
                                r1::cache_aligned_allocate(new_size * sizeof(slot_id)));
            ...
            struct start_reduce;
            template <typename Range, typename Body, typename Partitioner>
            struct start_deterministic_reduce;
            ...
            struct node {
                ...
                node() = default;
                node(node* parent, int ref_count) : my_parent{parent}, m_ref_count{ref_count}
                {
            ...
            struct tree_node : public node {
                ...
                tree_node(node* parent, int ref_count, small_object_allocator& alloc)
                    : node{parent, ref_count}, m_allocator{alloc}
                {
                }

                void join(task_group_context*){
                    /*dummy, required only for reduction algorithms*/};

                template <typename Task>
                static void mark_task_stolen(Task& t)
                {
                    std::atomic<bool>& flag =
                        static_cast<tree_node*>(t.my_parent)->m_child_stolen;
#if TBB_USE_PROFILING_TOOLS
                    // Threading tools respect lock prefix but report false-positive data-race
                    // via plain store
                    flag.exchange(true);
#else
            ...
            // Context used to Assert cancellation state during reduction join process
            template <typename TreeNodeType>
            void fold_tree(node* n, const execution_data& ed)
            {
                for (;;) {
                    __TBB_ASSERT(n->m_ref_count.load(std::memory_order_relaxed) > 0,
                                 "The refcount must be positive.");
            ...
            //! Depth is a relative depth of recursive division inside a range pool. Relative
            //! depth allows infinite absolute depth of the recursion for heavily unbalanced
            //! workloads with range represented by a number that cannot fit into machine word.
            ...
            public:
                ...
                //! max_depth starts from 0, e.g. value 2 makes 3 ranges in the pool up to two
                //! 1/4 pieces
                void split_to_fill(depth_t max_depth)
                {
                    while (my_size < MaxCapacity && is_divisible(max_depth)) {
                        ...
                        new (my_pool.begin() + my_head)
                            T(my_pool.begin()[prev]);  // copy TODO: std::move?
                        my_pool.begin()[prev].~T();    // instead of assignment
                        new (my_pool.begin() + prev)
                            T(my_pool.begin()[my_head], detail::split());  // do 'inverse' split
            ...
            struct partition_type_base {
                ...
                template <typename Task>
                bool check_being_stolen(Task&, const execution_data&)
                {
                    return false;
                }  // part of old should_execute_range()
                ...
                {
                    // The algorithm in a few words ([]-denotes calls to decision methods of
                    // partitioner): [If this task is stolen, adjust depth and divisions if
                    // necessary, set flag]. If range is divisible {
                    //    Spread the work while [initial divisions left];
                    //    Create trap task [if necessary];
                    // }
                    // If not divisible or [max depth is reached], execute, else do the range
                    // pool part
                    if (range.is_divisible()) {
                        if (self().is_divisible()) {
                            do {  // split until is divisible
                                typename Partition::split_type split_obj =
            ...
            struct adaptive_mode : partition_type_base<Partition> {
                ...
                size_t            my_divisor;
                // For affinity_partitioner, my_divisor indicates the number of affinity array
                // indices the task reserves. A task which has only one index must produce the
                // right split without reserved index in order to avoid it to be overwritten in
                // note_affinity() of the created (right) task. I.e. a task created deeper than
                // the affinity array can remember must not save its affinity (LIFO order)
                static const unsigned factor = 1;
                adaptive_mode()
                    : my_divisor(get_initial_auto_partitioner_divisor() / 4 *
                                 my_partition::factor)
                {
                }
            ...
            template <typename T>
            using supports_proportional_splitting =
                typename std::is_constructible<T, T&, proportional_split&>;
            ...
            /** If the Range has proportional_split constructor,
               ...
               proportion; otherwise it represents equal-size split. */
            // TODO: Assert if this helper can be a nested class of proportional_mode.
            ...
            struct proportion_helper<
                Range,
                typename std::enable_if<supports_proportional_splitting<Range>::value>::type> {
            ...
            struct proportional_mode : adaptive_mode<Partition> {
                typedef Partition my_partition;
                using partition_type_base<Partition>::self;  // CRTP helper to get access to
                                                             // derived classes

                proportional_mode() : adaptive_mode<Partition>() {}
                proportional_mode(proportional_mode& src, split)
                    : adaptive_mode<Partition>(src, split())
                {
                }
                ...
                size_t do_split(proportional_mode& src, const proportional_split& split_obj)
                {
                    ...
                    portion =
                        (portion + my_partition::factor / 2) & (0ul - my_partition::factor);
                ...
                {
                    // Create a proportion for the number of threads expected to handle "this"
                    // subrange
            ...
            struct linear_affinity_mode : proportional_mode<Partition> {
                ...
                linear_affinity_mode()
                    : proportional_mode<Partition>(),
                      my_head(get_initial_partition_head()),
                      my_max_affinity(self().my_divisor)
                {
                }
                linear_affinity_mode(linear_affinity_mode& src, split)
                    : proportional_mode<Partition>(src, split()),
                      my_head((src.my_head + src.my_divisor) % src.my_max_affinity),
                      my_max_affinity(src.my_max_affinity)
                {
                }
                linear_affinity_mode(linear_affinity_mode&     src,
                                     const proportional_split& split_obj)
                    : proportional_mode<Partition>(src, split_obj),
                      my_head((src.my_head + src.my_divisor) % src.my_max_affinity),
                      my_max_affinity(src.my_max_affinity)
                {
                }
            ...
            struct dynamic_grainsize_mode : Mode {
                using Mode::self;
                enum { begin = 0, run, pass } my_delay;
                depth_t               my_max_depth;
                static const unsigned range_pool_size = __TBB_RANGE_POOL_CAPACITY;
                dynamic_grainsize_mode()
                    : Mode(), my_delay(begin), my_max_depth(__TBB_INIT_DEPTH)
                {
                }
                dynamic_grainsize_mode(dynamic_grainsize_mode& p, split)
                    : Mode(p, split()), my_delay(pass), my_max_depth(p.my_max_depth)
                {
                }
                dynamic_grainsize_mode(dynamic_grainsize_mode&   p,
                                       const proportional_split& split_obj)
                    : Mode(p, split_obj), my_delay(begin), my_max_depth(p.my_max_depth)
                {
                }
                template <typename Task>
                bool check_being_stolen(Task& t, const execution_data& ed)
                {  // part of old should_execute_range()
                    if (!(self().my_divisor /
                          Mode::my_partition::factor)) {  // if not from the top P tasks of
                                                          // binary tree
                        self().my_divisor =
                            1;  // TODO: replace by on-stack flag (partition_state's member)?
                        if (is_stolen_task(ed) &&
                            t.my_parent->m_ref_count >=
                                2) {  // runs concurrently with the left task
#if __TBB_USE_OPTIONAL_RTTI
                                // RTTI is available, Assert whether the cast is valid
                                // TODO: TBB_REVAMP_TODO
                                // __TBB_ASSERT(dynamic_cast<tree_node*>(t.m_parent), 0);
                                // correctness of the cast relies on avoiding the root task for
                                // which:
                                // - initial value of my_divisor != 0 (protected by separate
                                // assertion)
                        ...
                        do {
                            ...
                            if (self().check_for_demand(start)) {
                                if (range_pool.size() > 1) {
                                    start.offer_work(range_pool.front(),
                                                     range_pool.front_depth(), ed);
                                    range_pool.pop_front();
                                    continue;
                                }
                                if (range_pool.is_divisible(
                                        self().max_depth()))  // was not enough depth to fork a
                                                              // task
                                    continue;  // note: next split_to_fill() should split range
                                               // at least once
                            }
                            start.run_body(range_pool.back());
                            range_pool.pop_back();
                        } while (!range_pool.empty() &&
                                 !ed.context->is_group_execution_cancelled());
                    }
                }
                template <typename Task>
                bool check_for_demand(Task& t)
                {
                    if (pass == my_delay) {
                        if (self().my_divisor >
                            1)  // produce affinitized tasks while they have slot in array
                            ...
                            self().my_divisor = 0;  // once for each task; depth will be
                                                    // decreased in align_depth()
                            return true;
            ...
            class auto_partition_type
                : public dynamic_grainsize_mode<adaptive_mode<auto_partition_type>> {
            public:
                ...
                {  // part of old should_execute_range()
                    if (my_divisor > 1)
                        return true;
                    if (my_divisor &&
                        my_max_depth) {  // can split the task. TODO: on-stack flag instead
            ...
            public:
                ...
                void execute(StartType& start, Range& range, execution_data& ed)
                {
                    split_type split_obj =
                        split();  // start.offer_work accepts split_type as reference
            ...
            public:
                ...
                static_partition_type(static_partition_type&    p,
                                      const proportional_split& split_obj)
            ...
            class affinity_partition_type
                : public dynamic_grainsize_mode<linear_affinity_mode<affinity_partition_type>> {
                static const unsigned factor_power =
                    4;  // TODO: get a unified formula based on number of computing units
                slot_id* my_array;

            public:
                static const unsigned factor =
                    1 << factor_power;  // number of slots in affinity array per task
                typedef detail::proportional_split split_type;
                ...
                affinity_partition_type(affinity_partition_type&  p,
                                        const proportional_split& split_obj)
                    : dynamic_grainsize_mode<linear_affinity_mode<affinity_partition_type>>(
                          p, split_obj),
                      my_array(p.my_array)
                ...
                void spawn_task(task& t, task_group_context& ctx)
                {
                    if (my_divisor) {
                        if (!my_array[my_head]) {
                            // TODO: consider new ideas with my_array for both affinity and
                            // static partitioner's, then code reuse
            ...
            private:
                template <typename Range, typename Body, typename Partitioner>
                friend struct start_for;
                template <typename Range, typename Body, typename Partitioner>
                friend struct start_reduce;
                template <typename Range, typename Body, typename Partitioner>
                friend struct start_deterministic_reduce;
                template <typename Range, typename Body, typename Partitioner>
                ...
                public:
                    bool should_execute_range(const execution_data&)
                    {
                        return false;
                    }
            ...
            /** The range is initial divided into several large chunks.
                Chunks are further subdivided into smaller pieces if demand detected and they
               are divisible.
                @ingroup algorithms */
            class auto_partitioner {
            public:
            ...
            private:
                ...
                public:
                    ...
                    partition_type(const auto_partitioner&)
                        : num_chunks(get_initial_auto_partitioner_divisor() *
