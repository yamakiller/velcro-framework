...
#include "TBB/include/oneapi/tbb/detail/_concurrent_queue_base.h"
    ...
    namespace detail {
        namespace d2 {
            ...
            class concurrent_queue {
                ...
                using queue_allocator_traits =
                    tbb::detail::allocator_traits_base<queue_allocator_type>;
            ...
            public:
                ...
                using const_iterator =
                    concurrent_queue_iterator<concurrent_queue, const T, Allocator>;
                    ...
                    : my_allocator(a), my_queue_representation(nullptr)
                {
                    my_queue_representation = static_cast<queue_representation_type*>(
                        r1::cache_aligned_allocate(sizeof(queue_representation_type)));
                    queue_allocator_traits::construct(my_allocator, my_queue_representation);

                    __TBB_ASSERT(is_aligned(my_queue_representation, max_nfs_size),
                                 "alignment error");
                    __TBB_ASSERT(
                        is_aligned(&my_queue_representation->head_counter, max_nfs_size),
                        "alignment error");
                    __TBB_ASSERT(
                        is_aligned(&my_queue_representation->tail_counter, max_nfs_size),
                        "alignment error");
                    __TBB_ASSERT(is_aligned(&my_queue_representation->array, max_nfs_size),
                                 "alignment error");
                }

                template <typename InputIterator>
                concurrent_queue(InputIterator begin, InputIterator end,
                ...
                concurrent_queue(const concurrent_queue& src, const allocator_type& a)
                    : concurrent_queue(a)
                {
                    my_queue_representation->assign(*src.my_queue_representation, my_allocator,
                                                    copy_construct_item);
                }
                ...
                concurrent_queue(concurrent_queue&& src, const allocator_type& a)
                    : concurrent_queue(a)
                {
                    // checking that memory allocated by one instance of allocator can be
                    // deallocated with another
                    if (my_allocator == src.my_allocator) {
                        internal_swap(src);
                    } else {
                        // allocators are different => performing per-element move
                        my_queue_representation->assign(*src.my_queue_representation,
                                                        my_allocator, move_construct_item);
                        src.clear();
                    }
                }
                ...
                //------------------------------------------------------------------------
                // The iterators are intended only for debugging.  They are slow and not thread
                // safe.
                //------------------------------------------------------------------------

                iterator unsafe_begin()
                {
                    return concurrent_queue_iterator_provider::get<iterator>(*this);
                }
                iterator unsafe_end()
                {
                    return iterator();
                ...
                {
                    return concurrent_queue_iterator_provider::get<const_iterator>(*this);
                }
            ...
            private:
                ...
                void internal_push(Args&&... args)
                {
                    ...
                    my_queue_representation->choose(k).push(
                        k, *my_queue_representation, my_allocator, std::forward<Args>(args)...);
                }
                ...
                bool internal_try_pop(void* dst)
                {
                    ticket_type k;
                    do {
                        k = my_queue_representation->head_counter.load(
                            std::memory_order_relaxed);
                        do {
                            if (static_cast<std::ptrdiff_t>(
                                    my_queue_representation->tail_counter.load(
                                        std::memory_order_relaxed) -
                                    k) <= 0) {
                                // Queue is empty
                                return false;
                            ...
                            // Queue had item with ticket k when we looked. Attempt to get that
                            // item. Another thread snatched the item, retry.
                        } while (!my_queue_representation->head_counter.compare_exchange_strong(
                            k, k + 1));
                    } while (!my_queue_representation->choose(k).pop(
                        dst, k, *my_queue_representation, my_allocator));
                    return true;
                }
                ...
                {
                    // TODO: use allocator_traits_base for copy construction
                    new (location) value_type(*static_cast<const value_type*>(src));
                    // queue_allocator_traits::construct(my_allocator, location,
                    // *static_cast<const T*>(src));
                }

                static void move_construct_item(T* location, const void* src)
                {
                    // TODO: use allocator_traits_base for move construction
            ...
            static constexpr size_t cbq_slots_avail_tag = 0;
            static constexpr size_t cbq_items_avail_tag = 1;
        }  // namespace d2

        namespace r1 {
            class concurrent_monitor;

            TBB_EXPORT std::u8_t* __TBB_EXPORTED_FUNC
                       allocate_bounded_queue_rep(size_t queue_rep_size);
            TBB_EXPORT void __TBB_EXPORTED_FUNC
            deallocate_bounded_queue_rep(std::u8_t* mem, size_t queue_rep_size);
            TBB_EXPORT void __TBB_EXPORTED_FUNC
            abort_bounded_queue_monitors(concurrent_monitor* monitors);
            TBB_EXPORT void __TBB_EXPORTED_FUNC notify_bounded_queue_monitor(
                concurrent_monitor* monitors, size_t monitor_tag, size_t ticket);
            TBB_EXPORT void __TBB_EXPORTED_FUNC
            wait_bounded_queue_monitor(concurrent_monitor* monitors, size_t monitor_tag,
        ...
        namespace d2 {
            ...
            class concurrent_bounded_queue {
                ...
                void internal_wait(r1::concurrent_monitor* monitors, size_t monitor_tag,
                                   std::ptrdiff_t target, FuncType pred)
                {
            ...
            public:
                ...
                using iterator =
                    concurrent_queue_iterator<concurrent_bounded_queue, T, Allocator>;
                using const_iterator =
                    concurrent_queue_iterator<concurrent_bounded_queue, const T, Allocator>;
                ...
                explicit concurrent_bounded_queue(const allocator_type& a)
                    : my_allocator(a),
                      my_capacity(0),
                      my_abort_counter(0),
                      my_queue_representation(nullptr)
                {
                    ...
                    my_monitors =
                        reinterpret_cast<r1::concurrent_monitor*>(my_queue_representation + 1);
                    queue_allocator_traits::construct(my_allocator, my_queue_representation);
                    my_capacity = size_t(-1) / (queue_representation_type::item_size > 1
                                                    ? queue_representation_type::item_size
                                                    : 2);
                ...
                ~concurrent_bounded_queue()
                {
                    ...
                    my_queue_representation->clear(my_allocator);
                    queue_allocator_traits::destroy(my_allocator, my_queue_representation);
                    r1::deallocate_bounded_queue_rep(
                        reinterpret_cast<std::u8_t*>(my_queue_representation),
            ...
            private:
                ...
                static constexpr std::ptrdiff_t infinite_capacity =
                ...
                void internal_push(Args&&... args)
                {
                    ...
                    if (static_cast<std::ptrdiff_t>(my_queue_representation->head_counter.load(
                            std::memory_order_relaxed)) <= target) {  // queue is full
                        auto pred = [&] {
                            ...
                            return static_cast<std::ptrdiff_t>(
                                       my_queue_representation->head_counter.load(
                        ...
                        try_call([&] {
                            ...
                            my_queue_representation->choose(ticket).abort_push(
                                ticket, *my_queue_representation, my_allocator);
                        });
                    }
                    __TBB_ASSERT(
                        (static_cast<std::ptrdiff_t>(my_queue_representation->head_counter.load(
                             std::memory_order_relaxed)) > target),
                        nullptr);
                    my_queue_representation->choose(ticket).push(
                        ticket, *my_queue_representation, my_allocator,
                ...
                {
                    ticket_type ticket =
                        my_queue_representation->tail_counter.load(std::memory_order_relaxed);
                    do {
                        if (static_cast<std::ptrdiff_t>(
                                ticket - my_queue_representation->head_counter.load(
                                             std::memory_order_relaxed)) >= my_capacity) {
                            // Queue is full
                            return false;
                        }
                        // Queue had empty slot with ticket k when we looked. Attempt to claim
                        // that slot. Another thread claimed the slot, so retry.
                    } while (!my_queue_representation->tail_counter.compare_exchange_strong(
                        ticket, ticket + 1));
                ...
                bool internal_pop(void* dst)
                {
                    ...
                    // This loop is a single pop operation; abort_counter should not be re-read
                    // inside
                    unsigned old_abort_counter =
                        my_abort_counter.load(std::memory_order_relaxed);
                    ...
                    do {
                        ...
                        if (static_cast<std::ptrdiff_t>(
                            ...
                            }).on_exception([&] { my_queue_representation->head_counter--; });
                        }
                        __TBB_ASSERT(static_cast<std::ptrdiff_t>(
                                         my_queue_representation->tail_counter.load(
                                             std::memory_order_relaxed)) > target,
                                     nullptr);
                    } while (!my_queue_representation->choose(target).pop(
                        dst, target, *my_queue_representation, my_allocator));
                ...
                bool internal_pop_if_present(void* dst)
                {
                    ...
                    do {
                        ...
                        do {
                            ...
                            ticket, ticket + 1));
                    } while (!my_queue_representation->choose(ticket).pop(
                        dst, ticket, *my_queue_representation, my_allocator));
