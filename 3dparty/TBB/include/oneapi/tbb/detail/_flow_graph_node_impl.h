...
public:
    ...
    typedef typename allocator_traits_base<A>::template rebind_alloc<input_queue_type>
        allocator_type;
...
private:
    ...
    graph_task* create_body_task(const input_type& input)
    {
        ...
        graph_task*                                            t =
            allocator.new_object<task_type>(my_graph_ref, allocator, *this, input, my_priority);
    ...
    inline graph_task* create_forward_task()
    {
        ...
        graph_task*                             t =
            allocator.new_object<task_type>(graph_reference(), allocator, *this, my_priority);
        graph_reference().reserve_wait();
        return t;
    }
...
class function_input
    : public function_input_base<Input, Policy, A, function_input<Input, Output, Policy, A>> {
public:
    typedef Input                                           input_type;
    ...
    function_input(graph& g, size_t max_concurrency, Body& body, node_priority_t a_priority)
    ...
    Body copy_function_object()
    {
        ...
        return dynamic_cast<function_body_leaf<input_type, output_type, Body>&>(body_ref)
            .get_body();
    }
...
class multifunction_input
    : public function_input_base<Input, Policy, A,
                                 multifunction_input<Input, OutputPortSet, Policy, A>> {
public:
    ...
    multifunction_input(const multifunction_input& src)
        : base_type(src),
          ...
          my_init_body(src.my_init_body->clone()),
          my_output_ports(
              init_output_ports<output_ports_type>::call(src.my_graph_ref, my_output_ports))
    {
    }
    ...
    Body copy_function_object()
    {
        ...
        return *static_cast<Body*>(
            dynamic_cast<multifunction_body_leaf<input_type, output_ports_type, Body>&>(
                body_ref)
                .get_body_ptr());
    }
    ...
    // TODO: consider moving common parts with implementation in function_input into separate
    // function
    graph_task* apply_body_impl_bypass(const input_type& i)
    {
        fgt_begin_body(my_body);
...
public:
    ...
    template <typename Body>
    continue_input(graph& g, int number_of_predecessors, Body& body, node_priority_t a_priority)
...
protected:
    ...
    #pragma warning(pop)
            ...
            graph_task* t = allocator.new_object<task_type>(graph_reference(), allocator, *this,
                                                            continue_msg(), my_priority);
            graph_reference().reserve_wait();
            return t;
        }
    }
...
void add_nodes_impl(CompositeType* c_node, bool visible, const NodeType1& n1,
                    const NodeTypes&... n)
{
