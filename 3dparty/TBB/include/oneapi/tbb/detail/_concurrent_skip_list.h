...
#include "TBB/include/oneapi/tbb/enumerable_thread_specific.h"
#include <utility>
...
namespace tbb {
    namespace detail {
        namespace d2 {
            ...
            private:
                using allocator_traits_base =
                    tbb::detail::allocator_traits_base<container_allocator_type>;

                // Allocator is the same as the container allocator=> allocates unitptr_t
                // It is required to rebind it to value_type to get the correct pointer and
                // const_pointer
                using value_allocator_traits =
                    typename allocator_traits_base::template rebind_traits<value_type>;
            ...
            public:
                ...
                // however this seems technically impractical due to use of
                // allocator_traits_base.

                // Should not be called directly, instead use create method
                skip_list_node(size_type levels) : my_height(levels), my_index_number(0) {}
                ...
                static skip_list_node* create(container_allocator_type& alloc, size_type height)
                {
                    ...
                    static_assert(
                        std::is_same<typename allocator_traits_base::value_type,
                                     std::u8_t>::value,
                        "skip_list_node assumes that passed in allocator operates on bytes");
                    auto* node = reinterpret_cast<skip_list_node*>(
                        allocator_traits_base::allocate(alloc, sz));

                    // Construct the node itself
                    allocator_traits_base::construct(alloc, node, height);

                    // Construct the level pointers
                    for (size_type l = 0; l < height; ++l) {
                        allocator_traits_base::construct(alloc, &node->get_atomic_next(l),
                                                         nullptr);
                    }
                ...
                static void destroy(container_allocator_type& alloc, skip_list_node* node)
                {
                    ...
                    for (size_type l = 0; l < node->height(); ++l) {
                        allocator_traits_base::destroy(alloc, &node->atomic_next(l));
                    }
                    size_type sz = calc_node_size(node->height());
                    // Destroy the node itself
                    allocator_traits_base::destroy(alloc, node);

                    // Deallocate the node
                    allocator_traits_base::deallocate(alloc, reinterpret_cast<std::u8_t*>(node),
                                                      sz);
                }
                ...
                node_ptr next(size_type level) const
                {
                    ...
                    __TBB_ASSERT(res == nullptr || res->height() > level,
                ...
                atomic_node_ptr& atomic_next(size_type level)
                {
                    ...
                    __TBB_ASSERT(node == nullptr || node->height() > level,
                                 "Broken internal structure");
#endif
                    return res;
                }
                ...
                {
                    __TBB_ASSERT(n == nullptr || n->height() > level,
                                 "Broken internal structure");
            ...
            private:
                ...
                {
                    static_assert(alignof(skip_list_node) >= alignof(atomic_node_ptr),
                                  "Incorrect alignment");
                ...
                {
                    const atomic_node_ptr* arr =
                        reinterpret_cast<const atomic_node_ptr*>(this + 1);
                    return arr[level];
                }
            ...
            public:
                ...
                skip_list_iterator(
                    const skip_list_iterator<node_type, typename node_type::value_type>& other)
            ...
            private:
                ...
                friend bool operator==(const skip_list_iterator& lhs,
                                       const skip_list_iterator& rhs)
                {
            ...
            protected:
                ...
                using node_allocator_type =
                    typename allocator_traits_type::template rebind_alloc<std::u8_t>;
                ...
                using node_type =
                    d1::node_handle<key_type, value_type, list_node_type, allocator_type>;
                ...
                using random_level_generator_type =
                    typename container_traits::random_level_generator_type;
            ...
            public:
                ...
                explicit concurrent_skip_list(const key_compare&    comp,
                ...
                concurrent_skip_list(InputIterator first, InputIterator last,
                ...
                concurrent_skip_list(std::initializer_list<value_type> init,
                                     const key_compare&                comp = key_compare(),
                                     const allocator_type&             alloc = allocator_type())
                ...
                {
                    internal_copy(other);
                    __TBB_ASSERT(my_size == other.my_size,
                                 "Wrong size of copy-constructed container");
                }
                ...
                std::pair<iterator, bool> insert(node_type&& nh)
                {
                    if (!nh.empty()) {
                        ...
                        std::pair<iterator, bool> insert_result =
                            internal_insert_node(insert_node);
                ...
                node_type unsafe_extract(const_iterator pos)
                {
                    ...
                    return extract_result.first
                               ? d1::node_handle_accessor::construct<node_type>(
                ...
                const_iterator upper_bound(const key_type& key) const
                {
                    return const_iterator(
                        internal_get_bound(key, not_greater_compare(my_compare)));
                }
                ...
                void clear() noexcept
                {
                    ...
                    if (head == nullptr)
                        return;  // Head is not allocated => container is empty
                ...
                {
                    return container_traits::value_comp(my_compare);
                }
                ...
                public:
                    ...
                    {
                        return my_level != 0 ? my_begin.my_node_ptr->next(my_level - 1) !=
                                                   my_end.my_node_ptr
                                             : false;
                    }
                    ...
                    size_type size() const
                    {
                        return std::distance(my_begin, my_end);
                    }

                    const_range_type(const_range_type& r, split) : my_end(r.my_end)
                    {
            ...
            private:
                ...
                {
                    my_head_ptr.store(other.my_head_ptr.load(std::memory_order_relaxed),
                ...
                void
                internal_move_construct_with_allocator(concurrent_skip_list&& other,
                ...
                {
                    return allow_multimapping ? internal_find_multi(key)
                                              : internal_find_unique(key);
                }
                ...
                node_ptr internal_find_multi(const K& key) const
                {
                    ...
                    if (prev == nullptr)
                        return nullptr;  // If the head node is not allocated - exit

                    node_ptr curr = nullptr;
                    node_ptr old_curr = curr;

                    for (size_type h = my_max_height.load(std::memory_order_acquire); h > 0;
                         --h) {
                ...
                node_ptr internal_find_unique(const K& key) const
                {
                    ...
                    return (it == end() || my_compare(key, container_traits::get_key(*it)))
                               ? nullptr
                               : it.my_node_ptr;
                }
                ...
                std::pair<iterator, iterator> internal_equal_range(const K& key) const
                {
                    ...
                    if (found(lb.my_node_ptr, key)) {
                        if (!allow_multimapping) {
                            ...
                            // For multi containers - find the upper bound starting from the
                            // lower bound
                            ...
                            for (size_type h = prev->height(); h > 0; --h) {
                                ...
                                while (curr && cmp(get_key(curr), key)) {
                                    prev = curr;
                                    // If the height of the next node is greater than the
                                    // current one - jump to its height
                                    if (h < curr->height()) {
                                        h = curr->height();
                                    }
                                    curr = prev->next(h - 1);
                                                ...
                                                const Comparator& cmp) const
                {
                    ...
                    while (curr && cmp(get_key(curr), get_key(node))) {
                        if (allow_multimapping && cmp(get_key(node), get_key(curr)) &&
                            curr->index_number() > node->index_number()) {
                            break;
                        }
                ...
                template <typename Comparator>
                void fill_prev_curr_arrays(array_type& prev_nodes, array_type& curr_nodes,
                                           node_ptr node, const key_type& key,
                                           const Comparator& cmp, node_ptr head)
                {
                    ...
                    if (curr_max_height < node_height) {
                        std::fill(prev_nodes.begin() + curr_max_height,
                                  prev_nodes.begin() + node_height, head);
                        std::fill(curr_nodes.begin() + curr_max_height,
                                  curr_nodes.begin() + node_height, nullptr);
                    }
                ...
                std::pair<iterator, bool> internal_insert_node(node_ptr new_node)
                {
                    ...
                    auto       compare =
                        select_comparator(std::integral_constant<bool, allow_multimapping>{});

                    node_ptr head_node = create_head_if_necessary();

                    for (;;) {
                        fill_prev_curr_arrays(prev_nodes, curr_nodes, new_node,
                                              get_key(new_node), compare, head_node);
                        ...
                        // If the node was successfully linked on the first level - it will be
                        // linked on other levels Insertion cannot fail starting from this point

                        // If the height of inserted node is greater than maximum - increase
                        // maximum
                        size_type max_height = my_max_height.load(std::memory_order_acquire);
                        for (;;) {
                            if (new_height <= max_height ||
                                my_max_height.compare_exchange_strong(max_height, new_height)) {
                                // If the maximum was successfully updated by current thread
                                // or by an other thread for the value, greater or equal to
                                // new_height
                                break;
                            }
                        }

                        for (size_t level = 1; level < new_height; ++level) {
                            ...
                            for (;;) {
                                ...
                                __TBB_ASSERT(new_node->height() > level,
                                             "Detail structure break");
                                if (prev->atomic_next(level).compare_exchange_strong(
                                        next, new_node)) {
                                    break;
                                }

                                for (size_type lev = level; lev != new_height; ++lev) {
                                    curr_nodes[lev] = internal_find_position(
                                        lev, prev_nodes[lev], new_node, compare);
                                }
                            }
                        }
                        ++my_size;
                ...
                // Returns node_ptr to the extracted node and node_ptr to the next node after
                // the extracted
                ...
                void internal_merge(SourceType&& source)
                {
                    ...
                    for (source_iterator it = source.begin(); it != source.end();) {
                        ...
                        if (allow_multimapping ||
                            ...
                            __TBB_ASSERT(handle.empty(),
                                         "Node handle should be empty after the insertion");
                        }
                    }
                }
            ...
            private:
                ...
                node_ptr create_value_node(Args&&... args)
                {
                    ...
                    auto value_guard = make_raii_guard([&] { delete_node(node); });

                    // Construct the value inside the node
                    node_allocator_traits::construct(my_node_allocator, node->storage(),
                ...
                node_ptr create_head_if_necessary()
                {
                    ...
                    if (current_head == nullptr) {
                        ...
                        if (my_head_ptr.compare_exchange_strong(current_head, new_head)) {
                            ...
                            // If an other thread has already created the head node - destroy
                            // new_head current_head now points to the actual head node
                            delete_node(new_head);
                        }
                    }
                    __TBB_ASSERT(my_head_ptr.load(std::memory_order_relaxed) != nullptr,
                                 nullptr);
                ...
                void internal_move_assign(concurrent_skip_list&& other,
                                          /*POCMA || is_always_equal =*/std::true_type)
                {
                    internal_move(std::move(other));
                }

                void internal_move_assign(concurrent_skip_list&& other,
                                          /*POCMA || is_always_equal =*/std::false_type)
                {
                ...
                void internal_swap(concurrent_skip_list& other,
                                   /*POCMA || is_always_equal =*/std::true_type)
                {
                ...
                void internal_swap(concurrent_skip_list& other,
                                   /*POCMA || is_always_equal =*/std::false_type)
                {
                    __TBB_ASSERT(my_node_allocator == other.my_node_allocator,
            ...
            template <typename Traits>
            bool operator==(const concurrent_skip_list<Traits>& lhs,
                            const concurrent_skip_list<Traits>& rhs)
            {
            ...
            operator<=>(const concurrent_skip_list<Traits>& lhs,
                        const concurrent_skip_list<Traits>& rhs)
            {
            ...
            public:
                static constexpr size_t max_level = MaxLevel;
                ...
                concurrent_geometric_level_generator()
                    : engines(std::minstd_rand::result_type(time(nullptr)))
                {
                }
                ...
                size_t operator()()
                {
                    ...
                    // -1 is required to have an ability to return 0 from the generator
                    // (max_level - log2(2^31) - 1)
                    size_t result =
                        max_level - size_t(tbb::detail::log2(engines.local()() + 1)) - 1;
