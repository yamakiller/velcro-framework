...
struct is_element_of {
    ...
    static const bool                                       value =
        std::is_same<Q, T_i>::value || is_element_of<Q, N - 1, Tuple>::value;
};
...
// Tuple tells us the allowed types that variant can hold.  It determines the alignment of the
// space in Wrapper, and how big Wrapper is.
//
// the object can only be tested for type, and a read-only reference can be fetched by
// cast_to<T>().

using tbb::detail::punned_cast;
struct tagged_null_type {};
template <typename TagType, typename T0, typename T1 = tagged_null_type,
          typename T2 = tagged_null_type, typename T3 = tagged_null_type,
          typename T4 = tagged_null_type, typename T5 = tagged_null_type,
          typename T6 = tagged_null_type, typename T7 = tagged_null_type,
          typename T8 = tagged_null_type, typename T9 = tagged_null_type>
class tagged_msg {
...
private:
    ...
    public:
        variant()
        {
            (void)new (&my_space) Wrapper<default_constructed>(default_constructed());
        }
        ...
        {
            const Wrapper<U>* h =
                dynamic_cast<const Wrapper<U>*>(punned_cast<const WrapperBase*>(&my_space));
            if (!h) {
        ...
        {
            return variant_is_a<default_constructed>();
        }
...
public:
    ...
    {
        return my_msg.variant_is_default_constructed();
    }
};  // class tagged_msg
