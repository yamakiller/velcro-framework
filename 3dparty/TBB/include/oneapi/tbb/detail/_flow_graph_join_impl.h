...
struct join_helper {
    ...
    static inline bool get_my_item(InputTuple& my_input, OutputTuple& out)
    {
        ...
        return join_helper<N - 1>::get_my_item(my_input, out) &&
               res;  // do get on other inputs before returning
    }
    ...
    {
        __TBB_ASSERT(std::get<N - 1>(other_inputs).get_my_key_func(),
                     "key matching join node should not be instantiated without functors.");
        std::get<N - 1>(my_inputs).set_my_key_func(
            std::get<N - 1>(other_inputs).get_my_key_func()->clone());
...
struct join_helper<1> {
    ...
    static inline void copy_key_functors(KeyFuncTuple& my_inputs, KeyFuncTuple& other_inputs)
    {
        __TBB_ASSERT(std::get<0>(other_inputs).get_my_key_func(),
                     "key matching join node should not be instantiated without functors.");
        std::get<0>(my_inputs).set_my_key_func(
            std::get<0>(other_inputs).get_my_key_func()->clone());
    }
    template <typename InputTuple>
    static inline void reset_inputs(InputTuple& my_input, reset_flags f)
    {
...
private:
    ...
    public:
        ...
        reserving_port_operation(const predecessor_type& s, op_type t)
            : type(char(t)), my_pred(const_cast<predecessor_type*>(&s))
        {
        }
...
private:
    ...
    public:
        ...
        queueing_port_operation(const T& e, op_type t)
            : type(char(t)), my_val(e), bypass_t(nullptr)
        {
        }
        // constructor for pointer parameter
        queueing_port_operation(const T* p, op_type t)
            : type(char(t)), my_arg(const_cast<T*>(p)), bypass_t(nullptr)
        {
        }
        // constructor with no parameter
        queueing_port_operation(op_type t) : type(char(t)), bypass_t(nullptr) {}
    };
    ...
    void handle_operations(queueing_port_operation* op_list)
    {
        ...
        while (op_list) {
            ...
            case try__put_task: {
                ...
                if (was_empty)
                    rtask = my_join->decrement_port_count(false);
                else
...
class key_matching_port
    : public receiver<typename TraitsType::T>,
      public hash_buffer<typename TraitsType::K, typename TraitsType::T,
                         typename TraitsType::TtoK, typename TraitsType::KHash> {
public:
    ...
    typedef hash_buffer<key_type, input_type, type_to_key_func_type, hash_compare_type>
        buffer_type;
...
private:
    ...
    public:
        ...
        input_type* my_arg;
        // constructor for value parameter
        key_matching_port_operation(const input_type& e, op_type t) : type(char(t)), my_val(e)
        {
        }
        // constructor for pointer parameter
    ...
    void handle_operations(key_matching_port_operation* op_list)
    {
        ...
        while (op_list) {
            ...
            case try__put: {
                ...
                current->status.store(was_inserted ? SUCCEEDED : FAILED,
...
public:
    ...
    // Suppress superfluous diagnostic about virtual keyword absence in a destructor of an
    // inherited class while the parent class has the virtual keyword for the destrocutor.
    virtual
#endif
        ~key_matching_port()
    {
...
public:
    ...
    typedef join_node_base<reserving, InputTuple, OutputTuple>
        base_node_type;  // for forwarding
    ...
    join_node_FE(const join_node_FE& other)
        : reserving_forwarding_base((other.reserving_forwarding_base::graph_ref)),
          my_node(nullptr)
    {
        ports_with_no_inputs = N;
        join_helper<N>::set_join_node_pointer(my_inputs, this);
...
protected:
    ...
    std::atomic<size_t> ports_with_no_inputs;
...
public:
    ...
    join_node_FE(const join_node_FE& other)
        : queueing_forwarding_base((other.queueing_forwarding_base::graph_ref)),
          my_node(nullptr)
    {
        ports_with_no_items = N;
        join_helper<N>::set_join_node_pointer(my_inputs, this);
...
protected:
    ...
    input_type          my_inputs;
    base_node_type*     my_node;
    std::atomic<size_t> ports_with_no_items;
...
class join_node_FE<key_matching<K, KHash>, InputTuple, OutputTuple>
    : public matching_forwarding_base<K>,
      ...
      public hash_buffer<                 // typedefed below to key_to_count_buffer_type
          ...
          type_to_key_function_body<count_element<typename std::decay<K>::type>,
...
public:
    ...
    typedef type_to_key_function_body<count_element_type, unref_key_type&>
        TtoK_function_body_type;
    typedef type_to_key_function_body_leaf<count_element_type, unref_key_type&,
                                           key_to_count_func>
    ...
    typedef hash_buffer<unref_key_type&, count_element_type, TtoK_function_body_type,
                        key_hash_compare>
                                     key_to_count_buffer_type;
    typedef item_buffer<output_type> output_buffer_type;
    typedef join_node_base<key_matching<key_type, key_hash_compare>, InputTuple, OutputTuple>
                                               base_node_type;  // for forwarding
...
private:
    ...
    typedef join_node_FE<key_matching<key_type, key_hash_compare>, InputTuple, OutputTuple>
        class_type;
    ...
    public:
        ...
        graph_task*    bypass_t;
        // constructor for value parameter
        key_matching_FE_operation(const unref_key_type& e, op_type t)
            : type(char(t)), my_val(e), my_output(nullptr), bypass_t(nullptr)
        {
        }
        key_matching_FE_operation(output_type* p, op_type t)
            : type(char(t)), my_output(p), bypass_t(nullptr)
        {
        }
        // constructor with no parameter
        key_matching_FE_operation(op_type t)
            : type(char(t)), my_output(nullptr), bypass_t(nullptr)
        {
        }
    };
    ...
    graph_task* fill_output_buffer(unref_key_type& t)
    {
        ...
        if (join_helper<N>::get_items(my_inputs, l_out)) {  //  <== call back
            ...
            if (do_fwd) {  // we enqueue if receiving an item from predecessor, not if successor
                           // asks for item
                small_object_allocator                      allocator{};
                typedef forward_task_bypass<base_node_type> task_type;
    ...
    void handle_operations(key_matching_FE_operation* op_list)
    {
        ...
        while (op_list) {
            ...
            case may_succeed:  // called from BE
                current->status.store(this->buffer_empty() ? FAILED : SUCCEEDED,
                                      std::memory_order_release);
                break;
...
public:
    ...
    join_node_FE(graph& g, FunctionTuple& TtoK_funcs)
        : forwarding_base_type(g), my_node(nullptr)
    {
        join_helper<N>::set_join_node_pointer(my_inputs, this);
    ...
    join_node_FE(const join_node_FE& other)
        : forwarding_base_type((other.forwarding_base_type::graph_ref)),
          key_to_count_buffer_type(),
          output_buffer_type()
    {
        my_node = nullptr;
    ...
    graph_task* increment_key_count(unref_key_type const& t) override
    {  // called from input_ports
...
//! join_node_base
template <typename JP, typename InputTuple, typename OutputTuple>
class join_node_base : public graph_node,
                       public join_node_FE<JP, InputTuple, OutputTuple>,
...
private:
    ...
    public:
        ...
        join_node_base_operation(const successor_type& s, op_type t)
            : type(char(t)), my_succ(const_cast<successor_type*>(&s)), bypass_t(nullptr)
        {
        }
        ...
        while (op_list) {
            ...
            case reg_succ: {
                ...
                if (tuple_build_may_succeed() && !forwarder_busy && is_graph_active(my_graph)) {
                    small_object_allocator allocator{};
                    typedef forward_task_bypass<join_node_base<JP, InputTuple, OutputTuple>>
                                task_type;
            ...
            case do_fwrd_bypass: {
                ...
                // again. Without reservation of the FE, the methods accessing it must be
                // locked. We could remember the keys of the objects we forwarded, and then
                // remove them from the input ports after forwarding is complete?
...
public:
    ...
    join_node_base(const join_node_base& other)
        : graph_node(other.graph_node::my_graph),
...
struct join_base {
    typedef join_node_base<JP, typename wrap_tuple_elements<N, PT, OutputTuple>::type,
                           OutputTuple>
        type;
};
...
struct join_base<N, key_matching_port, OutputTuple, key_matching<K, KHash>> {
    ...
    typedef join_node_base<key_traits_type,
                           // ports type
                           typename wrap_key_tuple_elements<N, key_matching_port,
                                                            key_traits_type, OutputTuple>::type,
                           OutputTuple>
        type;
};

//! unfolded_join_node : passes input_ports_type to join_node_base.  We build the input port
//! type
//  using tuple_element.  The class PT is the port type (reserving_port, queueing_port,
//  key_matching_port) and should match the typename.
...
// key_matching unfolded_join_node.  This must be a separate specialization because the
// constructors differ.
...
class unfolded_join_node<2, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<2, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;

public:
    typedef typename wrap_key_tuple_elements<2, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        new type_to_key_function_body_leaf<T0, K, key_from_message_body<K, T0>>(
                            key_from_message_body<K, T0>()),
                        new type_to_key_function_body_leaf<T1, K, key_from_message_body<K, T1>>(
                            key_from_message_body<K, T1>())))
    {
    }
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    ...
    unfolded_join_node(graph& g, Body0 body0, Body1 body1)
        : base_type(
              g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                       new type_to_key_function_body_leaf<T1, K, Body1>(body1)))
    {
        static_assert(std::tuple_size<OutputTuple>::value == 2,
...
class unfolded_join_node<3, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<3, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;

public:
    typedef typename wrap_key_tuple_elements<3, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T1, K, key_from_message_body<K, T1>>(
                            key_from_message_body<K, T1>()),
                        new type_to_key_function_body_leaf<T2, K, key_from_message_body<K, T2>>(
                            key_from_message_body<K, T2>())))
    {
    }
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 3,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<4, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<4, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
...
public:
    typedef typename wrap_key_tuple_elements<4, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T2, K, key_from_message_body<K, T2>>(
                            key_from_message_body<K, T2>()),
                        new type_to_key_function_body_leaf<T3, K, key_from_message_body<K, T3>>(
                            key_from_message_body<K, T3>())))
    {
    }
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 4,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<5, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<5, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
    typedef typename std::tuple_element<3, OutputTuple>::type T3;
...
public:
    typedef typename wrap_key_tuple_elements<5, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T3, K, key_from_message_body<K, T3>>(
                            key_from_message_body<K, T3>()),
                        new type_to_key_function_body_leaf<T4, K, key_from_message_body<K, T4>>(
                            key_from_message_body<K, T4>())))
    {
    }
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 5,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<6, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<6, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
    typedef typename std::tuple_element<3, OutputTuple>::type T3;
...
public:
    typedef typename wrap_key_tuple_elements<6, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T4, K, key_from_message_body<K, T4>>(
                            key_from_message_body<K, T4>()),
                        new type_to_key_function_body_leaf<T5, K, key_from_message_body<K, T5>>(
                            key_from_message_body<K, T5>())))
    {
    }
    #endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4,
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 6,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<7, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<7, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
    typedef typename std::tuple_element<3, OutputTuple>::type T3;
...
public:
    typedef typename wrap_key_tuple_elements<7, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T5, K, key_from_message_body<K, T5>>(
                            key_from_message_body<K, T5>()),
                        new type_to_key_function_body_leaf<T6, K, key_from_message_body<K, T6>>(
                            key_from_message_body<K, T6>())))
    {
    }
    #endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4,
    ...
    unfolded_join_node(graph& g, Body0 body0, Body1 body1, Body2 body2, Body3 body3,
                       Body4 body4, Body5 body5, Body6 body6)
        : base_type(
              g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                       new type_to_key_function_body_leaf<T1, K, Body1>(body1),
                                       new type_to_key_function_body_leaf<T2, K, Body2>(body2),
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 7,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<8, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<8, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
    typedef typename std::tuple_element<3, OutputTuple>::type T3;
...
public:
    typedef typename wrap_key_tuple_elements<8, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T6, K, key_from_message_body<K, T6>>(
                            key_from_message_body<K, T6>()),
                        new type_to_key_function_body_leaf<T7, K, key_from_message_body<K, T7>>(
                            key_from_message_body<K, T7>())))
    {
    }
    #endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4,
    ...
    unfolded_join_node(graph& g, Body0 body0, Body1 body1, Body2 body2, Body3 body3,
                       Body4 body4, Body5 body5, Body6 body6, Body7 body7)
        : base_type(
              g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                       new type_to_key_function_body_leaf<T1, K, Body1>(body1),
                                       new type_to_key_function_body_leaf<T2, K, Body2>(body2),
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 8,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<9, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<9, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
    typedef typename std::tuple_element<3, OutputTuple>::type T3;
...
public:
    typedef typename wrap_key_tuple_elements<9, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
private:
    ...
    typedef std::tuple<f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p, f7_p, f8_p>
        func_initializer_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T7, K, key_from_message_body<K, T7>>(
                            key_from_message_body<K, T7>()),
                        new type_to_key_function_body_leaf<T8, K, key_from_message_body<K, T8>>(
                            key_from_message_body<K, T8>())))
    {
    }
    #endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4,
    ...
    unfolded_join_node(graph& g, Body0 body0, Body1 body1, Body2 body2, Body3 body3,
                       Body4 body4, Body5 body5, Body6 body6, Body7 body7, Body8 body8)
        : base_type(
              g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                       new type_to_key_function_body_leaf<T1, K, Body1>(body1),
                                       new type_to_key_function_body_leaf<T2, K, Body2>(body2),
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 9,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
...
class unfolded_join_node<10, key_matching_port, OutputTuple, key_matching<K, KHash>>
    : public join_base<10, key_matching_port, OutputTuple, key_matching<K, KHash>>::type {
    typedef typename std::tuple_element<0, OutputTuple>::type T0;
    typedef typename std::tuple_element<1, OutputTuple>::type T1;
    typedef typename std::tuple_element<2, OutputTuple>::type T2;
    typedef typename std::tuple_element<3, OutputTuple>::type T3;
...
public:
    typedef typename wrap_key_tuple_elements<10, key_matching_port, key_matching<K, KHash>,
                                             OutputTuple>::type input_ports_type;
    typedef OutputTuple                                         output_type;
...
private:
    ...
    typedef std::tuple<f0_p, f1_p, f2_p, f3_p, f4_p, f5_p, f6_p, f7_p, f8_p, f9_p>
        func_initializer_type;
...
public:
    ...
    unfolded_join_node(graph& g)
        : base_type(g,
                    func_initializer_type(
                        ...
                        new type_to_key_function_body_leaf<T8, K, key_from_message_body<K, T8>>(
                            key_from_message_body<K, T8>()),
                        new type_to_key_function_body_leaf<T9, K, key_from_message_body<K, T9>>(
                            key_from_message_body<K, T9>())))
    {
    }
    #endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4,
    ...
    {
        static_assert(std::tuple_size<OutputTuple>::value == 10,
                      "wrong number of body initializers");
    }
    unfolded_join_node(const unfolded_join_node& other) : base_type(other) {}
};
