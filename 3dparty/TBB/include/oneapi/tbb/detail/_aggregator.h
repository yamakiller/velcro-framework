...
#include "_utils.h"
#include <atomic>
#if !__TBBMALLOC_BUILD  // TODO: Assert this macro with TBB Malloc
...
namespace tbb {
    namespace detail {
        namespace d1 {
            ...
            // m_base class for aggregated operation
            template <typename Derived>
            class aggregated_operation {
            public:
                // Zero value means "wait" status, all other values are "user" specified values
                // and are defined into the scope of a class which uses "status"
            ...
            /* An aggregator for collecting operations coming from multiple sources and
               executing them serially on a single thread.  OperationType must be derived from
               aggregated_operation. The parameter HandlerType is a functor that will be passed
               the list of operations and is expected to handle each operation appropriately,
               setting the status of each operation to non-zero. */
            ...
            public:
                ...
                /* Places an operation into the waitlist (pending_operations), and either
                   handles the list, or waits for the operation to complete, or returns. The
                   long_life_time parameter specifies the life time of the given operation
                   object. Operations with long_life_time == true may be accessed after
                   execution. A "short" life time operation (long_life_time == false) can be
                   destroyed during execution, and so any access to it after it was put into the
                   waitlist, including status Assert, is invalid. As a consequence, waiting for
                   completion of such operation causes undefined behavior. */
                             ...
                             bool long_life_time = true)
                {
                    ...
                    // ITT note: &(op->status) tag is used to cover accesses to this op node.
                    // This thread has created the operation, and now releases it so that the
                    // handler thread may handle the associated operation w/o triggering a race
                    // condition; thus this tag will be acquired just before the operation is
                    // handled in the handle_operations functor.
                    ...
                    if (!res) {  // first in the list; handle the operations
                        // ITT note: &pending_operations tag covers access to the handler_busy
                        // flag, which this waiting handler thread will try to set before
                        // entering handle_operations.
                    ...
                    // Not first; wait for op to be ready
                        ...
                        __TBB_ASSERT(long_life_time, "Waiting for an operation object that "
                                                     "might be destroyed during processing");
            ...
            private:
                ...
                void start_handle_operations(HandlerType& handle_operations)
                {
                    ...
                    // ITT note: &handler_busy tag covers access to pending_operations as it is
                    // passed between active and waiting handlers.  Below, the waiting handler
                    // waits until the active handler releases, and the waiting handler acquires
                    // &handler_busy as it becomes the active_handler. The release point is at
                    // the end of this function, when all operations in pending_operations have
                    // been handled by the owner of this aggregator.
                    ...
                    // acquire fence not necessary here due to causality rule and surrounding
                    // atomics
                    ...
                    // handler_busy has been set and a new active handler will now process that
                    // list's operations.
            ...
            public:
                ...
                void initialize_handler(HandlerType h)
                {
                    handle_operations = h;
                }
