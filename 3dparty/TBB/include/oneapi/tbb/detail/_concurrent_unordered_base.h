    ...
    namespace detail {
        namespace d1 {
            ...
            private:
                ...
                friend bool operator==(const solist_iterator<M, T>& i,
            ...
            public:
                ...
                solist_iterator(
                    const solist_iterator<Container, typename Container::value_type>& other)
            ...
            private:
                using aligned_storage_type =
                    typename std::aligned_storage<sizeof(value_type)>::type;
            ...
            private:
                ...
                // TODO: Assert assert conditions for different C++ standards
                static_assert(
                    std::is_same<typename allocator_traits_type::value_type, value_type>::value,
                    "value_type of the container must be the same as its allocator");
                using sokey_type = size_t;
            ...
            private:
                ...
                using value_node_allocator_traits =
                    tbb::detail::allocator_traits_base<value_node_allocator_type>;

                static constexpr size_type round_up_to_power_of_two(size_type bucket_count)
                {
                    return size_type(1) << size_type(tbb::detail::log2(
                               uintptr_t(bucket_count == 0 ? 1 : bucket_count) * 2 - 1));
                }

                template <typename T>
                using is_transparent =
                    dependent_bool<has_transparent_key_equal<key_type, hasher, key_equal>, T>;

            public:
                using node_type =
                    node_handle<key_type, value_type, value_node_type, allocator_type>;

                explicit concurrent_unordered_base(
                ...
                concurrent_unordered_base(size_type bucket_count, const hasher& hash,
                ...
                explicit concurrent_unordered_base(const allocator_type& alloc)
                    : concurrent_unordered_base(initial_bucket_count, hasher(), key_equal(),
                                                alloc)
                {
                }
                ...
                concurrent_unordered_base(InputIterator first, InputIterator last,
                    ...
                    : concurrent_unordered_base(first, last, bucket_count, hasher(),
                                                key_equal(), alloc)
                {
                }
                ...
                concurrent_unordered_base(InputIterator first, InputIterator last,
                                          ...
                                          const allocator_type& alloc)
                    : concurrent_unordered_base(first, last, bucket_count, hash, key_equal(),
                                                alloc)
                {
                }
                ...
                {
                    try_call([&] { internal_copy(other); }).on_exception([&] { clear(); });
                }
                      ...
                      my_segments(std::move(other.my_segments), alloc)
                {
                    ...
                    internal_move_construct_with_allocator(std::move(other), alloc,
                                                           is_always_equal());
                }
                ...
                concurrent_unordered_base(std::initializer_list<value_type> init,
                                          ...
                                          const key_equal& equal = key_equal(),
                                          const allocator_type& alloc = allocator_type())
                    : concurrent_unordered_base(init.begin(), init.end(), bucket_count, hash,
                                                equal, alloc)
                {
                }
                ...
                concurrent_unordered_base(std::initializer_list<value_type> init,
                                          size_type bucket_count, const allocator_type& alloc)
                    : concurrent_unordered_base(init, bucket_count, hasher(), key_equal(),
                                                alloc)
                {
                }
                ...
                concurrent_unordered_base& operator=(const concurrent_unordered_base& other)
                {
                    if (this != &other) {
                        ...
                        my_bucket_count.store(
                            other.my_bucket_count.load(std::memory_order_relaxed),
                            std::memory_order_relaxed);
                        my_max_load_factor = other.my_max_load_factor;
                        my_hash_compare = other.my_hash_compare;
                        my_segments = other.my_segments;
                ...
                concurrent_unordered_base&
                operator=(concurrent_unordered_base&& other) noexcept(
                    unordered_segment_table::is_noexcept_assignment)
                {
                    if (this != &other) {
                        clear();
                        my_size.store(other.my_size.load(std::memory_order_relaxed),
                        ...
                        internal_move_assign(
                            std::move(other),
                            tbb::detail::disjunction<pocma_type, is_always_equal>());
                    }
                    return *this;
                }
                ...
                void swap(concurrent_unordered_base& other) noexcept(
                    unordered_segment_table::is_noexcept_swap)
                {
                    if (this != &other) {
                        using pocs_type =
                        ...
                        using is_always_equal = typename allocator_traits_type::is_always_equal;
                        internal_swap(other,
                                      tbb::detail::disjunction<pocs_type, is_always_equal>());
                    }
                }

                allocator_type get_allocator() const noexcept
                {
                    return my_segments.get_allocator();
                }

                iterator begin() noexcept
                {
                    return iterator(first_value_node(&my_head));
                }
                const_iterator begin() const noexcept
                {
                    return const_iterator(first_value_node(const_cast<node_ptr>(&my_head)));
                }
                ...
                __TBB_nodiscard bool empty() const noexcept
                {
                    return size() == 0;
                }
                ...
                std::pair<iterator, bool> insert(node_type&& nh)
                {
                    if (!nh.empty()) {
                        ...
                        auto           init_node =
                            [&insert_node](sokey_type order_key) -> value_node_ptr {
                ...
                template <typename K>
                typename std::enable_if<is_transparent<K>::value, iterator>::type
                find(const K& key)
                {
                    value_node_ptr result = internal_find(key);
                    return result == nullptr ? end() : iterator(result);
                ...
                template <typename K>
                typename std::enable_if<is_transparent<K>::value, const_iterator>::type
                ...
                template <typename K>
                typename std::enable_if<is_transparent<K>::value, size_type>::type
                ...
                template <typename K>
                typename std::enable_if<is_transparent<K>::value, bool>::type
                ...
                {
                    auto bucket_begin =
                        first_value_node(const_cast<self_type*>(this)->get_bucket(n));
                ...
                local_iterator unsafe_end(size_type n)
                {
                    ...
                    return n != bucket_count - 1 ? unsafe_begin(get_next_bucket_index(n))
                ...
                {
                    return my_bucket_count.load(std::memory_order_relaxed);
                }
                ...
                {
                    return float(size() /
                                 float(my_bucket_count.load(std::memory_order_acquire)));
                }
                ...
                void rehash(size_type bucket_count)
                {
                    size_type current_bucket_count =
                        my_bucket_count.load(std::memory_order_acquire);
                    if (current_bucket_count < bucket_count) {
                        // TODO: do we need do-while here?
                        my_bucket_count.compare_exchange_strong(
                            current_bucket_count, round_up_to_power_of_two(bucket_count));
                    }
                }
                    ...
                    do {
                        ...
                        while (necessary_bucket_count * max_load_factor() < elements_count) {
                            necessary_bucket_count <<= 1;
                             ...
                             !my_bucket_count.compare_exchange_strong(current_bucket_count,
                ...
                {
                    return my_hash_compare.hash_function();
                }
                key_equal key_eq() const
                {
                    return my_hash_compare.key_eq();
                }
                ...
                public:
                    ...
                    {
                        return iterator(my_instance.first_value_node(my_begin_node));
                    }
                    iterator end() const
                    {
                        return iterator(my_instance.first_value_node(my_end_node));
                    }

                    const_range_type(const concurrent_unordered_base& table)
                        : my_instance(table),
                          my_begin_node(const_cast<node_ptr>(&table.my_head)),
                          my_end_node(nullptr)
                    {
                        set_midpoint();
                    }
                ...
                private:
                    void set_midpoint() const
                    {
                        if (my_begin_node == my_end_node) {
                            ...
                            sokey_type begin_key = my_begin_node != nullptr
                                                       ? my_begin_node->order_key()
                                                       : invalid_key;
                            sokey_type end_key =
                                my_end_node != nullptr ? my_end_node->order_key() : invalid_key;

                            size_type mid_bucket =
                                reverse_bits(begin_key + (end_key - begin_key) / 2) %
                                my_instance.my_bucket_count.load(std::memory_order_relaxed);
                            while (my_instance.my_segments[mid_bucket].load(
                            ...
                            if (reverse_bits(mid_bucket) > begin_key) {
                                ...
                                my_midpoint_node = my_instance.first_value_node(
                                    my_instance.my_segments[mid_bucket].load(
                                        std::memory_order_relaxed));
                            } else {
                ...
                public:
                    ...
                    iterator begin() const
                    {
                        return iterator(const_range_type::begin().get_node_ptr());
                    }
                    iterator end() const
                    {
                        return iterator(const_range_type::end().get_node_ptr());
                    }
                };  // class range_type
            ...
            private:
                ...
                static constexpr size_type pointers_per_embedded_table =
                    sizeof(size_type) * 8 - 1;

                class unordered_segment_table
                    : public segment_table<std::atomic<node_ptr>, allocator_type,
                                           unordered_segment_table,
                                           pointers_per_embedded_table> {
                    using self_type = unordered_segment_table;
                    using atomic_node_ptr = std::atomic<node_ptr>;
                    using base_type =
                        segment_table<std::atomic<node_ptr>, allocator_type,
                                      unordered_segment_table, pointers_per_embedded_table>;
                    ...
                    using segment_allocator_traits =
                        tbb::detail::allocator_traits_base<segment_allocator_type>;

                public:
                    // Segment table for unordered containers should not be extended in the
                    // wait- free implementation
                    static constexpr bool allow_table_extending = false;
                    static constexpr bool is_noexcept_assignment =
                        std::is_nothrow_move_assignable<hasher>::value &&
                        std::is_nothrow_move_assignable<key_equal>::value &&
                        segment_allocator_traits::is_always_equal::value;
                    static constexpr bool is_noexcept_swap =
                        tbb::detail::is_nothrow_swappable<hasher>::value &&
                        tbb::detail::is_nothrow_swappable<key_equal>::value &&
                        segment_allocator_traits::is_always_equal::value;

                    // TODO: using base_type::base_type is not compiling on Windows and Intel
                    // Compiler - investigate
                    unordered_segment_table(
                    ...
                    unordered_segment_table(const unordered_segment_table& other,
                    ...
                    unordered_segment_table(unordered_segment_table&&  other,
                                            const base_allocator_type& alloc)
                    ...
                    unordered_segment_table&
                    operator=(const unordered_segment_table&) = default;

                    unordered_segment_table& operator=(unordered_segment_table&&) = default;

                    segment_type
                    create_segment(typename base_type::segment_table_type,
                                   typename base_type::segment_index_type segment_index,
                                   size_type)
                    {
                        ...
                        for (size_type i = 0; i != seg_size; ++i) {
                            segment_allocator_traits::construct(alloc, new_segment + i,
                                                                nullptr);
                        }
                        return new_segment;
                    }
                    ...
                    // in unordered, it is also necessary to call the destructor during
                    // deallocation
                    ...
                    void destroy_segment(segment_type address, size_type index)
                    {
                        ...
                        segment_allocator_traits::deallocate(alloc, address,
                                                             this->segment_size(index));
                    }

                    void copy_segment(size_type index, segment_type, segment_type to)
                    {
                        if (index == 0) {
                            // The first element in the first segment is embedded into the table
                            // (my_head) so the first pointer should not be stored here It would
                            // be stored during move ctor/assignment operation
                    ...
                    // allocate_long_table is required by the segment_table base class, but
                    // unused for unordered containers
                    typename base_type::segment_table_type
                    allocate_long_table(const typename base_type::atomic_segment*, size_type)
                    {
                    ...
                    // destroy_elements is required by the segment_table base class, but unused
                    // for unordered containers this function call but do nothing
                    void destroy_elements() {}
                };  // struct unordered_segment_table

                void internal_clear()
                {
                    // TODO: consider usefulness of two versions of clear() - with dummy nodes
                    // deallocation and without it
...
// GCC 11.1 issues a warning here that incorrect destructor might be called for dummy_nodes
    ...
    !__INTEL_COMPILER
                ...
                struct internal_insert_return_type {
                    // If the insertion failed - the remaining_node points to the node, which
                    // was failed to insert This node can be allocated in process of insertion
                    value_node_ptr remaining_node;
                    // If the insertion failed - node_with_equal_key points to the node in the
                    // list with the key, equivalent to the inserted, otherwise it points to the
                ...
                std::pair<iterator, bool> internal_insert_value(ValueType&& value)
                {
                    ...
                    if (insert_result.remaining_node != nullptr) {
                        // If the insertion fails - destroy the node which was failed to insert
                        // if it exist
                ...
                // Creates a node using the specified callback after the place for insertion was
                // found Returns internal_insert_return_type object, where:
                //     - If the insertion succeeded:
                //         - remaining_node is nullptr
                //         - node_with_equal_key point to the inserted node
                //         - inserted is true
                //     - If the insertion failed:
                //         - remaining_node points to the node, that was failed to insert if it
                //         was created.
                //           nullptr if the node was not created, because the requested key was
                //           already presented in the list
                //         - node_with_equal_key point to the element in the list with the key,
                //         equivalent to
                //           to the requested key
                //         - inserted is false
                template <typename ValueType, typename CreateInsertNode>
                internal_insert_return_type internal_insert(ValueType&&      value,
                                                            CreateInsertNode create_insert_node)
                {
                    static_assert(
                        std::is_same<typename std::decay<ValueType>::type, value_type>::value,
                    ...
                    while (!try_insert(prev, new_node, curr)) {
                        ...
                        if (search_result.second) {
                            return internal_insert_return_type{new_node, search_result.first,
                                                               false};
                        }
                    ...
                    return internal_insert_return_type{
                        nullptr, static_cast<value_node_ptr>(new_node), true};
                }

                // Searches the node with the key, equivalent to key with requested order key
                // after the node prev Returns the existing node and true if the node is already
                // in the list Returns the first node with the order key, greater than requested
                // and false if the node is not presented in the list
                std::pair<value_node_ptr, bool>
                search_after(node_ptr& prev, sokey_type order_key, const key_type& key)
                {
                    // NOTE: static_cast<value_node_ptr>(curr) should be done only after we
                    // would ensure that the node is not a dummy node

                    node_ptr curr = prev->next();

                    while (curr != nullptr &&
                           (curr->order_key() < order_key ||
                            (curr->order_key() == order_key &&
                             !my_hash_compare(traits_type::get_key(
                    ...
                    if (curr != nullptr && curr->order_key() == order_key &&
                        !allow_multimapping) {
                ...
                void adjust_table_size(size_type total_elements, size_type current_size)
                {
                    ...
                    if ((float(total_elements) / float(current_size)) > my_max_load_factor) {
                        // Double the size of the hash only if size hash not changed in between
                        // loads
                        my_bucket_count.compare_exchange_strong(current_size,
                                                                2u * current_size);
                    }
                }
                ...
                node_ptr insert_dummy_node(node_ptr parent_dummy_node, sokey_type order_key)
                {
                    ...
                    do {
                        ...
                        // Move forward through the list while the order key is less than
                        // requested
                        ...
                        if (next_node != nullptr && next_node->order_key() == order_key) {
                            // Another dummy node with the same order key was inserted by
                            // another thread Destroy the node and exit
                ...
                static bool try_insert(node_ptr prev_node, node_ptr new_node,
                ...
                {
                    size_type bucket =
                        hash_key % my_bucket_count.load(std::memory_order_acquire);
                ...
                void init_bucket(size_type bucket)
                {
                    ...
                    while (my_segments[parent_bucket].load(std::memory_order_acquire) ==
                           nullptr) {
                        // Initialize all of the parent buckets
                        init_bucket(parent_bucket);
                    }

                    __TBB_ASSERT(my_segments[parent_bucket].load(std::memory_order_acquire) !=
                                     nullptr,
                                 "Parent bucket should be initialized");
                    node_ptr parent =
                        my_segments[parent_bucket].load(std::memory_order_acquire);

                    // Insert dummy node into the list
                    node_ptr dummy_node =
                        insert_dummy_node(parent, split_order_key_dummy(bucket));
                    // TODO: consider returning pair<node_ptr, bool> to avoid store operation if
                    // the bucket was stored by an other thread or move store to
                    // insert_dummy_node Add dummy_node into the segment table
                ...
                value_node_ptr create_node(sokey_type order_key, Args&&... args)
                {
                    ...
                    value_node_allocator_traits::construct(
                        value_node_allocator, new_node->storage(), std::forward<Args>(args)...);
                    value_guard.dismiss();
                    return new_node;
                }
                ...
                {
                    // TODO: consider reimplementation without equal_range - it is not effective
                    // to perform lookup over a bucket for each unsafe_erase call
                ...
                void internal_extract(value_node_ptr node_to_extract)
                {
                    ...
                    for (node_ptr node = prev_node->next(); node != nullptr;
                        ...
                        if (node == node_to_extract) {
                            ...
                            my_size.store(my_size.load(std::memory_order_relaxed) - 1,
                                          std::memory_order_relaxed);
                            return;
                        }
                ...
                void internal_merge(SourceType&& source)
                {
                    static_assert(
                        std::is_same<node_type,
                                     typename std::decay<SourceType>::type::node_type>::value,
                        "Incompatible containers cannot be merged");

                    for (node_ptr source_prev = &source.my_head;
                         source_prev->next() != nullptr;) {
                        if (!source_prev->next()->is_dummy()) {
                            value_node_ptr curr =
                                static_cast<value_node_ptr>(source_prev->next());
                            ...
                            if (allow_multimapping ||
                                !contains(traits_type::get_key(curr->value()))) {
                                ...
                                // Node handle with curr cannot be used directly in insert call,
                                // because the destructor of node_type will destroy curr
                                node_type curr_node =
                                    node_handle_accessor::construct<node_type>(curr);

                                // If the insertion fails - return ownership of the node to the
                                // source
                                if (!insert(std::move(curr_node)).second) {
                                    ...
                                    __TBB_ASSERT(source_prev->next() == next_node,
                                                 "Concurrent operations with the source "
                                                 "container in merge are prohibited");

                                    // Initialize the node with the old order key, because the
                                    // order key can change during the insertion
                                    curr->init(old_order_key);
                                    __TBB_ASSERT(old_order_key >= source_prev->order_key() &&
                                                     (next_node == nullptr ||
                                                      old_order_key <= next_node->order_key()),
                                                 "Wrong nodes order in the source container");
                                    // Merge is unsafe for source container, so the insertion
                                    // back can be done without compare_exchange
            ...
            private:
                ...
                void unlink_node(node_ptr prev_node, node_ptr node_to_unlink,
                                 node_ptr next_node)
                {
                            ...
                            return nullptr;
                                   ...
                                   my_hash_compare(
                                       traits_type::get_key(
                                           static_cast<value_node_ptr>(curr)->value()),
                                       key)) {
                            // The fact that order keys match does not mean that the element is
                            // found. Key function comparison has to be performed to Assert
                            // whether this is the right element. If not, keep searching while
                            // order key is the same.
                    ...
                    while (curr != nullptr) {
                        if (curr->order_key() > order_key) {
                            ...
                            do {
                                ...
                                allow_multimapping && last != nullptr && !last->is_dummy() &&
                                my_hash_compare(traits_type::get_key(
                ...
                {
                    if (allow_multimapping) {
                        // TODO: consider reimplementing the internal_equal_range with elements
                        // counting to avoid std::distance
                        auto eq_range = equal_range(key);
                ...
                void internal_copy(const concurrent_unordered_base& other)
                {
                    ...
                    for (node_ptr node = other.my_head.next(); node != nullptr;
                         node = node->next()) {
                        node_ptr new_node;
                        if (!node->is_dummy()) {
                            // The node in the right table contains a value
                            new_node = create_node(node->order_key(),
                            ...
                            my_segments[reverse_bits(node->order_key())].store(
                ...
                void move_content(concurrent_unordered_base&& other)
                {
                    ...
                    other.my_bucket_count.store(initial_bucket_count,
                                                std::memory_order_relaxed);
                ...
                void
                internal_move_construct_with_allocator(concurrent_unordered_base&& other,
                                                       ...
                                                       /*is_always_equal = */ std::false_type)
                {
                    ...
                    if (alloc == other.my_segments.get_allocator()) {
                        ...
                        try_call([&] { internal_move(std::move(other)); }).on_exception([&] {
                            clear();
                        });
                    }
                }

                // Move assigns the hash table to other is any instances of allocator_type are
                // always equal or propagate_on_container_move_assignment is true
                void internal_move_assign(concurrent_unordered_base&& other,
                ...
                // Move assigns the hash table to other is any instances of allocator_type are
                // not always equal and propagate_on_container_move_assignment is false
                void internal_move_assign(concurrent_unordered_base&& other,
                ...
                void internal_swap(concurrent_unordered_base& other,
                ...
                {
                    __TBB_ASSERT(my_segments.get_allocator() ==
                                     other.my_segments.get_allocator(),
                ...
                void internal_swap_fields(concurrent_unordered_base& other)
                {
                    ...
                    // swap() method from segment table swaps all of the segments including the
                    // first segment We should restore it to my_head. Without it the first
                    // segment of the container will point to other.my_head.
                    my_segments[0].store(&my_head, std::memory_order_relaxed);
                ...
                {
                    size_type bits =
                        tbb::detail::log2(my_bucket_count.load(std::memory_order_relaxed));
