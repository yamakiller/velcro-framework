...
namespace tbb {
    namespace detail {
        inline namespace d0 {
            ...
            struct is_always_equal_detector<
                Allocator, tbb::detail::void_t<typename Allocator::is_always_equal>> {
                using type = typename Allocator::is_always_equal;
            };
#endif  // !__TBB_CPP17_ALLOCATOR_IS_ALWAYS_EQUAL_PRESENT

            template <typename Allocator>
            class allocator_traits_base : public std::allocator_traits_base<Allocator> {
                using base_type = std::allocator_traits_base<Allocator>;
            ...
            public:
                ...
                template <typename T>
                using rebind_traits = typename tbb::detail::allocator_traits_base<
                    typename base_type::template rebind_alloc<T>>;
            ...
            template <typename Allocator>
            void copy_assign_allocators_impl(Allocator&, const Allocator&,
                                             /*pocca = */ std::false_type)
            {
            }
            ...
            {
                using pocca_type = typename allocator_traits_base<
                    Allocator>::propagate_on_container_copy_assignment;
                copy_assign_allocators_impl(lhs, rhs, pocca_type());
            }

            template <typename Allocator>
            void move_assign_allocators_impl(Allocator& lhs, Allocator& rhs,
                                             /*pocma = */ std::true_type)
            {
                lhs = std::move(rhs);
            }
            ...
            template <typename Allocator>
            void move_assign_allocators_impl(Allocator&, Allocator&,
                                             /*pocma = */ std::false_type)
            {
            }
            ...
            {
                using pocma_type = typename allocator_traits_base<
                    Allocator>::propagate_on_container_move_assignment;
                move_assign_allocators_impl(lhs, rhs, pocma_type());
            }

            template <typename Allocator>
            void swap_allocators_impl(Allocator& lhs, Allocator& rhs,
                                      /*pocs = */ std::true_type)
            {
                using std::swap;
                swap(lhs, rhs);
            ...
            {
                using pocs_type =
                    typename allocator_traits_base<Allocator>::propagate_on_container_swap;
