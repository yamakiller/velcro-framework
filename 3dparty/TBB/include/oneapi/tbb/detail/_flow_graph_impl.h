    ...
    namespace detail {
        ...
        namespace d1 {
            ...
            public:
                ...
                {
                    return ((my_graph == other.my_graph) &&
                            (current_node == other.current_node));
                }
            ...
            enum reset_flags {
                rf_reset_protocol = 0,
                rf_reset_bodies = 1 << 0,  // delete the current node body, reset to a copy of
                                           // the initial node body.
            ...
            typedef tbb::concurrent_priority_queue<graph_task*, graph_task_comparator>
            ...
            public:
                ...
                /** The graph will not return from wait_for_all until a matching number of
                release_wait calls is made. */
                void reserve_wait() override;

                //! Deregisters an external entity that may have interacted with the graph.
                /** The graph will not return from wait_for_all until all the number of
                reserve_wait calls matches the number of release_wait calls. */
                void release_wait() override;

                //! Wait until graph is idle and the number of release_wait calls equals to the
                //! number of reserve_wait calls.
                /** The waiting thread will go off and steal work while it is blocked in the
                 * wait_for_all. */
                void wait_for_all()
                {
                    cancelled = false;
                    ...
                    try_call([this] {
                        my_task_arena->execute([this] { wait(my_wait_context, *my_context); });
                    ...
                    // TODO: the "if" condition below is just a work-around to support the
                    // concurrent wait mode. The cancellation and exception mechanisms are still
                    // broken in this mode. Consider using task group not to re-implement the
                    // same functionality.
            ...
            protected:
                ...
                {
                    // TODO revamp: propagate graph_reference() method to all the reference
                    // places.
                    return my_graph;
                }
            ...
            inline graph_task* prioritize_task(graph& g, graph_task& gt)
            {
                ...
                //! Non-preemptive priority pattern. The original task is submitted as a work
                //! item to the priority queue, and a new critical task is created to take and
                //! execute a work item with the highest known priority. The reference counting
                //! responsibility is transferred (via allocate_continuation) to the new task.
                task* critical_task = gt.my_allocator.new_object<priority_task_selector>(
                    g.my_priority_queue, gt.my_allocator);
            ...
            {
                if (is_graph_active(g)) {
                    __TBB_ASSERT(g.my_task_arena && g.my_task_arena->is_active(),
                                 "Is graph's arena initialized and active?");
