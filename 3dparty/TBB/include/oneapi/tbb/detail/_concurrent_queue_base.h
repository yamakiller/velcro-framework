...
#include "TBB/include/oneapi/tbb/profiling.h"
#include "TBB/include/oneapi/tbb/spin_mutex.h"
#include "TBB/include/oneapi/tbb/cache_aligned_allocator.h"
...
namespace tbb {
    ...
    #pragma warning(disable : 4146)
            ...
            public:
                ...
                using page_allocator_type =
                    typename allocator_traits_type::template rebind_alloc<padded_page>;

            protected:
                using page_allocator_traits =
                    tbb::detail::allocator_traits_base<page_allocator_type>;
            ...
            public:
                ...
                size_type prepare_page(ticket_type k, queue_rep_type& base,
                                       page_allocator_type page_allocator, padded_page*& p)
                {
                    __TBB_ASSERT(p == nullptr, "Invalid page argument for prepare_page");
                    k &= -queue_rep_type::n_queue;
                    size_type index =
                        modulo_power_of_two(k / queue_rep_type::n_queue, items_per_page);
                    if (!index) {
                        try_call([&] {
                    ...
                    if (tail_counter.load(std::memory_order_relaxed) != k)
                ...
                void push(ticket_type k, queue_rep_type& base, queue_allocator_type& allocator,
                          Args&&... args)
                {
                         ...
                         queue_allocator_type& allocator)
                {
                    ...
                    bool success = false;
                    {
                        ...
                        micro_queue_pop_finalizer<self_type, value_type, page_allocator_type>
                            finalizer(*this, page_allocator, k + queue_rep_type::n_queue,
                                    ...
                                    item_constructor_type construct_item)
                {
                    ...
                    if (is_valid_page(srcp)) {
                        ...
                        size_type index =
                            modulo_power_of_two(head_counter.load(std::memory_order_relaxed) /
                                                    queue_rep_type::n_queue,
                                                items_per_page);
                        size_type end_in_first_page = (index + n_items < items_per_page)
                                                          ? (index + n_items)
                                                          : items_per_page;

                        try_call([&] {
                            head_page.store(make_copy(allocator, srcp, index, end_in_first_page,
                        ...
                        try_call([&] {
                            if (srcp != src.tail_page.load(std::memory_order_relaxed)) {
                                for (srcp = srcp->next;
                                     srcp != src.tail_page.load(std::memory_order_relaxed);
                                     srcp = srcp->next) {
                                    cur_page->next =
                                        make_copy(allocator, srcp, 0, items_per_page, g_index,
                                                  construct_item);
                                    cur_page = cur_page->next;
                                }

                                __TBB_ASSERT(srcp ==
                                                 src.tail_page.load(std::memory_order_relaxed),
                                             nullptr);
                                size_type last_index = modulo_power_of_two(
                                    ...
                                    items_per_page);
                                if (last_index == 0)
                                    last_index = items_per_page;

                                cur_page->next = make_copy(allocator, srcp, 0, last_index,
                                                           g_index, construct_item);
                                cur_page = cur_page->next;
                            }
                ...
                padded_page* make_copy(queue_allocator_type& allocator,
                                       const padded_page* src_page, size_type begin_in_page,
                ...
                void invalidate_page(ticket_type k)
                {
                    ...
                    {
                        spin_mutex::scoped_lock lock(page_mutex);
                        tail_counter.store(k + queue_rep_type::n_queue + 1,
                                           std::memory_order_relaxed);
                        padded_page* q = tail_page.load(std::memory_order_relaxed);
                        if (is_valid_page(q)) {
                            q->next = invalid_page;
                ...
                void clear(queue_allocator_type& allocator)
                {
                    ...
                    size_t              index = head_counter.load(std::memory_order_relaxed);
                    page_allocator_type page_allocator(allocator);
            ...
            private:
                ...
                friend class micro_queue_pop_finalizer<self_type, value_type,
                                                       page_allocator_type>;
                ...
                void copy_item(padded_page& dst, size_type dindex, const padded_page& src,
                ...
                void spin_wait_until_my_turn(std::atomic<ticket_type>& counter, ticket_type k,
    ...
    #pragma warning(pop)
            ...
            public:
                ...
                micro_queue_pop_finalizer(Container& queue, Allocator& alloc, ticket_type k,
                                          padded_page* p)
                ...
                ~micro_queue_pop_finalizer()
                {
                    ...
                    if (is_valid_page(p)) {
                        ...
                        allocator_traits_type::deallocate(allocator,
                                                          static_cast<padded_page*>(p), 1);
                    }
                }
    ...
    #pragma warning(disable : 4324)
            ...
            public:
                ...
                void clear(queue_allocator_type& alloc)
                {
                    ...
                    for (size_type i = 0; i < n_queue; ++i) {
                        ...
                        if (is_valid_page(tail_page)) {
                            __TBB_ASSERT(array[i].get_head_page() == tail_page,
                                         "at most one page should remain");
                            page_allocator_traits::destroy(
                                page_allocator, static_cast<padded_page*>(tail_page));
                            page_allocator_traits::deallocate(
                                page_allocator, static_cast<padded_page*>(tail_page), 1);
                            array[i].set_tail_page(nullptr);
                        } else {
                            __TBB_ASSERT(!is_valid_page(array[i].get_head_page()),
                            ...
                            item_constructor_type construct_item)
                {
                    ...
                    tail_counter.store(src.tail_counter.load(std::memory_order_relaxed),
                                       std::memory_order_relaxed);
                    n_invalid_entries.store(
                        src.n_invalid_entries.load(std::memory_order_relaxed),
                        std::memory_order_relaxed);
                    ...
                    try_call([&] {
                        for (; queue_idx < n_queue; ++queue_idx) {
                            array[queue_idx].assign(src.array[queue_idx], alloc,
                                                    construct_item);
                        }
                    }).on_exception([&] {
                ...
                bool empty() const
                {
                    ...
                    // if tc!=r.tail_counter, the queue was not empty at some point between the
                    // two reads.
                    return tc == tail_counter.load(std::memory_order_relaxed) &&
                           std::ptrdiff_t(tc - hc -
                                          n_invalid_entries.load(std::memory_order_relaxed)) <=
                               0;
                }
    ...
    #pragma warning(pop)
            ...
            protected:
                ...
                concurrent_queue_iterator_base(queue_rep_type* queue_rep)
                    : my_queue_rep(queue_rep),
                      my_head_counter(
                          my_queue_rep->head_counter.load(std::memory_order_relaxed))
                {
                    for (size_t i = 0; i < queue_rep_type::n_queue; ++i) {
                        my_array[i] = my_queue_rep->array[i].get_head_page();
                    }

                    if (!get_item(my_item, my_head_counter))
                        advance();
                }
                ...
                void advance()
                {
                    __TBB_ASSERT(my_item,
                                 "Attempt to increment iterator past end of the queue");
                    size_t k = my_head_counter;
#if TBB_USE_ASSERT
                    Value* tmp;
                    get_item(tmp, k);
                    __TBB_ASSERT(my_item == tmp, nullptr);
#endif
                    size_t i = modulo_power_of_two(k / queue_rep_type::n_queue,
                                                   my_queue_rep->items_per_page);
                    ...
                    // Advance k
                    my_head_counter = ++k;
                    if (!get_item(my_item, k))
                        advance();
                }

                concurrent_queue_iterator_base&
                operator=(const concurrent_queue_iterator_base& other)
                {
                    this->assign(other);
                    return *this;
                }

                bool get_item(Value*& item, size_t k)
                {
            ...
            class concurrent_queue_iterator
                : public concurrent_queue_iterator_base<typename std::remove_cv<Value>::type,
                                                        Allocator> {
                using base_type =
                    concurrent_queue_iterator_base<typename std::remove_cv<Value>::type,
                                                   Allocator>;
            ...
            public:
                ...
                /** If Value==Container::value_type, then this routine is the copy constructor.
                    If Value==const Container::value_type, then this routine is a conversion
                   constructor. */
                concurrent_queue_iterator(
                    const concurrent_queue_iterator<Container, typename Container::value_type,
                                                    Allocator>& other)
                    : base_type(other)
                {
                }
            ...
            public:
                ...
                friend bool operator==(const concurrent_queue_iterator& lhs,
                                       const concurrent_queue_iterator& rhs)
                {
