...
protected:
    ...
    typedef typename allocator_traits_base<A>::template rebind_alloc<buffer_item_type>
                           allocator_type;
    ...
    {
        __TBB_ASSERT(!(size_type(&(my_array[i & (my_array_size - 1)].begin()->second)) %
                       alignment_of<buffer_item_state>::value),
                     NULL);
        __TBB_ASSERT(!(size_type(&(my_array[i & (my_array_size - 1)].begin()->first)) %
                       alignment_of<item_type>::value),
                     NULL);
    ...
    void grow_my_array(size_t minimum_size)
    {
        ...
        for (size_type i = 0; i < new_size; ++i) {
            new_array[i].begin()->second = no_item;
        }
...
public:
    //! Constructor
    item_buffer() : my_array(NULL), my_array_size(0), my_head(0), my_tail(0)
    {
...
protected:
    ...
    {
        if (my_reserved || !my_item_valid(this->my_head))
            return false;
        my_reserved = true;
        // reserving the head
        v = this->front();
