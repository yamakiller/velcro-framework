...
#include "_assert.h"
#include "_utils.h"
#include "TBB/include/oneapi/tbb/spin_rw_mutex.h"
...
namespace tbb {
    namespace detail {
        ...
        namespace d1 {
            ...
            constexpr size_t speculation_granularity = 64;
            ...
            public:
                ...
                public:
                    ...
                    constexpr scoped_lock()
                        : m_mutex(nullptr), m_transaction_state(rtm_type::rtm_not_in_mutex)
                    {
                    }

                    //! Acquire lock on given mutex.
                    scoped_lock(rtm_rw_mutex& m, bool write = true)
                        : m_mutex(nullptr), m_transaction_state(rtm_type::rtm_not_in_mutex)
                    {
                        acquire(m, write);
                    }
                    ...
                    /** Returns whether the upgrade happened without releasing and re-acquiring
                     * the lock */
    ...
    #pragma warning(pop)  // 4324 warning
        ...
        namespace r1 {
            //! Detail acquire write lock.
            // only_speculate == true if we're doing a try_lock, else false.
            TBB_EXPORT void __TBB_EXPORTED_FUNC acquire_writer(d1::rtm_rw_mutex&,
                                                               d1::rtm_rw_mutex::scoped_lock&,
                                                               bool only_speculate = false);
            //! Detail acquire read lock.
            // only_speculate == true if we're doing a try_lock, else false.
            TBB_EXPORT void __TBB_EXPORTED_FUNC acquire_reader(d1::rtm_rw_mutex&,
                                                               d1::rtm_rw_mutex::scoped_lock&,
                                                               bool only_speculate = false);
            //! Detail upgrade reader to become a writer.
            TBB_EXPORT bool __TBB_EXPORTED_FUNC upgrade(d1::rtm_rw_mutex::scoped_lock&);
            //! Detail downgrade writer to become a reader.
            TBB_EXPORT bool __TBB_EXPORTED_FUNC downgrade(d1::rtm_rw_mutex::scoped_lock&);
            //! Detail try_acquire write lock.
            TBB_EXPORT bool __TBB_EXPORTED_FUNC
            try_acquire_writer(d1::rtm_rw_mutex&, d1::rtm_rw_mutex::scoped_lock&);
            //! Detail try_acquire read lock.
            TBB_EXPORT bool __TBB_EXPORTED_FUNC
            try_acquire_reader(d1::rtm_rw_mutex&, d1::rtm_rw_mutex::scoped_lock&);
            //! Detail release lock.
        ...
        namespace d1 {
            ...
            void rtm_rw_mutex::scoped_lock::release()
            {
                ...
                __TBB_ASSERT(m_transaction_state != rtm_type::rtm_not_in_mutex,
            ...
            {
                __TBB_ASSERT(m_mutex, "lock is not acquired");
                if (m_transaction_state == rtm_type::rtm_transacting_writer ||
                    m_transaction_state == rtm_type::rtm_real_writer) {
            ...
            bool rtm_rw_mutex::scoped_lock::downgrade_to_reader()
            {
                ...
                if (m_transaction_state == rtm_type::rtm_transacting_reader ||
                    m_transaction_state == rtm_type::rtm_real_reader) {
            ...
            {
                __TBB_ASSERT(m_mutex, "lock is not acquired");
                return m_transaction_state == rtm_type::rtm_transacting_writer ||
                       m_transaction_state == rtm_type::rtm_real_writer;
            }
