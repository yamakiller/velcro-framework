...
// Included in namespace tbb::detail::d1 (in flow_graph.h)
    ...
    // Visual Studio 2019 reports an error while calling predecessor_selector::get and
    // successor_selector::get Seems like the well-formed expression in trailing decltype is
    // treated as ill-formed
    // TODO: investigate problems with decltype in trailing return types or find the
...
public:
    // TODO: consider making graph_reference() public and consistent interface to get a
    // reference to the graph and remove get_graph_helper
    template <typename T>
...
follows(FirstPredecessor& first_predecessor, Predecessors&... predecessors)
{
    ...
    return node_set<order::following, FirstPredecessor, Predecessors...>(first_predecessor,
                                                                         predecessors...);
}
...
{
    static_assert(
        (conjunction<has_input_type<FirstSuccessor>, has_input_type<Successors>...>::value),
    ...
    return node_set<order::preceding, FirstSuccessor, Successors...>(first_successor,
                                                                     successors...);
}
...
public:
    ...
    #else
    static auto get(NodeType& node)
        -> decltype(get_impl(node, has_input_ports_type<NodeType>()))
    #endif
    {
...
{
    const size_t SetSize = std::tuple_size<decltype(s.nodes)>::value;
