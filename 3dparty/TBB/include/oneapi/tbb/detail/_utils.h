...
namespace tbb {
    namespace detail {
        inline namespace d0 {
            ...
            template <typename... T>
            void suppress_unused_warning(T&&...)
            {
            }
            ...
            constexpr size_t max_nfs_size_exp = 7;
            static_assert(1 << max_nfs_size_exp == max_nfs_size,
                          "max_nfs_size_exp must be a log2(max_nfs_size)");
            ...
            T spin_wait_while_eq(const std::atomic<T>& location, const U value,
            ...
            /** T and U should be comparable types. */
            template <typename T, typename U>
            T spin_wait_until_eq(const std::atomic<T>& location, const U value,
                                 std::memory_order order = std::memory_order_acquire)
            {
                return spin_wait_while(
                    location, [&value](T t) { return t != value; }, order);
            ...
            constexpr bool is_power_of_two_at_least(ArgIntegerType     arg,
                                                    DivisorIntegerType divisor)
            {
                // Divisor should be a power of two
                static_assert(
                    std::is_integral<ArgIntegerType>::value,
            ...
            template <typename ArgIntegerType, typename DivisorIntegerType>
            inline ArgIntegerType modulo_power_of_two(ArgIntegerType     arg,
                                                      DivisorIntegerType divisor)
            {
            ...
            //! A function to Assert if passed in pointer is aligned on a specific border
            template <typename T>
            ...
            {
                p = reinterpret_cast<T*>(poisoned_ptr);
            }

            template <typename T>
            inline void poison_pointer(std::atomic<T*>& p)
            {
                p.store(reinterpret_cast<T*>(poisoned_ptr), std::memory_order_relaxed);
            }

            /** Expected to be used in assertions only, thus no empty form is defined. **/
            template <typename T>
            inline bool is_poisoned(T* p)
            {
                return p == reinterpret_cast<T*>(poisoned_ptr);
            }

            template <typename T>
            inline bool is_poisoned(const std::atomic<T*>& p)
            {
                return is_poisoned(p.load(std::memory_order_relaxed));
            }
#else
            template <typename T>
            ...
            template <size_t alignment = 0, typename T>
            bool assert_pointer_valid(T* p, const char* comment = nullptr)
            {
                ...
                __TBB_ASSERT(is_aligned(p, alignment == 0 ? Std::alignof(T) : alignment),
                             comment);
#endif
            ...
            //! m_base class for types that should not be assigned.
            class no_assign {
            public:
            ...
            //! m_base class for types that should not be copied or assigned.
            ...
            /** /param initializer Pointer to function without arguments
                       The variant that returns bool is used for cases when initialization can
               fail and it is OK to continue execution, but the state should be reset so that
            ...
            void atomic_do_once(const F& initializer, std::atomic<do_once_state>& state)
            {
                ...
                while (state.load(std::memory_order_acquire) != do_once_state::executed) {
                    if (state.load(std::memory_order_relaxed) == do_once_state::uninitialized) {
                        ...
                        if (((std::atomic<enum_type>&)state)
                                .compare_exchange_strong((enum_type&)expected_state,
                                                         (enum_type)do_once_state::pending)) {
#else
                        if (state.compare_exchange_strong(expected_state,
                                                          do_once_state::pending)) {
#endif
            ...
            concept boolean_testable =
                boolean_testable_impl<T> && requires(T&& t) {
                                                {
                                                    !std::forward<T>(t)
            ...
            using synthesized_three_way_result = decltype(synthesized_three_way_comparator{}(
