...
//! A node_cache maintains a std::queue of elements of type T.  Each operation is protected by a
//! lock.
...
public:
    ...
    {
        typename mutex_type::scoped_lock l(my_mutex, true);
        for (successors_type::iterator i = my_successors.begin(); i != my_successors.end();
             ++i) {
            if (*i == &r) {
                __TBB_ASSERT(my_owner, "Cache of successors must have an owner.");
                // TODO: Assert if we need to test for continue_receiver before removing from r.
