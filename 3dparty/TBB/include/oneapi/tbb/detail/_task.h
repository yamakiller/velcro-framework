...
namespace tbb {
    namespace detail {
        ...
        namespace r1 {
            ...
            TBB_EXPORT void __TBB_EXPORTED_FUNC spawn(d1::task& t, d1::task_group_context& ctx,
                                                      d1::slot_id id);
            TBB_EXPORT void __TBB_EXPORTED_FUNC execute_and_wait(d1::task&               t,
                                                                 d1::task_group_context& t_ctx,
                                                                 d1::wait_context&,
                                                                 d1::task_group_context& w_ctx);
            TBB_EXPORT void __TBB_EXPORTED_FUNC wait(d1::wait_context&,
                                                     d1::task_group_context& ctx);
            TBB_EXPORT                          d1::slot_id __TBB_EXPORTED_FUNC
                                                execution_slot(const d1::execution_data*);
            ...
            TBB_EXPORT void __TBB_EXPORTED_FUNC suspend(suspend_callback_type suspend_callback,
                                                        void*                 user_callback);
        ...
        namespace d1 {
            ...
            {
                // Copy user function to a new stack after the context switch to avoid a race
                // when the previous suspend point is resumed while the user_callback is being
                // called.
            ...
            public:
                // Despite the internal reference count is uin64_t we limit the user interface
                // with uint32_t to preserve a part of the internal reference count for special
                // needs.
                wait_context(std::uint32_t ref_count) : m_ref_count{ref_count}
                {
            ...
            inline void execute_and_wait(task& t, task_group_context& t_ctx,
                                         wait_context& wait_ctx, task_group_context& w_ctx)
            {
            ...
            static constexpr size_t task_alignment = 64;

            //! m_base class for user-defined tasks.
