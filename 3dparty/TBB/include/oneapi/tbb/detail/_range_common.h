...
namespace tbb {
    namespace detail {
        inline namespace d0 {
            ...
            //! Type enables transmission of splitting proportion from partitioners to range
            //! objects
            /**
            ...
            struct range_split_object_provider {
                ...
                static split get(PartitionerSplitType&)
                {
                    return split();
                }
            };
            ...
            struct range_split_object_provider<
                Range, typename std::enable_if<std::is_constructible<
                           Range, Range&, proportional_split&>::value>::type> {
                template <typename PartitionerSplitType>
                static PartitionerSplitType& get(PartitionerSplitType& split_obj)
                {
                    return split_obj;
                }
            };
            ...
            concept blocked_range_value =
                std::copyable<Value> && requires(const std::remove_reference_t<Value>& lhs,
                                                 const std::remove_reference_t<Value>& rhs) {
                                            {
                                                lhs < rhs
                                                } -> relaxed_convertible_to<bool>;
                                            {
                                                lhs - rhs
            ...
            template <typename Range>
            concept tbb_range = std::copy_constructible<Range> && splittable<Range> &&
            ...
            concept iterator_satisfies =
                requires(IteratorTag tag) { requires iterator_concept_helper<Iterator>(tag); };

            template <typename Sequence, typename IteratorTag>
            concept container_based_sequence = requires(Sequence& seq) {
                                                   {
                                                       std::begin(seq)
                                                       } -> iterator_satisfies<IteratorTag>;
                                                   {
                                                       std::end(seq)
                                                       } -> iterator_satisfies<IteratorTag>;
                                               };
