...
template <typename Key,  // type of key within ValueType
          typename ValueType,
          typename ValueToKey,   // abstract method that returns "const Key" or "const Key&"
                                 // given ValueType
          typename HashCompare,  // has hash and equal
          typename Allocator =
              tbb::cache_aligned_allocator<typename aligned_pair<ValueType, void*>::type>>
...
public:
    ...
    typedef
        typename std::allocator_traits_base<Allocator>::template rebind_alloc<list_array_type>
            pointer_array_allocator_type;
    typedef typename std::allocator_traits_base<Allocator>::template rebind_alloc<element_type>
...
private:
    ...
    void grow_array()
    {
        ...
        list_array_type    new_free_list = NULL;
        {
            ...
            for (size_t i = 0; i < new_size; ++i)
                new_pointer_array[i] = NULL;
            set_up_free_list(&new_free_list, new_elements_array, my_size);

            for (size_t i = 0; i < my_size; ++i) {
                for (element_type* op = pointer_array[i]; op;
                     op = (element_type*)(op->second)) {
    ...
    void internal_insert_with_key(element_type** p_pointer_array, size_t p_sz,
                                  list_array_type& p_free_list, const value_type& v)
    {
        ...
        element_type* my_elem = p_free_list;
        p_free_list = (element_type*)(p_free_list->second);
    ...
    static void internal_free_buffer(pointer_array_type& pa, list_array_type& el, size_t& sz,
                                     size_t& ne)
    {
        if (pa) {
        ...
        for (element_type* p = pointer_array[h]; p; prev = p, p = (element_type*)(p->second)) {
            ...
            if (this->equal((*my_key)(*vp), k)) {
                ...
                if (prev)
                    ...
                    pointer_array[h] = (element_type*)(p->second);
                p->second = free_list;
                free_list = p;
                --nelements;
                return;
