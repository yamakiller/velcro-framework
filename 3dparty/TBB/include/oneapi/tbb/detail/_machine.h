...
#elif __TBBMALLOC_BUILD && WIN32
            // Use Windows API for yield in tbbmalloc to avoid dependency on C++ runtime with
    ...
    #pragma intrinsic(_mm_mfence)
            ...
            static inline void atomic_fence(std::memory_order order)
            {
                ...
                if (order == std::memory_order_seq_cst || order == std::memory_order_acq_rel ||
                    order == std::memory_order_acquire || order == std::memory_order_release) {
                    _mm_mfence();
                    return;
                }
            ...
            static inline void machine_pause(i32_t delay)
            {
#if __TBB_x86_64 || __TBB_x86_32
                while (delay-- > 0) {
                    _mm_pause();
            ...
            namespace gnu_builtins {
                inline uintptr_t clz(unsigned int x)
                {
                    return __builtin_clz(x);
                }
                inline uintptr_t clz(unsigned long int x)
                {
                    return __builtin_clzl(x);
                }
                inline uintptr_t clz(unsigned long long int x)
                {
                    return __builtin_clzll(x);
                }
            }  // namespace gnu_builtins
#elif defined(_MSC_VER)
    #pragma intrinsic(__TBBW(_BitScanReverse))
            namespace msvc_intrinsics {
                static inline uintptr_t bit_scan_reverse(uintptr_t i)
                {
                    unsigned long j;
                    __TBBW(_BitScanReverse)(&j, i);
                    return j;
                }
            }  // namespace msvc_intrinsics
#endif
            ...
            static inline uintptr_t machine_log2(uintptr_t x)
            {
                ...
                if (sizeof(x) > 4 && (uintptr_t tmp = x >> 32)) {
                    x = tmp;
                    result += 32;
                }
                if (uintptr_t tmp = x >> 16) {
                    x = tmp;
                    result += 16;
                }
                if (uintptr_t tmp = x >> 8) {
                    x = tmp;
                    result += 8;
                }
                if (uintptr_t tmp = x >> 4) {
                    x = tmp;
                    result += 4;
                }
                if (uintptr_t tmp = x >> 2) {
                    x = tmp;
                    result += 2;
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            namespace llvm_builtins {
                inline u8_t builtin_bitreverse(u8_t x)
                {
                    return __builtin_bitreverse8(x);
                }
                inline u16_t builtin_bitreverse(u16_t x)
                {
                    return __builtin_bitreverse16(x);
                }
                inline uint32_t builtin_bitreverse(uint32_t x)
                {
                    return __builtin_bitreverse32(x);
                }
                inline uint64_t builtin_bitreverse(uint64_t x)
                {
    ...
    #define __TBB_CPU_CTL_ENV_PRESENT 1
            struct cpu_ctl_env {
                unsigned int x87cw{};
    #if (__TBB_x86_64)
                // Changing the infinity mode or the floating-point precision is not supported
                // on x64. The attempt causes an assertion. See
                ...
                unsigned int                  mxcsr{};
                static constexpr unsigned int MXCSR_CONTROL_MASK =
                    ~0x3fu; /* all except last six status bits */
    #endif
    ...
    #define __TBB_CPU_CTL_ENV_PRESENT 1
            struct cpu_ctl_env {
                ...
                static const int MXCSR_CONTROL_MASK =
                    ~0x3f; /* all except last six status bits */
...
namespace tbb {
    namespace detail {
        ...
        namespace r1 {
            void* __TBB_EXPORTED_FUNC cache_aligned_allocate(size_t size);
        ...
        namespace d1 {
            ...
            public:
                ...
                // It is possible not to copy memory but just to copy pointers but the following
                // issues should be addressed:
                //   1. The arena lifetime and the context lifetime are independent;
                //   2. The user is allowed to recapture different FPU settings to context so
                //   'current FPU settings' inside dispatch loop may become invalid.
                // But do we really want to improve the fenv implementation? It seems to be
                // better to replace the fenv implementation with a platform specific
                // implementation.
                ...
                bool operator!=(const cpu_ctl_env& ctl) const
                {
                    ...
                    return std::memcmp((void*)my_fenv_ptr, (void*)ctl.my_fenv_ptr,
                                       sizeof(fenv_t));
                }
                void get_env()
                {
