...
// Therefore we use different ways to detect its version.
    ...
    // Its format should match the __TBB_GCC_VERSION above, e.g. 70301 for libstdc++ coming with
    // GCC 7.3.1.
...
// The additional level of indirection is needed to expand macros A and B (not to get the AB
// macro). See [cpp.subst] and [cpp.concat] for more details.
...
#if _M_X64 || _M_ARM64
    #define __TBBW(name) name##64
#else
    #define __TBBW(name) name
#endif
...
/** User controlled TBB features & modes **/
    ...
    2. "_DEBUG defined to something that is evaluated to 0" (including "garbage", as per
    [cpp.cond]) means "no debug";
    ...
    */
        ...
        #define __TBB_IS__DEBUG_EMPTY                                                          \
            (__TBB_IS_MACRO_EMPTY(_DEBUG, IGNORED) == __TBB_MACRO_EMPTY)
...
#if WIN32 || WIN64
    #define __TBB_USEWINAPI 1
#else
    #define __TBB_USE_POSIX 1
#endif

/** Detail TBB features & modes **/

/** __TBB_DYNAMIC_LOAD_ENABLED describes the system possibility to load shared libraries at run
 * time **/
#ifndef __TBB_DYNAMIC_LOAD_ENABLED
    #define __TBB_DYNAMIC_LOAD_ENABLED 1
#endif

/** __TBBWIN8UI_SUPPORT enables support of Windows* Store Apps and limit a possibility to load
    shared libraries at run time only from application container **/
#if defined(WINAPI_FAMILY) && WINAPI_FAMILY == WINAPI_FAMILY_APP
    #define __TBBWIN8UI_SUPPORT 1
#else
    #define __TBBWIN8UI_SUPPORT 0
#endif

/** __TBBWEAK_SYMBOLS_PRESENT denotes that the system supports the weak symbol mechanism **/
#ifndef __TBBWEAK_SYMBOLS_PRESENT
    #define __TBBWEAK_SYMBOLS_PRESENT                                                          \
        (!WIN32 && !__APPLE__ && !__sun && (__TBB_GCC_VERSION >= 40000 || __INTEL_COMPILER))
#endif
...
// TODO: Remove the condition(__INTEL_COMPILER > 2021) from the
// __TBB_CPP17_DEDUCTION_GUIDES_PRESENT macro when this feature start working correctly on this
// compiler.
#if __INTEL_COMPILER && (!_MSC_VER || __INTEL_CXX11_MOVE__)
    #define __TBB_CPP14_VARIABLE_TEMPLATES_PRESENT (__TBB_LANG >= 201402L)
    #define __TBB_CPP17_DEDUCTION_GUIDES_PRESENT                                               \
    ...
    #endif
#elif __GNUC__
    #define __TBB_CPP14_VARIABLE_TEMPLATES_PRESENT                                             \
        (__TBB_LANG >= 201402L && __TBB_GCC_VERSION >= 50000)
...
#elif _MSC_VER
    #define __TBB_CPP14_VARIABLE_TEMPLATES_PRESENT                                             \
        (_MSC_FULL_VER >= 190023918 && (!__INTEL_COMPILER || __INTEL_COMPILER >= 1700))
    #define __TBB_CPP17_DEDUCTION_GUIDES_PRESENT                                               \
        (_MSC_VER >= 1914 && __TBB_LANG >= 201703L &&                                          \
         (!__INTEL_COMPILER || __INTEL_COMPILER > 2021))
    #define __TBB_CPP20_CONCEPTS_PRESENT                                                       \
        (_MSC_VER >= 1923 && __TBB_LANG >= 202002L)  // TODO: INTEL_COMPILER?
#else
    #define __TBB_CPP14_VARIABLE_TEMPLATES_PRESENT (__TBB_LANG >= 201402L)
...
#define __TBB_CPP11_GET_NEW_HANDLER_PRESENT                                                    \
    (_MSC_VER >= 1900 || __TBB_GLIBCXX_VERSION >= 40900 && __GXX_EXPERIMENTAL_CXX0X__ ||       \
     _LIBCPP_VERSION)
// GCC4.8 on RHEL7 does not support std::is_trivially_copyable
#define __TBB_CPP11_TYPE_PROPERTIES_PRESENT                                                    \
    (_LIBCPP_VERSION || _MSC_VER >= 1700 ||                                                    \
     (__TBB_GLIBCXX_VERSION >= 50000 && __GXX_EXPERIMENTAL_CXX0X__))

#define __TBB_CPP17_MEMORY_RESOURCE_PRESENT                                                    \
    (_MSC_VER >= 1913 && (__TBB_LANG > 201402L) ||                                             \
...
#define __TBB_CPP17_IS_SWAPPABLE_PRESENT              (__TBB_LANG >= 201703L)
    ...
    #define __TBB_CPP20_COMPARISONS_PRESENT                                                    \
...
#define __TBB_RESUMABLE_TASKS (!__TBBWIN8UI_SUPPORT && !__ANDROID__ && !__QNXNTO__)

/* This macro marks incomplete code or comments describing ideas which are considered for the
 * future. See also for plain comment with TODO and FIXME marks for small improvement
 * opportunities.
 */
#define __TBB_TODO            0
...
/** Target OS is either iOS* or iOS* simulator **/
    ...
    #define __TBB_IOS 1
        ...
        #define __TBB_MACOS_TARGET_VERSION                                                     \
            (100000 + 10 * (__ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ - 1000))
    #else
    ...
    #define __TBB_GCCWARNING_IGNORED_ATTRIBUTES_PRESENT (__TBB_GCC_VERSION >= 60100)
#endif
...
#define __TBB_CPP17_UNCAUGHT_EXCEPTIONS_PRESENT                                                \
    (_MSC_VER >= 1900 || __GLIBCXX__ && __cpp_lib_uncaught_exceptions ||                       \
     _LIBCPP_VERSION >= 3700 &&                                                                \
         (!__TBB_MACOS_TARGET_VERSION || __TBB_MACOS_TARGET_VERSION >= 101200))

#define __TBB_TSX_INTRINSICS_PRESENT                                                           \
    (__RTM__ || __INTEL_COMPILER || (_MSC_VER >= 1700 && (__TBB_x86_64 || __TBB_x86_32)))

#define __TBBWAITPKG_INTRINSICS_PRESENT                                                        \
    ((__INTEL_COMPILER >= 1900 || __TBB_GCC_VERSION >= 110000 ||                               \
      __TBB_CLANG_VERSION >= 120000) &&                                                        \
     !__ANDROID__)
...
**/
#if (WIN32 || WIN64) && (__TBB_SOURCE_DIRECTLY_INCLUDED || TBB_USE_PREVIEW_BINARY)
    #define __TBB_NO_IMPLICIT_LINKAGE       1
    #define __TBBMALLOC_NO_IMPLICIT_LINKAGE 1
#endif

#if (__TBB_BUILD || __TBBMALLOC_BUILD || __TBBMALLOCPROXY_BUILD || __TBBBIND_BUILD) &&         \
    !defined(__TBB_NO_IMPLICIT_LINKAGE)
    #define __TBB_NO_IMPLICIT_LINKAGE 1
#endif
...
#define __TBB_RECYCLE_TO_ENQUEUE __TBB_BUILD  // keep non-official
...
#if TBB_PREVIEWWAITING_FORWORKERS || __TBB_BUILD
    #define __TBB_SUPPORTSWORKERSWAITING_IN_TERMINATE 1
#endif
    ...
    (WIN32 || WIN64 || __APPLE__ || (__unix__ && !__ANDROID__))
...
// instantiation site, which is too late for suppression of the corresponding messages for
// internal stuff.
#if !defined(__INTEL_COMPILER) &&                                                              \
    (!defined(TBB_SUPPRESS_DEPRECATED_MESSAGES) || (TBB_SUPPRESS_DEPRECATED_MESSAGES == 0))
...
#if (!defined(TBB_SUPPRESS_DEPRECATED_MESSAGES) || (TBB_SUPPRESS_DEPRECATED_MESSAGES == 0)) && \
    !(__TBB_LANG >= 201103L || _MSC_VER >= 1900)
    #pragma message(                                                                           \
        "TBB Warning: Support for C++98/03 is deprecated. Please use the compiler that supports C++11 features at least.")
#endif
...
// Many OS versions (Android 4.0.[0-3] for example) need workaround for dlopen to avoid
// non-recursive loader lock hang Setting the workaround for all compile targets ($APP_PLATFORM)
// below Android 4.4 (android-19)
#if __ANDROID__
