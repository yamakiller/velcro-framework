...
namespace tbb {
    namespace detail {
        namespace d1 {
            ...
            template <typename T, typename Allocator, typename DerivedType,
                      size_t PointersPerEmbeddedTable>
            class segment_table {
            public:
                using value_type = T;
            ...
            protected:
                using segment_table_allocator_traits =
                    tbb::detail::allocator_traits_base<segment_table_allocator_type>;
            ...
            public:
                ...
                segment_table(segment_table&& other)
                    : my_segment_table_allocator(std::move(other.my_segment_table_allocator)),
                      my_segment_table(my_embedded_table),
                      my_first_block{},
                      my_size{},
                      my_segment_table_allocation_failed{}
                ...
                {
                    if (this != &other) {
                        copy_assign_allocators(my_segment_table_allocator,
                                               other.my_segment_table_allocator);
                        internal_transfer(other, copy_segment_body_type{*this});
                    }
                    return *this;
                ...
                segment_table&
                operator=(segment_table&& other) noexcept(derived_type::is_noexcept_assignment)
                {
                    ...
                    if (this != &other) {
                        move_assign_allocators(my_segment_table_allocator,
                                               other.my_segment_table_allocator);
                ...
                void swap(segment_table& other) noexcept(derived_type::is_noexcept_swap)
                {
                    using is_equal_type =
                        typename segment_table_allocator_traits::is_always_equal;
                    using pocs_type =
                    ...
                    if (this != &other) {
                        swap_allocators(my_segment_table_allocator,
                                        other.my_segment_table_allocator);
                        internal_swap(other,
                                      tbb::detail::disjunction<is_equal_type, pocs_type>());
                    }
                }
                ...
                void enable_segment(segment_type& segment, segment_table_type table,
                                    segment_index_type seg_index, size_type index)
                {
                    ...
                    if (new_segment != nullptr) {
                        // Store (new_segment - segment_base) into the segment table to allow
                        // access to the table by index via
                        ...
                        if (!table[seg_index].compare_exchange_strong(
                                disabled_segment, new_segment - segment_base(seg_index))) {
                            // compare_exchange failed => some other thread has already enabled
                            // this segment Deallocate the memory
                            self()->deallocate_segment(new_segment, seg_index);
                        }
                    }

                    segment = table[seg_index].load(std::memory_order_acquire);
                    __TBB_ASSERT(segment != nullptr, "If create_segment returned nullptr, the "
                                                     "element should be stored in the table");
                }
                ...
                void delete_segment(segment_index_type seg_index)
                {
                    ...
                    segment_type segment_to_delete =
                        get_table()[seg_index].exchange(disabled_segment);
                ...
                {
                    // Check for an active table, if it is embedded table - return the number of
                    // embedded segments Otherwise - return the maximum number of segments
                    return table == my_embedded_table ? pointers_per_embedded_table
                ...
                size_type capacity() const noexcept
                {
                    ...
                    for (size_type seg_index = 0; seg_index < num_segments; ++seg_index) {
                        ...
                        if (table[seg_index].load(std::memory_order_relaxed) <=
                            segment_allocation_failure_tag) {
                ...
                void reserve(size_type n)
                {
                    ...
                    segment_index_type start_seg_idx =
                        size == 0 ? 0 : segment_index_of(size - 1) + 1;
                    for (segment_index_type seg_idx = start_seg_idx; segment_base(seg_idx) < n;
                         ++seg_idx) {
                ...
                void clear_table()
                {
                    ...
                    if (current_segment_table != my_embedded_table) {
                        // If the active table is not the embedded one - deallocate the active
                        // table
                        for (size_type i = 0; i != pointers_per_long_table; ++i) {
                            segment_table_allocator_traits::destroy(my_segment_table_allocator,
                                                                    &current_segment_table[i]);
                        }

                        segment_table_allocator_traits::deallocate(my_segment_table_allocator,
                ...
                void extend_table_if_necessary(segment_table_type& table, size_type start_index,
                                               size_type end_index)
                {
                    ...
                    if (table == my_embedded_table && end_index > embedded_table_size) {
                        if (start_index <= embedded_table_size) {
                            try_call([&] {
                                ...
                                // It is possible that the table was extended by the thread that
                                // allocated first_block. In this case it is necessary to
                                // re-read the current table.
                                ...
                                my_segment_table_allocation_failed.store(
                                    true, std::memory_order_relaxed);
                            });
                        } else {
                            atomic_backoff backoff;
                            do {
                                if (my_segment_table_allocation_failed.load(
            ...
            private:
                ...
                void internal_transfer(const segment_table& other,
                                       TransferBody         transfer_segment)
                {
                    ...
                    size_type          end_segment_size =
                        segment_size(other.find_last_allocated_segment(other_table));

                    // If an exception occurred in other, then the size may be greater than the
                    // size of the end segment.
                    size_type other_size =
                        end_segment_size < other.my_size.load(std::memory_order_relaxed)
                            ? other.my_size.load(std::memory_order_relaxed)
                            : end_segment_size;
                    other_size =
                        my_segment_table_allocation_failed ? embedded_table_size : other_size;
                    ...
                    for (segment_index_type i = 0; segment_base(i) < other_size; ++i) {
                        ...
                        if (other_table[i].load(std::memory_order_relaxed) ==
                            ...
                            transfer_segment(
                                i,
                                other.get_table()[i].load(std::memory_order_relaxed) +
                                    segment_base(i),
                                get_table()[i].load(std::memory_order_relaxed) +
                                    segment_base(i));
                        }
                    }
                }
                ...
                void internal_move(segment_table&& other)
                {
                    ...
                    clear();
                    my_first_block.store(other.my_first_block.load(std::memory_order_relaxed),
                                         std::memory_order_relaxed);
                    my_size.store(other.my_size.load(std::memory_order_relaxed),
                                  std::memory_order_relaxed);
                    // If an active table in other is embedded - restore all of the embedded
                    // segments
                    if (other.get_table() == other.my_embedded_table) {
                        for (size_type i = 0; i != pointers_per_embedded_table; ++i) {
                            segment_type other_segment =
                                other.my_embedded_table[i].load(std::memory_order_relaxed);
                            my_embedded_table[i].store(other_segment,
                                                       std::memory_order_relaxed);
                            other.my_embedded_table[i].store(nullptr,
                                                             std::memory_order_relaxed);
                        }
                        my_segment_table.store(my_embedded_table, std::memory_order_relaxed);
                    } else {
                        my_segment_table.store(other.my_segment_table,
                                               std::memory_order_relaxed);
                ...
                void
                internal_move_construct_with_allocator(segment_table&& other,
                                                       ...
                                                       /*is_always_equal = */ std::false_type)
                {
                    if (other.my_segment_table_allocator == alloc) {
                        ...
                        // If allocators are not equal - perform per element move with
                        // reallocation
                        try_call([&] {
                ...
                // Move assigns the segment table to other is any instances of allocator_type
                // are always equal or propagate_on_container_move_assignment is true
                void internal_move_assign(segment_table&& other,
                ...
                // Move assigns the segment table to other is any instances of allocator_type
                // are not always equal and propagate_on_container_move_assignment is false
                void internal_move_assign(segment_table&& other,
                ...
                void internal_swap(segment_table& other,
                                   /*is_always_equal || POCS = */ std::true_type)
                {
                    internal_swap_fields(other);
                }

                // Swaps two segment tables if any instances of allocator_type are not always
                // equal and propagate_on_container_swap is false According to the C++ standard,
                // swapping of two containers with unequal allocators is an undefined behavior
                // scenario
                void internal_swap(segment_table& other,
                ...
                {
                    // If an active table in either *this segment table or other is an embedded
                    // one - swaps the embedded tables
                    if (get_table() == my_embedded_table ||
                        other.get_table() == other.my_embedded_table) {
                        for (size_type i = 0; i != pointers_per_embedded_table; ++i) {
                            segment_type current_segment =
                                my_embedded_table[i].load(std::memory_order_relaxed);
                            segment_type other_segment =
                                other.my_embedded_table[i].load(std::memory_order_relaxed);

                            my_embedded_table[i].store(other_segment,
                                                       std::memory_order_relaxed);
                            other.my_embedded_table[i].store(current_segment,
                                                             std::memory_order_relaxed);
                        }
                    }
                    ...
                    if (current_segment_table == my_embedded_table) {
                        ...
                        other.my_segment_table.store(current_segment_table,
                                                     std::memory_order_relaxed);
                    }
            ...
            protected:
                ...
                const segment_type segment_allocation_failure_tag =
                    reinterpret_cast<segment_type>(1);
                static constexpr size_type embedded_table_size =
                    segment_size(pointers_per_embedded_table);
