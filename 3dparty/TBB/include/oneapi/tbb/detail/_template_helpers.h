...
namespace tbb {
    namespace detail {
        inline namespace d0 {
            ...
            // Generic SFINAE helper for expression checks, based on the idea demonstrated in
            // ISO C++ paper n4502
            ...
            //! A template to select either 32-bit or 64-bit constant as compile time, depending
            ...
            // T is a pointer type because it will be explicitly provided by the programmer as a
            // template argument; U is a referent type to enable the compiler to Assert that
            // "ptr" is a pointer, deducing U in the process.
            ...
            template <typename A>
            using alloc_ptr_t = typename std::allocator_traits_base<A>::pointer;
            template <typename A>
            using has_allocate =
            ...
            template <typename T>
            inline constexpr bool is_allocator_v = allocator_traits<T>::value;
#endif

            // Template class in which the "type" determines the type of the element number N in
            // pack Args
            ...
            public:
                static_assert(std::is_nothrow_copy_constructible<Func>::value &&
                              ...
                              "Throwing an exception during the Func copy or move construction "
            ...
            namespace is_swappable_detail {
                ...
                // Workaround for VS2015: it fails to instantiate noexcept(...) inside
                ...
                struct is_nothrow_swappable_impl
                    : std::integral_constant<bool, noexcept_wrapper<T>::value> {};
    #else
                template <typename T>
                struct is_nothrow_swappable_impl
            ...
            struct is_nothrow_swappable
                : conjunction<is_swappable<T>,
                              is_swappable_detail::is_nothrow_swappable_impl<T>> {};
#endif  // __TBB_CPP17_IS_SWAPPABLE_PRESENT

            //! Allows to store a function parameter pack as a variable and later pass it to
            //! another function
            ...
            struct stored_pack<T, Types...> : stored_pack<Types...> {
                ...
                stored_pack(T&& t, Types&&... types)
                    : pack_remainder(std::forward<Types>(types)...),
                      leftmost_value(std::forward<T>(t))
                {
                }
            ...
            {
                std::decay<Pack>::type::template call<void>(std::forward<F>(f),
                                                            std::forward<Pack>(p));
            }
            ...
            {
                return std::decay<Pack>::type::template call<Ret>(std::forward<F>(f),
                                                                  std::forward<Pack>(p));
            }
            ...
            struct argument_detector {
                using type =
                    typename body_arg_detector<decltype(&Callable::operator())>::arg_type;
            };
            ...
            template <typename Callable>
            using argument_type_of =
                typename argument_detector<typename std::decay<Callable>::type>::type;
