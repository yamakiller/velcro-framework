...
struct indexer_helper {
                                                ...
                                                graph& g)
    {
        ...
        graph_task* (*indexer_node_put_task)(const T&, void*) =
            do_try_put<IndexerNodeBaseType, T, N - 1>;
...
public:
    ...
    // Some versions of Intel(R) C++ Compiler fail to generate an implicit constructor for the
    // class which has std::tuple as a member.
...
//! indexer_node_base
template <typename InputTuple, typename OutputType, typename StructTypes>
class indexer_node_base : public graph_node,
                          public indexer_node_FE<InputTuple, OutputType, StructTypes>,
...
private:
    ...
    public:
        ...
        indexer_node_base_operation(const output_type* e, op_type t) : type(char(t)), my_arg(e)
        {
        }
        indexer_node_base_operation(const successor_type& s, op_type t)
            : type(char(t)), my_succ(const_cast<successor_type*>(&s))
        {
        }
    };
        ...
        while (op_list) {
            ...
            case try__put_task: {
                ...
                current->status.store(
                    SUCCEEDED,
                    std::memory_order_release);  // return of try_put_task actual return value
            } break;
            }
        }
    }
...
template <int N, typename InputTuple>
struct input_types;
...
struct input_types<4, InputTuple> {
    ...
    typedef tagged_msg<size_t, first_type, second_type, third_type, fourth_type> type;
};
...
struct input_types<5, InputTuple> {
    ...
    typedef tagged_msg<size_t, first_type, second_type, third_type, fourth_type, fifth_type>
        type;
};
...
struct input_types<8, InputTuple> {
    ...
    typedef tagged_msg<size_t, first_type, second_type, third_type, fourth_type, fifth_type,
                       sixth_type, seventh_type, eighth_type>
        type;
};
...
struct input_types<9, InputTuple> {
    ...
    typedef tagged_msg<size_t, first_type, second_type, third_type, fourth_type, fifth_type,
                       sixth_type, seventh_type, eighth_type, nineth_type>
        type;
};
...
struct input_types<10, InputTuple> {
    ...
    typedef tagged_msg<size_t, first_type, second_type, third_type, fourth_type, fifth_type,
                       sixth_type, seventh_type, eighth_type, nineth_type, tenth_type>
        type;
};
...
struct indexer_types : public input_types<std::tuple_size<OutputTuple>::value, OutputTuple> {
    ...
    typedef
        typename wrap_tuple_elements<N, indexer_input_port, OutputTuple>::type input_ports_type;
