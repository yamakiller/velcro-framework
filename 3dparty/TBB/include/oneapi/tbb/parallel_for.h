    ...
    namespace detail {
        ...
        inline namespace d0 {
            ...
            concept parallel_for_body = std::copy_constructible<Body> &&
                                        requires(const std::remove_reference_t<Body>& body,
                                                 Range& range) { body(range); };

            template <typename Index>
            concept parallel_for_index =
                std::constructible_from<Index, int> && std::copyable<Index> &&
                requires(const std::remove_reference_t<Index>& lhs,
                         const std::remove_reference_t<Index>& rhs) {
                    {
                        lhs < rhs
                        } -> adaptive_same_as<bool>;
                    {
                        lhs - rhs
            ...
            concept parallel_for_function = requires(
                const std::remove_reference_t<Function>& func, Index index) { func(index); };
        ...
        namespace d1 {
            ...
            struct start_for : public task {
                                ...
                                task_group_context& context)
                {
                    if (!range.empty()) {
                        small_object_allocator alloc{};
                        start_for&             for_task =
                            *alloc.new_object<start_for>(range, body, partitioner, alloc);
            ...
            private:
                ...
                void offer_work_impl(execution_data& ed, Args&&... constructor_args)
                {
                    // New right child
                    small_object_allocator alloc{};
                    start_for&             right_child = *alloc.new_object<start_for>(
                        ed, std::forward<Args>(constructor_args)..., alloc);

                    // New root node as a continuation and ref count. Left and right child
                    // attach to the new parent.
                    right_child.my_parent = my_parent =
                        alloc.new_object<tree_node>(ed, my_parent, 2, alloc);
            ...
            public:
                ...
                {
                    // A set of local variables to help the compiler with vectorization of the
                    // following loop.
                    Index b = r.begin();
                    Index e = r.end();
                    Index ms = my_step;
    ...
    #pragma ivdep
        ...
        #pragma vector always assert
            ...
            /** \page parallel_for_body_req Requirements on parallel_for body
                ...
                - \code void Body::operator()( Range& r ) const; \endcode   Function call
            operator applying the body to range \c r.
            **/

            /** \name parallel_for
                See also requirements on \ref range_req "Range" and \ref parallel_for_body_req
               "parallel_for Body". **/
            //@{
            ...
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                const Range& range, const Body& body)
            {
                start_for<Range, Body, const __TBB_DEFAULT_PARTITIONER>::run(
                    range, body, __TBB_DEFAULT_PARTITIONER());
            }
            ...
            template <typename Range, typename Body>
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                const Range& range, const Body& body, const simple_partitioner& partitioner)
            {
            ...
            template <typename Range, typename Body>
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                const Range& range, const Body& body, const auto_partitioner& partitioner)
            {
            ...
            template <typename Range, typename Body>
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                const Range& range, const Body& body, const static_partitioner& partitioner)
            {
            ...
            template <typename Range, typename Body>
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                const Range& range, const Body& body, affinity_partitioner& partitioner)
            {
                start_for<Range, Body, affinity_partitioner>::run(range, body, partitioner);
            }

            //! Parallel iteration over range with default partitioner and user-supplied
            //! context.
            /** @ingroup algorithms **/
            template <typename Range, typename Body>
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                const Range& range, const Body& body, task_group_context& context)
            {
                start_for<Range, Body, const __TBB_DEFAULT_PARTITIONER>::run(
                    range, body, __TBB_DEFAULT_PARTITIONER(), context);
            }
            ...
            //! Parallel iteration over range with affinity_partitioner and user-supplied
            //! context.
            /** @ingroup algorithms **/
            template <typename Range, typename Body>
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
            ...
            //! Implementation of parallel iteration over stepped range of integers with
            //! explicit step and partitioner
            template <typename Index, typename Function, typename Partitioner>
            void parallel_for_impl(Index first, Index last, Index step, const Function& f,
                                   Partitioner& partitioner)
            {
                if (step <= 0)
                    throw_exception(
                        exception_id::nonpositive_step);  // throws std::invalid_argument
                else if (first < last) {
            ...
            //! Parallel iteration over a range of integers with a step provided and default
            //! partitioner
            template <typename Index, typename Function>
            __TBB_requires(parallel_for_index<Index>&& parallel_for_function<
                           Function, Index>) void parallel_for(Index first, Index last,
                                                               Index step, const Function& f)
            {
                parallel_for_impl<Index, Function, const auto_partitioner>(first, last, step, f,
                                                                           auto_partitioner());
            }
            ...
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
                    Function, Index>) void parallel_for(Index first, Index last, Index step,
                                                        const Function&           f,
                                                        const simple_partitioner& partitioner)
            {
                parallel_for_impl<Index, Function, const simple_partitioner>(first, last, step,
                                                                             f, partitioner);
            }
            ...
            {
                parallel_for_impl<Index, Function, const static_partitioner>(first, last, step,
                                                                             f, partitioner);
            }
            //! Parallel iteration over a range of integers with a step provided and affinity
            //! partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
            ...
            //! Parallel iteration over a range of integers with a default step value and simple
            //! partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
            ...
            //! Parallel iteration over a range of integers with a default step value and auto
            //! partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
            ...
            //! Parallel iteration over a range of integers with a default step value and static
            //! partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
            ...
            //! Implementation of parallel iteration over stepped range of integers with
            //! explicit step, task group context, and partitioner
            template <typename Index, typename Function, typename Partitioner>
            void parallel_for_impl(Index first, Index last, Index step, const Function& f,
            ...
            //! context, and default partitioner
            template <typename Index, typename Function>
            __TBB_requires(parallel_for_index<Index>&& parallel_for_function<
                           Function, Index>) void parallel_for(Index first, Index last,
                                                               Index step, const Function& f,
            ...
            //! Parallel iteration over a range of integers with a default step value, explicit
            //! task group context, and default partitioner
            template <typename Index, typename Function>
            __TBB_requires(parallel_for_index<Index>&& parallel_for_function<
                           Function, Index>) void parallel_for(Index first, Index last,
                                                               const Function&     f,
            ...
            {
                parallel_for_impl<Index, Function, const auto_partitioner>(
                    first, last, static_cast<Index>(1), f, auto_partitioner(), context);
            }
            //! Parallel iteration over a range of integers with a default step value, explicit
            //! task group context, and simple partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
                    Function, Index>) void parallel_for(Index first, Index last,
            ...
            //! Parallel iteration over a range of integers with a default step value, explicit
            //! task group context, and auto partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
                    Function, Index>) void parallel_for(Index first, Index last,
            ...
            //! Parallel iteration over a range of integers with a default step value, explicit
            //! task group context, and static partitioner
            template <typename Index, typename Function>
            __TBB_requires(
                parallel_for_index<Index>&& parallel_for_function<
                    Function, Index>) void parallel_for(Index first, Index last,
