...
#include "TBB/include/oneapi/tbb/detail/_intrusive_list_node.h"
...
namespace tbb {
    namespace detail {
        ...
        namespace r1 {
            ...
            TBB_EXPORT void __TBB_EXPORTED_FUNC execute(d1::task_arena_base&,
                                                        d1::delegate_base&);
            TBB_EXPORT void __TBB_EXPORTED_FUNC isolate_within_arena(d1::delegate_base&,
                                                                     std::intptr_t);
            ...
            TBB_EXPORT bool __TBB_EXPORTED_FUNC
            is_group_execution_cancelled(d1::task_group_context&);
        ...
        namespace d2 {
            ...
            private:
                ...
                {
                    __TBB_ASSERT(ed.context == &this->ctx(),
                                 "The task group context should be used for all tasks");
            ...
            public:
                ...
                function_task(FF&& f, d1::wait_context& wo, d1::task_group_context& ctx,
                              d1::small_object_allocator& alloc)
                    : task_handle_task{wo, ctx, alloc}, m_func(std::forward<FF>(f))
                {
                }
            };
            ...
            namespace {
                ...
                d1::task* task_ptr_or_nullptr(F&& f)
                {
                    using is_void_t = std::is_void<decltype(std::forward<F>(f)())>;
        ...
        namespace d1 {
            ...
            // This structure is left here for backward compatibility Assert
            ...
            unhandled exceptions intercepted during tasks execution. Intercepting an exception
            results in generating internal cancellation requests (which is processed in exactly
            the same way as external ones).

                The context is associated with one or more root tasks and defines the
            cancellation group that includes all the descendants of the corresponding root
            task(s). Association is established when a context object is passed as an argument
            to the task::allocate_root() method. See task_group_context::task_group_context for
            more details.
                ...
                cancellation propagation direction. If a task in a cancellation group is
            cancelled all the other tasks in this group and groups bound to it (as children) get
            cancelled too.
            **/
            ...
            private:
                ...
                enum class task_group_context_version : std::u8_t {
                    gold_2021U1 =
                        0,  // version of task_group_context released in oneTBB 2021.1 GOLD
                    proxy_support =
                        1  // backward compatible support for 'this' context to act as a proxy
                };
                ...
                struct context_traits {
                    ...
                    bool bound : 1;
                    bool
                        proxy : 1;  // true if 'this' acts as a proxy for user-specified context
                    bool reserved1 : 1;
                    bool reserved2 : 1;
                    bool reserved3 : 1;
                    bool reserved4 : 1;
                ...
                static constexpr std::u8_t may_have_children = 1;
                //! The context internal state (currently only may_have_children).
                std::atomic<std::u8_t> my_state;

                enum class lifetime_state : std::u8_t {
                    created,
                    locked,
                    isolated,
                    bound,
                ...
                static_assert(
                    sizeof(std::atomic<r1::thread_data*>) == sizeof(r1::context_list*),
                    "To preserve backward compatibility these types should have the same size");

                //! Used to form the thread specific list of contexts without additional memory
                //! allocation.
                /** A context is included into the list of the current thread when its binding
                   to its parent happens. Any context can be present in the list of one thread
                   only. **/
                intrusive_list_node my_node;
                static_assert(
                    sizeof(intrusive_list_node) == sizeof(context_list_node),
                    "To preserve backward compatibility these types should have the same size");

                //! Pointer to the container storing exception being propagated across this task
                //! group.
                std::atomic<r1::tbb_exception_ptr*> my_exception;
                static_assert(sizeof(std::atomic<r1::tbb_exception_ptr*>) ==
                                  sizeof(r1::tbb_exception_ptr*),
                              "backward compatibility Assert");
                ...
                char padding[max_nfs_size - sizeof(std::uint64_t)   // my_cpu_ctl_env
                             - sizeof(std::atomic<std::uint32_t>)   // my_cancellation_requested
                             - sizeof(std::u8_t)                    // my_version
                             - sizeof(context_traits)               // my_traits
                             - sizeof(std::atomic<std::u8_t>)       // my_state
                ...
                {
                    my_traits = t;  // GCC4.8 issues warning list initialization for bitset
                      ...
                      my_actual_context{actual_context}
                {
                    ...
                    // no need to initialize 'this' context as it acts as a proxy for
                    // my_actual_context, which initialization is a user-side responsibility.
                }
#endif

                static context_traits make_traits(kind_type      relation_with_parent,
                                                  std::uintptr_t user_traits)
                {
                    context_traits ct;
                ...
                bool is_proxy() const
                {
                    return my_version >= task_group_context_version::proxy_support &&
                           my_traits.proxy;
                }
#endif
...
#if __TBB_PREVIEW_TASK_GROUP_EXTENSIONS
                    if (is_proxy()) {
                        __TBB_ASSERT(my_actual_context,
                                     "Actual task_group_context is not set.");
            ...
            public:
                ...
                /** By default a bound context is created. That is this context will be bound
                   ...
                   associated with this context will never be affected by events in any other
                   context.
                    ...
                    an external thread. That is if a particular algorithm is invoked directly
                   from the external thread (not from a TBB task), supplying it with explicitly
                ...
                // Do not introduce any logic on user side since it might break state
                // propagation assumptions
                ~task_group_context()
                {
#if __TBB_PREVIEW_TASK_GROUP_EXTENSIONS
                    // When 'this' serves as a proxy, the initialization does not happen - nor
                    // should the destruction.
                    if (!is_proxy())
#endif
                    {
                        r1::destroy(*this);
                ...
                //! Forcefully reinitializes the context after the task tree it was associated
                //! with is completed.
                /** Because the method assumes that all the tasks that used to be associated
                   with this context have already finished, calling it while the context is
                   still in use somewhere in the task hierarchy leads to undefined behavior.
                ...
                /** \return false if cancellation has already been requested, true otherwise.
                    ...
                    context or to one of its ancestors (if this context is bound). It is
                   guaranteed that when this method is concurrently called on the same not yet
            ...
            static_assert(sizeof(task_group_context) == 128,
                          "Wrong size of task_group_context");
            ...
            public:
                function_task(const F& f, wait_context& wo, small_object_allocator& alloc)
                    : m_func(f), m_wait_ctx(wo), m_allocator(alloc)
                {
                }

                function_task(F&& f, wait_context& wo, small_object_allocator& alloc)
                    : m_func(std::move(f)), m_wait_ctx(wo), m_allocator(alloc)
                {
                }
            };
            ...
            protected:
                ...
                task_group_status internal_run_and_wait(const F& f)
                {
                    ...
                    try_call([&] {
                        ...
                        // TODO: the reset method is not thread-safe. Ensure the correct
                        // behavior.
                ...
                task* prepare_task(F&& f)
                {
                    ...
                    return alloc.new_object<function_task<typename std::decay<F>::type>>(
                        std::forward<F>(f), m_wait_ctx, alloc);
                }
                ...
                d2::task_handle prepare_task_handle(F&& f)
                {
                    ...
                    d2::task_handle_task* function_task_p = alloc.new_object<function_task_t>(
                        std::forward<F>(f), m_wait_ctx, context(), alloc);
            ...
            public:
                task_group_base(uintptr_t traits = 0)
                    : m_wait_ctx(0),
                      m_context(task_group_context::bound,
                                task_group_context::default_traits | traits)
                {
                }

#if __TBB_PREVIEW_TASK_GROUP_EXTENSIONS
                task_group_base(task_group_context& ctx) : m_wait_ctx(0), m_context(&ctx) {}
#endif
...
#endif
                        // Always attempt to do proper cleanup to avoid inevitable memory
                        // corruption in case of missing wait (for the sake of better
                ...
                {
                    bool cancellation_status = false;
                    try_call([&] { d1::wait(m_wait_ctx, context()); }).on_completion([&] {
                        // TODO: the reset method is not thread-safe. Ensure the correct
                        // behavior.
