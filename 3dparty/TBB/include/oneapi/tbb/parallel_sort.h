...
namespace tbb {
    namespace detail {
        ...
        inline namespace d0 {
            ...
            concept compare =
                requires(const std::remove_reference_t<Compare>&            comp,
        ...
        namespace d1 {
            ...
            /** The split operation selects a splitter and places all elements less than or
               equal to the value in the first range and the remaining elements in the second
               range.
                @ingroup algorithms */
            template <typename RandomAccessIterator, typename Compare>
            class quick_sort_range {
                size_t median_of_three(const RandomAccessIterator& array, size_t l, size_t m,
                                       size_t r) const
                {
                    return comp(array[l], array[m])
                               ? (comp(array[m], array[r]) ? m
                                                           : (comp(array[l], array[r]) ? r : l))
                               : (comp(array[r], array[m])
                                      ? m
                                      : (comp(array[r], array[l]) ? r : l));
                }

                size_t pseudo_median_of_nine(const RandomAccessIterator& array,
                                             const quick_sort_range&     range) const
                {
                    size_t offset = range.size / 8u;
                    return median_of_three(
                        array, median_of_three(array, 0, offset, offset * 2),
            ...
            public:
                ...
                quick_sort_range(RandomAccessIterator begin_, size_t size_,
                                 const Compare& comp_)
            ...
            public:
                ...
                quick_sort_pretest_body(const Compare& _comp, task_group_context& _context)
                    : comp(_comp), context(_context)
                {
                }
                ...
                void operator()(const blocked_range<RandomAccessIterator>& range) const
                {
                    ...
                    int i = 0;
                    // TODO: consider using std::is_sorted() for each 64 iterations (requires
                    // performance measurements)
                    for (RandomAccessIterator k = range.begin(); k != my_end; ++k, ++i) {
                        if (i % 64 == 0 && context.is_group_execution_cancelled())
                            break;

                        // The k - 1 is never out-of-range because the first chunk starts at
                        // begin+serial_cutoff+1
                        if (comp(*(k), *(k - 1))) {
            ...
            struct quick_sort_body {
                void
                operator()(const quick_sort_range<RandomAccessIterator, Compare>& range) const
                {
            ...
            /** @ingroup algorithms */
            template <typename RandomAccessIterator, typename Compare>
            void do_parallel_quick_sort(RandomAccessIterator begin, RandomAccessIterator end,
                                        const Compare& comp)
            {
                parallel_for(
                    quick_sort_range<RandomAccessIterator, Compare>(begin, end - begin, comp),
            ...
            /** @ingroup algorithms */
            template <typename RandomAccessIterator, typename Compare>
            void parallel_quick_sort(RandomAccessIterator begin, RandomAccessIterator end,
                                     const Compare& comp)
            {
                task_group_context my_context(PARALLEL_SORT);
                constexpr int      serial_cutoff = 9;

                __TBB_ASSERT(begin + serial_cutoff < end,
                ...
                parallel_for(
                    blocked_range<RandomAccessIterator>(k + 1, end),
                    quick_sort_pretest_body<RandomAccessIterator, Compare>(comp, my_context),
                    auto_partitioner(), my_context);
            ...
            /** \page parallel_sort_iter_req Requirements on iterators for parallel_sort
                Requirements on the iterator type \c It and its value type \c T for \c
            parallel_sort:

                - \code void iter_swap( It a, It b ) \endcode Swaps the values of the elements
            the given iterators \c a and \c b are pointing to. \c It should be a random access
            iterator.

                - \code bool Compare::operator()( const T& x, const T& y ) \endcode True if x
            comes before y;
            **/

            /** \name parallel_sort
                See also requirements on \ref parallel_sort_iter_req "iterators for
               parallel_sort". **/
            //@{

            //! Sorts the data in [begin,end) using the given comparator
            /** The compare function object is used for all comparisons between elements during
               sorting. The compare object must define a bool operator() function.
                @ingroup algorithms **/
            template <typename RandomAccessIterator, typename Compare>
            __TBB_requires(std::random_access_iterator<RandomAccessIterator>&& compare<
                           Compare,
                           RandomAccessIterator>) void parallel_sort(RandomAccessIterator begin,
            ...
            //! Sorts the data in [begin,end) with a default comparator \c
            //! std::less<RandomAccessIterator>
            /** @ingroup algorithms **/
            template <typename RandomAccessIterator>
            __TBB_requires(
                std::random_access_iterator<RandomAccessIterator>&& less_than_comparable<
                    typename std::iterator_traits<RandomAccessIterator>::
                        value_type>) void parallel_sort(RandomAccessIterator begin,
                                                        RandomAccessIterator end)
            {
                parallel_sort(
                    begin, end,
            ...
            __TBB_requires(
                container_based_sequence<Range, std::random_access_iterator_tag>&&
                    compare<Compare,
                            range_iterator_type<Range>>) void parallel_sort(Range&         rng,
                                                                            const Compare& comp)
            {
                parallel_sort(std::begin(rng), std::end(rng), comp);
            }

            //! Sorts the data in rng with a default comparator \c
            //! std::less<RandomAccessIterator>
            /** @ingroup algorithms **/
            template <typename Range>
            __TBB_requires(
                container_based_sequence<Range, std::random_access_iterator_tag>&&
                    less_than_comparable<typename std::iterator_traits<
                        range_iterator_type<Range>>::value_type>) void parallel_sort(Range& rng)
            {
