...
/** The "posix_memalign" analogue.
    @ingroup memory_allocation */
TBBMALLOC_EXPORT int __TBB_EXPORTED_FUNC scalable_posix_memalign(void** memptr,
                                                                 size_t alignment, size_t size);

/** The "_aligned_malloc" analogue.
    @ingroup memory_allocation */
TBBMALLOC_EXPORT void* __TBB_EXPORTED_FUNC scalable_aligned_malloc(size_t size,
                                                                   size_t alignment);

/** The "_aligned_realloc" analogue.
    @ingroup memory_allocation */
TBBMALLOC_EXPORT void* __TBB_EXPORTED_FUNC scalable_aligned_realloc(void* ptr, size_t size,
                                                                    size_t alignment);
...
namespace rml {
    ...
    typedef void* (*rawAllocType)(std::intptr_t pool_id, size_t& bytes);
    // returns non-zero in case of error
    typedef int (*rawFreeType)(std::intptr_t pool_id, void* raw_ptr, size_t raw_bytes);
    ...
    struct MemPoolPolicy {
        ...
        unsigned fixedPool : 1,
            ...
            keepAllMemory : 1, reserved : 30;

        MemPoolPolicy(rawAllocType pAlloc_, rawFreeType pFree_, size_t granularity_ = 0,
                      bool fixedPool_ = false, bool keepAllMemory_ = false)
            : pAlloc(pAlloc_),
              pFree(pFree_),
              granularity(granularity_),
              version(TBBMALLOC_POOL_VERSION),
              fixedPool(fixedPool_),
              keepAllMemory(keepAllMemory_),
              reserved(0)
        {
        }
    };
    ...
    TBBMALLOC_EXPORT MemPoolError pool_create_v1(std::intptr_t        pool_id,
                                                 const MemPoolPolicy* policy,
                                                 rml::MemoryPool**    pool);

    TBBMALLOC_EXPORT bool        pool_destroy(MemoryPool* memPool);
    TBBMALLOC_EXPORT void*       pool_malloc(MemoryPool* memPool, size_t size);
    TBBMALLOC_EXPORT void*       pool_realloc(MemoryPool* memPool, void* object, size_t size);
    TBBMALLOC_EXPORT void*       pool_aligned_malloc(MemoryPool* mPool, size_t size,
                                                     size_t alignment);
    TBBMALLOC_EXPORT void*       pool_aligned_realloc(MemoryPool* mPool, void* ptr, size_t size,
                                                      size_t alignment);
    ...
    TBBMALLOC_EXPORT size_t      pool_msize(MemoryPool* memPool, void* object);
    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                __TBB_nodiscard T* allocate(size_t n)
                {
            ...
            inline bool operator==(const scalable_allocator<T>&,
            ...
            private:
                ...
                //! Memory allocated by one instance of scalable_resource_impl could be
                //! deallocated by any other instance of this class
