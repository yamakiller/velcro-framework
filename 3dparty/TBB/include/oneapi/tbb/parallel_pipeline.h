...
namespace tbb {
    namespace detail {
        ...
        namespace r1 {
            TBB_EXPORT void __TBB_EXPORTED_FUNC parallel_pipeline(task_group_context&, size_t,
                                                                  const d1::filter_node&);
        }
        ...
        namespace d1 {
            ...
            enum class filter_mode : unsigned int {
                ...
                //! processes items one at a time; all such filters process items in the same
                //! order
                serial_in_order = base_filter::filter_is_serial,
                //! processes items one at a time and in no particular order
                serial_out_of_order =
                    base_filter::filter_is_serial | base_filter::filter_is_out_of_order
            };
            ...
            class filter {
                ...
                friend void parallel_pipeline(size_t, const filter<void, void>&,
                                              task_group_context&);
            ...
            public:
                ...
                filter(filter_mode mode, const Body& body)
                    : my_root(new(r1::allocate_memory(
                          sizeof(filter_node_leaf<InputType, OutputType, Body>)))
                                  filter_node_leaf<InputType, OutputType, Body>(
                                      static_cast<unsigned int>(mode), body))
                {
                }
            ...
            {
                return filter_node_ptr(new (
                    r1::allocate_memory(sizeof(filter_node_leaf<InputType, OutputType, Body>)))
                                           filter_node_leaf<InputType, OutputType, Body>(
                                               static_cast<unsigned int>(mode), body));
            }
            ...
            /** @ingroup algorithms */
            template <typename Body>
            filter<filter_input<Body>, filter_output<Body>> make_filter(filter_mode mode,
                                                                        const Body& body)
            {
            ...
            {
                __TBB_ASSERT(left.my_root,
                             "cannot use default-constructed filter as left argument of '&'");
                __TBB_ASSERT(right.my_root,
                             "cannot use default-constructed filter as right argument of '&'");
                return filter_node_ptr(new (r1::allocate_memory(sizeof(filter_node)))
                                           filter_node(left.my_root, right.my_root));
            }
            ...
            /** @ingroup algorithms **/
            inline void parallel_pipeline(size_t                    max_number_of_live_tokens,
                                          const filter<void, void>& filter_chain,
                                          task_group_context&       context)
            {
                r1::parallel_pipeline(context, max_number_of_live_tokens,
                                      *filter_chain.my_root);
            }
            ...
            void parallel_pipeline(size_t max_number_of_live_tokens, const F1& filter1,
                                   const F2& filter2, FiltersContext&&... filters)
            {
                parallel_pipeline(max_number_of_live_tokens, filter1 & filter2,
                                  std::forward<FiltersContext>(filters)...);
            }
