...
namespace V
{
    namespace Test
    {
        ScopedAutoTempDirectory::ScopedAutoTempDirectory()
        {
            ...
            v_snprintf(tempDirectoryTemplate, V::IO::MaxPathLength, "%sUnitTest-XXXXXX", tempDirRoot.c_str());

            const char* tempDir = mkdtemp(tempDirectoryTemplate);    
            V_Error("VTest", tempDir, "Unable to create temp directory %s", tempDirectoryTemplate);
