...
namespace V
{
    namespace Test
    {
        ...
        VStd::string GetCurrentExecutablePath()
        {
            ...
            V:Utils::GetExecutableDirectory(exeDirectory, AZ_ARRAY_SIZE(exeDirectory));

            VStd::string executablePath = exeDirectory;
            return executablePath;
        }

        VStd::string ScopedAutoTempDirectory::Resolve(const char* path) const
        {
            VStd::string resolved = VStd::string::format("%s" AZ_CORRECT_FILESYSTEM_SEPARATOR_STRING "%s", m_tempDirectory, path);
            return resolved;
        }
