...
namespace V {
    namespace Test {
        ...
        void addTestEnvironments(std::vector<ITestEnvironment*> envs) {
            ...
            if (envs.empty()) {
                addTestEnvironment(V::Test::DefaultTestEnv());
            } else {
                for (auto env : envs) {
        ...
        std::vector<TestEnvironmentRegistry*> TestEnvironmentRegistry::_envs;
        ...
        void printUnusedParametersWarning(int argc, char** argv)
        {
         ...
         bool VelcroUnitTestMain::Run(int argc, char** argv)
        {
            using namespace V::Test;
            m_returnCode = 0;
            if (ContainsParameter(argc, argv, "--unittest") || ContainsParameter(argc, argv, "--unittests"))
            {
                ...
                if (waitForDebbugerIndex != -1) {
                    ...
                    V::Test::Platform& platform = V::Test::GetPlatform();
                ...
                V::Test::excludeIntegTests();
                V::Test::ApplyGlobalParameters(&argc, argv);
                V::Test::printUnusedParametersWarning(argc, argv);
                V::Test::addTestEnvironments(m_envs);
        ...
        bool VelcroUnitTestMain::Run(const char* commandLine)
        {
            int tokenSize;
            char** commandTokens = V::Test::SplitCommandLine(tokenSize, const_cast<char*>(commandLine));
        ...
        int RunTestsInLib(V::Test::Platform& platform, const std::string& lib, const std::string& symbol, int& argc, char** argv)
        {
#if defined(V_MONOLITHIC_BUILD)
            ::testing::InitGoogleMock(&argc, argv);
            ...
            for (auto env : V::Test::TestEnvironmentRegistry::s_envs)
            {
                if (env->m_module_name == lib_upper)
                {
                    V::Test::addTestEnvironments(env->m_envs);
...
#else // V_MONOLITHIC_BUILD
            int result = 0;
            std::shared_ptr<V::Test::IModuleHandle> module = platform.GetModule(lib);
            if (module->IsValid())
            {
