...
#include <tchar.h>
#include <stdio.h>
V_PUSH_DISABLE_WARNING(4091, "-Wmissing-declarations"); // 4091: 'typedef ': ignored on left of '' when no variable is declared
#   include <dbghelp.h>
...
namespace V {
    ...
    namespace Debug
    {
        ...
        static EnvironmentVariable<SymbolStorageDynamicallyLoadedModules> _dynamicallyLoadedModules = nullptr;

        // Mutex for use of _dynamicallyLoadedModules so that if there's one thread decoding frames while another is
        // handling DLL load and adding to the list we don't have issues
        static EnvironmentVariable<VStd::mutex> _dynamicallyLoadedModulesMutex;

        static VStd::mutex& GetDynamicallyLoadedModulesMutex()
        {
            if (!_dynamicallyLoadedModulesMutex)
            {
                _dynamicallyLoadedModulesMutex = Environment::CreateVariable<VStd::mutex>(V_CRC("SymbolStorageDynamicallyLoadedModulesMutex", 0x2b7dbaf2));
                V_Assert(_dynamicallyLoadedModulesMutex, "Unable to create SymbolStorageDynamicallyLoadedModulesMutex environment variable");
            }
            return *_dynamicallyLoadedModulesMutex;
        }

        static SymbolStorageDynamicallyLoadedModules& GetRegisteredLoadedModules()
        {
            if (!_dynamicallyLoadedModules)
            {
                _dynamicallyLoadedModules = Environment::CreateVariable<SymbolStorageDynamicallyLoadedModules>(V_CRC("SymbolStorageDynamicallyLoadedModules", 0xecf96588));
                V_Assert(_dynamicallyLoadedModules, "Unable to create SymbolStorageDynamicallyLoadedModules environment variable - dynamically loaded modules won't have symbols loaded!");
            }
            return *_dynamicallyLoadedModules;
        }
...
#define LOAD_FUNCTION(A)    { g_##A = (A##_t)GetProcAddress(g_dbgHelpDll, #A); V_Assert(g_##A != 0, ("Can not load %s function!",#A)); }

    using namespace V::Debug;

    bool                        g_dbgHelpLoaded = false;
    VStd::mutex                g_dbgLoadingMutex;
    ...
    VStd::fixed_vector<SymbolStorage::ModuleInfo, 2048> g_moduleInfo;
    ...
    class WindowsSymbols
    {
        ...
        // V::Debug::_dynamicallyLoadedModules.  When decoding frames, we make sure to 
    ...
    public:
        ...
        {
            VStd::lock_guard<VStd::mutex> locker(V::Debug::GetDynamicallyLoadedModulesMutex());
            V::Debug::SymbolStorageDynamicallyLoadedModules& loadedModules = V::Debug::GetRegisteredLoadedModules();
        ...
        )
        {
            ...
            if (NotificationReason == 1)
            {
                ...
                if (loadedModules.m_size < V_ARRAY_SIZE(loadedModules.m_modules))
                {
                    V::Debug::DynamicallyLoadedModuleInfo& modInfo = loadedModules.m_modules[loadedModules.m_size];
                    size_t numCharsConverted;
                    wcstombs_s(&numCharsConverted, modInfo.m_fileName, V_ARRAY_SIZE(modInfo.m_fileName), NotificationData->Loaded.FullDllName->Buffer, VStd::min(V_ARRAY_SIZE(modInfo.m_fileName) - 1, (size_t)NotificationData->Loaded.FullDllName->Length));
                    wcstombs_s(&numCharsConverted, modInfo.m_name, V_ARRAY_SIZE(modInfo.m_name), NotificationData->Loaded.BaseDllName->Buffer, VStd::min(V_ARRAY_SIZE(modInfo.m_name) - 1, (size_t)NotificationData->Loaded.BaseDllName->Length));
                    modInfo.m_baseAddress = (V::u64)NotificationData->Loaded.DllBase;
                    modInfo.m_size = (V::u32)NotificationData->Loaded.SizeOfImage;
                    ...
                    if (toWriteTo)
                    {
                        ...
                        memcpy(toWriteTo, &modInfo, sizeof(V::Debug::DynamicallyLoadedModuleInfo));
                    }
                    else
                    {
                ...
                {
                     V_TracePrintf("System", "Failed to store dynamic module information, increase size of V::Debug::SymbolStorageDynamicallyLoadedModules.m_modules to maintain stack tracer functionality!");
                }
            }
        }
        ...
        void RegisterModuleListeners()
        {
            ...
            if (m_LdrRegisterDllNotification)
            {
                ...
                {
                    V_TracePrintf("System", "Failed registering module load listeners - SymbolStorage frame decoding may be impacted.");
                }
            }
        }
        ...
        void UnregisterModuleListeners()
        {
            ...
            if (m_LdrUnregisterDllNotification)
            {
                ...
                {
                    V_TracePrintf("System", "Failed unregistering module load listeners");
                }
            }
        }
    ...
    public:
        ...
        BOOL LoadSymbols()
        {
            ...
            if (!m_szSymPath)
            {
                ...
                DWORD pathLen = GetModuleFileNameA(nullptr, g_reservedScratchSpace1, V_ARRAY_SIZE(g_reservedScratchSpace1));
                if (pathLen > 0)
                {
                    char* exeDirEnd = g_reservedScratchSpace1 + pathLen;
                    VStd::replace(g_reservedScratchSpace1, exeDirEnd, '\\', '/');
                ...
                v_strcat(g_reservedScratchSpace1, V_ARRAY_SIZE(g_reservedScratchSpace1), ";.");

                V::Debug::SymbolStorageDynamicallyLoadedModules& loadedModules = V::Debug::GetRegisteredLoadedModules();
                for (int j = 0; j < loadedModules.m_size; ++j)
                {
                    V::Debug::DynamicallyLoadedModuleInfo& module = loadedModules.m_modules[j];
                    v_strcpy(g_reservedScratchSpace2, g_scratchSpaceSize, module.m_fileName);
                    char* lastSlash = strrchr(g_reservedScratchSpace2, '\\');
                    if (lastSlash)
                    {
                        *lastSlash = 0; // cut off everything after and including the last backslash.
                        v_strcat(g_reservedScratchSpace2, g_scratchSpaceSize, ";");
                        // is it already there?
                        ...
                        if (strstr(g_reservedScratchSpace1, g_reservedScratchSpace2) == nullptr)
                        {
                            ...
                            {
                                v_strcat(g_reservedScratchSpace1, g_scratchSpaceSize, g_reservedScratchSpace2);
                            }
                        }
                    }
            ...
            {
                V_TracePrintf("System", "failed to call SymInitialize! Error %d", GetLastError());
            }
        ...
        V_INLINE void CleanUpSymbols()
        {
            m_isSymLoaded = false;
            g_SymCleanup(g_currentProcess);
        }

        V_FORCE_INLINE bool IsSymLoaded() const { return m_isSymLoaded ? true : false; }
        ...
        void DecodeFrames(const StackFrame* frames, unsigned int numFrames, SymbolStorage::StackLine* textLines)
        {
            ...
            for (unsigned int i = 0; i < numFrames; ++i)
            {
                ...
                DWORD64 pc = (DWORD64)frame.ProgramCounter;
                ...
                {
                    v_snprintf(textLine, textLineSize, "%s (%ld) : ", line.FileName, line.LineNumber);
                }
                else
                {
                    ...
                    {
                        v_snprintf(textLine, textLineSize, "%p (%s) : ", (LPVOID)pc, module.ModuleName);
                    }
                    else
                    {
                        v_snprintf(textLine, textLineSize, "%p (module-name not available) : ", (LPVOID)pc);
                    }
                }
                ...
                if (g_SymGetSymFromAddr64(g_currentProcess, pc, &offsetFromSmybol, &sym))
                {
                    ...
                    if (textLine[0] == 0)
                    {
                        v_strcpy(textLine, textLineSize, sym.Name);
                    }
                }
                else
                {
                    v_strcat(textLine, textLineSize, "(function-name not available)");
                }
            }
        ...
        void FindFunctionFromIP(void* ip, SymbolStorage::StackLine* func_name, SymbolStorage::StackLine* file_name, SymbolStorage::StackLine* module_name, int& line_num, void*& baseAddr)
        {
            ...
            EnterCriticalSection(&g_csDbgHelpDll);
            {
                ...
                if (g_SymGetLineFromAddr64(g_currentProcess, (DWORD64)ip, &displacement, &line))
                {
                    if (line.FileName[0])
                    {
                        v_strcpy(*file_name, sizeof(*file_name), line.FileName);
                ...
                if (GetModuleInfo(g_currentProcess, (DWORD64)ip, &module))
                {
                    ...
                    {
                        v_strcpy(*module_name, sizeof(*module_name), module.ModuleName);
                    }
                }
    ...
    private:
        ...
        DWORD LoadModule(HANDLE hProcess, LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size)
        {
            ...
            {
                if (strcmp(g_moduleInfo[i].FileName, img) == 0)
                {
                    return result;
                }
            }
            ...
            CHAR szImg[1024];
            CHAR szMod[1024];
            v_strcpy(szImg, V_ARRAY_SIZE(szImg), img);
            v_strcpy(szMod, V_ARRAY_SIZE(szMod), mod);
            ...
            if (szImg != NULL)
            {
                ...
                if (g_moduleInfo.size() <  g_moduleInfo.capacity())
                {
                    ...
                    modInfo.BaseAddress = (u64)baseAddr;
                    v_strcpy(modInfo.ModName, V_ARRAY_SIZE(modInfo.ModName), szMod);
                    v_strcpy(modInfo.FileName, V_ARRAY_SIZE(modInfo.FileName), img);
                    modInfo.FileVersion = (u64)fileVersion;
                    modInfo.Size = size;
                }
                else
                {
                    // Warn that we can't store information about all modules... if we move this AZCore we can allocate this dynamically.
                    V_TracePrintf("System", "Failed to store module information, increase size of g_moduleInfo to maintain stack tracer functionality!");
                }
            }
            return result;
        }
...
#pragma pack( pop )
        ...
        bool GetModuleListTH32(HANDLE hProcess, DWORD pid)
        {
            ...
            // Load dynamically loaded modules
            {
                ...
                V::Debug::SymbolStorageDynamicallyLoadedModules loadedModules;
                {
                    // scoped for lock_guard
                    VStd::lock_guard<VStd::mutex> locker(V::Debug::GetDynamicallyLoadedModulesMutex());
                    // note well that this is operator=, we are copying
                    loadedModules = V::Debug::GetRegisteredLoadedModules();
...
cleanup:
    ...
    void    LoadDbgHelp()
    {
        VStd::lock_guard<VStd::mutex> lock(g_dbgLoadingMutex);
    ...
    StackRecorder::Record(StackFrame* frames, unsigned int maxNumOfFrames, unsigned int suppressCount, void* nativeThread)
    {
        ...
        if (nativeThread == NULL)
        {
            ...
            static_assert(V_ARRAY_SIZE(myFrames) <= 0xffff, "RtlCaptureStackBackTrace only supports a maximum of MAXUSHORT (0xffff) frames captured.");
            V_Assert(maxNumOfFrames <= V_ARRAY_SIZE(myFrames), "You need to increase the size of myFrames array %u (needed %zu)!", maxNumOfFrames, V_ARRAY_SIZE(myFrames));
            
            numFrames = RtlCaptureStackBackTrace(suppressCount, maxNumOfFrames, myFrames, NULL);
            for (unsigned int frame = 0; frame < numFrames; ++frame)
            {
                frames[frame].ProgramCounter = (uintptr_t)myFrames[frame];
            }
        }
        else
        {
            ...
            CONTEXT alignas(16) context; // Without this alignment the function randomly crashes in release.
            ...
            for (; frame < (s32)maxNumOfFrames; ++frame)
            {
                ...
                if (frame >= 0)
                {
                    frames[numFrames++].ProgramCounter = sf.AddrPC.Offset;
                }
            }
