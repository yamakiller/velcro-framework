...
//#include <vcore/debug/trace_messages_driller_bus.h>
...
namespace V
{
    ...
    namespace Debug
    {
        namespace Platform
        {
            ...
            {
                VStd::fixed_wstring<g_maxMessageLength> tmpW;
                if(window)
                {
                    VStd::to_wstring(tmpW, window);
                    tmpW += L": ";
                    OutputDebugStringW(tmpW.c_str());
                    tmpW.clear();
                }
                VStd::to_wstring(tmpW, message);
                OutputDebugStringW(tmpW.c_str());
            }
        }
    }
    ...
    {
        extern V_THREAD_LOCAL bool g_suppressEBusCalls; // used when it would be dangerous to use ebus broadcasts.
    }
