...
namespace VStd
{
    ...
    namespace Internal
    {
        ...
        unsigned __stdcall thread_run_function(void* param)
        {
            ...
            ThreadDetectorEventBus::Broadcast(&ThreadDetectorEventBus::Events::OnThreadExit, this_thread::get_id()); // goes to the profiler.
        ...
        HANDLE create_thread(const thread_desc* desc, thread_info* ti, unsigned int* id)
        {
            ...
            *id = native_thread_invalid_id;
            ...
            ThreadDetectorEventBus::Broadcast(&ThreadDetectorEventBus::Events::OnThreadEnter, thread::id(*id), desc);
    ...
    thread::~thread()
    {
        V_Assert(!joinable(), "You must call detach or join before you delete a thread!");
    }

    void thread::join()
    {
        V_Assert(joinable(), "Thread must be joinable!");
