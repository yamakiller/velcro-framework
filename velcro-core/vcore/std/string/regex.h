 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_STRING_REGEX_H
#define V_FRAMEWORK_CORE_STD_STRING_REGEX_H
...
#include <vcore/std/exceptions.h>
#include <vcore/std/containers/vector.h>
#include <vcore/memory/system_allocator.h>

#include <limits>

// used for std::pointer_traits \note do an VStd version
#include <memory>

 #ifndef V_REGEX_MAX_COMPLEXITY_COUNT
  #define V_REGEX_MAX_COMPLEXITY_COUNT 10000000L   /* set to 0 to disable */
 #endif /* V_REGEX_MAX_COMPLEXITY_COUNT */

 #ifndef V_REGEX_MAX_STACK_COUNT
  #define V_REGEX_MAX_STACK_COUNT  1000L   /* set to 0 to disable */
 #endif /* V_REGEX_MAX_STACK_COUNT */
...
namespace VStd
{
  #define V_REGEX_DIFFT(iter)  typename iterator_traits<iter>::difference_type
  #define V_REGEX_VALT(iter)   typename iterator_traits<iter>::value_type
    ...
    namespace regex_constants
    {
        ...
        V_DEFINE_ENUM_BITWISE_OPERATORS(syntax_option_type)
        ...
        V_DEFINE_ENUM_BITWISE_OPERATORS(match_flag_type)
    ...
    public:
        ...
        typedef typename VStd::basic_string<Element> string_type;
        ...
        {
            Element result;
            return (VStd::str_transform(&result, &ch, 1) == 1) ? result : ch;
        }
        ...
        {   // apply locale-specific transformation
            ...
            if (length)
            {
                VStd::str_transform(result.data(), first, length);
            }
            return result;
        }
        ...
        {   // apply locale-specific case-insensitive transformation
            ...
            {   // non-empty string, transform it
                ...
                VStd::to_lower(temp.data(), temp.data() + temp.size());
                VStd::str_transform(result.data(), temp.data(), temp.size());
            }
            return result;
        }
        ...
        bool isctype(Element ch, char_class_type fx) const
        {
            ...
            {
            case Ch_alnum:
                return VStd::is_alnum(ch) != 0;
            case Ch_alpha:
                return VStd::is_alpha(ch) != 0;
            case Ch_cntrl:
                return VStd::is_cntrl(ch) != 0;
            case Ch_digit:
                return VStd::is_digit(ch) != 0;
            case Ch_graph:
                return VStd::is_graph(ch) != 0;
            case Ch_lower:
                return VStd::is_lower(ch) != 0;
            case Ch_print:
                return VStd::is_print(ch) != 0;
            case Ch_punct:
                return VStd::is_punct(ch) != 0;
            case Ch_space:
                return VStd::is_space(ch) != 0;
            case Ch_upper:
                return VStd::is_upper(ch) != 0;
            case Ch_xdigit:
                return VStd::is_xdigit(ch) != 0;
            default:
                return (ch == '_' || VStd::is_alnum(ch) != 0);             // assumes L'_' == '_'
            }
        }
    ...
    public:
        typedef V_REGEX_VALT (BidirectionalIterator) value_type;
        typedef V_REGEX_DIFFT (BidirectionalIterator) difference_type;
    ...
    template<class BidirectionalIterator>
    inline bool operator==(const V_REGEX_VALT(BidirectionalIterator)* left, const sub_match<BidirectionalIterator>& right) { return (right.compare(left) == 0); }

    template<class BidirectionalIterator>
    inline bool operator!=(const V_REGEX_VALT(BidirectionalIterator)* left, const sub_match<BidirectionalIterator>& right) { return (!(left == right)); }

    template<class BidirectionalIterator>
    inline bool operator<(const V_REGEX_VALT(BidirectionalIterator)* left, const sub_match<BidirectionalIterator>& right) { return (0 < right.compare(left)); }

    template<class BidirectionalIterator>
    inline bool operator>(const V_REGEX_VALT(BidirectionalIterator)* left, const sub_match<BidirectionalIterator>& right)  { return (right < left); }
    ...
    template<class BidirectionalIterator>
    inline bool operator==(const sub_match<BidirectionalIterator>& left, const V_REGEX_VALT(BidirectionalIterator)* right) { return (left.compare(right) == 0); }

    template<class BidirectionalIterator>
    inline bool operator!=(const sub_match<BidirectionalIterator>& left, const V_REGEX_VALT(BidirectionalIterator)* right) { return (!(left == right)); }

    template<class BidirectionalIterator>
    inline bool operator<(const sub_match<BidirectionalIterator>& left, const V_REGEX_VALT(BidirectionalIterator)* right) { return (left.compare(right) < 0); }

    template<class BidirectionalIterator>
    inline bool operator>(const sub_match<BidirectionalIterator>& left, const V_REGEX_VALT(BidirectionalIterator)* right) { return (right < left); }
    ...
    template<class BidirectionalIterator>
    inline bool operator==(const V_REGEX_VALT(BidirectionalIterator)& left,    const sub_match<BidirectionalIterator>& right) { typename sub_match<BidirectionalIterator>::string_type string(1, left); return (right.compare(string) == 0);   }

    template<class BidirectionalIterator>
    inline bool operator!=(const V_REGEX_VALT(BidirectionalIterator)& left,    const sub_match<BidirectionalIterator>& right)  { return (!(left == right)); }

    template<class BidirectionalIterator>
    inline bool operator<(const V_REGEX_VALT(BidirectionalIterator)& left, const sub_match<BidirectionalIterator>& right)  { typename sub_match<BidirectionalIterator>::string_type string(1, left); return (0 < right.compare(string)); }
    ...
    template<class BidirectionalIterator>
    inline bool operator==(const sub_match<BidirectionalIterator>& left, const V_REGEX_VALT(BidirectionalIterator)& right) { typename sub_match<BidirectionalIterator>::string_type string(1, right); return (left.compare(string) == 0); }
    ...
    inline bool operator==(const sub_match<BidirectionalIterator>& left, const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& right)    { return (left.compare(right.c_str()) == 0); }

    template<class BidirectionalIterator, class Traits, class Allocator>
    inline bool operator!=(const sub_match<BidirectionalIterator>& left, const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& right)    { return (!(left == right)); }

    template<class BidirectionalIterator, class Traits, class Allocator>
    inline bool operator<(const sub_match<BidirectionalIterator>& left, const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& right) { return (left.compare(right.c_str()) < 0); }

    template<class BidirectionalIterator, class Traits, class Allocator>
    inline  bool operator>(const sub_match<BidirectionalIterator>& left, const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& right) { return (right < left); }
    ...
    template<class BidirectionalIterator, class Traits, class Allocator>
    inline bool operator==(const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& left, const sub_match<BidirectionalIterator>& right) { return (right.compare(left.c_str()) == 0); }

    template<class BidirectionalIterator, class Traits, class Allocator>
    inline bool operator!=(const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& left, const sub_match<BidirectionalIterator>& right) { return (!(left == right)); }

    template<class BidirectionalIterator, class Traits, class Allocator>
    inline bool operator<(const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& left, const sub_match<BidirectionalIterator>& right) {   return (0 < right.compare(left.c_str())); }

    template<class BidirectionalIterator, class Traits, class Allocator>
    inline bool operator>(const basic_string<V_REGEX_VALT(BidirectionalIterator), Traits, Allocator>& left, const sub_match<BidirectionalIterator>& right) {   return (right < left); }
    ...
    template<class BidirectionalIterator, class Allocator = VStd::allocator >
    class match_results;
    ...
    public:
        ...
        typedef V_REGEX_VALT (BidirectionalIterator) char_type;
        ...
        match_results(this_type&& right)
            : m_isReady(right.m_isReady)
            , m_original(right.m_original)
            , m_matches(VStd::move(right.m_matches))
            , m_prefix(right.m_prefix)
            , m_suffix(right.m_suffix)
        ...
        difference_type position(size_type index = 0) const { return VStd:: distance(m_original, (*this)[index].first); }
        ...
        {   // format text, replacing matches
            ...
            format(VStd:: back_inserter(string), format.begin(), format.end(), flags);
            return (string);
        }
        ...
        {   // format text, replacing matches
            ...
            string_type str;
            format(VStd:: back_inserter(str), format.begin(), format.end(), flags);
            return str;
        }
        ...
        {   // exchange contents with right
            VStd::swap(m_isReady, right.m_isReady);
            VStd::swap(m_original, right.m_original);
            m_matches.swap(right.m_matches);
            VStd::swap(m_prefix, right.m_prefix);
            VStd::swap(m_suffix, right.m_suffix);
        }
    ...
    bool operator==(const match_results<BidirectionalIterator, Allocator>& left, const match_results<BidirectionalIterator, Allocator>& right)
    {
        ...
        else
        {
            return (left.size() == right.size()
                    ...
                    && VStd:: equal(left.begin(), left.end(), right.begin()));
        }
    }
    ...
    struct RegExBuffer
    {
        V_CLASS_ALLOCATOR(RegExBuffer, V::SystemAllocator, 0);
        ...
        {
            V_Assert(index < Size, "Invalid index");
        ...
        void Insert(Element ch)
        {
            if (m_capacity <= Size)
            {
                Expand(Size + BUFFER_INCREMENT);
            }
            m_chars[Size++] = ch;
        }

        // remove and return last character
        Element Delete()        { return (m_chars[--Size]); }
    ...
    private:
        ...
        {   // expand buffer to hold length characters
            ...
            V_Assert(m_chars != nullptr, "VStd::allocator failed to allocate %u bytes!", requiredSize);
        }

        unsigned int m_capacity;
        unsigned int Size;
        Element* m_chars;
        VStd::allocator m_allocator;
    };

    struct RegExBitmap
    {   // accelerator table for small character values
        V_CLASS_ALLOCATOR(RegExBitmap, V::SystemAllocator, 0)
    ...
    {   // holds sequences of Size elements
        V_CLASS_ALLOCATOR(RegExSequence, V::SystemAllocator, 0)
        RegExSequence(unsigned int len)
            : Size(len) { }

        unsigned int Size;
        RegExBuffer<Element> Data;
        RegExSequence* Next;
    };
    ...
    public:
        V_CLASS_ALLOCATOR(NodeBase, V::SystemAllocator, 0)
    ...
    {   // root of parse tree
    public:
        V_CLASS_ALLOCATOR(RootNode, V::SystemAllocator, 0);
        RootNode()
            : NodeBase(NT_begin)
            , Loops(0)
            , Marks(0)
    ...
    public:
        V_CLASS_ALLOCATOR(NodeEndGroup, V::SystemAllocator, 0);
    ...
    public:
        V_CLASS_ALLOCATOR(NodeAssert, V::SystemAllocator, 0);
    ...
    public:
        V_CLASS_ALLOCATOR(NodeCapture, V::SystemAllocator, 0);
    ...
    public:
        V_CLASS_ALLOCATOR(NodeBack, V::SystemAllocator, 0);
    ...
    public:
        V_CLASS_ALLOCATOR(NodeString, V::SystemAllocator, 0);
    ...
    public:
        V_CLASS_ALLOCATOR(NodeClass, V::SystemAllocator, 0);
        ...
        RegExSequence<Element>* Coll;
        RegExBitmap* Small;
        RegExBuffer<Element>* Large;
        RegExBuffer<Element>* Ranges;
        typename RegExTraits::char_class_type _Classes;
        RegExSequence<Element>* Equiv;
    };
    ...
    public:
        V_CLASS_ALLOCATOR(NodeEndif, V::SystemAllocator, 0);
        NodeEndif()
    ...
    public:
        V_CLASS_ALLOCATOR(NodeIf, V::SystemAllocator, 0);
        NodeIf(NodeBase* m_end)
            : NodeBase(NT_if, NFLG_none)
            , Endif((NodeEndif*)m_end)
            , Child(0)
        {
        }
        ...
        {   // destroy branches of if node
            NodeIf* current = Child;
            while (current)
            {   // destroy branch
                NodeIf* tmp = current;
                current = current->Child;
                tmp->Child = 0;
                DestroyNode(tmp, Endif);
            }
        }
    ...
    public:
        V_CLASS_ALLOCATOR(NodeEndRepetition, V::SystemAllocator, 0);
        NodeEndRepetition()
            : NodeBase(NT_end_rep)
            , BeginRep(0)
        {
    ...
    public:
        V_CLASS_ALLOCATOR(NodeRepetition, V::SystemAllocator, 0);
        ...
        const int Min;
        const int Max;
        NodeEndRepetition* EndRep;
        unsigned int LoopNumber;
        int SampleLoop;   // -1 undetermined, 0 contains if/do, 1 simple
    ...
    public:
        typedef V_REGEX_DIFFT (ForwardIterator) DiffType;
        ...
        NodeBase* BeginCaptureGroup(unsigned int idx);
        void AddBackreference(unsigned int idx);
    ...
    public:
        Matcher(Iterator first, Iterator last, const RegExTraits& traits, RootNode* root, int _Nx, regex_constants::syntax_option_type sflags, regex_constants::match_flag_type mflags)
            ...
            , m_isMatched(false)
            , _Ncap(_Nx)
            , _Longest((root->Flags & NFLG_longest) && !(mflags & regex_constants::match_any))
            , m_traits(traits)
        {
            m_loopVals.resize(root->Loops);
        }
        ...
        {   // try to match
            m_begin = m_first;
            m_tgtState.Current = m_first;
            m_tgtState.GroupValid.resize(_Ncap);
            m_tgtState.Groups.resize(_Ncap);
            _Cap = matches != 0;
            m_isFull = isFullMatch;
            _Max_complexity_count = V_REGEX_MAX_COMPLEXITY_COUNT;
            _Max_stack_count = V_REGEX_MAX_STACK_COUNT;
            ...
            else if ((m_mflags& regex_constants::match_partial) != 0 && m_tgtState.Current == m_end && m_first != m_end)
            ...
            {   // copy results to m_matches
                ...
                for (int idx = 0; idx < _Ncap; ++idx)
                {   // copy submatch Index
                    if (m_result.GroupValid[idx])
                    {   // copy successful match
                        matches->at(idx).matched = true;
                        matches->at(idx).first = m_result.Groups[idx].m_begin;
                        matches->at(idx).second = m_result.Groups[idx].m_end;
                    }
                    else
                    {   // copy failed match
                        matches->at(idx).matched = false;
                        matches->at(idx).first = m_end;
                        matches->at(idx).second = m_end;
                    }
                }
    ...
    public:
        ...
        unsigned int _Loop_count() const        { return (m_rootNode != 0 ? m_rootNode->Loops : 0); }
        // return number of capture groups
        unsigned int mark_count() const         { return (m_rootNode != 0 ? m_rootNode->Marks - 1 : 0); }
        ...
        { 
            VStd::swap(m_rootNode, right.m_rootNode); 
            VStd::swap(m_error, right.m_error);
        }
    ...
    private:
        ...
        void Clear()
        {   // free all storage
            if (m_rootNode != 0 && --m_rootNode->Refs == 0)
            {
        ...
        {   // build regular expression from input iterators
            basic_string<V_REGEX_VALT(InputIterator)> string(first, last);
            Reset(string.begin(), string.end(),
    ...
    template<class BidirectionalIterator, class Allocator>
    void swap(match_results<BidirectionalIterator, Allocator>& left, match_results<BidirectionalIterator, Allocator>& right) VSTD_THROW0()
    {
        left.swap(right);
    }
    ...
    #define V_REGEX_ISDIGIT(x) ('0' <= (x) && (x) <= '9')
    ...
    {   // format with ECMAScript rules
        ...
        {   // process one character or escape sequence
            ...
            {   // replace $`
                output = VStd::copy(match.prefix().first, match.prefix().second, output);
                ++first;
            }
            else if (*first == '\'')
            {   // replace $'
                output = VStd::copy(match.suffix().first, match.suffix().second, output);
                ++first;
            }
            else
            {   // replace capture group descriptors
                ...
                else if (V_REGEX_ISDIGIT(*first))
                {   // replace $n, $nn
                    n = *first++ - '0';
                    if (first != last && V_REGEX_ISDIGIT(*first))
                ...
                {
                    output = VStd::copy(match.at(n).first, match.at(n).second, output);
                }
            }
        }
        return output;
    ...
    {   // format with sed rules
        ...
        {   // process one character or escape sequence
            ...
            {   // replace with full match
                output = VStd::copy(match.at(0).first, match.at(0).second, output);
                ++first;
            }
            else if (*first != '\\')
            {
    ...
    inline OutputIterator _Regex_replace(OutputIterator _Result, BidirectionalIterator first, BidirectionalIterator last, const basic_regex<Element, RegExTraits>& regEx,   const basic_string<Element, Traits, Allocator>& format, regex_constants::match_flag_type flags)
    {
        ...
        {   // replace at each match
            ...
            {
                _Result = VStd::copy(m_matches.prefix().first, m_matches.prefix().second, _Result);
            }
        ...
        return (flags & regex_constants::format_no_copy ? _Result : VStd::copy(position, last, _Result));
    }
    ...
    regex_replace(const basic_string<Element, _Traits1, _Alloc1>& string, const basic_regex<Element, RegExTraits>& regEx, const basic_string<Element, _Traits2, _Alloc2>& format,   regex_constants::match_flag_type flags = regex_constants::match_default)
    {
        ...
        regex_replace(VStd:: back_inserter(result), string.begin(), string.end(), regEx, format, flags);
        return result;
    }
    ...
    template<class BidirectionalIterator, class Element = V_REGEX_VALT(BidirectionalIterator), class RegExTraits = regex_traits<Element> >
    ...
    public:
        ...
        bool operator==(const this_type& right) const
        {
            ...
            return (m_begin == right.m_begin && m_end == right.m_end && Flags == right.Flags && m_value.at(0) == right.m_value.at(0));
        }
        ...
        this_type& operator++()
        {
            ...
            {   // handle zero-length match
                ...
                if (_Regex_search(_Start, m_end, &m_value, *m_regEx, Flags | regex_constants::match_not_null | regex_constants::match_continuous, m_begin))
                {
                    return (*this);
                }
                ++_Start;
            ...
            if (_Regex_search(_Start, m_end, &m_value, *m_regEx, Flags, m_begin))
            {
                return (*this);
            }
            else
    ...
    private:
        ...
        {   // check for suffix specifier
            return (VStd::find(m_subs.begin(), m_subs.end(), -1) != m_subs.end());
        }
    ...
    {   // test for beginning of expression or subexpression
        return (_Nx->Kind == NT_begin || _Nx->Kind == NT_group || _Nx->Kind == NT_capture);
    }
    ...
    {   // test for beginning of expression or subexpression
        return (BeginExpression(m_current) || (m_current->Kind == NT_bol && BeginExpression(m_current->Previous)));
    }

    template<class ForwardIterator, class Element, class RegExTraits>
    inline NodeBase* Builder<ForwardIterator, Element, RegExTraits>::LinkNode(NodeBase* _Nx)
    {   // insert _Nx at current location
        _Nx->Previous = m_current;
        if (m_current->Next)
        {   // set back pointer
            _Nx->Next = m_current->Next;
            m_current->Next->Previous = _Nx;
        }
        m_current->Next = _Nx;
        m_current = _Nx;
        return _Nx;
    ...
    {   // allocate and link simple node
        return LinkNode(vnew NodeBase(kind));
    }
    ...
    {   // add string node
        LinkNode(vnew NodeString<Element>);
    }

    template<class ForwardIterator, class Element, class RegExTraits>
    inline void Builder<ForwardIterator, Element, RegExTraits>::AddChar(Element ch)
    {   // append character
        if (m_current->Kind != NT_str || m_current->Flags & NFLG_final)
        {
            AddStringNode();
        }
    ...
    {   // add bracket expression node
        LinkNode(vnew NodeClass<Element, RegExTraits>);
    }
    ...
    {   // append character to character array
        ...
        if (!node->Large)
        {
            node->Large = vnew RegExBuffer<Element>;
        }
        node->Large->Insert(ch);
    }

    template<class ForwardIterator, class Element, class RegExTraits>
    inline void Builder<ForwardIterator, Element, RegExTraits>::AddCharToClass(Element ch)
    {   // add character to bracket expression
        static int _max = BITMAP_max;  // to quiet diagnostics
        static int _min = 0;

        if (_min <= static_cast<int>(ch) && static_cast<int>(ch) < _max)
        {
            AddCharToBitmap(ch);
        }
        else
    ...
    V_FORCE_INLINE bool IsGreaterEqualToZero(char ch)      { return ch >= 0; }
    V_FORCE_INLINE bool IsGreaterEqualToZero(wchar_t ch)   { (void)ch; return true; }
    ...
    {   // add character range to set
        ...
        {       // store remaining range as pair
            if (!node->Ranges)
            {
                node->Ranges = vnew RegExBuffer<Element>;
            }
            node->Ranges->Insert(_E0);
            node->Ranges->Insert(e1);
        }
    }
    ...
    {   // add contents of named class to bracket expression
        /*static*/
        int max = BITMAP_max;             // to quiet diagnostics
        NodeClass<Element, RegExTraits>* node = (NodeClass<Element, RegExTraits>*)m_current;
        AddElts(node, _Cl, m_traits);
        if (static_cast<Element>(max) < (std::numeric_limits<Element>::max)())
        {
    ...
    {   // add collation element to element sequence
        while (*current && (unsigned int)diff < (*current)->Size)
        {
            current = &(*current)->Next;
        }
        if (!(*current) || (unsigned int)diff != (*current)->Size)
        {   // add new sequence holding elements of the same length
            RegExSequence<Element>* node = *current;
            *current = vnew RegExSequence<Element>((unsigned int)diff);
            (*current)->Next = node;
        }
        (*current)->Data.Insert(first, last);
    }
    ...
    {   // add elements of equivalence class to bracket expression
        ...
        if (static_cast<Element>(max) < (std::numeric_limits<Element>::max)())
        {   // map range
            RegExSequence<Element>** current = &node->Equiv;
    ...
    {   // add collation element to bracket expression
        NodeClass<Element, RegExTraits>* node = (NodeClass<Element, RegExTraits>*)m_current;
        RegExSequence<Element>** current = &node->Coll;
        CharToElts(first, last, diff, current);
    }
    ...
    {   // add end of group node
        NodeType _Elt = back->Kind == NT_group ? NT_end_group
            : back->Kind == NT_assert ? NT_end_assert
            : back->Kind == NT_neg_assert ? NT_end_assert
            : NT_end_capture;
        LinkNode(vnew NodeEndGroup(_Elt, NFLG_none, back));
    }

    template<class ForwardIterator, class Element, class RegExTraits>
    inline NodeBase* Builder<ForwardIterator, Element, RegExTraits>::BeginAssertGroup(bool _Neg)
    {   // add assert node
        NodeAssert* _Node1 = vnew NodeAssert(_Neg
                ? NT_neg_assert : NT_assert);

        NodeBase* _Node2;
        _Node2 = vnew NodeBase(NT_nop);
        LinkNode(_Node1);
        _Node1->Child = _Node2;
    ...
    template<class ForwardIterator, class Element, class RegExTraits>
    inline NodeBase* Builder<ForwardIterator, Element, RegExTraits>::BeginCaptureGroup(unsigned int idx)
    {   // add capture group node
        return (LinkNode(vnew NodeCapture(idx)));
    }

    template<class ForwardIterator, class Element, class RegExTraits>
    inline void Builder<ForwardIterator, Element, RegExTraits>::AddBackreference(unsigned int idx)
    {   // add back reference node
        LinkNode(vnew NodeBack(idx));
    }
    ...
    inline NodeBase* Builder<ForwardIterator, Element, RegExTraits>::BeginIf(NodeBase* _Start)
    {
        // add if node
        /* append endif node */
        NodeBase* result = vnew NodeEndif;
        LinkNode(result);

        /* insert if_node */
        NodeIf* _Node1 = vnew NodeIf(result);
        NodeBase* position = _Start->Next;
        _Node1->Previous = position->Previous;
        position->Previous->Next = _Node1;
        _Node1->Next = position;
        position->Previous = _Node1;
        return result;
    }
    ...
    {   // add else node
        NodeIf* parent = (NodeIf*)_Start->Next;
        NodeBase* first = end->Next;
        end->Next = 0;
        ...
        parent->Child = vnew NodeIf(end);
        parent->Child->Next = first;
        first->Previous = parent->Child;
    }

    template<class ForwardIterator, class Element, class RegExTraits>
    inline void Builder<ForwardIterator, Element, RegExTraits>::AddRep(int min, int max, bool isGreedy)
    {   // add repeat node
        if (m_current->Kind == NT_str && ((NodeString<Element>*)m_current)->Data.Size() != 1)
        {   // move final character to new string node
            NodeString<Element>* node = (NodeString<Element>*)m_current;
            AddChar(node->Data.Delete());
        }
        NodeBase* position = m_current;
        NodeEndRepetition* node0 = vnew NodeEndRepetition();
        NodeRepetition* nodeRep = vnew NodeRepetition(isGreedy, min, max, node0, m_root->Loops++);
        node0->BeginRep = nodeRep;
        LinkNode(node0);
        if (position->Kind == NT_end_group || position->Kind == NT_end_capture)
        {
            position = ((NodeEndGroup*)position)->Back;
        }
        position->Previous->Next = nodeRep;
        nodeRep->Previous = position->Previous;
        position->Previous = nodeRep;
        nodeRep->Next = position;
    }
    ...
    {   // apply if node
        ...
        {   // process one branch of if
            ...
            {   // record match
                int length = (int)VStd:: distance(start.m_current, m_tgtState.Current);
    ...
    {   // apply repetition to loop with no nested if/do
        ...
        for (; index < node->Min; ++index)
        {       // do minimum number of reps
            Iterator current = m_tgtState.Current;
            if (!_Match_pat(node->Next))
            {       // didn't match minimum number of reps, fail
                m_tgtState = _St;
                return (false);
            }
            else if (current == m_tgtState.Current)
            {
                index = node->Min - 1;    // skip matches that don't change state
            }
        }

        TgtState<Iterator> _Final = m_tgtState;
        bool isMatched = false;
        Iterator _Saved_pos = m_tgtState.Current;

        if (!_Match_pat(node->EndRep->Next))
        {
            ;   // no full match yet
        }
        else if (!isGreedy)
        ...
        while (node->Max == -1 || index++ < node->Max)
        {       // try another rep/tail match
            m_tgtState.Current = _Saved_pos;
            m_tgtState.GroupValid = _St.GroupValid;
            if (!_Match_pat(node->Next))
            {
                break;  // rep match failed, quit loop
            }
            Iterator _Mid = m_tgtState.Current;
            if (!_Match_pat(node->EndRep->Next))
            {
    ...
    {   // apply repetition
        if (node->SampleLoop < 0)
        {       // first time, determine if loop is simple
            NodeBase* _Nx = node->Next;
            for (; _Nx->Kind != NT_end_rep; _Nx = _Nx->Next)
            {
                if (_Nx->Kind == NT_if || _Nx->Kind == NT_rep)
                {
                    break;
                }
            }
            node->SampleLoop = _Nx->Kind == NT_end_rep ? 1 : 0;
        }
        if (0 <  node->SampleLoop)
        {
        ...
        bool isMatched = false;
        TgtState<Iterator> _St = m_tgtState;
        LoopValues* _Psav = &m_loopVals[node->LoopNumber];
        int _Loop_idx_sav = _Psav->LoopIdx;
        Iterator* _Loop_iter_sav = (Iterator*)_Psav->LoopIter;
        Iterator _Cur_iter = m_tgtState.Current;

        bool _Progress = _Init_idx == 0 || *_Loop_iter_sav != _Cur_iter;

        if (0 <= node->Max && node->Max <= _Init_idx)
        {
            isMatched = _Match_pat(node->EndRep->Next); // reps done, try tail
        }
        else if (_Init_idx < node->Min)
        {       // try a required rep
            if (!_Progress)
            {
                isMatched = _Match_pat(node->EndRep->Next); // empty, try tail
            }
            else
            {       // try another required match
                _Psav->LoopIdx = _Init_idx + 1;
                _Psav->LoopIter = &_Cur_iter;
                isMatched = _Match_pat(node->Next);
            }
        }
        else if (!isGreedy)
        {       // not greedy, favor minimum number of reps
            isMatched = _Match_pat(node->EndRep->Next);
        ...
        {       // greedy, favor maximum number of reps
            ...
            {       // rep failed, try tail
                _Psav->LoopIdx = _Loop_idx_sav;
                _Psav->LoopIter = _Loop_iter_sav;
                m_tgtState = _St;
                isMatched = _Match_pat(node->EndRep->Next);
            }
        }
    ...
    {   // check whether ch is in _Eq
        ...
        {   // look for sequence of elements that are the right size
            for (int index = 0; index < _Eq->Data.Size(); index += _Eq->Size)
            {   // look for ch
                _Str0.assign(_Eq->Data.String() + index, _Eq->Size);
    ...
    {   // look for collation element [first, last) in _Eq
        ...
        {   // look for sequence of elements that are the right size
            ...
            {   // look for character range
                ...
                for (size_t _Jx = 0; _Jx < _Eq->Size; ++_Jx)
                {   // check current character
                    if (*result++ != *(_Eq->Data.String() + index + _Jx))
                    {
                        break;
                    }
                }
    ...
    {   // apply bracket expression
        /*static*/
        int _max = BITMAP_max;             // to quiet diagnostics
        /*static*/ int _min = 0;

        bool _Found;
        Element ch = *m_tgtState.Current;
        if (m_sflags & regex_constants::icase)
        {
            ch = m_traits.translate_nocase(ch);
        }
        Iterator result = m_tgtState.Current;
        ++result;
        Iterator _Resx;
        NodeClass<Element, RegExTraits>* node = (NodeClass<Element, RegExTraits>*)_Nx;
        if (node->Coll && (_Resx = _Lookup_coll(m_tgtState.Current, m_end, node->Coll))
            != m_tgtState.Current)
        {   // check for collation element
            result = _Resx;
            _Found = true;
        }
        else if (node->Ranges && (_Lookup_range((Element)(m_sflags & regex_constants::collate ? (int)m_traits.translate(ch) : (int)ch), node->Ranges)))
        {
            _Found = true;
        }
        else if (_min <= static_cast<int>(ch) && static_cast<int>(ch) < _max)
        {
            _Found = node->Small && node->Small->Find(ch);
        }
        else if (node->Large && VStd::find(node->Large->String(), node->Large->String() + node->Large->Size(), ch) != node->Large->String() + node->Large->Size())
        {
            _Found = true;
        }
        ...
        else if (node->Equiv && _Lookup_equiv(ch, node->Equiv, m_traits))
        {
            _Found = true;
        }
        else
        ...
        if (_Found == (node->Flags & NFLG_negate))
        {
            return false;
        }
        else
        {   // record result
            m_tgtState.Current = result;
            return true;
        }
    }
    ...
    {   // check for better match under UNIX rules
        ...
        {       // check each capture group
            if (!m_result.GroupValid[index] || !m_tgtState.GroupValid[index])
            {
                ;
            }
            else if (m_result.Groups[index].m_begin
                     != m_tgtState.Groups[index].m_begin)
            {
                return (VStd::distance(m_begin, m_result.Groups[index].m_begin)
                        < VStd::distance(m_begin, m_tgtState.Groups[index].m_begin));
            }
            else if (m_result.Groups[index].m_end
                     != m_tgtState.Groups[index].m_end)
            {
                return (VStd::distance(m_begin, m_result.Groups[index].m_end)
                        < VStd::distance(m_begin, m_tgtState.Groups[index].m_end));
            }
        }
        return false;
    }
    ...
    {   // check for match
        ...
        {       // match current node
            ...
            case NT_bol:
                    ...
                    == regex_constants::match_not_bol)
                {
                    isFailed = true;
                         ...
                         || m_tgtState.Current != m_begin)
                {           // check for preceding newline
                    Iterator tmp = m_tgtState.Current;
                    isFailed = *--tmp != Meta_nl;
                }
                break;

            case NT_eol:
                if ((m_mflags& regex_constants::match_not_eol) != 0 || (m_tgtState.Current != m_end && *m_tgtState.Current != Meta_nl))
                {
                    isFailed = true;
                }
                break;
            ...
            {           // check for word boundary
                bool _Is_bound;
                if (((m_mflags& regex_constants::match_not_bow) && m_tgtState.Current == m_begin) || ((m_mflags& regex_constants::match_not_eow) && m_tgtState.Current == m_end))
                {
                    _Is_bound = false;
                }
                else
                {
                    _Is_bound = _Is_wc(m_tgtState.Current, -1) != _Is_wc(m_tgtState.Current, 0);
                }
                bool _Neg = (_Nx->Flags & NFLG_negate) != 0;
                if (_Is_bound == _Neg)
                {
                    isFailed = true;
                }
            ...
            case NT_dot:
                if (m_tgtState.Current == m_end || *m_tgtState.Current == Meta_nl || *m_tgtState.Current == Meta_cr)
                {
                    isFailed = true;
                }
                else
            ...
            {           // check for string match
                ...
                Iterator result;
                if ((result = _Compare(m_tgtState.Current, m_end,
                             node->Data.String(),
                             node->Data.String() + node->Data.Size(),
                             m_traits, m_sflags,
            ...
            {           // check for bracket expression match
                isFailed = m_tgtState.Current == m_end
                    || !_Do_class(_Nx);
                break;
            }
            ...
            case NT_assert:
            {           // check assert
                Iterator ch = m_tgtState.Current;
                bool _Neg = _Nx->Kind == NT_neg_assert;
                BackTracingState<Iterator> _St = m_tgtState;
                if (_Match_pat(((NodeAssert*)_Nx)->Child) == _Neg)
            ...
            {           // record current position
                ...
                m_tgtState.Groups[node->Index].m_begin = m_tgtState.Current;
                for (size_t index = m_tgtState.GroupValid.size();
                     node->Index < index; )
                {
                    m_tgtState.GroupValid[--index] = false;
                }
                break;
            }
            ...
            {           // record successful capture
                ...
                NodeCapture* node0 = (NodeCapture*)node->Back;
                if (_Cap || node0->Index != 0)
                {           // update capture data
                    m_tgtState.GroupValid[node0->Index] = true;
                    m_tgtState.Groups[node0->Index].m_end = m_tgtState.Current;
                }
                break;
            }
            ...
            {           // check back reference
                ...
                if (m_tgtState.GroupValid[node->Index])
                {           // check for match
                    Iterator result = m_tgtState.Current;
                    Iterator _Bx = m_tgtState.Groups[node->Index].m_begin;
                    Iterator _Ex = m_tgtState.Groups[node->Index].m_end;
                    if (_Bx != _Ex      // _Bx == _Ex for zero-length match
                        && (result = _Compare(m_tgtState.Current, m_end,
            ...
            {           // return at end of loop
                NodeRepetition* _Nr = ((NodeEndRepetition*)_Nx)->BeginRep;
                LoopValues* _Psav = &m_loopVals[_Nr->LoopNumber];

                if (_Nr->SampleLoop == 0 && !_Do_rep(_Nr,
                        (_Nr->Flags & NFLG_greedy) != 0, _Psav->LoopIdx))
                {
            ...
            case NT_end:
                if (((m_mflags & (regex_constants::match_not_null | regex_constants::match_not_null1)) && m_begin == m_tgtState.Current) || (m_isFull && m_tgtState.Current != m_end))
                {
                    isFailed = true;
                }
        ...
        {   // check current node
            ...
            {       // check for string match
                ...
                {           // look for starting match
                    ...
                    if (node->Coll
                        && _Lookup_coll(first, next, node->Coll)
                        != first)
                    {
                        _Found = true;
                    }
                    ...
                    else if (_min <= ch && ch < _max)
                    {
                        _Found = node->Small && node->Small->Find(ch);
                    }
                    else if (node->Large
                             && VStd::find(node->Large->String(), node->Large->String() + node->Large->Size(), ch)
                             != node->Large->String() + node->Large->Size())
                    {
                        _Found = true;
                    }
                             ...
                             && m_traits.isctype(ch, node->_Classes))
                    {
                        _Found = true;
                             ...
                             && _Lookup_equiv(ch, node->Equiv, m_traits))
                    {
                        _Found = true;
                    }
                    else
            ...
            {       // check for soonest string match
                ...
                for (; first != last && node != 0; node = node->Child)
                {
                    last = Skip(first, last, node->Next);
                }
                return last;
            }
    ...
    {   // handle delimited expressions within bracket expression
        ...
        V_REGEX_DIFFT(ForwardIterator) diff = 0;
    ...
    {   // check for character class escape
        typename RegExTraits::char_class_type _Cls;
        ForwardIterator _Ch0 = m_pat;
        if (_Ch0 == m_end || (_Cls = m_traits.lookup_classname(m_pat, ++_Ch0, (Flags& regex_constants::icase) != 0)) == 0)
        {
            return false;
        }
    ...
    {   // add capture group
        ...
        m_finishedGroups[((NodeCapture*)_Pos1)->Index] = true;
    }
    ...
    {   // add disjunction inside group
        ...
        else if (Flags & regex_constants::nosubs)
        {
            _Do_noncapture_group();
        }
        else
    ...
    {   // check for quantifier following atom
        ...
        {       // check for valid bracketed value
            ...
            if (m_metaChar != Meta_rbr || (max != -1 && max < min))
            {
                Error(regex_constants::error_badbrace);
            }
        }
        ...
        {       // add non-greedy repeat node
            Next();
            m_builder.AddRep(min, max, false);
        }
        else
        {
            m_builder.AddRep(min, max, true);
        }
    }
        ...
        , m_errors(errors)
    {
        ...
        m_lflags = (Flags & gmask) == ECMAScript || (Flags & gmask) == 0 ? _ECMA_flags
            : (Flags & gmask) == basic ? _Basic_flags
            : (Flags & gmask) == extended ? _Extended_flags
            : (Flags & gmask) == awk ? _Awk_flags
            : (Flags & gmask) == grep ? _Grep_flags
            : (Flags & gmask) == egrep ? _Egrep_flags
            : 0;
...
#endif // V_FRAMEWORK_CORE_STD_STRING_REGEX_H