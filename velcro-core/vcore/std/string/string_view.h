 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_STRING_STRING_VIEW_H
#define V_FRAMEWORK_CORE_STD_STRING_STRING_VIEW_H
...
namespace VStd {
    namespace StringInternal {
        ...
        constexpr SizeT char_find(const CharT* s, size_t count, CharT ch, SizeT npos = static_cast<SizeT>(-1)) noexcept
        {
            ...
            return foundIter ? VStd::distance(s, foundIter) : npos;
        }
    ...
    struct char_traits
    {
        ...
        using int_type = VStd::conditional_t<VStd::is_same_v<Element, char>, int,
            VStd::conditional_t<VStd::is_same_v<Element, wchar_t>, wint_t, int>
        >;
...
#if !defined(V_COMPILER_MSVC) || V_COMPILER_MSVC < 1915 || V_COMPILER_MSVC > 1916
            if constexpr (VStd::is_same_v<char_type, char>)
            {
                return __builtin_memcmp(s1, s2, count);
            }
            else if constexpr (VStd::is_same_v<char_type, wchar_t>)
            {
        ...
        {
            V_Assert(dest != nullptr && src != nullptr, "Invalid input!");
            if (count == 0)
            {
                return dest;
            }
     ...
     */
    template <class Element, class Traits = VStd::char_traits<Element>>
    ...
    public:
        ...
        template <typename It, typename End, typename = VStd::enable_if_t<
            Internal::satisfies_contiguous_iterator_concept_v<It>
            && is_same_v<typename VStd::iterator_traits<It>::value_type, value_type>
            && !is_convertible_v<End, size_type>>
        >
        constexpr basic_string_view(It first, End last)
            : m_begin(VStd::to_address(first))
            , m_size(VStd::to_address(last) - VStd::to_address(first))
        {}
        ...
        constexpr basic_string_view(VStd::nullptr_t) = delete;
        ...
        {
            V_Assert(index < size(), "pos value is out of range");
            return index >= size() ? value_type(0) : data()[index];
        }

        constexpr const_reference front() const
        {
            V_Assert(!empty(), "string_view::front(): string is empty");
            return data()[0];
        }

        constexpr const_reference back() const
        {
            V_Assert(!empty(), "string_view::back(): string is empty");
        ...
        {
            V_Assert(n <= size(), "Attempting to remove prefix larger than string size");
            m_begin += n;
            m_size -= n;
        }
        ...
        {
            V_Assert(n <= size(), "Attempting to remove suffix larger than string size");
            m_size -= n;
        }
        ...
        {
            V_Assert(pos <= size(), "Position of bytes to copy to destination is out of string_view range");
            if (pos > size())
            {
                return 0;
            }
        ...
        {
            V_Assert(pos <= size(), "Cannot create substring where position is larger than size");
        ...
        {
            V_Assert(count == 0 || s, "string_view::find(): received nullptr");
            return find(basic_string_view{ s, count }, pos);
        }

        constexpr size_type find(const_pointer s, size_type pos = 0) const
        {
            V_Assert(s, "string_view::find(): received nullptr");
        ...
        {
            V_Assert(count == 0 || s, "string_view::rfind(): received nullptr");
            return rfind(basic_string_view{ s, count }, pos);
        }

        constexpr size_type rfind(const_pointer s, size_type pos = npos) const
        {
            V_Assert(s, "string_view::rfind(): received nullptr");
        ...
        {
            V_Assert(count == 0 || s, "string_view::find_first_of(): received nullptr");
            return find_first_of(basic_string_view{ s, count }, pos);
        }

        constexpr size_type find_first_of(const_pointer s, size_type pos = 0) const
        {
            V_Assert(s, "string_view::find_first_of(): received nullptr");
        ...
        {
            V_Assert(count == 0 || s, "string_view::find_last_of(): received nullptr");
            return find_last_of(basic_string_view{ s, count }, pos);
        }

        constexpr size_type find_last_of(const_pointer s, size_type pos = npos) const
        {
            V_Assert(s, "string_view::find_last_of(): received nullptr");
        ...
        {
            V_Assert(count == 0 || s, "string_view::find_first_not_of(): received nullptr");
            return find_first_not_of(basic_string_view{ s, count }, pos);
        }

        constexpr size_type find_first_not_of(const_pointer s, size_type pos = 0) const
        {
            V_Assert(s, "string_view::find_first_not_of(): received nullptr");
        ...
        {
            V_Assert(count == 0 || s, "string_view::find_last_not_of(): received nullptr");
            return find_last_not_of(basic_string_view{ s, count }, pos);
        }

        constexpr size_type find_last_not_of(const_pointer s, size_type pos = npos) const
        {
            V_Assert(s, "string_view::find_last_not_of(): received nullptr");
...
#define V_STRING_FORMAT "%.*s"

#endif // V_FRAMEWORK_CORE_STD_STRING_STRING_VIEW_H