 ...
 *
 */
#ifndef V_FRAMEWORK_CORE_STD_UTILS_H
#define V_FRAMEWORK_CORE_STD_UTILS_H 1
...
#include <vcore/std/reference_wrapper.h>
#include <vcore/std/typetraits/remove_reference.h>
#include <vcore/std/typetraits/is_convertible.h>
#include <vcore/std/typetraits/is_lvalue_reference.h>
...
namespace VStd {
    ...
    // rvalue move
    template<class T>
    constexpr VStd::remove_reference_t<T>&& move(T && t)
    {
        return static_cast<VStd::remove_reference_t<T>&&>(t);
    }
    ...
    V_FORCE_INLINE void                    iter_swap(ForwardIterator1 a, ForwardIterator2 b) {
        VStd::swap(*a, *b);
    }

    template<class ForwardIterator1, class ForwardIterator2>
    V_FORCE_INLINE ForwardIterator2 swap_ranges(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2) {
        for (; first1 != last1; ++first1, ++first2) {
            VStd::iter_swap(first1, first2);
        }
    ...
    struct pair {   
        ...
        using TT1 = VStd::remove_reference_t<T1>;
        using TT2 = VStd::remove_reference_t<T2>;

        constexpr pair(TT1&& value1, TT2&& value2)
            : first(VStd::move(value1))
            , second(VStd::move(value2)) {}
        ...
        constexpr pair(Other1&& value1, Other2&& value2)
            : first(VStd::forward<Other1>(value1))
            , second(VStd::forward<Other2>(value2)) {}
        constexpr pair(pair&& rhs)
            : first(VStd::move(rhs.first))
            , second(VStd::move(rhs.second)) {}
        template<class Other1, class Other2>
        constexpr pair(pair<Other1, Other2>&& rhs)
            : first(VStd::forward<Other1>(rhs.first))
            , second(VStd::forward<Other2>(rhs.second)) {}
        ...
        constexpr pair(piecewise_construct_t, TupleType<Args1...>& first_args,
            TupleType<Args2...>& second_args, VStd::index_sequence<I1...>,
            VStd::index_sequence<I2...>);
        ...
        void swap(this_type& rhs) {
            ...
            if (this != &rhs) {   // different, worth swapping
                VStd::swap(first, rhs.first);
                VStd::swap(second, rhs.second);
            }
        }
    ...
    // pair
    template<class T1, class T2>
    constexpr void swap(VStd::pair<T1, T2>& left, VStd::pair<T1, T2>& _Right) {   // swap _Left and _Right pairs
        left.swap(_Right);
    }

    template<class L1, class L2, class R1, class R2, class = VStd::void_t<decltype(declval<L1>() == declval<R1>() && declval<L2>() == declval<R2>())>>
    constexpr bool operator==(const pair<L1, L2>& left, const pair<R1, R2>& right) {   // test for pair equality
        return left.first == right.first && left.second == right.second;
    }

    template<class L1, class L2, class R1, class R2, class = VStd::void_t<decltype(declval<L1>() == declval<R1>() && declval<L2>() == declval<R2>())>>
    constexpr bool operator!=(const pair<L1, L2>& left, const pair<R1, R2>& right) {   // test for pair inequality
        return !(left == right);
    }

    template<class L1, class L2, class R1, class R2, class = VStd::void_t<decltype(declval<L1>() < declval<R1>() || (!(declval<R1>() < declval<L1>()) && declval<L2>() < declval<R2>()))>>
    constexpr bool operator<(const pair<L1, L2>& left, const pair<R1, R2>& right) {   // test if left < right for pairs
    ...
    namespace Internal {
        ...
        struct addressof_impl {
            static V_FORCE_INLINE T* f(T& v, long) { return reinterpret_cast<T*>(&const_cast<char&>(reinterpret_cast<const volatile char&>(v))); }
    ...
    // make_pair
    template<class T1, class T2>
    constexpr pair<typename VStd::unwrap_reference<T1>::type, typename VStd::unwrap_reference<T2>::type>
    make_pair(T1&& value1, T2&& value2) {
        typedef pair<typename VStd::unwrap_reference<T1>::type, typename VStd::unwrap_reference<T2>::type> pair_type;
        return pair_type(VStd::forward<T1>(value1), VStd::forward<T2>(value2));
    }
    ...
    make_pair(const T1& value1, T2&& value2) {
        typedef pair<typename VStd::unwrap_reference<T1>::type, typename VStd::unwrap_reference<T2>::type> pair_type;
        return pair_type((typename VStd::unwrap_reference<T1>::type)value1, VStd::forward<T2>(value2));
    }
    ...
    make_pair(T1&& value1, const T2& value2) {
        typedef pair<typename VStd::unwrap_reference<T1>::type, typename VStd::unwrap_reference<T2>::type> pair_type;
        return pair_type(VStd::forward<T1>(value1), (typename VStd::unwrap_reference<T2>::type)value2);
    }
    template<class T1, class T2>
    constexpr pair<typename VStd::unwrap_reference<T1>::type, typename VStd::unwrap_reference<T2>::type>
    make_pair(const T1& value1, const T2& value2) {
        typedef pair<typename VStd::unwrap_reference<T1>::type, typename VStd::unwrap_reference<T2>::type> pair_type;
        return pair_type((typename VStd::unwrap_reference<T1>::type)value1, (typename VStd::unwrap_reference<T2>::type)value2);
    }
    //////////////////////////////////////////////////////////////////////////
...
#endif // V_FRAMEWORK_CORE_STD_UTILS_H