 ...
 *
 */
#ifndef V_FRAMEWORK_CORE_STD_SORT_H
#define V_FRAMEWORK_CORE_STD_SORT_H 1
...
namespace VStd
{
    ...
    */
    ...
    namespace Internal
    {
        ...
        public:
            ...
            V_FORCE_INLINE TemporaryBuffer(size_type capacity)
                : m_data(0)
                , m_capacity(capacity)
                , m_numConstructed(0)
                , m_size(0)
            ...
            V_FORCE_INLINE TemporaryBuffer(size_type capacity, const Allocator& allocator)
            ...
            V_FORCE_INLINE ~TemporaryBuffer()
            {
            ...
            V_FORCE_INLINE size_type   capacity() const    { return m_capacity; }
            ...
            V_FORCE_INLINE iterator begin()        { return m_data; }
            V_FORCE_INLINE iterator end()      { return m_data + m_size; }
    ...
    void merge_buffered(BidirectionalIterator first, BidirectionalIterator mid, BidirectionalIterator last, Difference count1, Difference count2, Buffer& buffer, Compare comp)
    {
        ...
        {   // buffer too small, divide and conquer
            ...
            {   // left larger, cut it in half and partition right to match
                ...
                firstn = first;
                VStd::advance(firstn, count1n);
                lastn = VStd::lower_bound(mid, last, *firstn, comp);
                count2n = VStd::distance(mid, lastn);
            }
            else
            {   // right larger, cut it in half and partition left to match
                count1n = 0, count2n = count2 / 2;
                lastn = mid;
                VStd::advance(lastn, count2n);
                firstn = VStd::upper_bound(first, mid, *lastn, comp);
                count1n = VStd::distance(first, firstn);
            }
    ...
    BidirectionalIterator buffered_rotate(BidirectionalIterator first, BidirectionalIterator mid, BidirectionalIterator last, Difference count1, Difference count2, Buffer& buffer)
    {
        ...
        {   // buffer too small, rotate in place
            VStd::rotate(first, mid, last);
            VStd::advance(first, count2);
            return first;
        }
    }
    //////////////////////////////////////////////////////////////////////////
    ...
    {
        VStd::make_heap(first, middle, comp);
        for (RandomAccessIterator next = middle; next < last; ++next)
        {
            if (comp(*next, *first))
            {
                VStd::pop_heap(first, middle, next, comp);
            }
        }
        VStd::sort_heap(first, middle, comp);
    }

    template <class RandomAccessIterator>
    void partial_sort(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last)
    {
        VStd::make_heap(first, middle);
        for (RandomAccessIterator next = middle; next < last; ++next)
        {
            if (*next < *first)
            {
                VStd::pop_heap(first, middle, next);
            }
        }
        VStd::sort_heap(first, middle);
    }
    //////////////////////////////////////////////////////////////////////////
        ...
        if (first != last)
        {
            for (RandomAccessIterator next = first; ++next != last; )
            {
                ...
                if (value < *first)
                {
                    ...
                    VStd::Internal::move_backward(first, next, ++next1, Internal::is_fast_copy<RandomAccessIterator, RandomAccessIterator>());
    ...
    */
    ...
    {
        template<class RandomAccessIterator>
        V_INLINE void med3(RandomAccessIterator first, RandomAccessIterator mid, RandomAccessIterator last)
        {   // sort median of three elements to middle
            if (*mid < *first)
            {
                VStd::iter_swap(mid, first);
            }
            if (*last < *mid)
            {
                VStd::iter_swap(last, mid);
            }
            if (*mid < *first)
            {
                VStd::iter_swap(mid, first);
        ...
        template<class RandomAccessIterator, class Compare>
        V_INLINE void med3(RandomAccessIterator first, RandomAccessIterator mid, RandomAccessIterator last, Compare comp)
        {   // sort median of three elements to middle
            if (comp(*mid, *first))
            {
                VStd::iter_swap(mid, first);
        ...
        template<class RandomAccessIterator>
        V_INLINE void median(RandomAccessIterator first, RandomAccessIterator mid, RandomAccessIterator last)
        {   // sort median element to middle
            if (40 < last - first)
            {   // median of nine
                VStd::size_t step = (last - first + 1) / 8;
        ...
        template<class RandomAccessIterator, class Compare>
        V_INLINE void median(RandomAccessIterator first, RandomAccessIterator mid, RandomAccessIterator last, Compare comp)
        {   // sort median element to middle
            if (40 < last - first)
            {   // median of nine
                VStd::size_t step = (last - first + 1) / 8;
        ...
        template<class RandomAccessIterator>
        VStd::pair<RandomAccessIterator, RandomAccessIterator> unguarded_partition(RandomAccessIterator first, RandomAccessIterator last)
        {   // partition [_First, _Last), using operator<
            ...
            {   // partition
                ...
                for (; first < glast; --glast)
                {
                    ...
                    {
                        VStd::iter_swap(--pfirst, glast - 1);
                    }
                }
                if (glast == first && gfirst == last)
                {
                    return VStd::pair<RandomAccessIterator, RandomAccessIterator>(pfirst, plast);
                }
                ...
                {           // no room at bottom, rotate pivot upward
                    if (plast != gfirst)
                    {
                        VStd::iter_swap(pfirst, plast);
                    }
                    ++plast;
                    VStd::iter_swap(pfirst++, gfirst++);
                }
                else if (gfirst == last)
                {           // no room at top, rotate pivot downward
                    if (--glast != --pfirst)
                    {
                        VStd::iter_swap(glast, pfirst);
                    }
                    VStd::iter_swap(pfirst, --plast);
                }
                else
                {
                    VStd::iter_swap(gfirst++, --glast);
                }
            }
        }

        template<class RandomAccessIterator, class Compare>
        VStd::pair<RandomAccessIterator, RandomAccessIterator> unguarded_partition(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
        {   // partition [_First, _Last), using operator<
            RandomAccessIterator mid = first + (last - first) / 2;  // sort median to mid
        ...
        void sort_recursive(RandomAccessIterator first, RandomAccessIterator last, Difference ideal)
        {
            ...
            {   // divide and conquer by quicksort
                VStd::pair<RandomAccessIterator, RandomAccessIterator> mid = VStd::Internal::unguarded_partition(first, last);
            ...
            {   // heap sort if too many divisions
                VStd::make_heap(first, last);
                VStd::sort_heap(first, last);
            }
            else if (count > 1)
            {
                VStd::insertion_sort(first, last); // small
            }
        }
        ...
        void sort_recursive(RandomAccessIterator first, RandomAccessIterator last, Difference ideal, Compare comp)
        {
            ...
            for (; AZSTD_MAX_INSERTION_SORT_SIZE < (count = last - first) && 0 < ideal; )
            {   // divide and conquer by quicksort
                VStd::pair<RandomAccessIterator, RandomAccessIterator> mid = VStd::Internal::unguarded_partition(first, last, comp);
                ideal /= 2, ideal += ideal / 2; // allow 1.5 log2(N) divisions
            ...
            if (count > AZSTD_MAX_INSERTION_SORT_SIZE)
            {   // heap sort if too many divisions
                VStd::make_heap(first, last, comp);
                VStd::sort_heap(first, last, comp);
            }
            else if (count > 1)
            {
                VStd::insertion_sort(first, last, comp);   // small
            }
        }
        ...
        void chunked_merge(BidirectionalIterator first, BidirectionalIterator last, OutputIterator result, DifferenceType chunk, DifferenceType count, Compare comp)
        {
            ...
            {   // copy merging pairs of adjacent chunks
                ...
                VStd::advance(mid1, chunk);
                BidirectionalIterator mid2 = mid1;
                VStd::advance(mid2, chunk);

                result = VStd::merge(first, mid1, mid1, mid2, result, comp);
                first = mid2;
            }
            ...
            else
            {
                ...
                VStd::advance(mid, chunk);

                VStd::merge(first, mid, mid, last, result, comp);
            }
        }
        ...
        void merge_sort_buffered(BidirectionalIterator first, BidirectionalIterator last, DifferenceType count, Buffer& buffer, Compare comp)
        {
            ...
            for (DifferenceType nleft = count; AZSTD_MAX_INSERTION_SORT_SIZE <= nleft; nleft -= AZSTD_MAX_INSERTION_SORT_SIZE)
            {
                ...
                VStd::advance(midend, (int)AZSTD_MAX_INSERTION_SORT_SIZE);

                VStd::insertion_sort(mid, midend, comp);
                mid = midend;
            }
            VStd::insertion_sort(mid, last, comp); // sort partial last chunk
...
#endif // V_FRAMEWORK_CORE_STD_SORT_H
