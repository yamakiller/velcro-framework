#ifndef V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_CONCURRENT_FIXED_UNORDERED_MAP_H
#define V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_CONCURRENT_FIXED_UNORDERED_MAP_H
...
namespace VStd
{
    namespace Internal
    {
        template<class Key, class MappedType, class Hasher, class EqualKey, bool isMultiMap, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, VStd::size_t NumLocks>
     ...
     */
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, VStd::size_t NumLocks = 8, class Hasher = VStd::hash<Key>, class EqualKey = VStd::equal_to<Key> >
    ...
    public:
        ...
        V_FORCE_INLINE concurrent_fixed_unordered_map()
            : base_type(hasher(), key_eq()) {}
        V_FORCE_INLINE concurrent_fixed_unordered_map(const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual) {}
        template<class Iterator>
        V_FORCE_INLINE concurrent_fixed_unordered_map(Iterator first, Iterator last)
            : base_type(hasher(), key_eq())
        {
        ...
        template<class Iterator>
        V_FORCE_INLINE concurrent_fixed_unordered_map(Iterator first, Iterator last, const hasher& hash, const key_eq& keyEqual)
    ...
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, VStd::size_t NumLocks, class Hasher, class EqualKey >
    V_FORCE_INLINE void swap(concurrent_fixed_unordered_map<Key, MappedType, FixedNumBuckets, FixedNumElements, NumLocks, Hasher, EqualKey>& left, concurrent_fixed_unordered_map<Key, MappedType, FixedNumBuckets, FixedNumElements, NumLocks, Hasher, EqualKey>& right)
    {
        left.swap(right);
    }
    ...
    public:
        ...
        V_FORCE_INLINE concurrent_fixed_unordered_multimap()
            : base_type(hasher(), key_eq()) {}
        V_FORCE_INLINE concurrent_fixed_unordered_multimap(const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual) {}
        template<class Iterator>
        V_FORCE_INLINE concurrent_fixed_unordered_multimap(Iterator first, Iterator last)
            : base_type(hasher(), key_eq())
        {
            for (; first != last; ++first)
            {
        ...
        template<class Iterator>
        V_FORCE_INLINE concurrent_fixed_unordered_multimap(Iterator first, Iterator last, const hasher& hash, const key_eq& keyEqual)
            : base_type(hash, keyEqual)
        {
            for (; first != last; ++first)
            {
    ...
    template<class Key, class MappedType, VStd::size_t FixedNumBuckets, VStd::size_t FixedNumElements, VStd::size_t NumLocks, class Hasher, class EqualKey >
    V_FORCE_INLINE void swap(concurrent_fixed_unordered_multimap<Key, MappedType, FixedNumBuckets, FixedNumElements, NumLocks, Hasher, EqualKey>& left, concurrent_fixed_unordered_multimap<Key, MappedType, FixedNumBuckets, FixedNumElements, NumLocks, Hasher, EqualKey>& right)
    {
        left.swap(right);
    }
} // namespace VStd

#endif // V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_CONCURRENT_FIXED_UNORDERED_MAP_H