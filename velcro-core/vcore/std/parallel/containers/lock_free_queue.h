#ifndef V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_LOCK_FREE_QUEUE_H
#define V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_LOCK_FREE_QUEUE_H
...
namespace VStd
{
    ...
    {
        V_Assert(m_allocator.is_lock_free(), "Lock-free container requires a lock-free allocator");
        //allocator must allow stale read access, as queue can access a node which has been deallocated
        V_Assert(m_allocator.is_stale_read_allowed(), "Allocator for lock_free_queue must allow stale reads");
        //allocator must not recycle while lock_free_queue operations are active, as queue is vulnerable to ABA problem
        V_Assert(m_allocator.is_delayed_recycling(),   "Allocator for lock_free_queue must not recycle"
...
#endif // V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_LOCK_FREE_QUEUE_H