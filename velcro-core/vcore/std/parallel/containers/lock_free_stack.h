#ifndef V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_LOCK_FREE_STACK_H
#define V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_LOCK_FREE_STACK_H
...
namespace VStd
{
    ...
    inline lock_free_stack<T, Allocator>::lock_free_stack()
    {
        ...
        V_Assert(m_allocator.is_stale_read_allowed(), "Allocator for lock_free_stack must allow stale reads");
        //allocator must not recycle while lock_free_stack operations are active, as pop is vulnerable to ABA problem
        V_Assert(m_allocator.is_delayed_recycling(),   "Allocator for lock_free_stack must not recycle"
...
#endif // V_FRAMEWORK_CORE_STD_PARALLEL_CONTAINERS_LOCK_FREE_STACK_H