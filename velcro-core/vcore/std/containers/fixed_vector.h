 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_FIXED_VECTOR_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_FIXED_VECTOR_H
...
namespace VStd::Internal {
    ...
    using fixed_storage_size_t =
        VStd::conditional_t<(N < std::numeric_limits<uint8_t>::max()), uint8_t,
        VStd::conditional_t<(N < std::numeric_limits<uint16_t>::max()), uint16_t,
        VStd::conditional_t<(N < std::numeric_limits<uint32_t>::max()), uint32_t,
        VStd::conditional_t<(N < std::numeric_limits<uint32_t>::max()), uint64_t, size_t
        >>>>;
    ...
    struct fixed_zero_size_storage {
        ...
        constexpr fixed_zero_size_storage(VStd::initializer_list<U> ilist) noexcept {
            VSTD_CONTAINER_ASSERT(ilist.size() == 0, "Zero size storage is only constructible with an empty initializer list");
        }
        ...
        static constexpr void emplace_back(Args&&...) noexcept {
            VSTD_CONTAINER_ASSERT(false, "emplace_back is not supported on zero-size storage");
        }
        //! Removes the last element of the storage.
        //! Always fails for zero-sized storage.
        static constexpr void pop_back() noexcept {
            VSTD_CONTAINER_ASSERT(false, "pop_back is not supported on zero-size storage");
        }
        ...
        static constexpr void resize_no_construct(size_t new_size) noexcept {
            VSTD_CONTAINER_ASSERT(new_size == 0, "Zero size storage can only be resized to zero");
        }
    ...
    struct fixed_trivial_storage {
        ...
        template <typename U, typename = enable_if_t<is_convertible_v<U, T>>>
        constexpr fixed_trivial_storage(VStd::initializer_list<U> ilist) noexcept
        {
            VSTD_CONTAINER_ASSERT(ilist.size() <= capacity(), "Initializer list cannot be larger than storage capacity");
            size_t index{};
        ...
        {
            VSTD_CONTAINER_ASSERT(!full(), "emplace_back cannot be invoked on full storage");
            reference new_element = *(data() + size());
            VStd::construct_at(&new_element, VStd::forward<Args>(args)...);
        ...
        {
            VSTD_CONTAINER_ASSERT(!empty(), "pop_back cannot be invoked on empty storage");
            resize_no_construct(size() - 1);
        }
        //! Changes the size of the storage without adding or
        //! removing elements (unsafe).
    ...
    struct fixed_non_trivial_storage
    {
        ...
        template <typename U, typename = enable_if_t<is_convertible_v<U, T>>>
        constexpr fixed_non_trivial_storage(VStd::initializer_list<U> ilist) noexcept(noexcept(emplace_back(VStd::declval<U>())))
        {
            VSTD_CONTAINER_ASSERT(ilist.size() <= capacity(), "Initializer list cannot be larger than storage capacity");
            for (const U& element : ilist)
            {
        ...
        //! Destructs elements in the range [begin, end).
        //! This does not modify the size of the storage
        //! Invokes the destuctor via the VStd::destroy method
        template <typename InputIt, typename = enable_if_t<Internal::is_input_iterator_v<InputIt>>>
        void unsafe_destroy(InputIt first, InputIt last) noexcept(is_nothrow_destructible_v<value_type>)
        {
            VSTD_CONTAINER_ASSERT(first >= data() && first <= data() + size(), "begin iterator is not in range of storage");
            VSTD_CONTAINER_ASSERT(last >= data() && last <= data() + size(), "last iterator is not in range of storage");
            VStd::destroy(first, last);
        }
        ...
        VStd::aligned_storage_for_t<T> m_data[Capacity];
...
{
    /**
     * This is the fixed capacity version of the \ref VStd::vector. All of the functionality is the same
     ...
     * Check the fixed_vector \ref VStdExamples.
     */
    template< class T, VStd::size_t Capacity>
    class fixed_vector
    ...
    public:
        ...
        using reverse_iterator = VStd::reverse_iterator<iterator>;
        using const_reverse_iterator = VStd::reverse_iterator<const_iterator>;
        ...
        constexpr explicit fixed_vector(size_type numElements, const_reference value = value_type())
        {
            ...
            VStd::uninitialized_fill_n(data(), numElements, value);
        }

        template <class InputIt, typename = VStd::enable_if_t<Internal::is_input_iterator_v<InputIt>>>
        constexpr fixed_vector(InputIt first, InputIt last)
        {
            resize_no_construct(VStd::distance(first, last));
            VStd::uninitialized_copy(first, last, data());
        }
        ...
        constexpr fixed_vector(const fixed_vector& rhs)
        {
            ...
            VStd::uninitialized_copy(rhs.data(), rhs.data() + rhs.size(), data());
        }

        // The move constructor is linear in assignment operations being O(n)
        // It performs an VStd::move on each of the fixed_vector elements instead
        ...
        {
            resize_no_construct(rhs.size());
            VStd::uninitialized_move(rhs.data(), rhs.data() + rhs.size(), data());
        ...
        template <typename VectorContainer, typename = VStd::enable_if_t<!VStd::is_same_v<VectorContainer, fixed_vector>
            && !VStd::is_convertible_v<VectorContainer, size_type>>>
        constexpr fixed_vector(VectorContainer&& rhs)
        {
            constexpr bool is_const_or_lvalue_reference = VStd::is_lvalue_reference_v<VectorContainer>
                || VStd::is_const_v<VectorContainer>;
        ...
        constexpr fixed_vector(VStd::initializer_list<value_type> ilist)
            : base_type(ilist)
        {
        }
        ...
        constexpr fixed_vector& operator=(fixed_vector&& rhs)
        {
            ...
            return assign_helper(VStd::move(rhs));
        }

        template <typename VectorContainer>
        constexpr VStd::enable_if_t<!VStd::is_same_v<VStd::remove_cvref_t<VectorContainer>, fixed_vector>, fixed_vector>& operator=(VectorContainer&& rhs)
        {
            return assign_helper(VStd::forward<VectorContainer>(rhs));
        }
        ...
        constexpr void reserve(size_type newCapacity)
        {
            ...
            VSTD_CONTAINER_ASSERT(newCapacity <= capacity(),
        ...
        {
            VSTD_CONTAINER_ASSERT(position < size(), "VStd::fixed_vector<>::at - position is out of range");
        ...
        {
            VSTD_CONTAINER_ASSERT(!empty(), "VStd::fixed_vector<>::front - container is empty!");
            return *data();
        }
        ...
        {
            VSTD_CONTAINER_ASSERT(!empty(), "VStd::fixed_vector<>::back - container is empty!");
            return *(data() + size() - 1);
        }
        ...
        constexpr void assign(VStd::initializer_list<value_type> ilist)
        {
            assign(ilist.begin(), ilist.end());
        }

        template <typename... Args, typename = VStd::enable_if_t<is_constructible_v<T, Args...>>>
        constexpr iterator emplace(const_iterator insertPos, Args&&... args)
        {
            VSTD_CONTAINER_ASSERT(!full(), "Cannot emplace on a full fixed_vector");
            VSTD_CONTAINER_ASSERT(insertPos >= cbegin() && insertPos <= cend(), "insert position must be in range of container");
            pointer dataEnd = data() + size();
            pointer insertPosPtr = data() + VStd::distance(cbegin(), insertPos);

            if (insertPosPtr == dataEnd)
            {
                reference newElement = emplace_back(VStd::forward<Args>(args)...);
                return &newElement;
            }

            VStd::uninitialized_move(insertPosPtr, dataEnd, insertPosPtr + 1);
            VStd::construct_at(insertPosPtr, VStd::forward<Args>(args)...);
        ...
        {
            VSTD_CONTAINER_ASSERT(insertPos >= cbegin() && insertPos <= cend(), "insert position must be in range of container");
            return emplace(insertPos, VStd::move(value));
        }
        ...
        constexpr void insert(const_iterator insertPos, size_type numElements, const_reference value)
        {
            ...
            VSTD_CONTAINER_ASSERT(insertPos >= cbegin() && insertPos <= cend(), "insert position must be in range of container");
            VSTD_CONTAINER_ASSERT(Capacity >= (size() + numElements), "VStd::fixed_vector::insert - capacity is reached!");
            ...
            size_type numInitializedToFill = VStd::distance(insertPosPtr, dataEnd);
            if (numInitializedToFill < numElements)
            {
                ...
                pointer newLast = VStd::uninitialized_move(insertPosPtr, dataEnd, insertPosPtr + numElements);

                // Add new elements to uninitialized elements.
                VStd::uninitialized_fill_n(dataEnd, numElements - numInitializedToFill, valueOverlapsInputRange ? value_type(value) : value);

                // Add new data
                VStd::fill_n(insertPosPtr, numInitializedToFill, valueOverlapsInputRange ? value_type(value) : value);

                // Update the size of the fixed_vector_storage
                resize_no_construct(VStd::distance(dataStart, newLast));
            }
            else
            {
                ...
                pointer newLast = VStd::uninitialized_move(nonOverlap, dataEnd, dataEnd);

                // move the area with overlapping
                VStd::move_backward(insertPosPtr, nonOverlap, dataEnd);

                // add new elements
                VStd::fill_n(insertPosPtr, numElements, valueOverlapsInputRange ? value_type(value) : value);
        ...
        constexpr void insert(const_iterator insertPos, VStd::initializer_list<value_type> ilist)
        {
        ...
        {
            VSTD_CONTAINER_ASSERT(first >= cbegin() && last <= cend(), "erase iterator must be inside the range of fixed_vector container");
            iterator dataStart = begin();
            iterator dataEnd = end();
            iterator firstPtr = begin() + VStd::distance(cbegin(), first);
            iterator lastPtr = begin() + VStd::distance(cbegin(), last);
            size_type offset = VStd::distance(dataStart, firstPtr);
            // unless we have 1 elements we have memory overlapping, so we need to use move.
            iterator newLast = VStd::move(lastPtr, dataEnd, firstPtr);
        ...
        constexpr void swap(fixed_vector& rhs)
        {
            ...
            {
                auto tmp = VStd::move(*first1);
                *first1 = VStd::move(*first2);
                *first2 = VStd::move(tmp);
            }
    ...
    private:
        ...
        constexpr fixed_vector& assign_helper(VectorContainer&& rhs)
        {
            ...
            if (dataSize >= newSize)
            {
                ...
                if constexpr (is_const_or_lvalue_reference)
                {
                    newLast = VStd::copy(rhsStart, rhsStart + newSize, dataStart);
                }
                else
                {
                    newLast = VStd::move(rhsStart, rhsStart + newSize, dataStart);
                }

                // Destroy the rest.
                VStd::destroy(newLast, dataStart + dataSize);
            }
            else
            {
                iterator newLast{};
                if constexpr (is_const_or_lvalue_reference)
                {
                    newLast = VStd::copy(rhsStart, rhsStart + dataSize, dataStart);
                    VStd::uninitialized_copy(rhsStart + dataSize, rhsStart + newSize, newLast);
                }
                else
                {
                    newLast = VStd::move(rhsStart, rhsStart + dataSize, dataStart);
                    VStd::uninitialized_move(rhsStart + dataSize, rhsStart + newSize, newLast);
                }
            }
        ...
        {
            size_type numElements = VStd::distance(first, last);
            if (numElements == 0)
            {
                return;
            }
            ...
            VSTD_CONTAINER_ASSERT(Capacity >= size() + numElements, "VStd::fixed_vector::insert_iter - capacity is reached!");
            ...
            if (numInitializedToFill < numElements)
            {
                ...
                VStd::uninitialized_move(insertPosPtr, dataEnd, insertPosPtr + numElements);
                // get last iterator to use move assignment operator
                Iterator lastToAssign = VStd::next(first, numInitializedToFill);

                // assign new data - The data up to previous dataEnd has already been constructed
                // so placement new should not be used for them
                insertPosPtr = VStd::copy(first, lastToAssign, insertPosPtr);
                // Add new elements to uninitialized elements.
                iterator newDataEnd = VStd::uninitialized_copy(lastToAssign, last, insertPosPtr);

                // Update the fixed storage size
                resize_no_construct(VStd::distance(dataStart, newDataEnd));
            }
            else
            {
                ...
                // first copy the data that will not overlap.
                pointer nonOverlap = dataEnd - numElements;
                iterator  newLast = VStd::uninitialized_move(nonOverlap, dataEnd, dataEnd);

                // copy the memory backwards while performing VStd::move on the existing elements the area with overlapping memory
                VStd::move_backward(insertPosPtr, nonOverlap, dataEnd);

                // add new elements
                VStd::move(first, last, insertPosPtr);
        ...
        constexpr void insert_iter(const_iterator insertPos, Iterator first, Iterator last, const input_iterator_tag&)
        {
            ...
            size_type offset = VStd::distance(dataStart, insertPos);
    ...
    //! Deduction Guide for VStd::fixed_vector
    template<class T, class... U>
    fixed_vector(T, U...) -> fixed_vector<T, 1 + sizeof...(U)>;

    template< class T, VStd::size_t Capacity1, VStd::size_t Capacity2>
    constexpr bool operator==(const fixed_vector<T, Capacity1>& a, const fixed_vector<T, Capacity2>& b)
    {
        return VStd::equal(a.begin(), a.end(), b.begin(), b.end());
    }
    ...
    constexpr bool operator<(const fixed_vector<T, Capacity1>& a, const fixed_vector<T, Capacity2>& b)
    {
        return VStd::lexicographical_compare(a.begin(), a.end(), b.begin(), b.end());
    }
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_FIXED_VECTOR_H