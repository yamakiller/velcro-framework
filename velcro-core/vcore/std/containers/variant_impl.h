 ...
 *
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_VARIANT_IMPL_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_VARIANT_IMPL_H

#include <vcore/std/containers/array.h>
#include <vcore/std/function/invoke.h>
#include <vcore/std/typetraits/add_const.h>
#include <vcore/std/typetraits/add_cv.h>
#include <vcore/std/typetraits/add_volatile.h>
...
#undef max
    ...
    using variant_index_t = VStd::conditional_t<(NumAlternatives < std::numeric_limits<uint8_t>::max()), uint8_t,
        VStd::conditional_t<(NumAlternatives < std::numeric_limits<uint16_t>::max()), uint16_t,
        VStd::conditional_t<(NumAlternatives < std::numeric_limits<uint32_t>::max()), uint32_t, uint64_t>>>;
    ...
    inline namespace find_type
    {
        ...
        {
            static_assert(!VStd::is_same_v<T, T>, "T cannot be found in an empty variadic Types list");
        };
    ...
    namespace variant_detail
    {
        ...
        namespace get_alternative
        {
            struct union_
            {
                ...
                {
                    return VStd::forward<Union>(unionData).m_head;
                }

                template <size_t AltIndex, class Union>
                static constexpr auto&& get_alt(Union&& unionData, in_place_index_t<AltIndex>)
                {
                    return get_alt(VStd::forward<Union>(unionData).m_tail, in_place_index_t<AltIndex - 1>{});
                }
            };
            ...
            struct impl
            {
                ...
                {
                    return union_::get_alt(VStd::forward<VariantImpl>(var).m_union_data, in_place_index_t<Index>{});
                }
            };
            ...
            struct variant
            {
                ...
                {
                    return impl::get_alt<Index>(VStd::forward<Variant>(var).m_impl);
                }
            };
        }
        ...
        namespace visitor
        {
            struct impl
            {
                ...
                {
                    return make_dispatch_for_index<Visitor&&, decltype(VStd::forward<Variants>(variants).as_base())...>()[index](VStd::forward<Visitor>(visitor), VStd::forward<Variants>(variants).as_base()...);
                }

                template <class Visitor, class... Variants>
                static constexpr decltype(auto) visit_alt(Visitor&& visitor, Variants&&... variants)
                {
                    return at(make_dispatch_for_all<Visitor&&, decltype(VStd::forward<Variants>(variants).as_base())...>(), variants.index()...)(VStd::forward<Visitor>(visitor), VStd::forward<Variants>(variants).as_base()...);
                }
            ...
            private:
                ...
                {
                    static_assert(conjunction<is_same<DispatchFunc1, DispatchFuncs>...>::value, "VStd::visit requires the visitor to have a single return type.");
                }
                ...
                static constexpr auto make_dispatcher_array(DispatchFuncs&&... dispatchFuncs)
                {
                    ...
                    return VisitorReturnArray{ {VStd::forward<DispatchFuncs>(dispatchFuncs)...} };
                }
                ...
                struct dispatcher
                {
                    ...
                    {
                        return VStd::invoke(VStd::forward<Visitor>(visitor), get_alternative::impl::get_alt<Indices>(VStd::forward<Variants>(variants))...);
                    }
                };
            ...
            struct variant
            {
                ...
                {
                    return impl::visit_alt_at(index, VStd::forward<Visitor>(visitor), VStd::forward<VariantTypes>(vs).m_impl...);
                }

                template <class Visitor, class... VariantTypes>
                static constexpr decltype(auto) visit_alt(Visitor&& visitor, VariantTypes&&... vs)
                {
                    return impl::visit_alt(VStd::forward<Visitor>(visitor), VStd::forward<VariantTypes>(vs).m_impl...);
                }
                ...
                {
                    return visit_alt_at(index, make_value_visitor(VStd::forward<Visitor>(visitor)), VStd::forward<VariantTypes>(vs)...);
                }

                template <class Visitor, class... VariantTypes>
                static constexpr decltype(auto) visit_value(Visitor&& visitor, VariantTypes&&... vs)
                {
                    return visit_alt(make_value_visitor(VStd::forward<Visitor>(visitor)), VStd::forward<VariantTypes>(vs)...);
                }

                template <class R, class Visitor, class... VariantTypes>
                static constexpr R visit_value_r(Visitor&& visitor, VariantTypes&&... vs)
                {
                    return visit_alt_r<R>(make_value_visitor(VStd::forward<Visitor>(visitor)), VStd::forward<VariantTypes>(vs)...);
                }
            ...
            private:
                ...
                struct value_visitor
                {
                    ...
                    {
                        static_assert(is_invocable_v<Visitor, decltype((VStd::forward<Alternatives>(alts).m_value))...>,
                            "visitor must be invocable with all supplied values");
                        return VStd::invoke(VStd::forward<Visitor>(m_visitor), VStd::forward<Alternatives>(alts).m_value...);
                    }
                ...
                {
                    return value_visitor<Visitor>{VStd::forward<Visitor>(visitor)};
                }
            };
        ...
        struct alternative_impl
        {
            ...
            explicit constexpr alternative_impl(in_place_t, Args&&... args)
                : m_value(VStd::forward<Args>(args)...) {}
        ...
        public:
            ...
            explicit constexpr variant_impl_base(in_place_index_t<Index>, Args&&... args)
                : m_union_data(in_place_index_t<Index>{}, VStd::forward<Args>(args)...)
                , m_index(Index)
            {}
        ...
        public:
            ...
            void destroy()
            {
                if (!this->valueless_by_exception())
                {
                    ...
                    {
                        (void)variantAlt;
                        using alternative_type = VStd::remove_cvref_t<decltype(variantAlt)>;
                        variantAlt.~alternative_type();
                    };
                    visitor::impl::visit_alt(VStd::move(destructVisitFunc), *this);
                }
                this->m_index = variant_index_t_npos<typename base_type::index_t>;
            }
        };
        ...
        protected:
            ...
            {
                new (&alt) alternative_impl<Index, T>{in_place, VStd::forward<Args>(args)...};
                return alt.m_value;
            }
            ...
            static void generic_construct(variant_impl_constructor& lhs, Rhs&& rhs)
            {
                ...
                {
                    construct_alt(lhs_alt, VStd::forward<decltype(rhs_alt)>(rhs_alt).m_value);
                };
                if (!rhs.valueless_by_exception())
                {
                    visitor::impl::visit_alt_at(rhs.index(), VStd::move(constructVisitFunc), lhs, VStd::forward<Rhs>(rhs));
        ...
        public:
            ...
            {
                this->generic_construct(*this, VStd::move(other));
            }
        ...
        public:
            ...
            {
                this->destroy();
                auto& result_alternative = this->construct_alt(get_alternative::impl::get_alt<Index>(*this), VStd::forward<Args>(args)...);
                this->m_index = Index;
        ...
        protected:
            ...
            void assign_alt(alternative_impl<Index, T>& alt, Arg&& arg)
            {
                ...
                if (this->index() == Index)
                {
                    alt.m_value = VStd::forward<Arg>(arg);
                    return;
                }

                constexpr bool is_emplacable = is_nothrow_constructible_v<T, Arg> || !is_nothrow_move_constructible_v<T>;
                emplace_alt<Index, T>(VStd::forward<Arg>(arg), bool_constant<is_emplacable>{});
            }
            template <size_t Index, class T, class Arg>
            void emplace_alt(Arg&& arg, VStd::true_type)
            {
                this->emplace<Index>(VStd::forward<Arg>(arg));
            }

            template <size_t Index, class T, class Arg>
            void emplace_alt(Arg&& arg, VStd::false_type)
            {
                this->emplace<Index>(T(VStd::forward<Arg>(arg)));
            }
            ...
            void generic_assign(OtherVariant&& other)
            {
                ...
                {
                    this->assign_alt(thisAlt, VStd::forward<decltype(otherAlt)>(otherAlt).m_value);
                };
                visitor::impl::visit_alt_at(other.index(), VStd::move(assignmentVisitor), *this, VStd::forward<OtherVariant>(other));
            }
        };
        ...
        public:
            ...
            {
                this->generic_assign(VStd::move(rhs));
                return *this;
            }
            variant_impl_move_assignment& operator=(const variant_impl_move_assignment&) = default;
        };
        ...
        public:
            ...
            void assign(Arg&& arg)
            {
                this->assign_alt(get_alternative::impl::get_alt<Index>(*this), VStd::forward<Arg>(arg));
            }
            ...
            void swap(impl& other)
            {
                ...
                else
                {
                    ...
                    impl tmp(VStd::move(*rhs));

                    this->generic_construct(*rhs, VStd::move(*lhs));
                    this->generic_construct(*lhs, VStd::move(tmp));
                }
            }
        };
        ...
        /*
         ...
         An example use case of uses of this struct is when dealing with a `variant<const char*, VStd::string>`
         If the variant is initializes using `variant<const char*, VStd::string> var1("Hello")`, the best union value to use
         is a const char*.
         If the variant that being used is of type `variant<int, VStd::string>`, then the best union value to use is VStd::string
         */
        ...
        using best_alternative_t = typename VStd::invoke_result_t<implicit_convertible_operators<Types...>, T>::type;
        ...
        inline constexpr auto&& generic_get(VariantType&& variantInst)
        {
            ...
            {
                VSTD_CONTAINER_ASSERT(false, "bad_variant_access: Variant does not contain alternative at index %zu", Index);
            }
            return get_alternative::variant::get_alt<Index>(VStd::forward<VariantType>(variantInst)).m_value;
        }
        ...
        inline constexpr auto* generic_get_if(VariantType* variantInst)
        {
            return variantInst != nullptr && holds_alternative_at_index<Index>(*variantInst)
                ? VStd::addressof(get_alternative::variant::get_alt<Index>(*variantInst).m_value)
                : nullptr;
        }
    }
} // namespace VStd
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_VARIANT_IMPL_H