 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_SLIST_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_SLIST_H
...
namespace VStd
{
     ...
     * Check the intrusive_slist \ref VStdExamples.
     */
    ...
    struct intrusive_slist_node
    {
        ...
        ~intrusive_slist_node()
        {
            VSTD_CONTAINER_ASSERT(m_next == 0, "VStd::intrusive_slist_node - intrusive forward_list node is being destroyed while still a member of a list!");
        }
#endif
        T* m_next;
    ...
    /**
     * Intrusive forward_list is a VStd extension container.
     * \par
    ...
    public:
        ...
        public:
            ...
            {
                VSTD_CONTAINER_ASSERT(m_node != 0, "AZSTD::intrusive_slist::const_iterator_impl invalid node!");
        ...
        public:
            ...
            V_FORCE_INLINE this_type& operator++()
            {
                VSTD_CONTAINER_ASSERT(base_type::m_node != 0, "AZSTD::intrusive_slist::iterator_impl invalid node!");
        ...
        V_FORCE_INLINE explicit intrusive_slist()
            : m_numElements(0)
        {
        ...
        V_FORCE_INLINE intrusive_slist(const InputIterator& first, const InputIterator& last)
            : m_numElements(0)
        {
            node_ptr_type head = get_head();
            hook_node_ptr_type headHook = Hook::to_node_ptr(head);
        ...
        V_FORCE_INLINE ~intrusive_slist()
        {
#ifdef V_DEBUG_BUILD
            clear();
#endif
        ...
        {
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, const_cast<this_type&>(*this).get_head()));
        }
        ...
        V_FORCE_INLINE iterator before_begin()
        {
            node_ptr_type head = get_head();
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, head));
        }
        V_FORCE_INLINE const_iterator before_begin() const
        {
            return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, const_cast<this_type&>(*this).get_head()));
        }
        ...
        V_FORCE_INLINE const_iterator last() const         { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, const_cast<node_ptr_type>(m_lastNode))); }
        ...
        V_INLINE iterator insert_after(iterator insertPos, const_reference value)
        {
            pointer node = const_cast<pointer>(&value);
            hook_node_ptr_type nodeHook = Hook::to_node_ptr(node);
#ifdef V_DEBUG_BUILD
            VSTD_CONTAINER_ASSERT(nodeHook->m_next == 0, "VStd::intrusive_slist::insert_after - this node is already in a list, erase it first.");
#endif
#ifdef VSTD_HAS_CHECKED_ITERATORS
        ...
        inline iterator erase_after(const_reference value)
        {
            ...
            hook_node_ptr_type nodeHook = Hook::to_node_ptr(node);
#ifdef V_DEBUG_BUILD
            VSTD_CONTAINER_ASSERT(nodeHook->m_next != 0, "VStd::intrusive_slist::erase_after - this node is not in a list.");
#endif
            ...
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, prevNodeHook->m_next));
        }
        inline iterator erase_after(iterator toEraseNext)
        {
            ...
            orphan_node(node);
#else
            VSTD_CONTAINER_ASSERT(toEraseNext.m_node != 0, "VStd::intrusive_slist::erase_after - invalid node!");
        ...
        V_FORCE_INLINE iterator erase(const_reference value)
        {
        ...
        {
            iterator prevFirst = previous(VStd::Internal::ConstIteratorCast<iterator>(first));
            return erase_after(prevFirst, VStd::Internal::ConstIteratorCast<iterator>(last));
        }
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "AZSTD::intrusive_slist::splice_after - splicing it self!");
            VSTD_CONTAINER_ASSERT(rhs.m_numElements > 0, "AZSTD::intrusive_slist::splice_after - No elements to splice!");
        ...
        {
            VSTD_CONTAINER_ASSERT(first != rhs.end(), "AZSTD::intrusive_slist::splice_after invalid iterator!");
            iterator last = first;
            ++last;
        ...
        {
            VSTD_CONTAINER_ASSERT(first != last, "AZSTD::intrusive_slist::splice_after - no elements for splice");
            VSTD_CONTAINER_ASSERT(this != &rhs || splicePos != last, "AZSTD::intrusive_slist::splice_after invalid iterators");
            ...
            while (Hook::to_node_ptr(preLastNode)->m_next != lastNode)
            {
                ...
                ++numElements;
                VSTD_CONTAINER_ASSERT(numElements <= rhs.m_numElements, "AZSTD::intrusive_slist::splice_after rhs container is corrupted!");
            }
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "AZSTD::forward_list::merge - can not merge itself!");
    ...
    protected:
        ...
        V_FORCE_INLINE intrusive_slist(const this_type& rhs);
        ...
        */
        ...
        // Debug
#ifdef VSTD_HAS_CHECKED_ITERATORS
        V_INLINE void orphan_node(node_ptr_type node) const
        {
    ...
    template< class T, class Hook >
    V_FORCE_INLINE bool operator==(const intrusive_slist<T, Hook>& left, const intrusive_slist<T, Hook>& right)
    {
        return &left == &right;
    }
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_SLIST_H