 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_SET_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_SET_H
...
namespace VStd
{
    #define VSTD_RBTREE_BLACK  0
    #define VSTD_RBTREE_RED    1
    #define VSTD_RBTREE_LEFT   0
    #define VSTD_RBTREE_RIGHT  1
     ...
     * Check the intrusive_multi_set \ref VStdExamples.
     */
    template< class T >
    struct intrusive_multiset_node
    {
        ...
        ~intrusive_multiset_node()
        {
            VSTD_CONTAINER_ASSERT(m_children[0] == nullptr && m_children[1] == nullptr && m_parentColorSide == nullptr, "VStd::intrusive_multiset_node - intrusive list node is being destroyed while still in a container!");
        }
#endif
        ...
        V_FORCE_INLINE ColorType  getColor() const { return static_cast<ColorType>((reinterpret_cast<VStd::size_t>(m_parentColorSide) >> BIT_COLOR) & 1); }
        V_FORCE_INLINE void       setColor(ColorType color)
        {
            VStd::size_t parentColorSide = reinterpret_cast<VStd::size_t>(m_parentColorSide);
            parentColorSide &= ~(VStd::size_t)(1 << BIT_COLOR);
            parentColorSide |= (VStd::size_t)(color << BIT_COLOR);
            m_parentColorSide = reinterpret_cast<T*>(parentColorSide);
        }
        V_FORCE_INLINE SideType getParentSide() const { return static_cast<SideType>((reinterpret_cast<VStd::size_t>(m_parentColorSide) >> BIT_PARENT_SIDE) & 1); }
        V_FORCE_INLINE void     setParentSide(SideType side)
        {
            VStd::size_t parentColorSide = reinterpret_cast<VStd::size_t>(m_parentColorSide);
            parentColorSide &= ~(VStd::size_t)(1 << BIT_PARENT_SIDE);
            parentColorSide |= (VStd::size_t)(side << BIT_PARENT_SIDE);
            m_parentColorSide = reinterpret_cast<T*>(parentColorSide);
        }
        V_FORCE_INLINE T*      getParent() const       { return reinterpret_cast<T*>(reinterpret_cast<VStd::size_t>(m_parentColorSide) & ~static_cast<VStd::size_t>(BIT_MASK)); }
        V_FORCE_INLINE void    setParent(T* parent)
        {
            VSTD_CONTAINER_ASSERT((reinterpret_cast<VStd::size_t>(parent) & BIT_MASK) == 0, "We require nodes to be at least 4 bytes aligned!");
            VStd::size_t newParentColorSide = reinterpret_cast<VStd::size_t>(parent);
            VStd::size_t parentColorSide = reinterpret_cast<VStd::size_t>(m_parentColorSide);
        ...
        V_FORCE_INLINE T*      prev() const { return m_neighbours[VSTD_RBTREE_LEFT]; }
        V_FORCE_INLINE T*      next() const { return m_neighbours[VSTD_RBTREE_RIGHT];  }

    protected:
        T* m_children[2];
        T* m_neighbours[2];
        T* m_parentColorSide;
    } V_MAY_ALIAS; // we access the object in such way that we potentially can break strict aliasing rules
    ...
    /**
     * Intrusive multiset container is a VStd extension container.
     * \par
     ...
     */
    template< class T, class Hook, class Compare = VStd::less<T> >
    ...
    public:
        ...
        typedef typename VStd::size_t          difference_type;
        typedef typename VStd::size_t          size_type;
        ...
        public:
            ...
            V_FORCE_INLINE const_iterator_impl()
                : m_node(nullptr) {}
            V_FORCE_INLINE const_iterator_impl(const_node_ptr_type node)
                : m_node(const_cast<node_ptr_type>(node)) {}
            V_FORCE_INLINE const_reference operator*() const   { return *m_node; }
            V_FORCE_INLINE pointer operator->() const  { return m_node; }
            V_FORCE_INLINE this_type& operator++()
            {
                VSTD_CONTAINER_ASSERT(m_node != 0, "VSTD::intrusive_list::const_iterator_impl invalid node!");
                m_node = PredOrSucc(m_node, VSTD_RBTREE_RIGHT);
                return *this;
            }
            ...
            {
                VSTD_CONTAINER_ASSERT(m_node != 0, "VSTD::intrusive_list::const_iterator_impl invalid node!");
                m_node = PredOrSucc(m_node, VSTD_RBTREE_LEFT);
                return *this;
            }
            ...
            V_FORCE_INLINE bool operator==(const this_type& rhs) const { return (m_node == rhs.m_node); }
            V_FORCE_INLINE bool operator!=(const this_type& rhs) const { return (m_node != rhs.m_node); }
        protected:
            V_FORCE_INLINE static bool isNil(const_node_ptr_type node) { return node == Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_RIGHT]; }
            ...
            inline static node_ptr_type PredOrSucc(const_node_ptr_type node, SideType side)
            {
                ...
                {
                    VSTD_CONTAINER_ASSERT(!isNil(cur), "This node can't be nil");
        ...
        public:
            ...
            V_FORCE_INLINE iterator_impl(node_ptr_type node)
                : const_iterator_impl(node) {}
            V_FORCE_INLINE reference operator*() const { return *base_type::m_node; }
            V_FORCE_INLINE pointer operator->() const { return base_type::m_node; }
            V_FORCE_INLINE this_type& operator++()
            {
                VSTD_CONTAINER_ASSERT(base_type::m_node != 0, "VSTD::intrusive_list::iterator_impl invalid node!");
                base_type::m_node = base_type::PredOrSucc(base_type::m_node, VSTD_RBTREE_RIGHT);
                return *this;
            }
            ...
            {
                VSTD_CONTAINER_ASSERT(base_type::m_node != 0, "VSTD::intrusive_list::iterator_impl invalid node!");
                base_type::m_node = base_type::PredOrSucc(base_type::m_node, VSTD_RBTREE_LEFT);
                return *this;
            }
        ...
        class reverse_iterator_impl
        {
            ...
            typedef intrusive_multiset<T, Hook> tree_type;
        public:
            using value_type = typename VStd::iterator_traits<Iter>::value_type;
            using difference_type = typename VStd::iterator_traits<Iter>::difference_type;
            using pointer = typename VStd::iterator_traits<Iter>::pointer;
            using reference = typename VStd::iterator_traits<Iter>::reference;
            using iterator_category = typename VStd::iterator_traits<Iter>::iterator_category;
            using iterator_type = Iter;

            V_FORCE_INLINE reverse_iterator_impl()
                : m_node(nullptr) {}
            V_FORCE_INLINE reverse_iterator_impl(pointer node, const value_type* head)
                : m_node(node)
                , m_headNode(head)
            {}
            ...
            {
                VSTD_CONTAINER_ASSERT(m_node != nullptr, "VStd::intrusive_set::reference_iterator_impl invoked base() on invalid node!");
                ...
                return m_node == m_headNode
                    ? MinOrMax(Hook::to_node_ptr(m_node)->m_children[VSTD_RBTREE_LEFT], VSTD_RBTREE_LEFT)
                    : iterator_type(GetPredicateOrSuccessorNode(m_node, VSTD_RBTREE_RIGHT));
            }
            V_FORCE_INLINE reference operator*() const { return *m_node; }
            V_FORCE_INLINE pointer operator->() const { return m_node; }
            V_FORCE_INLINE this_type& operator++()
            {
                VSTD_CONTAINER_ASSERT(m_node != nullptr, "VStd::intrusive_set::reference_iterator_impl invalid node!");
                m_node = GetPredicateOrSuccessorNode(m_node, VSTD_RBTREE_LEFT);
                return *this;
            }
            ...
            {
                VSTD_CONTAINER_ASSERT(m_node != nullptr, "VStd::intrusive_set::reference_iterator_impl invalid node!");
                m_node = GetPredicateOrSuccessorNode(m_node, VSTD_RBTREE_RIGHT);
                return *this;
            }
        ...
        protected:
            ...
            static bool IsNilLeafNode(const_node_ptr_type node) { return node == Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_RIGHT]; }
            ...
            static node_ptr_type GetPredicateOrSuccessorNode(const_node_ptr_type node, SideType childNodeSide)
            {
                ...
                {
                    VSTD_CONTAINER_ASSERT(!IsNilLeafNode(cur), "This node can't be nil");
        ...
        V_FORCE_INLINE explicit intrusive_multiset()
            : m_numElements(0)
        {
            setNil(get_head());
        }

        V_FORCE_INLINE intrusive_multiset(const KeyCompare& keyEq)
            : m_numElements(0)
            , m_keyCompare(keyEq)
        {
            setNil(get_head());
        ...
        V_FORCE_INLINE intrusive_multiset(const InputIterator& first, const InputIterator& last)
            : m_numElements(0)
        {
            setNil(get_head());
            insert(first, last);
        ...
        V_FORCE_INLINE intrusive_multiset(const this_type& rhs)
        {
            (void)rhs;
            VSTD_CONTAINER_ASSERT(rhs.empty(), "You can't copy an intrusive linked list, use move semantics!");
            setNil(get_head());
            m_numElements = 0;
        }
        ...
        V_FORCE_INLINE const_iterator begin() const { return minimum(); }
        V_FORCE_INLINE iterator begin() { return minimum(); }
        V_FORCE_INLINE const_iterator end() const { return const_iterator(get_head()); }
        V_FORCE_INLINE iterator end() { return iterator(get_head()); }

        V_FORCE_INLINE reverse_iterator rbegin()               { return reverse_iterator(MinOrMax(get_root(), VSTD_RBTREE_RIGHT), get_head()); }
        V_FORCE_INLINE const_reverse_iterator rbegin() const   { return const_reverse_iterator(MinOrMax(get_root(), VSTD_RBTREE_RIGHT), get_head()); }
        const_reverse_iterator crbegin() const { return const_reverse_iterator(MinOrMax(get_root(), VSTD_RBTREE_RIGHT), get_head()); }
        V_FORCE_INLINE reverse_iterator rend()                 { return reverse_iterator(get_head(), get_head()); }
        V_FORCE_INLINE const_reverse_iterator rend() const     { return const_reverse_iterator(get_head(), get_head()); }
        const_reverse_iterator crend() const { return const_reverse_iterator(get_head(), get_head()); }

        V_FORCE_INLINE const_iterator root() const { return const_iterator(get_root()); }
        V_FORCE_INLINE iterator root() { return iterator(get_root()); }
        V_FORCE_INLINE const_iterator minimum() const { return const_iterator(MinOrMax(get_root(), VSTD_RBTREE_LEFT)); }
        V_FORCE_INLINE iterator minimum() { return iterator(MinOrMax(get_root(), VSTD_RBTREE_LEFT)); }
        V_FORCE_INLINE const_iterator maximum() const { return const_iterator(MinOrMax(get_root(), VSTD_RBTREE_RIGHT)); }
        V_FORCE_INLINE iterator maximum() { return iterator(MinOrMax(get_root(), VSTD_RBTREE_RIGHT)); }

        V_FORCE_INLINE iterator lower_bound(const KeyType& key) { return iterator(DoLowerBound(key)); }
        V_FORCE_INLINE const_iterator lower_bound(const KeyType& key) const { return const_iterator(DoLowerBound(key)); }
        ...
        V_FORCE_INLINE iterator upper_bound(const KeyType& key) { return iterator(DoUpperBound(key)); }
        V_FORCE_INLINE const_iterator upper_bound(const KeyType& key) const { return const_iterator(DoUpperBound(key)); }
        template<class ComparableToKey>
        ...
        V_FORCE_INLINE iterator find(const KeyType& key)
        {
        ...
        V_FORCE_INLINE const_iterator find(const KeyType& key) const
        {
        ...
        iterator    insert(T* value)
        {
            ...
            //          (void)dbgValueHook;
            //          VSTD_CONTAINER_ASSERT(dbgValueHook->m_children[0] == nullptr && dbgValueHook->m_children[1] == nullptr && dbgValueHook->m_parentColorSide == nullptr,
        ...
        V_FORCE_INLINE iterator insert(T& value)
        {
            return insert(&value);
        }

        template <class InputIterator>
        V_FORCE_INLINE void insert(InputIterator first, InputIterator last)
        {
            while (first != last)
            {
                insert(*first++);
        ...
        void        erase(T* value)
        {
            ...
            VSTD_CONTAINER_ASSERT(nodeHook->m_children[0] != nullptr || nodeHook->m_children[1] != nullptr || nodeHook->m_parentColorSide != nullptr ||
                nodeHook->m_neighbours[0] != nullptr || nodeHook->m_neighbours[1] != nullptr, "This node is NOT attached to this list");
#endif
            if (nodeHook->m_neighbours[VSTD_RBTREE_LEFT] != node)
            {
                if (nodeHook->getParent() == nullptr)
                {
                    VSTD_CONTAINER_ASSERT(nodeHook->m_children[VSTD_RBTREE_LEFT] == nullptr, "");
                    VSTD_CONTAINER_ASSERT(nodeHook->m_children[VSTD_RBTREE_RIGHT] == nullptr, "");
                    Unlink(node);
                }
                else
                {
                    T* repl = nodeHook->m_neighbours[VSTD_RBTREE_RIGHT];
                    VSTD_CONTAINER_ASSERT(repl != get_head(), "");
                    VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(repl)->getParent() == nullptr, "");
                    VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(repl)->m_children[VSTD_RBTREE_LEFT] == nullptr, "");
                    VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(repl)->m_children[VSTD_RBTREE_RIGHT] == nullptr, "");
                    Switch(repl, node);
                    Unlink(node);
                }
            }
            else
            {
                ...
                SideType s = VSTD_RBTREE_LEFT;
                if (nodeHook->m_children[VSTD_RBTREE_RIGHT] != endNode)
                {
                    if (nodeHook->m_children[VSTD_RBTREE_LEFT] != endNode)
                    {
                        repl = nodeHook->m_children[VSTD_RBTREE_RIGHT];
                        replHook = Hook::to_node_ptr(repl);
                        while (replHook->m_children[VSTD_RBTREE_LEFT] != endNode)
                        {
                            repl = replHook->m_children[VSTD_RBTREE_LEFT];
                            replHook = Hook::to_node_ptr(repl);
                        }
                    }
                    s = VSTD_RBTREE_RIGHT;
                }
                VSTD_CONTAINER_ASSERT(replHook->m_children[1 - s] == endNode, "");
                bool red = replHook->getColor() == VSTD_RBTREE_RED;
        ...
        V_FORCE_INLINE void        erase(T& value)
        {
            erase(&value);
        }

        V_FORCE_INLINE iterator    erase(const_iterator where)
        {
            T* node = const_cast<T*>(&*where);
            ++where;
            erase(node);
            return VStd::Internal::ConstIteratorCast<iterator>(where);
        }

        template <class InputIterator>
        V_FORCE_INLINE void        erase(InputIterator first, InputIterator last)
        {
            while (first != last)
            {
                erase(*first++);
        ...
        V_FORCE_INLINE VStd::size_t size() const { return m_numElements; }

        V_FORCE_INLINE bool empty() const { return get_root() == get_head(); }
        ...
        inline unsigned int validateHeight(const_node_ptr_type node) const
        {
            ...
            if (Hook::to_node_ptr(node)->getColor() == VSTD_RBTREE_BLACK)
            {
                return validateHeight(Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_LEFT]) + validateHeight(Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_RIGHT]) + 1;
            }
            VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_LEFT])->getColor() == VSTD_RBTREE_BLACK
                && Hook::to_node_ptr(Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_RIGHT])->getColor() == VSTD_RBTREE_BLACK, "");
            const unsigned lh = validateHeight(Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_LEFT]);
            const unsigned rh = validateHeight(Hook::to_node_ptr(node)->m_children[VSTD_RBTREE_RIGHT]);
            VSTD_CONTAINER_ASSERT(lh == rh, "");
            return lh;
        }
        ...
        {
            VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(get_head())->getColor() == VSTD_RBTREE_BLACK, "");
            VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(get_head())->m_children[VSTD_RBTREE_RIGHT] == get_head(), "");
            VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(get_head())->m_children[VSTD_RBTREE_LEFT] == get_head() || Hook::to_node_ptr(Hook::to_node_ptr(get_head())->m_children[VSTD_RBTREE_LEFT])->getColor() == VSTD_RBTREE_BLACK, "");
            validateHeight(Hook::to_node_ptr(get_head())->m_children[VSTD_RBTREE_LEFT]);
            return true;
        }
#endif
    ...
    protected:
        node_ptr_type DoLowerBound(const KeyType& key)
        {
            ...
            while (curNode != endNode)
            {
                ...
                {
                    curNode = Hook::to_node_ptr(curNode)->m_children[VSTD_RBTREE_RIGHT];
                }
                else
                {
                    bestNode = curNode;
                    curNode = Hook::to_node_ptr(curNode)->m_children[VSTD_RBTREE_LEFT];
                }
            }
            return bestNode;
        }
        ...
        inline void AttachTo(node_ptr_type node, node_ptr_type parent, SideType s) const
        {
            ...
            hookNode->m_neighbours[VSTD_RBTREE_LEFT] = node;
            hookNode->m_neighbours[VSTD_RBTREE_RIGHT] = node;
            hookNode->m_children[VSTD_RBTREE_LEFT] = hookParent->m_children[s];
            hookNode->m_children[VSTD_RBTREE_RIGHT] = hookParent->m_children[s];
            hookNode->setParent(parent);
            hookNode->setParentSide(s);
            hookParent->m_children[s] = node;
            hookNode->setColor(VSTD_RBTREE_RED);
        }

        inline void Rotate(node_ptr_type node, SideType side) const
        {
            hook_node_ptr_type hookNode = Hook::to_node_ptr(node);
            VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(hookNode->getParent())->m_children[node->getParentSide()] == node, "Invalid node structure");
        ...
        {
            VSTD_CONTAINER_ASSERT(node1 != node2, "Nodes must be different");
            hook_node_ptr_type node1Hook = Hook::to_node_ptr(node1);
            hook_node_ptr_type node2Hook = Hook::to_node_ptr(node2);
            VSTD_CONTAINER_ASSERT(node2Hook->getParent() != nullptr, "It must be the head");
            SideType nps = node2Hook->getParentSide();
            node1Hook->m_children[VSTD_RBTREE_LEFT] = node2Hook->m_children[VSTD_RBTREE_LEFT];
            node1Hook->m_children[VSTD_RBTREE_RIGHT] = node2Hook->m_children[VSTD_RBTREE_RIGHT];
            node1Hook->setParent(node2Hook->getParent());
            node1Hook->setParentSide(nps);
            Hook::to_node_ptr(node2Hook->m_children[VSTD_RBTREE_LEFT])->setParent(node1);
            Hook::to_node_ptr(node2Hook->m_children[VSTD_RBTREE_LEFT])->setParentSide(VSTD_RBTREE_LEFT);
            Hook::to_node_ptr(node2Hook->m_children[VSTD_RBTREE_RIGHT])->setParent(node1);
            Hook::to_node_ptr(node2Hook->m_children[VSTD_RBTREE_RIGHT])->setParentSide(VSTD_RBTREE_RIGHT);
        ...
        {
            hook_node_ptr_type node1Hook = Hook::to_node_ptr(node1);
            node1Hook->m_neighbours[VSTD_RBTREE_LEFT] = Hook::to_node_ptr(node2)->m_neighbours[VSTD_RBTREE_LEFT];
            node1Hook->m_neighbours[VSTD_RBTREE_RIGHT] = node2;
            Hook::to_node_ptr(node1Hook->m_neighbours[VSTD_RBTREE_RIGHT])->m_neighbours[VSTD_RBTREE_LEFT] = node1;
            Hook::to_node_ptr(node1Hook->m_neighbours[VSTD_RBTREE_LEFT])->m_neighbours[VSTD_RBTREE_RIGHT] = node1;
            node1Hook->m_children[VSTD_RBTREE_LEFT] = nullptr;
            node1Hook->m_children[VSTD_RBTREE_RIGHT] = nullptr;
            node1Hook->setParent(nullptr);
            node1Hook->setParentSide(VSTD_RBTREE_LEFT);
            node1Hook->setColor(VSTD_RBTREE_RED);
        }

        inline void Unlink(node_ptr_type node) const
        {
            hook_node_ptr_type nodeHook = Hook::to_node_ptr(node);
            Hook::to_node_ptr(nodeHook->m_neighbours[VSTD_RBTREE_RIGHT])->m_neighbours[VSTD_RBTREE_LEFT] = nodeHook->m_neighbours[VSTD_RBTREE_LEFT];
            Hook::to_node_ptr(nodeHook->m_neighbours[VSTD_RBTREE_LEFT])->m_neighbours[VSTD_RBTREE_RIGHT] = nodeHook->m_neighbours[VSTD_RBTREE_RIGHT];
        }
        ...
        void InsertFixup(node_ptr_type node)
        {
            ...
            while (parentHook->getColor() == VSTD_RBTREE_RED)
            {
                node_ptr_type pp = parentHook->getParent();
                VSTD_CONTAINER_ASSERT(pp != get_head(), "This should not happen");
                ...
                if (ppRightHook->getColor() == VSTD_RBTREE_RED)
                {
                    parentHook->setColor(VSTD_RBTREE_BLACK);
                    ppRightHook->setColor(VSTD_RBTREE_BLACK);
                    ppHook->setColor(VSTD_RBTREE_RED);
                    cur = pp;
            ...
            Hook::to_node_ptr(get_root())->setColor(VSTD_RBTREE_BLACK);
        }
        ...
        void EraseFixup(node_ptr_type node)
        {
            node_ptr_type cur = node;
            hook_node_ptr_type curHook = Hook::to_node_ptr(cur);
            while (curHook->getColor() != VSTD_RBTREE_RED && cur != get_root())
            {
                ...
                VSTD_CONTAINER_ASSERT(w != get_head(), "This should not happen");
                hook_node_ptr_type wHook = Hook::to_node_ptr(w);
                if (wHook->getColor() == VSTD_RBTREE_RED)
                {
                    VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(wHook->m_children[VSTD_RBTREE_LEFT])->getColor() == VSTD_RBTREE_BLACK &&
                        Hook::to_node_ptr(wHook->m_children[VSTD_RBTREE_RIGHT])->getColor() == VSTD_RBTREE_BLACK, "Invalid tree");
                    wHook->setColor(VSTD_RBTREE_BLACK);
                    parentHook->setColor(VSTD_RBTREE_RED);
                    w = wHook->m_children[s];
                    VSTD_CONTAINER_ASSERT(w != get_head(), "This should not happen");
                    wHook = Hook::to_node_ptr(w);
                    Rotate(p, s);
                }
                if (Hook::to_node_ptr(wHook->m_children[VSTD_RBTREE_LEFT])->getColor() == VSTD_RBTREE_BLACK &&
                    Hook::to_node_ptr(wHook->m_children[VSTD_RBTREE_RIGHT])->getColor() == VSTD_RBTREE_BLACK)
                {
                    wHook->setColor(VSTD_RBTREE_RED);
                    cur = p;
                    curHook = Hook::to_node_ptr(cur);
                }
                else
                {
                    if (Hook::to_node_ptr(wHook->m_children[o])->getColor() == VSTD_RBTREE_BLACK)
                    {
                        Hook::to_node_ptr(wHook->m_children[s])->setColor(VSTD_RBTREE_BLACK);
                        wHook->setColor(VSTD_RBTREE_RED);
                    ...
                    VSTD_CONTAINER_ASSERT(Hook::to_node_ptr(wHook->m_children[o])->getColor() == VSTD_RBTREE_RED, "This should not happen");
                    wHook->setColor(parentHook->getColor());
                    parentHook->setColor(VSTD_RBTREE_BLACK);
                    Hook::to_node_ptr(wHook->m_children[o])->setColor(VSTD_RBTREE_BLACK);
                    Rotate(p, s);
                    cur = get_root();
                    curHook = Hook::to_node_ptr(cur);
                }
            }
            curHook->setColor(VSTD_RBTREE_BLACK);
        }
        ...
        VStd::size_t   m_numElements;
        KeyCompare      m_keyCompare;


        V_FORCE_INLINE node_ptr_type get_head() { return reinterpret_cast<node_ptr_type>(&m_head); }
        V_FORCE_INLINE node_ptr_type get_root() { return Hook::to_node_ptr(get_head())->m_children[VSTD_RBTREE_LEFT]; }

        V_FORCE_INLINE const_node_ptr_type get_head() const { return reinterpret_cast<const_node_ptr_type>(&m_head); }
        V_FORCE_INLINE const_node_ptr_type get_root() const { return Hook::to_node_ptr(get_head())->m_children[VSTD_RBTREE_LEFT]; }
    ...
    V_FORCE_INLINE bool operator==(const intrusive_multiset<T, Hook, Compare>& left, const intrusive_multiset<T, Hook, Compare>& right)
    {
        return &left == &right;
    }
    ...
    #undef VSTD_RBTREE_RED
    #undef VSTD_RBTREE_BLACK
    #undef VSTD_RBTREE_LEFT
    #undef VSTD_RBTREE_RIGHT
}

#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_SET_H