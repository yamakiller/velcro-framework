 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_LIST_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_LIST_H
...
namespace VStd {
    ...
    public:
        ...
        V_FORCE_INLINE list_const_iterator()
            : m_node(0) {}
        V_FORCE_INLINE list_const_iterator(base_node_ptr_type node)
            : m_node(node) {}
        V_FORCE_INLINE reference operator*() const { return static_cast<node_ptr_type>(m_node)->m_value; }
        V_FORCE_INLINE pointer operator->() const { return &static_cast<node_ptr_type>(m_node)->m_value; }
        V_FORCE_INLINE this_type& operator++()
        {
            VSTD_CONTAINER_ASSERT(m_node != 0, "VSTD::list::const_iterator_impl invalid node!");
    ...
    public:
        ...
        V_FORCE_INLINE list_iterator() {}
        V_FORCE_INLINE list_iterator(base_node_ptr_type node)
            : list_const_iterator<T>(node)   {}
        V_FORCE_INLINE this_type& operator++() {
            VSTD_CONTAINER_ASSERT(base_type::m_node != 0, "VSTD::list::iterator_impl invalid node!");
            base_type::m_node = base_type::m_node->m_next;
            return *this;
        }

        V_FORCE_INLINE reference operator*() const { return static_cast<node_ptr_type>(base_type::m_node)->m_value; }
        V_FORCE_INLINE pointer operator->() const { return &(static_cast<node_ptr_type>(base_type::m_node)->m_value); }
        V_FORCE_INLINE this_type operator++(int) {
            VSTD_CONTAINER_ASSERT(base_type::m_node != 0, "VSTD::list::iterator_impl invalid node!");
            this_type temp = *this;
            base_type::m_node = base_type::m_node->m_next;
        ...
        V_FORCE_INLINE this_type& operator--() {
            VSTD_CONTAINER_ASSERT(base_type::m_node != 0, "VSTD::list::const_iterator invalid node!");
    ...
    * To find the node type use VStd::list::node_type.
    ...
    */
    ...
    public:
        ...
        V_FORCE_INLINE explicit list()
            : m_numElements(0)
        {
            m_head.m_next = m_head.m_prev = &m_head;
        }
        V_FORCE_INLINE explicit list(const allocator_type& allocator)
            : m_numElements(0)
        ...
        V_FORCE_INLINE explicit list(size_type numElements, const_reference value = value_type())
            : m_numElements(0)
        {
            m_head.m_next = m_head.m_prev = &m_head;
            insert(begin(), numElements, value);
        }
        V_FORCE_INLINE explicit list(size_type numElements, const_reference value, const allocator_type& allocator)
            : m_numElements(0)
            , m_allocator(allocator)
        {
        ...
        V_FORCE_INLINE list(const InputIterator& first, const InputIterator& last)
            : m_numElements(0)
        {
            m_head.m_next = m_head.m_prev = &m_head;
        ...
        template<class InputIterator>
        V_FORCE_INLINE list(const InputIterator& first, const InputIterator& last, const allocator_type& allocator)
            : m_numElements(0)
            , m_allocator(allocator)
        {
            m_head.m_next = m_head.m_prev = &m_head;
        ...
        V_FORCE_INLINE list(const this_type& rhs)
            : m_numElements(0)
        ...
        V_FORCE_INLINE list(std::initializer_list<T> list)
            : m_numElements(0)
        {
            m_head.m_next = m_head.m_prev = &m_head;
        ...
        V_INLINE void assign(size_type numElements, const_reference value)
        {
            iterator i = begin();
            iterator last = end();
        ...
        V_FORCE_INLINE iterator begin()                    { return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, m_head.m_next)); }
        V_FORCE_INLINE const_iterator begin() const        { return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, m_head.m_next)); }
        ...
        V_FORCE_INLINE void resize(size_type newNumElements, const_reference value = value_type())
        ...
        list(this_type&& rhs, typename VStd::enable_if_t<VStd::is_default_constructible<MyAllocator>::value>* = nullptr)
            : m_numElements(0)
        {
            m_head.m_next = m_head.m_prev = &m_head;
            swap(rhs);
        ...
        {
            insert_element(begin(), VStd::forward<T>(value));
        }

        void push_back(T&& value)
        {
            insert_element(end(), VStd::forward<T>(value));
        }

        iterator insert(const_iterator inserPos, T&& value)
        {
            return emplace(inserPos, VStd::forward<T>(value));
        }

        template<class ... ArgumentsInputs>
        void emplace_front(ArgumentsInputs&& ... agruments)
        {
            insert_element(begin(), VStd::forward<ArgumentsInputs>(agruments) ...);
        }

        template<class ... ArgumentsInputs>
        void emplace_back(ArgumentsInputs&& ... agruments)
        {
            insert_element(end(), VStd::forward<ArgumentsInputs>(agruments) ...);
        }

        template<class ... ArgumentsInputs>
        iterator emplace(const_iterator insertPos, ArgumentsInputs&& ... agruments)
        {
            insert_element(insertPos, VStd::forward<ArgumentsInputs>(agruments) ...);
        ...
        inline iterator insert(const_iterator insertPos, size_type numElements, const_reference value)
        {
            ...
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, insNode));
        }

        template <class InputIterator>
        V_FORCE_INLINE iterator insert(const_iterator insertPos, InputIterator first, InputIterator last)
        {
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "VStd::list::swap - it's pointless to swap the vector itself!");
            if (m_allocator == rhs.m_allocator)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        VStd::swap(m_head.m_next->m_prev, rhs.m_head.m_next->m_prev);
                        VStd::swap(m_head.m_prev->m_next, rhs.m_head.m_prev->m_next);
            ...
            else
            {
                ...
                {
                    temp.push_back(VStd::move(element));
                }
                clear();
                for (auto& element : rhs)
                {
                    push_back(VStd::move(element));
                }
                rhs.clear();
                for (auto& element : temp)
                {
                    rhs.push_back(VStd::move(element));
                }
            }
        }
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "VSTD::list::splice - splicing it self!");
            VSTD_CONTAINER_ASSERT(rhs.m_numElements > 0, "VSTD::list::splice - No elements to splice!");
            ...
            else
            {
                ...
                {
                    insert(splicePos, VStd::move(*cur));
                }
                rhs.clear();
            }
        }
        ...
        {
            VSTD_CONTAINER_ASSERT(first != rhs.end(), "VSTD::list::splice invalid iterator!");
            iterator last = first;
            ++last;
            ...
            {
                insert(splicePos, VStd::move(*first));
                rhs.erase(first);
            }
        }
        void splice(iterator splicePos, this_type& rhs, iterator first, iterator last)
        {
            VSTD_CONTAINER_ASSERT(first != last, "VSTD::list::splice - no elements for splice");
            VSTD_CONTAINER_ASSERT(this != &rhs || splicePos != last, "VSTD::list::splice invalid iterators");
            ...
            else
            {
                ...
                {
                    splicePos = insert(splicePos, VStd::move(*cur));
                    ++splicePos;
                }
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "VSTD::list::merge - can not merge itself!");
        ...
        */
        ...
        iterator relink(const_iterator insertPos, const base_node_ptr_type nodeToLink)
        {
            ...
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, nodeToLink));
        }
        /// @}
    protected:
        ...
        template <class InputIterator>
        V_FORCE_INLINE iterator insert_iter(const_iterator insertPos, const InputIterator& first, const InputIterator& last, const true_type& /* is_integral<InputIterator> */)
        {
            return insert(insertPos, (size_type)first, (value_type)last);
        }

        template <class InputIterator>
        V_FORCE_INLINE iterator insert_iter(const_iterator insertPos, const InputIterator& first, const InputIterator& last, const false_type& /* !is_integral<InputIterator> */)
        {
            if (first == last)
            {
        ...
        void insert_element(const_iterator insertPos, ArgumentsInputs&& ... agruments)
        {
            ...
            // construct
            pointer ptr = &newNode->m_value;
            Internal::construct<pointer>::single(ptr, VStd::forward<ArgumentsInputs>(agruments) ...);
    ...
    V_FORCE_INLINE bool operator==(const list<T, Allocator>& left, const list<T, Allocator>& right)
    {
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_LIST_H