 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_SET_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_SET_H
...
namespace VStd
{
    ...
    */
    template<class Key, class Compare = VStd::less<Key>, class Allocator = VStd::allocator>
    class set
    {
        enum
        {
    ...
    public:
        ...
        V_FORCE_INLINE explicit set(const Compare& comp = Compare(), const Allocator& alloc = Allocator())
            : m_tree(comp, alloc) {}
        explicit set(const Allocator& alloc)
            : m_tree(alloc) {}
        template <class InputIterator>
        V_FORCE_INLINE set(InputIterator first, InputIterator last, const Compare& comp = Compare(), const Allocator& alloc = Allocator())
            : m_tree(comp, alloc)
        {
        ...
        V_FORCE_INLINE set(const this_type& rhs)
            : m_tree(rhs.m_tree)  {}
        V_FORCE_INLINE set(const this_type& rhs, const Allocator& alloc)
        ...
        V_FORCE_INLINE value_compare value_comp() const    { return m_tree.key_comp(); }
        ...
        V_FORCE_INLINE iterator insert(iterator insertPos, const value_type& value)    { return m_tree.insert_unique(insertPos, value); }
        template <class InputIterator>
        ...
        auto find(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, const_iterator> { return m_tree.find(key); }
        template<typename ComparableToKey>
        auto find(const ComparableToKey& key) -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, iterator> { return m_tree.find(key); }

        template<typename ComparableToKey>
        auto contains(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, bool> { return m_tree.contains(key); }

        template<typename ComparableToKey>
        auto count(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, size_type> { return m_tree.count(key); }

        template<typename ComparableToKey>
        auto lower_bound(const ComparableToKey& key) -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, iterator> { return m_tree.lower_bound(key); }
        template<typename ComparableToKey>
        auto lower_bound(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, const_iterator> { return m_tree.lower_bound(key); }

        template<typename ComparableToKey>
        auto upper_bound(const ComparableToKey& key) -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, iterator> { return m_tree.upper_bound(key); }
        template<typename ComparableToKey>
        auto upper_bound(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, const_iterator> { return m_tree.upper_bound(key); }

        template<typename ComparableToKey>
        auto equal_range(const ComparableToKey& key) -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, pair<iterator, iterator>> { return m_tree.equal_range_unique(key); }
        template<typename ComparableToKey>
        auto equal_range(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, pair<const_iterator, const_iterator>> { return m_tree.equal_range_unique(key); }
    ...
    V_FORCE_INLINE void swap(set<Key, Compare, Allocator>& left, set<Key, Compare, Allocator>& right)
    {
        left.swap(right);
    }
    ...
    */
    ...
    public:
        ...
        V_FORCE_INLINE explicit multiset(const Compare& comp = Compare(), const Allocator& alloc = Allocator())
            : m_tree(comp, alloc) {}
        explicit multiset(const Allocator& alloc)
            : m_tree(alloc) {}
        template <class InputIterator>
        V_FORCE_INLINE multiset(InputIterator first, InputIterator last, const Compare& comp = Compare(), const Allocator& alloc = Allocator())
            : m_tree(comp, alloc)
        {
        ...
        V_FORCE_INLINE multiset(const this_type& rhs)
            : m_tree(rhs.m_tree) {}
        V_FORCE_INLINE multiset(const this_type& rhs, const Allocator& alloc)
            : m_tree(rhs.m_tree, alloc)   {}
        // Add move semantics...
        V_FORCE_INLINE this_type& operator=(const this_type& rhs) { m_tree = rhs.m_tree; return *this; }
        V_FORCE_INLINE key_compare key_comp() const        { return m_tree.key_comp(); }
        ...
        V_FORCE_INLINE iterator insert(iterator insertPos, const value_type& value)    { return m_tree.insert_equal(insertPos, value); }
        template <class InputIterator>
        ...
        template<typename ComparableToKey>
        auto equal_range(const ComparableToKey& key) -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, pair<iterator, iterator>> { return m_tree.equal_range(key); }
        template<typename ComparableToKey>
        auto equal_range(const ComparableToKey& key) const -> enable_if_t<::VStd::Internal::is_transparent<key_compare, ComparableToKey>::value, pair<const_iterator, const_iterator>> { return m_tree.equal_range(key); }
    ...
    template<class Key, class Compare, class Allocator>
    V_FORCE_INLINE void swap(multiset<Key, Compare, Allocator>& left, multiset<Key, Compare, Allocator>& right)
    {
        left.swap(right);
    }
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_SET_H