 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_LIST_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_LIST_H
...
namespace VStd
{
     ...
     * Check the intrusive_list \ref VStdExamples.
     */
    template <class T>
    struct intrusive_list_node
    {
        ...
        ~intrusive_list_node()
        {
            VSTD_CONTAINER_ASSERT(m_prev == nullptr && m_next == nullptr, "VStd::intrusive_list_node - intrusive list node is being destroyed while still a member of a list!");
        }
#endif
        T* m_next = nullptr;
        T* m_prev = nullptr;
    ...
    /**
     * Intrusive list is a VStd extension container.
     * \par
    ...
    public:
        ...
        public:
            ...
            V_FORCE_INLINE const_iterator_impl()
                : m_node(0) {}
            V_FORCE_INLINE const_iterator_impl(node_ptr_type node)
                : m_node(node) {}
        ...
        public:
            ...
            V_FORCE_INLINE reverse_iterator_impl()
                : m_node(nullptr) {}
            V_FORCE_INLINE reverse_iterator_impl(node_ptr_type node)
                : m_node(node) {}

            iterator_type base() const
            {
                VSTD_CONTAINER_ASSERT(m_node != nullptr, "VStd::intrusive_list::reverse_iterator_impl base invoked on invalid node!");
            ...
            V_FORCE_INLINE this_type& operator++()
            {
                VSTD_CONTAINER_ASSERT(m_node != nullptr, "VStd::intrusive_list::reverse_iterator_impl invalid node!");
                m_node = Hook::to_node_ptr(m_node)->m_prev;
                return *this;
            }
        ...
        V_FORCE_INLINE explicit intrusive_list()
            : m_numElements(0)
        {
        ...
        V_FORCE_INLINE intrusive_list(const InputIterator& first, const InputIterator& last)
            : m_numElements(0)
        {
            node_ptr_type head = get_head();
            hook_node_ptr_type headHook = Hook::to_node_ptr(head);
        ...
        V_FORCE_INLINE intrusive_list(const this_type& rhs)
            : m_numElements(0)
        {
            (void)rhs;
            V_Assert(rhs.m_numElements == 0, "You can't copy an intrusive linked list, use move semantics!");
            node_ptr_type head = get_head();
            hook_node_ptr_type headHook = Hook::to_node_ptr(head);
            headHook->m_next = headHook->m_prev = head;
        }
        ...
        V_FORCE_INLINE intrusive_list(this_type&& rhs)
        {
        ...
        {
            assign_rv(VStd::forward(rhs));
        }

        V_FORCE_INLINE ~intrusive_list()
        {
#ifdef V_DEBUG_BUILD
            clear();
#endif
        ...
        V_FORCE_INLINE size_type   max_size() const    { return size_type(-1); }
        V_FORCE_INLINE bool    empty() const           { return (m_numElements == 0); }

        V_FORCE_INLINE iterator begin()
        {
            hook_node_ptr_type headHook = Hook::to_node_ptr(get_head());
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, headHook->m_next));
        }
        V_FORCE_INLINE const_iterator begin() const
        {
            const hook_node_type* headHook = Hook::to_node_ptr(const_cast<this_type&>(*this).get_head());
            return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, headHook->m_next));
        }
        V_FORCE_INLINE iterator end()
        {
            node_ptr_type head = get_head();
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, head));
        }
        V_FORCE_INLINE const_iterator end() const
        {
            return const_iterator(VSTD_CHECKED_ITERATOR(const_iterator_impl, const_cast<this_type&>(*this).get_head()));
        }
        V_FORCE_INLINE reverse_iterator rbegin()
        { 
            hook_node_ptr_type headHook = Hook::to_node_ptr(get_head());
            return reverse_iterator(VSTD_CHECKED_ITERATOR(reverse_iterator<iterator>, headHook->m_prev));
        }
        V_FORCE_INLINE const_reverse_iterator rbegin() const
        { 
            const hook_node_type* headHook = Hook::to_node_ptr(const_cast<this_type&>(*this).get_head());
            return const_reverse_iterator(VSTD_CHECKED_ITERATOR(reverse_iterator<const_iterator>, headHook->m_prev));
        }
        ...
        V_FORCE_INLINE reverse_iterator rend()
        { 
            node_ptr_type head = get_head();
            return reverse_iterator(VSTD_CHECKED_ITERATOR(reverse_iterator<iterator>, head));
        }
        V_FORCE_INLINE const_reverse_iterator rend() const
        { 
            return const_reverse_iterator(VSTD_CHECKED_ITERATOR(reverse_iterator<const_iterator>, const_cast<this_type&>(*this).get_head()));
        }
        ...
        V_FORCE_INLINE reference back()                        { return (*(--end())); }
        V_FORCE_INLINE const_reference back() const            { return (*(--end())); }

        V_FORCE_INLINE void push_front(const_reference value)  { insert(begin(), value); }
        V_FORCE_INLINE void pop_front()                        { erase(begin()); }
        V_FORCE_INLINE void push_back(const_reference value)   { insert(end(), value); }
        V_FORCE_INLINE void pop_back()                         { erase(--end()); }

        V_INLINE iterator insert(iterator insertPos, const_reference value)
        {
            pointer node = const_cast<pointer>(&value);
            hook_node_ptr_type nodeHook = Hook::to_node_ptr(node);
#ifdef V_DEBUG_BUILD
            VSTD_CONTAINER_ASSERT(nodeHook->m_prev == 0 && nodeHook->m_next == 0, "VStd::intrusive_list::insert - this node is already in a list, erase it first.");
#endif
#ifdef VSTD_HAS_CHECKED_ITERATORS
            ...
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, node));
        }
        ...
        inline iterator erase(const_reference value)
        {
            ...
            hook_node_ptr_type nodeHook = Hook::to_node_ptr(node);
#ifdef V_DEBUG_BUILD
            VSTD_CONTAINER_ASSERT(nodeHook->m_prev != 0 && nodeHook->m_next != 0, "VStd::intrusive_list::erase - this node is not in a list.");
#endif
            ...
            --m_numElements;
            return iterator(VSTD_CHECKED_ITERATOR(iterator_impl, nextNode));
        }
        ...
        V_FORCE_INLINE iterator erase(const_iterator first, const_iterator last)
        {
            ...
            return VStd::Internal::ConstIteratorCast<iterator>(last);
        }
        ...
        void swap(this_type& rhs)
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "VStd::intrusive_list::swap - it's pointless to swap the list itself!");
            node_ptr_type head = get_head();
            node_ptr_type rhsHead = rhs.get_head();
            hook_node_ptr_type headHook = Hook::to_node_ptr(head);
            hook_node_ptr_type rhsHeadHook = Hook::to_node_ptr(rhsHead);
            ...
            else
            {
                ...
                else
                {
                    ...
                    VStd::swap(Hook::to_node_ptr(headHook->m_next)->m_prev, Hook::to_node_ptr(rhsHeadHook->m_next)->m_prev);
                    VStd::swap(Hook::to_node_ptr(headHook->m_prev)->m_next, Hook::to_node_ptr(rhsHeadHook->m_prev)->m_next);
                    VStd::swap(headHook->m_next, rhsHeadHook->m_next);
                    VStd::swap(headHook->m_prev, rhsHeadHook->m_prev);
                }
            }
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "VSTD::intrusive_list::splice - splicing it self!");
            VSTD_CONTAINER_ASSERT(rhs.m_numElements > 0, "VSTD::intrusive_list::splice - No elements to splice!");
        ...
        {
            VSTD_CONTAINER_ASSERT(first != rhs.end(), "VSTD::intrusive_list::splice invalid iterator!");
        ...
        {
            VSTD_CONTAINER_ASSERT(first != last, "VSTD::instrusive_list::splice - no elements for splice");
            VSTD_CONTAINER_ASSERT(this != &rhs || splicePos != last, "VSTD::instrusive_list::splice invalid iterators");
        ...
        {
            VSTD_CONTAINER_ASSERT(this != &rhs, "VSTD::intrusive_list::merge - can not merge itself!");
    ...
    template< class T, class Hook >
    V_FORCE_INLINE bool operator==(const intrusive_list<T, Hook>& left, const intrusive_list<T, Hook>& right)
    {
        return &left == &right;
    }
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_INTRUSIVE_LIST_H