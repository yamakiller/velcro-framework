 ...
 *
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_VARIANT_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_VARIANT_H

#include <vcore/std/containers/variant_impl.h>
...
#include <vcore/std/typetraits/is_swappable.h>

/* Microsoft C++ ABI puts 1 byte of padding between each empty base class when multiple inheritance is being used
 * VStd::variant implementation inherits from both make_constructor_overloads and make_assignment_overloads
 * which are both empty base classes. The second class make_assignment_overloads was put at offset 1.
 * Therefore the VStd::variant_detail::impl class is placed at offset 8 in order to be aligned and causes
...
namespace VStd
{
     ...
     * using SerializationType = VStd::variant<char*, VStd::array<float, 4>>;
     * ```
     * 
     ...
     * and binary data a variant with the types of `VStd::variant<double, int64_t, VStd::string, VStd::vector<uint8_t>>`
     ...
     *  Ex.
     *  using AnyFactoryCallback = VStd::any(*)();
    ...
    class VSTD_VARIANT_EMPTY_BASE_OPTIMIZATION variant
    ...
    /* monostate is a unit type intended to act as an empty alternative for an VStd::variant.
    ...
    {
        template<typename... VariantTypes, typename = VStd::enable_if_t<hash_enabled_concept_v<VariantTypes...>>>
        constexpr size_t operator()(const variant<VariantTypes...>& variantKey) const
        {
            ...
            {
                using value_type = VStd::remove_cvref_t<decltype(variantAlt)>;
...
#include <vcore/std/containers/variant.inl>

// undefine Visual Studio Empty Base Class Optimization Macro
#undef VSTD_VARIANT_EMPTY_BASE_OPTIMIZATION

#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_VARIANT_H