 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_CONTAINERS_NODE_HANDLE_H
#define V_FRAMEWORK_CORE_STD_CONTAINERS_NODE_HANDLE_H
...
namespace VStd {
    ...
    inline namespace AssociativeInternal {
        ...
        private:
            ...
            template <class Traits> friend class VStd::rbtree;
            template <class Traits> friend class VStd::hash_table;
        ...
        public:
            ...
            node_handle(node_handle&& other)
                : m_node(other.m_node)
                , m_allocator(VStd::move(other.m_allocator))
            {
                other.release_node();
            }
            ...
            node_handle& operator=(node_handle&& other)
            {
                ...
                if (m_allocator == VStd::nullopt || allocator_traits::propagate_on_container_move_assignment::value)
                {
                    m_allocator = VStd::move(other.m_allocator);
                }
            ...
            {
                using VStd::swap;
                node_handle temp{ VStd::move(*this) };
...
#endif // V_FRAMEWORK_CORE_STD_CONTAINERS_NODE_HANDLE_H