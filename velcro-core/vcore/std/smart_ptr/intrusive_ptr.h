#ifndef V_FRAMEWORK_CORE_STD_SMART_PTR_INTRUSIVE_PTR_H
#define V_FRAMEWORK_CORE_STD_SMART_PTR_INTRUSIVE_PTR_H
...
namespace VStd {
     ...
     *          static V_FORCE_INLINE void add_ref(MyObject* p)    { your code to add a reference }
     *          static V_FORCE_INLINE void release(MyObject* p)    { your code to remove a reference }
     *   }
     */
    template<class T>
    struct IntrusivePtrCountPolicy
    {
        static V_FORCE_INLINE void add_ref(T* p)   { p->add_ref(); }
        static V_FORCE_INLINE void release(T* p)   { p->release(); }
    };
     ...
     *
     * This pointer is not part of the C++ standard yet. It is considered VStd
    ...
    public:
        ...
        T& operator*() const
        {
            V_Assert(px != 0, "You can't dereference a null pointer");
            return *px;
        }
...
#endif // V_FRAMEWORK_CORE_STD_SMART_PTR_INTRUSIVE_PTR_H