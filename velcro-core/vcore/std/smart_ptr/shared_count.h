 ...
 * 
 */
#ifndef V_FRAMEWORK_CORE_STD_SMART_PTR_SHARED_COUNT_H
#define V_FRAMEWORK_CORE_STD_SMART_PTR_SHARED_COUNT_H
...
#include <vcore/std/smart_ptr/checked_delete.h>
#include <vcore/std/typetraits/alignment_of.h>

namespace VStd {
    namespace Internal {
        typedef VStd::type_id sp_typeinfo;
        ...
        private:
            ...
            VStd::atomic<long> use_count_;        // #shared
            VStd::atomic<long> weak_count_;       // #weak + (#shared != 0)
        ...
        public:
            ...
            void add_ref_copy()
            {
                use_count_.fetch_add(1, VStd::memory_order_acq_rel);
            }

            bool add_ref_lock() // true on success
            {
                long tmp = use_count_.load(VStd::memory_order_acquire);
                for (;; )
                {
                    if (tmp == 0)
                    {
                    ...
                    if (use_count_.compare_exchange_weak(tmp, tmp + 1, VStd::memory_order_acq_rel, VStd::memory_order_acquire))
                    {
                        return true;
                    }
                }
            ...
            {
                if (use_count_.fetch_sub(1, VStd::memory_order_acq_rel) == 1)
                {
                    dispose();
                    weak_release();
                }
            ...
            {
                weak_count_.fetch_add(1, VStd::memory_order_acq_rel);
            }

            void weak_release() // nothrow
            {
                if (weak_count_.fetch_sub(1, VStd::memory_order_acq_rel) == 1)
                {
                    destroy();
                }
            }
            ...
            {
                return use_count_.load(VStd::memory_order_acquire);
            }
        };
        ...
        public:
            ...
            {
                VStd::checked_delete(px_);
            }
            void destroy() override // nothrow
            {
                this->~this_type();
                a_.deallocate(this, sizeof(this_type), VStd::alignment_of<this_type>::value);
            }
        ...
        public:
                ...
                : pi_(0)
            {
                ...
                A a2(a);
                pi_ = reinterpret_cast<impl_type*>(a2.allocate(sizeof(impl_type), VStd::alignment_of<impl_type>::value));
                if (pi_ != 0)
                {
            ...
            explicit shared_count(std::auto_ptr<Y>& r, const A& a)
                : pi_(0)
#if defined(VSTD_SP_ENABLE_DEBUG_HOOKS)
                , id_(shared_count_id)
#endif
            {
                typedef sp_counted_impl_pa<Y, A> impl_type;
            ...
            friend inline bool operator<(shared_count const& a, shared_count const& b)
            {
                //return VStd::less<sp_counted_base *>()( a.pi_, b.pi_ );
                return (a.pi_ < b.pi_);
            }
...
#endif
        {
            ...
            {
                V_Assert(pi_ != 0, "Bad weak pointer");
            }
        }
...
#endif // V_FRAMEWORK_CORE_STD_SMART_PTR_SHARED_COUNT_H