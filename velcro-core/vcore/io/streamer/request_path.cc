...
namespace V
{
    namespace IO
    {
        ...
        void RequestPath::InitFromRelativePath(VStd::string path)
        {
            m_path = VStd::move(path);
            m_relativePathOffset = FindAliasOffset(m_path);
            m_absolutePathHash = s_emptyPathHash;
        }

        void RequestPath::InitFromAbsolutePath(VStd::string path)
        {
            m_path = VStd::move(path);
            m_relativePathOffset = 0;
            m_absolutePathHash = VStd::hash<VStd::string>{}(m_path);
        }
        ...
        void RequestPath::ResolvePath() const
        {
            ...
            {
                V_Assert(FileIOBase::GetInstance(),
                    "Trying to resolve a path in RequestPath before the low level file system has been initialized.");

                char fullPath[V::IO::MaxPathLength];
                if (!FileIOBase::GetInstance()->ResolvePath(m_path.c_str(), fullPath, V_ARRAY_SIZE(fullPath)))
                {
        ...
        size_t RequestPath::FindAliasOffset(const VStd::string& path) const
        {
