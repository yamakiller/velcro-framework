...
namespace V
{
    namespace IO
    {
        ...
        {
            V_Assert(m_hasOffsetEndSet, "Calling GetSize on a FileRange that doesn't have a size specified.");
            return m_offsetEnd - m_offsetBegin;
        }

        u64 FileRange::GetEndPoint() const
        {
            V_Assert(m_hasOffsetEndSet, "Calling GetEndPoint on a FileRange that doesn't have an end offset specified.");
            return m_offsetEnd;
        }
    } // namespace IO
} // namesapce V
