...
namespace V {
    namespace IO
    {
        ...
        void FileIOBase::SetInstance(FileIOBase* instance)
        {
            ...
            {
                V_Error("FileIO", false, "FileIOBase::SetInstance was called without first destroying the old instance and setting it to nullptr");
            }
        ...
        void FileIOBase::SetDirectInstance(FileIOBase* instance)
        {
            ...
            {
                V_Error("FileIO", false, "FileIOBase::SetDirectInstance was called without first destroying the old instance and setting it to nullptr");
            }

            (*g_directFileIOInstance) = instance;
        }

        VStd::optional<V::IO::FixedMaxPath> FileIOBase::ConvertToAlias(const V::IO::PathView& path) const
        {
            V::IO::FixedMaxPath convertedPath;
        ...
        VStd::optional<V::IO::FixedMaxPath> FileIOBase::ResolvePath(const V::IO::PathView& path) const
        {
            V::IO::FixedMaxPath resolvedPath;
        ...
        const char* GetStringModeFromOpenMode(OpenMode mode)
        {
            ...
            V_Error("FileIO", false, "A bad open mode was sent to GetStringModeFromOpenMode()");
            return "";
        }

        bool NameMatchesFilter(const char* name, const char* filter)
        {
            return VStd::wildcard_match(filter, name);
        }
        ...
        FileIOStream::FileIOStream(HandleType fileHandle, V::IO::OpenMode mode, bool ownsHandle)
            : m_handle(fileHandle)
            , m_mode(mode)
            , m_ownsHandle(ownsHandle)
        {
            ...
            V_Assert(fileIO, "FileIO is not initialized.");
            VStd::array<char, MaxPathLength> resolvedPath{ {0} };
            fileIO->GetFilename(m_handle, resolvedPath.data(), resolvedPath.size() - 1);
            m_filename = resolvedPath.data();
        }

        FileIOStream::FileIOStream(const char* path, V::IO::OpenMode mode, bool errorOnFailure)
            : m_handle(InvalidHandle)
            , m_mode(mode)
        ...
        {
            V_Assert(FileIOBase::GetInstance(), "FileIO is not initialized.");
            FileIOBase* fileIO = FileIOBase::GetInstance();
            ...
            // TODO:: 跟踪分析 开始
            //V_PROFILE_INTERVAL_START_COLORED(core, &m_filename, 0xff0000ff, "FileIO: %s", m_filename.c_str());
            return result;
        }
        ...
        void FileIOStream::Close()
        {
            if (m_handle != InvalidHandle)
            {
                ...
                m_ownsHandle = false;
                // TODO:: 跟踪分析 结束
               //V_PROFILE_INTERVAL_END(core, &m_filename);
            }
        }
        ...
        */
        ...
        \return OpenMode
        */
        V::IO::OpenMode FileIOStream::GetModeFlags() const
        {
            return m_mode;
        }
        ...
        {
            V_Assert(FileIOBase::GetInstance(), "FileIO is not initialized.");
            V_Assert(IsOpen(), "Cannot seek on a FileIOStream that is not open.");
            ...
            const Result result = FileIOBase::GetInstance()->Seek(m_handle, static_cast<V::s64>(bytes), seekType);
            (void)result;
            V_Error("FileIOStream", result.GetResultCode() == ResultCode::Success, "Seek failed.");
        }

        SizeType FileIOStream::Read(SizeType bytes, void* oBuffer)
        {
            V_Assert(FileIOBase::GetInstance(), "FileIO is not initialized.");
            V_Assert(IsOpen(), "Cannot read from a FileIOStream that is not open.");

            V::u64 bytesRead = 0;
            const Result result = FileIOBase::GetInstance()->Read(m_handle, oBuffer, bytes, m_errorOnFailure, &bytesRead);
            (void)result;
            V_Error("FileIOStream", result.GetResultCode() == ResultCode::Success, "Read failed in file %s.", m_filename.empty() ? "NULL" : m_filename.c_str());
        ...
        {
            V_Assert(FileIOBase::GetInstance(), "FileIO is not initialized.");
            V_Assert(IsOpen(), "Cannot write to a FileIOStream that is not open.");

            V::u64 bytesWritten = 0;
            const Result result = FileIOBase::GetInstance()->Write(m_handle, iBuffer, bytes, &bytesWritten);
            (void)result;
            V_Error("FileIOStream", result.GetResultCode() == ResultCode::Success, "Write failed.");
        ...
        {
            V_Assert(FileIOBase::GetInstance(), "FileIO is not initialized.");
            V_Assert(IsOpen(), "Cannot use a FileIOStream that is not open.");

            V::u64 currentPosition = 0;
            const Result result = FileIOBase::GetInstance()->Tell(m_handle, currentPosition);
            (void)result;
            V_Error("FileIOStream", result.GetResultCode() == ResultCode::Success, "GetCurPos failed.");
        ...
        SizeType FileIOStream::GetLength() const
        {
            ...
            {
                V_Error("FileIOStream", false, "GetLength failed.");
            }
