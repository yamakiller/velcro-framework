...
namespace V::IO::Internal
{
    ...
    template <typename InputIt, typename EndIt, typename = VStd::enable_if_t<VStd::Internal::is_input_iterator_v<InputIt>>>
    static constexpr bool HasDrivePrefix(InputIt first, EndIt last)
    {
        size_t prefixSize = VStd::distance(first, last);
        if (prefixSize < 2 || *VStd::next(first, 1) != ':')
        {
    ...
    static constexpr bool HasDrivePrefix(VStd::string_view prefix)
    {
    ...
    constexpr auto ConsumeRootName(InputIt entryBeginIter, InputIt entryEndIter, const char preferredSeparator)
        -> VStd::enable_if_t<VStd::Internal::is_forward_iterator_v<InputIt>, InputIt>
    {
...
#if defined(V_TRAIT_CUSTOM_PATH_ROOT_SEPARATOR)
            const VStd::string_view path{ entryBeginIter, entryEndIter };
            const auto positionOfPathSeparator = path.find(V_TRAIT_CUSTOM_PATH_ROOT_SEPARATOR);
            if (positionOfPathSeparator != VStd::string_view::npos)
            {
                return VStd::next(entryBeginIter, positionOfPathSeparator + 1);
            }
#endif
        ...
        else
        {
            ...
            VStd::string_view path{ entryBeginIter, entryEndIter };
            ...
            if (HasDrivePrefix(path))
            {
                ...
                return VStd::next(entryBeginIter, 2);
            }
            ...
            if (pathInUncForm)
            {
                ...
                if (slashQuestionMark || questionMarkTwice)
                {
                    ...
                    return VStd::next(entryBeginIter, 3);
                }
            }
            ...
            if (path.size() >= 3 && Internal::IsSeparator(path[1]) && !Internal::IsSeparator(path[2]))
            {
                ...
                constexpr VStd::string_view PathSeparators = { "/\\" };
                size_t nextPathSeparatorOffset = path.find_first_of(PathSeparators, 3);
                return VStd::next(entryBeginIter, nextPathSeparatorOffset != VStd::string_view::npos ? nextPathSeparatorOffset : path.size());
            }
    ...
    {
        return VStd::find_if_not(entryBeginIter, entryEndIter, [](const char elem) { return Internal::IsSeparator(elem); });
    }
    ...
    {
        return VStd::find_if(entryBeginIter, entryEndIter, [](const char elem) { return Internal::IsSeparator(elem); });
    }
    ...
    {
        size_t pathSize = VStd::distance(first, last);
        ...
        {
#if defined(V_TRAIT_CUSTOM_PATH_ROOT_SEPARATOR)
            const VStd::string_view path{ first, last };
            return path.find(V_TRAIT_CUSTOM_PATH_ROOT_SEPARATOR) != VStd::string_view::npos;
#else
        ...
        else
        {
            if (Internal::HasDrivePrefix(first, last))
            {
                ...
                return pathSize > 2 && Internal::IsSeparator(*VStd::next(first, 2));
            }

            return first != ConsumeRootName(first, last, preferredSeparator);
        }
    }
    static constexpr bool IsAbsolute(VStd::string_view pathView, const char preferredSeparator)
    {
    ...
    static int ComparePathSegment(VStd::string_view left, VStd::string_view right, bool exactCaseCompare)
    {
        const size_t maxCharsToCompare = (VStd::min)(left.size(), right.size());
...
namespace V::IO::parser
{
    using parser_path_type = PathView;
    using string_view_pair = VStd::pair<VStd::string_view, VStd::string_view>;
    ...
    struct PathParser
    {
        VStd::string_view m_path_view;
        VStd::string_view m_path_raw_entry;
        ParserState m_parser_state{};
        const char m_preferred_separator{ V_TRAIT_OS_PATH_SEPARATOR };

        constexpr PathParser(VStd::string_view path, ParserState state, const char preferredSeparator) noexcept
            : m_path_view(path)
        ...
        constexpr PathParser(VStd::string_view path, VStd::string_view entry, ParserState state, const char preferredSeparator) noexcept
            : m_path_view(path)
        ...
        constexpr static PathParser CreateBegin(VStd::string_view path, const char preferredSeparator) noexcept
        {
        ...
        constexpr static PathParser CreateEnd(VStd::string_view path, const char preferredSeparator) noexcept
        {
        ...
        constexpr void Increment() noexcept
        {
            ...
            case PS_AtEnd:
                V_Assert(false, "Path Parser cannot be incremented when it is in the AtEnd state");
            }
        }
        ...
        constexpr void Decrement() noexcept
        {
            ...
            case PS_AtEnd:
            {
                ...
                */
                ...
                if (Internal::IsSeparator(*(filenameEnd - 1)))
                {
                    ...
                    filenameEnd = Internal::ConsumeSeparator(VStd::make_reverse_iterator(filenameEnd),
                        VStd::make_reverse_iterator(rootDirEnd)).base();
                }

                // The previous state will be Filename, so the beginning of the filename is searched found
                auto filenameBegin = Internal::ConsumeName(VStd::make_reverse_iterator(filenameEnd),
                    VStd::make_reverse_iterator(rootDirEnd)).base();
            ...
            case PS_InFilenames:
            {
                ...
                */
                ...
                auto filenameEnd = Internal::ConsumeSeparator(VStd::make_reverse_iterator(currentPathEntry),
                    VStd::make_reverse_iterator(rootDirEnd)).base();
                auto filenameBegin = Internal::ConsumeName(VStd::make_reverse_iterator(filenameEnd),
                    VStd::make_reverse_iterator(rootDirEnd)).base();
                return MakeState(PS_InFilenames, filenameBegin, filenameEnd);
            ...
            case PS_InRootName:
                ...
                return MakeState(PS_BeforeBegin);
            case PS_BeforeBegin:
                V_Assert(false, "Path Parser cannot be decremented when it is in the BeforeBegin State");
            }
        }

        //! Return a view of the current element in the path processor state
        constexpr VStd::string_view operator*() const noexcept
        {
            switch (m_parser_state)
            {
            case PS_BeforeBegin:
    ...
    private:
        constexpr void MakeState(ParserState newState, typename VStd::string_view::iterator start, typename VStd::string_view::iterator end) noexcept
        {
            m_parser_state = newState;
            m_path_raw_entry = VStd::string_view(start, end);
        }
        ...
        constexpr typename VStd::string_view::iterator getNextTokenStartPos() const noexcept
        {
            switch (m_parser_state)
            {
            case PS_BeforeBegin:
        ...
        constexpr typename VStd::string_view::iterator getCurrentTokenStartPos() const noexcept
        {
            switch (m_parser_state)
            {
            case PS_BeforeBegin:
    ...
    constexpr string_view_pair SeparateFilename(const VStd::string_view& srcView)
    {
        ...
        if (pos == VStd::string_view::npos || pos == 0)
        {
            return string_view_pair{ srcView, VStd::string_view{} };
        }
    ...
    constexpr int CompareRootName(PathParser* lhsPathParser, PathParser* rhsPathParser)
    {
        ...
        auto GetRootName = [](PathParser* pathParser) constexpr -> VStd::string_view
        {
    ...
    {
        size_t hash_value = 0;
        const bool hashExactPath = pathParser.m_preferred_separator == V::IO::PosixPathSeparator;
        while (pathParser)
        {
            ...
            case PS_InRootName:
            case PS_InFilenames:
                VStd::hash_combine(hash_value, HashSegment(*pathParser, hashExactPath));
                break;
            case PS_InRootDir:
                ...
                VStd::hash_combine(hash_value, HashSegment("/", hashExactPath));
                break;
            default:
    ...
    constexpr PathPartKind ClassifyPathPart(const PathParser& parser)
    {
        ...
        VStd::string_view pathPart = *parser;
        if (pathPart == ".")
        {
