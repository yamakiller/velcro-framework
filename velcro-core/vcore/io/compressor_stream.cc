...
namespace V
{
    ...
    */
    CompressorStream::CompressorStream(const char* filename, OpenMode flags /*= 0*/)
        : CompressorStream(vnew FileIOStream(filename, flags), true)
    {
    }
    ...
    */
    ...
    {
        if (m_stream->IsOpen() && m_compressorData && m_compressorData->CompressorHandle)
        {
            m_compressorData->CompressorHandle->Close(this);
        }
    ...
    {
        return m_compressorData && m_compressorData->CompressorHandle ? m_compressorData->CompressorHandle->Read(this, bytes, GetCurPos(), oBuffer) : m_stream->Read(bytes, oBuffer);
    }

    SizeType CompressorStream::Write(SizeType bytes, const void* iBuffer)
    {
        return m_compressorData && m_compressorData->CompressorHandle ? m_compressorData->CompressorHandle->Write(this, bytes, iBuffer, GetCurPos()) : m_stream->Write(bytes, iBuffer);
    }

    V::IO::SizeType CompressorStream::GetCurPos() const
    {
    ...
    */
    ...
    {
        return m_compressorData && m_compressorData->CompressorHandle ? m_compressorData->CompressorHandle->Read(this, bytes, offset, oBuffer) : m_stream->ReadAtOffset(bytes, oBuffer, offset);
    }
    ...
    */
    ...
    {
        return m_compressorData && m_compressorData->CompressorHandle ? m_compressorData->CompressorHandle->Write(this, bytes, iBuffer, offset) : m_stream->WriteAtOffset(bytes, iBuffer, offset);
    }
    ...
    */
    V::IO::GenericStream* CompressorStream::GetWrappedStream() const
    {
        return m_stream;
    }
    ...
    */
    bool CompressorStream::WriteCompressedHeader(V::u32 compressorId, int compressionLevel, SizeType autoSeekDataSize)
    {
        if (m_compressorData)
        {
            V_Warning("IO", false, "Compressor already started!");
            return false;
        }
        if (GetCompressedLength() > 0)
        {
            V_Warning("IO", false, "We can start compressing a stream only before we have written any data to it! Currently you have %llu bytes written!", GetCompressedLength());
            return false;
        }

        if (!CreateCompressor(compressorId))
        {
            V_Warning("IO", false, "Compressor is non-existent, unable to write compressed header");
            return false;
        }
    ...
    */
    bool CompressorStream::ReadCompressedHeader()
    {
        ...
        VStd::array<V::u8, Compressor::m_maxHeaderSize> dataBuffer;
        ...
        if (dataSize > sizeof(CompressorHeader)) // at lea
        {
            ...
            if (hdr->IsValid())
            {
                VStd::endian_swap(hdr->CompressorId);
                VStd::endian_swap(hdr->UncompressedSize);
                V::u8* data = dataBuffer.data() + sizeof(CompressorHeader);
                dataSize -= sizeof(CompressorHeader);

                Compressor* compressor = CreateCompressor(hdr->CompressorId);
                V_Assert(compressor, "Can't create compressor 0x%08x for stream! We can't load this stream!", hdr->CompressorId);

                if (compressor && compressor->ReadHeaderAndData(this, data, dataSize))
                {
                    m_compressorData->UncompressedSize = hdr->UncompressedSize;
                }
                else
                {
                    V_Assert(false, "Failed to setup compressor 0x%08x for stream!", hdr->CompressorId);
                }
    ...
    */
    ...
    {
        return m_compressorData && m_compressorData->CompressorHandle ? m_compressorData->CompressorHandle->WriteSeekPoint(this) : true;
    }
    ...
    */
    ...
    {
        return m_compressorData ? m_compressorData->UncompressedSize : GetCompressedLength();
    }
    ...
    */
    Compressor* CompressorStream::CreateCompressor(V::u32 compressorId)
    {
        m_compressor.reset(nullptr);
        if (compressorId == CompressorZLib::TypeId())
        {
            m_compressor.reset(vnew CompressorZLib);
        }
        else if (compressorId == CompressorZStd::TypeId())
        {
            m_compressor.reset(vnew CompressorZStd);
        }
        else
        {
            V_Assert(false, "Unable to create compressor with type id [0x%08x]", compressorId);
        }
