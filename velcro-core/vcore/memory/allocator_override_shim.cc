...
namespace V {
    namespace Internal {
        AllocatorOverrideShim* AllocatorOverrideShim::Create(IAllocator* owningAllocator, IAllocatorAllocate* shimAllocationSource) {
            void* memory = shimAllocationSource->Allocate(sizeof(AllocatorOverrideShim), VStd::alignment_of<AllocatorOverrideShim>::value, 0);
            ...
            if (IsOverridden()) {
                ...
                if (m_records.erase(ptr))  {
                    ...
                    if (newSize) {
                        ...
                        memcpy(newPtr, ptr, VStd::min(newSize, oldSize));
                    }
