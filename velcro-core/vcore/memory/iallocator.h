#ifndef V_FRAMEWORK_CORE_MEMORY_IALLOCATOR_H
#define V_FRAMEWORK_CORE_MEMORY_IALLOCATOR_H
...
namespace V {
    ...
    struct AllocatorDebugConfig {
        /// 设置记录堆栈跟踪时从调用堆栈顶部省略的条目数.
        AllocatorDebugConfig& SetStackRecordLevels(int levels) { StackRecordLevels = levels; return *this; }

        /// 如果此分配器不应由 MemoryDriller 等系统记录和分析其记录，则设置为 true.
        AllocatorDebugConfig& SetExcludeFromDebugging(bool exclude = true) { ExcludeFromDebugging = exclude; return *this; }

        /// 如果此分配器使用保护部分扩展分配以检测溢出，则设置为 true.
        AllocatorDebugConfig& SetUsesMemoryGuards(bool use = true) { UsesMemoryGuards = use; return *this; }

        /// 如果此分配器写入未分配的内存，则设置为 true，以便可以在运行时检测到.
        AllocatorDebugConfig& SetMarksUnallocatedMemory(bool marks = true) { MarksUnallocatedMemory = marks; return *this; }

        unsigned int StackRecordLevels = 0;
        bool ExcludeFromDebugging = false;
        bool UsesMemoryGuards = false;
        bool MarksUnallocatedMemory = false;
    };
    ...
    class IAllocator {
        public:
            ...
            // @{ 每个系统分配器都需要提供名称, 这就是它向分配器管理器注册的方式.
