#ifndef V_FRAMEWORK_CORE_MEMORY_POOL_ALLOCATOR_H
#define V_FRAMEWORK_CORE_MEMORY_POOL_ALLOCATOR_H
...
namespace V
{
        ...
        public:
                ...
                : public Schema::Descriptor
            {
                Descriptor()
                    ...
                    , IsMarkUnallocatedMemory(false)
                    , IsMemoryGuards(false)
                    , StackRecordLevels(5)
                {}

                bool AllocationRecords; ///< True if we want to track memory allocations, otherwise false.
                bool IsMarkUnallocatedMemory; ///< Sets unallocated memory to default values (if m_allocationRecords is true)
                bool IsMemoryGuards; ///< Enables memory guards for stomp detection. Keep in mind that this will change the memory "profile" a lot as we usually double every pool allocation.
                unsigned char StackRecordLevels; ///< If stack recording is enabled, how many stack levels to record. (if m_allocationRecords is true)
            };

            bool Create(const Descriptor& descriptor)
            {
                V_Assert(this->IsReady() == false, "Allocator was already created!");
                if (this->IsReady())
                {
                    return false;
                }
    ...
    {
    public:
        V_CLASS_ALLOCATOR(PoolAllocator, SystemAllocator, 0);
        VOBJECT(PoolAllocator, "{f2760c0f-9e71-4c13-904d-9c6a8c94a48e}");
    ...
    {
    public:
        V_CLASS_ALLOCATOR(ThreadPoolAllocator, SystemAllocator, 0);
        VOBJECT(ThreadPoolAllocator, "{08e67def-942f-490d-be7a-e9814ee84b1f}");
...
#endif // V_FRAMEWORK_CORE_MEMORY_POOL_ALLOCATOR_H