#ifndef V_FRAMEWORK_CORE_MATH_UUID_H
#define V_FRAMEWORK_CORE_MATH_UUID_H
...
namespace V
{
    struct Uuid
    {
        ...
        enum Version
        {
            ...
            // V Custom version, same as VER_RANDOM except we store a CRC32 in the first 4 bytes
            // you can add and combine in this 32 bytes.
            //VER_V_RANDOM_CRC32 = 6, // 0 1 1 0
        };
        ...
        */
        ...
        inline StringType ToString(bool isBrackets = true, bool isDashes = true) const
        {
            ...
            ToString(output, V_ARRAY_SIZE(output), isBrackets, isDashes);
        ...
        V_MATH_INLINE bool operator==(const Uuid& rhs) const
        {
            const V::u64* lhs64 = reinterpret_cast<const V::u64*>(data);
            const V::u64* rhs64 = reinterpret_cast<const V::u64*>(rhs.data);
        ...
        V_MATH_INLINE bool operator!=(const Uuid& rhs) const { return !(*this == rhs); }
        ...
        operator GUID() const;
        V_MATH_INLINE Uuid& operator=(const GUID& guid)          { *this = Uuid(guid); return *this; }
        V_MATH_INLINE bool operator==(const GUID& guid) const    { return *this == Uuid(guid); }
        V_MATH_INLINE bool operator!=(const GUID& guid) const    { return !(*this == guid);  }
    #endif

        /// Adding two UUID generates SHA1 Uuid based on the data of both uuids
        Uuid operator+(const Uuid& rhs) const;
        V_MATH_INLINE Uuid& operator+=(const Uuid& rhs)      { *this = *this + rhs; return *this; }
        ...
        V_MATH_INLINE iterator begin()               { return data; }
        V_MATH_INLINE iterator end()                 { return data + V_ARRAY_SIZE(data); }
        V_MATH_INLINE const_iterator begin() const   { return data; }
        V_MATH_INLINE const_iterator end() const     { return data + V_ARRAY_SIZE(data); }
        //////////////////////////////////////////////////////////////////////////
...
namespace VStd
{
    ...
    struct hash<V::Uuid>
    {
        typedef V::Uuid    argument_type;
        typedef size_t      result_type;
        V_FORCE_INLINE size_t operator()(const V::Uuid& id) const
        {
            return id.GetHash();
        }
    };
...
#endif // V_FRAMEWORK_CORE_MATH_UUID_H