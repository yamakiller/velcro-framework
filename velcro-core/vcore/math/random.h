#ifndef V_FRAMEWORK_CORE_MATH_RANDOM_H
#define V_FRAMEWORK_CORE_MATH_RANDOM_H

#include <vcore/base.h>
#include <vcore/math/random_platform.h>
...
namespace V {
    ...
    */
    enum class RandomDistributionType : V::u32
    {
        Normal,
        UniformReal
    };
    ...
    public:
        ...
        HaltonSequence(VStd::array<uint32_t, Dimensions> bases)
            : m_bases(bases)
        {
        ...
        //! Entries are expected to be, or implicitly converted to, VStd::array<float, Dimensions>.
        template<typename Iterator>
        void FillHaltonSequence(Iterator begin, Iterator end)
        {
            VStd::array<uint32_t, Dimensions> indices = m_offsets;

            // Generator that returns the Halton number for all bases for a single entry.
            auto f = [&]()
            {
                VStd::array<float, Dimensions> item;
            ...
            VStd::generate(begin, end, f);
        }
        
        //! Returns a Halton sequence in an array of N length.
        template<uint32_t N>
        VStd::array<VStd::array<float, Dimensions>, N> GetHaltonSequence()
        {
            VStd::array<VStd::array<float, Dimensions>, N> result;
        ...
        void SetOffsets(VStd::array<uint32_t, Dimensions> offsets)
        {
            m_offsets = offsets;

            // Halton sequences start at index 1, so increment all the indices.
            VStd::for_each(m_offsets.begin(), m_offsets.end(), [](uint32_t &n){ n++; });
        }
        ...
        void SetIncrements(VStd::array<int32_t, Dimensions> increments)
        {
    ...
    private:
        ...
        VStd::array<uint32_t, Dimensions> m_bases;
        VStd::array<uint32_t, Dimensions> m_offsets;
        VStd::array<int32_t, Dimensions> m_increments;

    };
}

#endif // V_FRAMEWORK_CORE_MATH_RANDOM_H