#ifndef V_FRAMEWORK_CORE_STATISTICS_STATISTICAL_PROFILER_H
#define V_FRAMEWORK_CORE_STATISTICS_STATISTICAL_PROFILER_H

#include <vcore/event_bus/bus_impl.h> //Just to get V::NullMutex
#include <vcore/statistics/statistics_manager.h>
...
namespace V
{
    namespace Statistics
    {
        ...
        //! The StatisticalProfilerProxy OWNS an array of StatisticalProfiler<VStd::string, VStd::shared_spin_mutex>.
        //! You can "manage" one of those StatisticalProfiler by getting a reference to it and
        //! add Running statistics etc.
        template <class StatIdType = VStd::string, class MutexType = V::NullMutex>
        ...
        public:
            ...
            {
                m_statisticsManager = VStd::move(other.m_statisticsManager);
                m_perFrameAggregates = VStd::move(other.m_perFrameAggregates);
            }
            ...
            V::Statistics::StatisticsManager<StatIdType>& GetStatsManager()
            {
            ...
            {
                VStd::scoped_lock<MutexType> lock(m_mutex);
                ...
                for (StatisticalAggregate& aggregate : m_perFrameAggregates)
                {
                    ...
                    for (const V::Statistics::NamedRunningStatistic* stat : aggregate.m_statsForPerFrameCalculation)
                    {
            ...
            void LogAndResetStats(const char* windowName)
            {
                ...
                for (V::Statistics::NamedRunningStatistic* stat : m_statsVector)
                {
                    if (stat->GetNumSamples() == 0)
                    {
                        continue;
                    }
                    const VStd::string& statReport = stat->GetFormatted();
                    V_Printf(windowName, "%s\n", statReport.c_str());
                    stat->Reset();
                }
                for (StatisticalAggregate& aggregate : m_perFrameAggregates)
                {
            ...
            {
                VStd::scoped_lock<MutexType> lock(m_mutex);
                V::Statistics::NamedRunningStatistic* stat = m_statisticsManager.GetStatistic(statId);
                if (!stat)
                {
                    return;
                }
            ...
            const V::Statistics::NamedRunningStatistic* GetStatistic(const StatIdType& statId)
            {
                return m_statisticsManager.GetStatistic(statId);
            }

            int AddPerFrameStatisticalAggregate(const VStd::vector<StatIdType>& statIds,
                const StatIdType& timePerFrameStatId,
                const VStd::string& timePerFrameStatName)
            {
                VStd::scoped_lock<MutexType> lock(m_mutex);
                ...
                for (const StatIdType& statId : statIds)
                {
                    ...
                    auto const& itor = VStd::find(aggregate.m_statsForPerFrameCalculation.begin(), aggregate.m_statsForPerFrameCalculation.end(), stat);
                ...
                {
                    V_Warning("StatisticalProfiler", false, "Per frame stat with name %s already exists\n", timePerFrameStatName.c_str());
                    m_perFrameAggregates.pop_back();
                    return 0;
                }
            ...
            const V::Statistics::NamedRunningStatistic* GetFirstStatPerFrame() const
            {
                if (m_perFrameAggregates.size() < 1)
                {
                    return nullptr;
        ...
        protected:
            ...
            MutexType m_mutex;
            V::Statistics::StatisticsManager<StatIdType> m_statisticsManager;
            VStd::vector<V::Statistics::NamedRunningStatistic*> m_statsVector;
            ...
            struct StatisticalAggregate
            {
                ...
                V::Statistics::NamedRunningStatistic* m_statPerFrame;
                VStd::vector<V::Statistics::NamedRunningStatistic*> m_statsForPerFrameCalculation;
            ...
            VStd::vector<StatisticalAggregate> m_perFrameAggregates;
...
#endif // V_FRAMEWORK_CORE_STATISTICS_STATISTICAL_PROFILER_H